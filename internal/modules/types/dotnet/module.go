...
// Images returns the container images required for a .NET pod.
...
// Create provisions the .NET pod via the legacy CreateSitePod shim.
...
func (Module) Create(ctx context.Context, client modules.PodmanClient, cfg modules.PodConfig) error {
	...
	if !ok {
		return fmt.Errorf("dotnet.Module.Create: expected *modules.PodmanClientAdapter")
	}
...
// SeedConfigs returns default config maps for a .NET site.
...
// ScaffoldDir sets up the filesystem for a .NET site.
