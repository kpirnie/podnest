...
// scaffoldDir creates the filesystem structure and writes config files for a Node.js site.
func scaffoldDir(dir string, cfg modules.ScaffoldConfig) error {
	...
	if err := os.Chown(dir+"/redis", cfg.SiteUID, cfg.SiteUID); err != nil {
		logger.Warn("could not chown redis dir: %v", err)
	}
	...
	nginxSite, err := config.RenderNginxSite(marshalCfg(cfg.Configs[models.ConfigNginx]), models.SiteTypeDotNet, vEnabled)
	if err != nil {
