...
// Images returns the container images required for a PHP pod.
...
// Create provisions the PHP pod via the legacy CreateSitePod shim.
...
func (Module) Create(ctx context.Context, client modules.PodmanClient, cfg modules.PodConfig) error {
	...
	if !ok {
		return fmt.Errorf("php.Module.Create: expected *modules.PodmanClientAdapter")
	}
...
// SeedConfigs returns default config maps for a PHP site.
...
// ScaffoldDir sets up the filesystem for a PHP site.
