"use strict";(()=>{var d={async _req(t,e,a,s=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),s),o={method:t,headers:{"Content-Type":"application/json"},signal:n.signal};t!=="GET"&&t!=="HEAD"&&(o.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(o.body=JSON.stringify(a));try{let r=await fetch("/api"+e,o);clearTimeout(i);let l=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);return l}catch(r){throw clearTimeout(i),r}},get:t=>d._req("GET",t),post:(t,e)=>d._req("POST",t,e),put:(t,e)=>d._req("PUT",t,e),delete:t=>d._req("DELETE",t),patch:(t,e)=>d._req("PATCH",t,e)};var ct=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",k);let u=UIkit.modal(`#${n}`),m=document.getElementById("kp-sync-ok"),b=document.getElementById("kp-sync-cancel"),v=h=>{u.hide(),setTimeout(()=>document.getElementById(n)?.remove(),300),s(h)};m.addEventListener("click",()=>v(!0),{once:!0}),b.addEventListener("click",()=>v(!1),{once:!0}),document.getElementById(n).addEventListener("hidden",()=>v(!1),{once:!0}),u.show()})}var g={routes:{},_ownHashChange:!1,register(t,e){this.routes[t]=e},async go(t,e={}){let a=Object.keys(e).length?t+"/"+Object.values(e).join("/"):t;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(i=>{i.classList.toggle("kp-active",i.dataset.view===t)}),document.querySelectorAll(".kp-bn-item[data-view]").forEach(i=>{i.classList.toggle("kp-active",i.dataset.view===t)});let s=this.routes[t];if(!s)return;let n=document.getElementById("kp-view");n.innerHTML=ct();try{await s(n,e)}catch(i){n.innerHTML=L(i.message)}}};function at(){let e=(window.location.hash.replace("#","")||"dashboard").split("/"),a=e[0],s={};return a==="site-detail"&&e[1]&&(s.id=e[1]),{view:a,params:s}}var c={show(t,e="info",a=7e3){let s={success:"check",error:"warning",info:"info"},n=document.createElement("div");n.className=`kp-toast kp-toast-${e}`,n.innerHTML=`<span uk-icon="${s[e]||"info"}"></span><span>${t}</span>`,document.getElementById("kp-toasts").appendChild(n),UIkit.icon(n.querySelector("[uk-icon]")),setTimeout(()=>n.remove(),a)},success:t=>c.show(t,"success"),error:t=>c.show(t,"error"),info:t=>c.show(t,"info")};async function W(t){let e=`
...
`).map(S=>S.trim()).filter(Boolean),install_wordpress:v===1?b.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;x("Creating Site",w);try{await d.post("/sites",f),y(),c.success(`Site '${f.name}' created`),g.go("sites")}catch(S){y(),c.error(S.message),u.disabled=!1,u.innerHTML=m}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var z=null;function T(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}function jt(t){return`${t.toFixed(1)}%`}var V=null;function st(){return V||(V=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),V)}function nt(t,e){return`
        ...
        </table>`}function pt(t,e,a){let s=a===6,n=null;function i(){if(s)return;let l=t.querySelector("#stats-pod-indicator"),p=t.querySelector("#stats-pod-table-wrap");if(!p)return;let k=location.protocol==="https:"?"wss":"ws";n=new WebSocket(`${k}://${location.host}/api/sites/${e}/stats/pod`),n.onopen=()=>{l&&(l.className="kp-status kp-status-running",l.textContent="Live")},n.onmessage=u=>{try{let m=JSON.parse(u.data);p.innerHTML=Ot(m.containers??[])}catch{}},n.onerror=()=>{l&&(l.className="kp-status kp-status-error",l.textContent="Error")},n.onclose=()=>{l&&l.textContent==="Live"&&(l.className="kp-status kp-status-stopped",l.textContent="Disconnected")}}function o(){n&&n.readyState===WebSocket.OPEN&&n.close(),n=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{dt(e)}),i();let r=new MutationObserver(()=>{document.getElementById("stats-panel")||(o(),r.disconnect())});r.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ut(t,e){let a=e===6;await Wt(t),a||await dt(t)}async function kt(t){let e=await d.get("/sites")??[];t.innerHTML=`
        ...
        ${e.length===0?U("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>O()),Vt()}function zt(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,n=t.ParentID>0?e.find(i=>i.ID===t.ParentID)??null:null;return`
        ...
        </div>`}function Vt(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),n=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let i=null,o=!0,r=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],l=()=>{let m=r().length;e.textContent=`${m} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush"].forEach(v=>{let h=document.getElementById(v);h&&(h.disabled=m===0)});let b=document.querySelectorAll(".kp-site-row-check");a.indeterminate=m>0&&m<b.length,a.checked=b.length>0&&m===b.length},p=()=>{let u=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(m=>{let b=m.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",v=m.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";m.style.display=!u||b.includes(u)||v.includes(u)?"":"none"})},k=u=>{i===u?o=!o:(i=u,o=!0),document.querySelectorAll(".kp-sort-icon").forEach(b=>{b.dataset.col===u?b.textContent=o?" \u2191":" \u2193":b.textContent=" \u2195"});let m=[...n.querySelectorAll("tr")];m.sort((b,v)=>{let h="",f="";return u==="name"?(h=b.querySelector(".kp-site-row-name")?.textContent??"",f=v.querySelector(".kp-site-row-name")?.textContent??""):u==="status"?(h=b.dataset.status??"",f=v.dataset.status??""):u==="type"?(h=b.dataset.type??"",f=v.dataset.type??""):u==="domain"&&(h=b.querySelector("td:nth-child(6)")?.textContent.trim()??"",f=v.querySelector("td:nth-child(6)")?.textContent.trim()??""),o?h.localeCompare(f):f.localeCompare(h)}),m.forEach(b=>n.appendChild(b))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(u=>{u.checked=a.checked}),l()}),n?.addEventListener("change",u=>{u.target.classList.contains("kp-site-row-check")&&l()}),s?.addEventListener("input",p),document.querySelectorAll(".kp-sortable").forEach(u=>{u.addEventListener("click",()=>k(u.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush"].forEach(u=>{let m=u.replace("bulk-","");document.getElementById(u)?.addEventListener("click",()=>{let b=r().map(v=>v.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:m,ids:b}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(u=>{u.textContent=" \u2195"}),l()}var J=null;async function bt(t){let[e,a,s]=await Promise.all([d.get("/sites").catch(()=>[]),d.get("/stats/traffic").catch(()=>null),d.get("/stats/pod").catch(()=>null)]),n=e.filter(r=>r.SiteStatus===1).length,i=e.filter(r=>r.SiteStatus===2).length,o=e.filter(r=>r.SiteStatus===4).length;if(t.innerHTML=`
        <div class="kp-view-header">
    ...
    `,e.admin_domain&&gt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),r=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p={admin_domain:new FormData(i.target).get("admin_domain").trim()};try{await d.put("/settings",p),c.success("Settings saved"),gt(p.admin_domain)}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async i=>{let o=i.target.files[0];if(!o)return;let r=new FormData;r.append("file",o);try{let l=await fetch("/api/settings/import",{method:"POST",body:r}),p=l.status===204?null:await l.json().catch(()=>null);if(!l.ok)throw new Error(p?.error||`HTTP ${l.status}`);c.success("Settings imported")}catch(l){c.error(l.message)}finally{i.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),r=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(i.target),p={backup_schedule:l.get("backup_schedule").trim(),backup_retain_days:l.get("backup_retain_days").trim()};try{await d.put("/settings/backup",p),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),r=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(i.target),p={s3_endpoint:l.get("s3_endpoint").trim(),s3_bucket:l.get("s3_bucket").trim(),s3_region:l.get("s3_region").trim(),s3_access_key:l.get("s3_access_key").trim()},k=l.get("s3_secret_key").trim();k&&(p.s3_secret_key=k);try{await d.put("/settings/backup",p),c.success("S3 settings saved")}catch(u){c.error(u.message)}finally{o.disabled=!1,o.innerHTML=r}})}function ft(t){return`
        ...
        <tr>
            ...
            <td>
                <div class="uk-flex" style="gap:6px">
                    ...
                    </div>`}else o.innerHTML="";let r=t.querySelector("#backup-list-wrap");r&&(r.innerHTML=Kt(s))}catch(a){let s=t.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function yt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let a={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${e}/backup-repo`,a),c.success("Backup destinations saved")}catch(s){c.error(s.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let a=0;try{a=(await d.get(`/sites/${e}/backups`))?.length??0}catch{}try{await d.post(`/sites/${e}/backups`,{label:"manual"})}catch(i){c.error(i.message);return}x("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let s=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${e}/backups`))?.length??0)>a||Date.now()>s)&&(clearInterval(n),y(),await R(t,e),Date.now()<=s?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async a=>{let s=a.target.closest(".backup-restore-btn");if(s){let o=s.dataset.id;if(!await $("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${e}/backups/${o}/restore`)}catch(u){c.error(u.message);return}x("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let l=Date.now(),p=Date.now()+900*1e3,k=setInterval(async()=>{try{let u=await d.get(`/sites/${e}/backups/restore-status`);(!u?.active||Date.now()>p)&&(clearInterval(k),y(),u?.active?c.error("Restore timed out"):c.success("Restore complete"),await R(t,e))}catch{}},3e3);return}let n=a.target.closest(".backup-delete-btn");if(n){let o=n.dataset.id;if(!await $("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;x("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${e}/backups/${o}`),y(),c.success("Snapshot deleted"),await R(t,e)}catch(l){y(),c.error(l.message)}}let i=a.target.closest(".backup-download-btn");if(i){let o=i.dataset.id;x("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${e}/backups/${o}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{y()},5e3)},300);return}})}var G={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function A(t,e,a){let s=a?Object.entries(a):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
        ...
        </div>`}function Tt(t,e){let a=null,s=!1,n=t.querySelector("#log-output"),i=t.querySelector("#log-connect"),o=t.querySelector("#log-disconnect"),r=t.querySelector("#log-clear"),l=t.querySelector("#log-autoscroll"),p=t.querySelector("#log-status");function k(b){for(b.split(`
`).forEach(v=>{if(!v)return;let h=document.createElement("div");h.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",h.textContent=v,n.appendChild(h)});n.childElementCount>Gt;)n.removeChild(n.firstChild);l.checked&&(n.scrollTop=n.scrollHeight)}function u(){a&&(a.close(),a=null),s=!1,i.disabled=!1,o.disabled=!0,p&&(p.textContent="Disconnected")}i.addEventListener("click",()=>{u();let b=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,h=location.protocol==="https:"?"wss":"ws",f=b==="waf"?`${h}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:`${h}://${location.host}/api/sites/${e}/logs?container=${b}&tail=${v}`;a=new WebSocket(f),a.onopen=()=>{s=!0,i.disabled=!0,o.disabled=!1,p&&(p.textContent=`Connected \u2014 ${b}`)},a.onmessage=w=>k(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,i.disabled=!1,o.disabled=!0,p&&(p.textContent="Disconnected")}}),o.addEventListener("click",u),r.addEventListener("click",()=>{n.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(u(),i.click())});let m=g.go.bind(g);g.go=function(b,v={}){return a&&u(),m(b,v)}}function Qt(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Ct(t,e){try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Qt(a.status))}catch{}}function Pt(t){t.forEach(e=>Ct(e.Domain,e.ID))}function It(t,e,a,s=0,n=null){let i=t.SiteType!==3&&t.PMAPort>0;return`
...
`).forEach(v=>{if(!v)return;let h=document.createElement("div");b?h.className=b:h.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",h.textContent=v,a.appendChild(h)}),a.scrollTop=a.scrollHeight}function k(m){if(m=m.trim(),!m)return;r.unshift(m),l=-1,p(`wp> ${m}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,o&&(o.textContent="Running...");let b=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${b}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:m}))},v.onmessage=h=>{let f=h.data;if(f.trim()==="[done]"){v.close();return}if(f.startsWith("[info]")){p(f,"kp-muted");return}if(f.startsWith("[error]")){p(f,"kp-log-line-err");return}p(f)},v.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,n.disabled=!1,o&&(o.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{k(s.value),s.value=""}),s.addEventListener("keydown",m=>{if(m.key==="Enter"){k(s.value),s.value="",l=-1;return}if(m.key==="ArrowUp"){m.preventDefault(),l<r.length-1&&(l++,s.value=r[l]);return}m.key==="ArrowDown"&&(m.preventDefault(),l>0?(l--,s.value=r[l]):(l=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(m=>{m.addEventListener("click",()=>{let b=m.dataset.cmd;if(b.startsWith("search-replace")){s.value=b,s.focus();let v=b.indexOf("''")+1;s.setSelectionRange(v,v);return}k(b)})}),i.addEventListener("click",()=>{a.innerHTML=""});let u=g.go.bind(g);g.go=function(m,b={}){return u(m,b)},s.focus()}var N=null;function Xt(){return`
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${It(a,s??[],n,a.ParentID??0,r.find(u=>u.ID===a.ParentID)?.Name??null)}</li>
            <li>${nt(e,a.SiteType)}</li>
            <li>${A(e,1,o[1])}</li>
            ${l?`<li>${A(e,2,o[2])}</li>`:""}
            <li>${A(e,3,o[3])}</li>
            ...
            <li>${ft(e)}</li>
            ${k?`<li>${$t(e)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>g.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>W(a)),document.getElementById("sd-site-nav")?.addEventListener("change",u=>{g.go("site-detail",{id:u.target.value})}),K(t),C(t),Tt(t,e),te(t,e),Rt(e),p){se(t,e),ae(e);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{x("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${e}/recreate`),y(),c.success("Pod recreated"),g.go("site-detail",{id:e})}catch(u){y(),c.error(u.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let u=await j(a.Name);if(u){x("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${e}/clone`,{name:u}),y(),c.success(`Site cloned as '${u}'`),g.go("sites")}catch(m){y(),c.error(m.message)}}}),t.querySelectorAll("[data-action]").forEach(u=>{u.addEventListener("click",async()=>{let m=u.dataset.action;if(m==="flush"){try{await d.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(v){c.error(v.message)}return}x(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[m]??m} Pod`,"Please wait...");try{await d.post(`/sites/${e}/${m}`),y(),c.success(`Site ${m} successful`),g.go("site-detail",{id:e})}catch(v){y(),c.error(v.message)}})}),xt(t,e),Dt(t,e),a.SiteType===1&&Mt(t,e),qt(t,e,a),yt(t,e),R(t,e),k&&(Lt(t,e),Q(t,e)),pt(t,e,a.SiteType),ut(e,a.SiteType),Zt(t),Pt(s??[])}async function Y(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((i,o)=>{QRCode.toDataURL(t,{width:220,margin:2},(r,l)=>{r?o(r):i(l)})});e.src=n,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function X(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{i.textContent="Copied!"});else{let o=document.createElement("textarea");o.value=n,o.style.cssText="position:fixed;opacity:0",document.body.appendChild(o),o.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}o.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),g.go("users")})}function At(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let i=UIkit.modal("#kp-edit-user-modal");i.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let k=p.target.querySelector('[type="submit"]'),u=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let m=new FormData(p.target),b={fname:m.get("fname").trim(),lname:m.get("lname").trim(),email:m.get("email").trim(),phone:m.get("phone").trim()};if(s){b.role=parseInt(m.get("role"));let h=m.get("uname");h&&(b.uname=h.trim())}let v=m.get("password");v&&(b.password=v);try{await d.put(`/users/${e}`,b),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("User updated"),g.go("users")}catch(h){c.error(h.message),k.disabled=!1,k.innerHTML=u}});let o=document.getElementById("totp-setup-btn");o&&o.addEventListener("click",async()=>{o.disabled=!0,o.textContent="Setting up\u2026";try{let p=await d.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await Y(p.uri)}catch(p){c.error(p.message),o.disabled=!1,o.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){c.error("Enter a 6-digit code");return}r.disabled=!0;try{let k=await d.post(`/users/${e}/totp/confirm`,{code:p});i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("TOTP enabled"),k.backup_codes?.length?X(k.backup_codes):g.go("users")}catch(k){c.error(k.message),r.disabled=!1}});let l=document.getElementById("totp-disable-btn");l&&l.addEventListener("click",async()=>{l.disabled=!0;try{await d.delete(`/users/${e}/totp`),c.success("TOTP disabled"),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),g.go("users")}catch(p){c.error(p.message),l.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function Ft(t){if(!D()){t.innerHTML=L("Access denied");return}let e=await d.get("/users");t.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>At(t)),ie(t)}function rt(t){let e=t.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${t.id}">
