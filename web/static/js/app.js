...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let o=new AbortController,n=setTimeout(()=>o.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:o.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let l=await fetch("/api"+e,r);clearTimeout(n);let i=l.status===204?null:await l.json().catch(()=>null);if(l.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!l.ok)throw new Error(i?.error||`HTTP ${l.status}`);return i}catch(l){throw clearTimeout(n),l}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var y=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function E(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let o=UIkit.modal(`#${a}`),n=document.getElementById("kp-clone-name"),r=document.getElementById("kp-clone-ok"),l=document.getElementById("kp-clone-cancel"),i=d=>{o.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(d)};r.addEventListener("click",()=>i(n.value.trim()||null),{once:!0}),l.addEventListener("click",()=>i(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>i(null),{once:!0}),o.show(),setTimeout(()=>n.focus(),150),n.addEventListener("keydown",d=>{d.key==="Enter"&&r.click()})})}function mt(t,e,a){return new Promise(s=>{let o="kp-sync-modal",n=t==="pull",r=n?"Pull From Parent":"Push To Parent",l=n?"cloud-download":"cloud-upload",i=n?a:e,d=n?e:a,u=`
            <div id="${o}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",u);let b=UIkit.modal(`#${o}`),k=document.getElementById("kp-sync-ok"),p=document.getElementById("kp-sync-cancel"),v=g=>{b.hide(),setTimeout(()=>document.getElementById(o)?.remove(),300),s(g)};k.addEventListener("click",()=>v(!0),{once:!0}),p.addEventListener("click",()=>v(!1),{once:!0}),document.getElementById(o).addEventListener("hidden",()=>v(!1),{once:!0}),b.show()})}var f={routes:{},_ownHashChange:!1,register(t,e){this.routes[t]=e},async go(t,e={}){let a=Object.keys(e).length?t+"/"+Object.values(e).join("/"):t;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)}),document.querySelectorAll(".kp-bn-item[data-view]").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)});let s=this.routes[t];if(!s)return;let o=document.getElementById("kp-view");o.innerHTML=Z();try{await s(o,e)}catch(n){o.innerHTML=_(n.message)}}};function kt(){let e=(window.location.hash.replace("#","")||"dashboard").split("/"),a=e[0],s={};return a==="site-detail"&&e[1]&&(s.id=e[1]),{view:a,params:s}}var c={show(t,e="info",a=7e3){let s={success:"check",error:"warning",info:"info"},o=document.createElement("div");o.className=`kp-toast kp-toast-${e}`,o.innerHTML=`<span uk-icon="${s[e]||"info"}"></span><span>${y(t)}</span>`,document.getElementById("kp-toasts").appendChild(o),UIkit.icon(o.querySelector("[uk-icon]")),setTimeout(()=>o.remove(),a)},success:t=>c.show(t,"success"),error:t=>c.show(t,"error"),info:t=>c.show(t,"info")};async function at(t){let e=`
        ...
        </div>`}function $e(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),o=t.querySelector("#admin-log-connect"),n=t.querySelector("#admin-log-disconnect"),r=t.querySelector("#admin-log-clear"),l=t.querySelector("#admin-log-autoscroll"),i=t.querySelector("#admin-log-status");function d(k){for(k.split(`
`).forEach(p=>{if(!p)return;let v=document.createElement("div");v.className=p.match(/WAF BLOCK/i)?"kp-log-line-err":p.match(/WAF DETECT/i)?"kp-log-line-warn":p.match(/error|crit|emerg/i)?"kp-log-line-err":p.match(/warn/i)?"kp-log-line-warn":p.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=p,s.appendChild(v)});s.childElementCount>xe;)s.removeChild(s.firstChild);l.checked&&(s.scrollTop=s.scrollHeight)}function u(){e&&(e.close(),e=null),a=!1,o.disabled=!1,n.disabled=!0,i&&(i.textContent="Disconnected")}o.addEventListener("click",()=>{u();let k=t.querySelector("#admin-log-source").value,p=t.querySelector("#admin-log-tail").value,v=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${v}://${location.host}/api/logs/waf?tail=${p}`:`${v}://${location.host}/api/logs/proxy?tail=${p}`;e=new WebSocket(g),e.onopen=()=>{a=!0,o.disabled=!0,n.disabled=!1,i&&(i.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=h=>d(h.data),e.onerror=()=>{},e.onclose=()=>{a=!1,o.disabled=!1,n.disabled=!0,i&&(i.textContent="Disconnected")}}),n.addEventListener("click",u),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(u(),o.click())});let b=f.go.bind(f);f.go=function(k,p={}){return e&&u(),b(k,p)}}function _t(t){t.innerHTML=Se(),$e(t)}var nt=50;function Le(t,e,a){let s=Math.max(1,Math.ceil(t.total/nt)),o=(t.entries??[]).map(Ct).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        ...
        ${s}
    </div>`}function Ee(t){return`<span class="kp-badge ${t>=500?"kp-badge-error":t>=400?"kp-badge-warn":t>=300?"kp-badge-info":"kp-badge-ok"}">${t}</span>`}var T=t=>y(t??"");async function It(t,e){let a=new URLSearchParams({page:e,page_size:nt});return t.username&&a.set("username",t.username),t.action&&a.set("action",t.action),t.target_type&&a.set("target_type",t.target_type),t.date_from&&a.set("date_from",t.date_from),t.date_to&&a.set("date_to",t.date_to),t.auth!==""&&a.set("auth",t.auth),m.get(`/audit?${a}`)}function Te(t){return{username:t.querySelector("#al-filter-user").value.trim(),action:t.querySelector("#al-filter-action").value.trim(),target_type:t.querySelector("#al-filter-target").value.trim(),date_from:t.querySelector("#al-filter-date-from").value,date_to:t.querySelector("#al-filter-date-to").value,auth:t.querySelector("#al-filter-auth").value}}async function _e(t,e,a){async function s(r,l){let i=await It(r,l);t.querySelector("#al-table-body").innerHTML=(i.entries??[]).map(Ct).join("")||'<tr><td colspan="8" class="uk-text-center kp-text-dim">No records found</td></tr>';let d=Math.max(1,Math.ceil(i.total/nt)),u=t.querySelector("#al-pager");u&&(u.innerHTML=d>1?Pt(l,d):""),t.querySelector("#al-record-count").textContent=`${i.total} record${i.total!==1?"s":""}`,o(t,r,l,d),e=r,a=l}function o(r,l,i,d){r.querySelector("#al-prev")?.addEventListener("click",()=>s(l,i-1)),r.querySelector("#al-next")?.addEventListener("click",()=>s(l,i+1))}t.querySelector("#al-filter-apply")?.addEventListener("click",()=>{s(Te(t),1)}),t.querySelector("#al-filter-clear")?.addEventListener("click",()=>{["al-filter-user","al-filter-action","al-filter-target","al-filter-date-from","al-filter-date-to"].forEach(r=>{let l=t.querySelector(`#${r}`);l&&(l.value="")}),t.querySelector("#al-filter-auth").value="",s({username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},1)});let n=Math.max(1,Math.ceil(parseInt(t.querySelector("#al-record-count")?.textContent??"0")/nt));o(t,e,a,n),t.querySelector("#audit-log-panel")?.addEventListener("click",r=>{let l=r.target.closest(".al-diff-btn");if(!l)return;r.preventDefault(),r.stopPropagation();let i=l.dataset.prior??"",d=l.dataset.new??"",u="";i&&d?u=`=== BEFORE ===
`+st(i)+`

=== AFTER ===
`+st(d):d?u=st(d):u=st(i),document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),o=document.getElementById("cs-node-version-wrap"),n=document.getElementById("cs-dotnet-version-wrap"),r=document.getElementById("cs-start-command-wrap"),l=document.getElementById("cs-wordpress-wrap");e.show();let i=document.getElementById("cs-domains-wrap"),d=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let u=parseInt(a.value);s.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),o.classList.toggle("uk-hidden",u!==4),n.classList.toggle("uk-hidden",u!==5),r.classList.toggle("uk-hidden",u!==4&&u!==5),l.classList.toggle("uk-hidden",u!==1||u===6),i.classList.toggle("uk-hidden",u===6),d.classList.toggle("uk-hidden",u!==6)}),document.getElementById("create-site-form").addEventListener("submit",async u=>{u.preventDefault();let b=u.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(u.target),v=parseInt(p.get("site_type")),g=null;v===4&&(g=parseInt(p.get("node_version"))),v===5&&(g=parseInt(p.get("dotnet_version")));let h={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:v,runtime_version:g,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(S=>S.trim()).filter(Boolean),install_wordpress:v===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${h.name}' as a reverse proxy...`:`Setting up '${h.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",w);try{await m.post("/sites",h,6e5),x(),c.success(`Site '${h.name}' created`),f.go("sites")}catch(S){x(),c.error(S.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function Ce(t){return`${t.toFixed(1)}%`}var it=null;function bt(){return it||(it=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),it)}function vt(t,e){return`
        ...
        <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
            ...
            <tbody>${t.map(a=>{let s=a.mem_limit>0?(a.mem_used/a.mem_limit*100).toFixed(1):0,o=s>80,n=a.name.split("-").pop();return`
            <tr>
        ...
        </table>`}function qe(t,e,a,s,o){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let n=[...t].sort((b,k)=>{let p,v;switch(a){case"time":p=b.time,v=k.time;break;case"method":p=b.method,v=k.method;break;case"site":p=b.site_name,v=k.site_name;break;case"ip":p=b.client_ip,v=k.client_ip;break;default:p=b.status,v=k.status;break}return p<v?s?1:-1:p>v?s?-1:1:0}),r=50,l=Math.ceil(n.length/r),d=n.slice(e*r,(e+1)*r).map(b=>{let k=y(b.ua),p=b.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
                <td class="kp-stats-table-cell-mono" style="white-space:nowrap">${b.time.slice(11,19)}</td>
                ${o?`<td class="kp-stats-table-cell-mono" style="font-size:0.8rem">${y(b.site_name)}</td>`:""}
        ...
        <div class="kp-table-wrap uk-overflow-auto">
            <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                <thead><tr>
                    ...
                    ${o?`<th style="color:var(--kp-text-dim);font-size:0.75rem;cursor:pointer;user-select:none" data-dd-col="site">Site ${a==="site"?s?"\u2193":"\u2191":"\u2195"}</th>`:""}
        ...
        ${u}`}async function gt(t,e,a,s=!1){let o=document.getElementById("stats-drilldown-modal"),n=document.getElementById("stats-drilldown-title"),r=document.getElementById("stats-drilldown-body");if(!o||!r)return;n.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,r.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(o).show();let l=[],i=0,d="time",u=!0;function b(){console.log(s),r.innerHTML=qe(l,i,d,u,s),r.querySelectorAll("th[data-dd-col]").forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.ddCol;d===p?u=!u:(d=p,u=!0),i=0,b()})}),r.querySelectorAll("[data-dd-page]").forEach(k=>{k.addEventListener("click",()=>{i=parseInt(k.dataset.ddPage,10),b()})})}try{l=await m.get(`${t}?hour=${encodeURIComponent(e)}&status=${a}`)}catch(k){r.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${y(k.message)}</p>`;return}b()}function ht(t,e,a){let s=a===6,o=null;function n(){if(s)return;let i=t.querySelector("#stats-pod-indicator"),d=t.querySelector("#stats-pod-table-wrap");if(!d)return;let u=location.protocol==="https:"?"wss":"ws";o=new WebSocket(`${u}://${location.host}/api/sites/${e}/stats/pod`),o.onopen=()=>{i&&(i.className="kp-status kp-status-running",i.textContent="Live")},o.onmessage=b=>{try{let k=JSON.parse(b.data);d.innerHTML=Ie(k.containers??[]),d.querySelectorAll(".kp-stats-pod-role-btn").forEach(p=>{p.addEventListener("click",async()=>{let v=p.style.color;p.style.color="var(--kp-warning)";let g=p.dataset.container.split("-").pop();try{await m.post(`/sites/${e}/containers/${g}/restart`),c.success(`${g} restarted`)}catch(h){p.style.color=v,c.error(h.message)}})})}catch{}},o.onerror=()=>{i&&(i.className="kp-status kp-status-error",i.textContent="Error")},o.onclose=()=>{i&&i.textContent==="Live"&&(i.className="kp-status kp-status-stopped",i.textContent="Disconnected")}}function r(){o&&o.readyState===WebSocket.OPEN&&o.close(),o=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{Bt(e)}),n();let l=new MutationObserver(()=>{document.getElementById("stats-panel")||(r(),l.disconnect())});l.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ft(t,e){let a=e===6;await Pe(t),a||await Bt(t)}async function Mt(t){let e=await m.get("/sites")??[];t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${e.length===0?tt("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>ot()),Me()}function Be(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,o=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
        ...
        </tr>`}function At(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,o=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
        ...
        </div>`}function Me(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),o=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let n=null,r=!0,l=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],i=()=>{let v=l().length;e.textContent=`${v} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(w=>{let S=document.getElementById(w);S&&(S.disabled=v===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=v===0);let h=document.querySelectorAll(".kp-site-row-check");a.indeterminate=v>0&&v<h.length,a.checked=h.length>0&&v===h.length},d=()=>{let p=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(v=>{let g=v.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",h=v.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";v.style.display=!p||g.includes(p)||h.includes(p)?"":"none"})},u=p=>{n===p?r=!r:(n=p,r=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===p?r?" \u2191":" \u2193":" \u2195"});let v=[...o.querySelectorAll("tr")];v.sort((g,h)=>{let w="",S="";return p==="name"?(w=g.querySelector(".kp-site-row-name")?.textContent??"",S=h.querySelector(".kp-site-row-name")?.textContent??""):p==="status"?(w=g.dataset.status??"",S=h.dataset.status??""):p==="type"?(w=g.dataset.type??"",S=h.dataset.type??""):p==="domain"&&(w=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",S=h.querySelector("td:nth-child(6)")?.textContent.trim()??""),r?w.localeCompare(S):S.localeCompare(w)}),v.forEach(g=>o.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(p=>{p.checked=a.checked}),i()}),o?.addEventListener("change",p=>{p.target.classList.contains("kp-site-row-check")&&i()}),s?.addEventListener("input",d),document.querySelectorAll(".kp-sortable").forEach(p=>{p.addEventListener("click",()=>u(p.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(p=>{let v=p.replace("bulk-","");document.getElementById(p)?.addEventListener("click",()=>{let g=l().filter(h=>h.dataset.siteType!=="6").map(h=>h.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:v,ids:g}}))})});let b=document.getElementById("kp-bulk-mobile-pill"),k=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",p=>{p.stopPropagation(),k.hidden=!k.hidden}),document.addEventListener("click",p=>{k&&!b?.contains(p.target)&&(k.hidden=!0)},{capture:!0}),["start","stop","restart","flush","recreate"].forEach(p=>{document.getElementById(`bulk-mobile-${p}`)?.addEventListener("click",v=>{v.preventDefault(),k.hidden=!0;let g=l().filter(h=>h.dataset.siteType!=="6").map(h=>h.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:p,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(p=>{p.textContent=" \u2195"}),i()}var N=null;async function Dt(t){let[e,a,s]=await Promise.all([m.get("/sites").catch(()=>[]),m.get("/stats/traffic").catch(()=>null),m.get("/stats/pod").catch(()=>null)]),o=e.filter(l=>l.SiteType!==6&&l.SiteStatus===1).length,n=e.filter(l=>l.SiteType===6).length,r=e.filter(l=>l.SiteType!==6&&l.SiteStatus===4).length;if(t.innerHTML=`
        ...
        <div class="uk-grid-small uk-child-width-1-2 uk-child-width-1-4@m uk-margin-medium-bottom" uk-grid>
            ...
            <div>
                <div class="kp-stat-card">
                    <div class="uk-flex uk-flex-between">
                        <div>
                            <div class="kp-stat-value" style="color:var(--kp-success)">${o}</div>
                    ...
                    <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                        ...
                        <tbody>
                            ${(a?.top_sites??[]).length===0?'<tr><td colspan="2" class="kp-muted uk-text-small">No traffic data</td></tr>':(a?.top_sites??[]).map(l=>`
                                    <tr>
                                        <td class="kp-mono" style="font-size:0.8rem">${y(l.name)}</td>
        ...
        `,a?.hits_per_hour?.length){await bt();let l=document.getElementById("dash-traffic-chart");l&&window.Chart&&(N&&(N.destroy(),N=null),N=new window.Chart(l,{type:"bar",data:{labels:a.hits_per_hour.map(i=>new Date(i.hour).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),datasets:[{label:"2xx",data:a.hits_per_hour.map(i=>i["2xx"]),backgroundColor:"rgba(39,174,96,0.75)",borderColor:"rgba(39,174,96,1)",borderWidth:1,borderRadius:3},{label:"3xx",data:a.hits_per_hour.map(i=>i["3xx"]),backgroundColor:"rgba(43,142,255,0.75)",borderColor:"rgba(43,142,255,1)",borderWidth:1,borderRadius:3},{label:"4xx",data:a.hits_per_hour.map(i=>i["4xx"]),backgroundColor:"rgba(255,171,0,0.75)",borderColor:"rgba(255,171,0,1)",borderWidth:1,borderRadius:3},{label:"5xx",data:a.hits_per_hour.map(i=>i["5xx"]),backgroundColor:"rgba(235,59,90,0.75)",borderColor:"rgba(235,59,90,1)",borderWidth:1,borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,onClick:(i,d)=>{if(!d||!d.length)return;let u=d[0].datasetIndex,b=N.data.datasets[u].label;if(b!=="4xx"&&b!=="5xx")return;let k=a.hits_per_hour[d[0].index]?.hour;k&&gt("/stats/drilldown",k,b,!0)},onHover:(i,d)=>{if(!d||!d.length){i.native.target.style.cursor="default";return}let u=N.data.datasets[d[0].datasetIndex].label;i.native.target.style.cursor=u==="4xx"||u==="5xx"?"pointer":"default"},plugins:{legend:{display:!0,labels:{color:"#6b8cae",font:{size:11}},onHover:i=>{i.native.target.style.cursor="pointer"},onLeave:i=>{i.native.target.style.cursor="default"}},tooltip:{mode:"index",backgroundColor:"#0c1530",borderColor:"#1a2a4a",borderWidth:1,titleColor:"#dde8f5",bodyColor:"#6b8cae"}},scales:{x:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10},maxRotation:45},grid:{color:"rgba(26,42,74,0.6)"}},y:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10}},grid:{color:"rgba(26,42,74,0.6)"},beginAtZero:!0}}}}))}document.getElementById("dash-new-site")?.addEventListener("click",()=>ot())}function V(t=null){let e=t?`/sites/${t}/security/ip`:"/security/ip",a=t?`/sites/${t}/security/ua`:"/security/ua",s=t?`/sites/${t}/waf`:"/settings/waf",o=t?`/sites/${t}/security/country`:"/security/country",n=t?`/sites/${t}/security/asn`:"/security/asn";return`
        <div id="security-panel" data-ip-base="${e}" data-ua-base="${a}" data-geo-base="${o}" data-asn-base="${n}" data-waf-base="${s}" ${t?`data-site-id="${t}"`:""}>
        ...
        </div>`}async function B(t){let e=t.querySelector("#security-panel");if(!e)return;let a=e.dataset.ipBase,s=e.dataset.uaBase,o=e.dataset.wafBase;try{let n=e.dataset.geoBase,r=e.dataset.asnBase,l=[m.get(a),m.get(s),m.get(n),m.get(r)];e.dataset.siteId||l.push(m.get(o),m.get("/settings/trusted-proxies"),m.get("/security/bypass"));let[i,d,u,b,k,p,v]=await Promise.all(l);if(!t.querySelector("#sec-ip-whitelist"))return;if(t.querySelector("#sec-ip-whitelist").value=i.whitelist??"",t.querySelector("#sec-ip-blacklist").value=i.blacklist??"",t.querySelector("#sec-ua-whitelist").value=d.whitelist??"",t.querySelector("#sec-ua-blacklist").value=d.blacklist??"",t.querySelector("#sec-geo-whitelist").value=u.whitelist??"",t.querySelector("#sec-geo-blacklist").value=u.blacklist??"",t.querySelector("#sec-asn-whitelist").value=b.whitelist??"",t.querySelector("#sec-asn-blacklist").value=b.blacklist??"",k){let g=t.querySelector("#sec-waf-enabled"),h=t.querySelector("#sec-waf-audit"),w=t.querySelector("#sec-waf-mode"),S=t.querySelector("#sec-waf-paranoia"),R=t.querySelector("#sec-waf-exclusions");g&&(g.checked=!!k.Enabled),h&&(h.checked=!!k.AuditLog),w&&(w.value=String(k.Mode??0)),S&&(S.value=String(k.ParanoiaLevel??1)),R&&(R.value=k.Exclusions??"")}if(p){let g=t.querySelector("#sec-tp-cidrs");g&&(g.value=p.trusted_proxies_custom??"")}if(v){let g=t.querySelector("#sec-bypass-cidrs");g&&(g.value=v.bypass??"")}}catch(n){c.error("Failed to load security rules: "+n.message)}}function lt(t){let e=t.querySelector("#security-panel");if(!e)return;let a=e.dataset.ipBase,s=e.dataset.uaBase,o=e.dataset.geoBase;t.querySelector("#sec-ip-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-ip-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put(a,{whitelist:t.querySelector("#sec-ip-whitelist").value,blacklist:t.querySelector("#sec-ip-blacklist").value}),c.success("IP rules saved")}catch(l){c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-ua-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-ua-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put(s,{whitelist:t.querySelector("#sec-ua-whitelist").value,blacklist:t.querySelector("#sec-ua-blacklist").value}),c.success("UA rules saved")}catch(l){c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-geo-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-geo-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{let l={whitelist:t.querySelector("#sec-geo-whitelist").value,blacklist:t.querySelector("#sec-geo-blacklist").value},i=await m.put(o,l);i?.status==="confirm"&&(await UIkit.modal.confirm(`${i.reason}. Save anyway?`),i=await m.put(o,{...l,confirm:!0})),c.success("Country rules saved")}catch(l){l instanceof Error&&c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-asn-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-asn-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{let l=t.querySelector("#security-panel").dataset.asnBase,i={whitelist:t.querySelector("#sec-asn-whitelist").value,blacklist:t.querySelector("#sec-asn-blacklist").value},d=await m.put(l,i);d?.status==="confirm"&&(await UIkit.modal.confirm(`${d.reason}. Save anyway?`),d=await m.put(l,{...i,confirm:!0})),c.success("ASN rules saved")}catch(l){l instanceof Error&&c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-asn-lookup")?.addEventListener("click",()=>Ae(t)),t.querySelector("#sec-tp-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-tp-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put("/settings/trusted-proxies",{trusted_proxies_custom:t.querySelector("#sec-tp-cidrs").value.trim()}),c.success("Trusted proxy ranges saved")}catch(l){c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-bypass-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-bypass-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put("/security/bypass",{bypass:t.querySelector("#sec-bypass-cidrs").value.trim()}),c.success("Bypass rules saved")}catch(l){c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-tp-import")?.addEventListener("change",async n=>{let r=n.target.files[0];if(!r)return;let l=new FormData;l.append("file",r);try{let i=await fetch("/api/settings/trusted-proxies/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:l}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("Trusted proxies imported")}catch(i){c.error(i.message)}finally{n.target.value=""}}),t.querySelector("#sec-waf-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-waf-save"),r=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put(e.dataset.wafBase,{enabled:t.querySelector("#sec-waf-enabled").checked,mode:parseInt(t.querySelector("#sec-waf-mode").value,10),paranoia_level:parseInt(t.querySelector("#sec-waf-paranoia").value,10),audit_log:t.querySelector("#sec-waf-audit").checked,exclusions:t.querySelector("#sec-waf-exclusions").value.trim()}),c.success("WAF settings saved \u2014 engine recompiling in background")}catch(l){c.error(l.message)}finally{n.disabled=!1,n.innerHTML=r}}),t.querySelector("#sec-ip-import")?.addEventListener("change",async n=>{let r=n.target.files[0];if(!r)return;let l=new FormData;l.append("file",r);try{let i=await fetch("/api"+a+"/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:l}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("IP rules imported")}catch(i){c.error(i.message)}finally{n.target.value=""}}),t.querySelector("#sec-ua-import")?.addEventListener("change",async n=>{let r=n.target.files[0];if(!r)return;let l=new FormData;l.append("file",r);try{let i=await fetch("/api"+s+"/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:l}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("UA rules imported")}catch(i){c.error(i.message)}finally{n.target.value=""}}),t.querySelector("#sec-waf-import")?.addEventListener("change",async n=>{let r=n.target.files[0];if(!r)return;let l=new FormData;l.append("file",r);try{let i=await fetch("/api/settings/waf/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:l}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("WAF settings imported")}catch(i){c.error(i.message)}finally{n.target.value=""}})}function Ae(t){document.getElementById("kp-asn-lookup-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
        <div id="kp-asn-lookup-modal" uk-modal>
            <div class="uk-modal-dialog kp-modal uk-modal-body">
                ...
                </button>`,n.querySelector("#asn-lookup-ip").textContent=r.ip,n.querySelector("#asn-lookup-org").textContent=r.org||"",n.querySelector("#asn-lookup-add").addEventListener("click",()=>{let l=t.querySelector("#sec-asn-blacklist"),i=`AS${r.asn}`;l.value.split(`
`).some(d=>d.trim().toUpperCase()===i)||(l.value=l.value.trim()?`${l.value.replace(/\s+$/,"")}
${i}`:i),a.hide(),c.success(`${i} added to blacklist \u2014 save to apply`)})}catch(r){n.innerHTML="",c.error(r.message)}};document.getElementById("asn-lookup-go").addEventListener("click",s),document.getElementById("asn-lookup-q").addEventListener("keydown",o=>{o.key==="Enter"&&s()})}async function Rt(t){if(!q()){t.innerHTML=_("Access denied");return}t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${V(null)}`,lt(t),B(t)}function De(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Ht(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Ft(t){if(!q()){t.innerHTML=_("Access denied");return}let[e,a,s,o,n,r]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Ht(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async l=>{l.preventDefault();let i=l.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(l.target).get("admin_domain").trim()};try{await m.put("/settings",b),c.success("Settings saved"),Ht(b.admin_domain)}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("settings-import").addEventListener("change",async l=>{let i=l.target.files[0];if(!i)return;let d=new FormData;d.append("file",i);try{let u=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:d}),b=u.status===204?null:await u.json().catch(()=>null);if(!u.ok)throw new Error(b?.error||`HTTP ${u.status}`);c.success("Settings imported")}catch(u){c.error(u.message)}finally{l.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async l=>{l.preventDefault();let i=l.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={backup_schedule:u.get("backup_schedule").trim(),backup_retain_days:u.get("backup_retain_days").trim()};try{await m.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("s3-form").addEventListener("submit",async l=>{l.preventDefault();let i=l.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={s3_endpoint:u.get("s3_endpoint").trim(),s3_bucket:u.get("s3_bucket").trim(),s3_region:u.get("s3_region").trim(),s3_access_key:u.get("s3_access_key").trim()},k=u.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await m.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("smtp-form").addEventListener("submit",async l=>{l.preventDefault();let i=l.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={smtp_host:u.get("smtp_host").trim(),smtp_port:u.get("smtp_port").trim(),smtp_username:u.get("smtp_username").trim(),smtp_from:u.get("smtp_from").trim(),smtp_tls:u.get("smtp_tls")?"true":"false"},k=u.get("smtp_password").trim();k&&(b.smtp_password=k);try{await m.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("sns-form").addEventListener("submit",async l=>{l.preventDefault();let i=l.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={aws_access_key:u.get("aws_access_key").trim(),aws_region:u.get("aws_region").trim(),aws_sns_sender_id:u.get("aws_sns_sender_id").trim()},k=u.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await m.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("resource-form").addEventListener("submit",async l=>{l.preventDefault();let i=l.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={resource_ram_reserve_gb:u.get("resource_ram_reserve_gb").trim(),resource_poll_interval:u.get("resource_poll_interval").trim(),resource_throttle_pct:u.get("resource_throttle_pct").trim(),resource_webhook_url:u.get("resource_webhook_url").trim()};try{await m.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=d}})}function Nt(t){return`
        ...
        </div>`}function Ut(t,e){let a=Date.now()+18e5,s=setInterval(async()=>{try{let o=await m.get(`/sites/${e}/backups/restore-status`);(!o?.active||Date.now()>a)&&(clearInterval(s),x(),o?.active?c.error("Import timed out \u2014 check server logs"):c.success("Import complete"),await W(t,e))}catch{}},3e3)}async function W(t,e){try{let[a,s]=await Promise.all([m.get(`/sites/${e}/backup-repo`),m.get(`/sites/${e}/backups`)]),o=t.querySelector("#backup-local-enabled"),n=t.querySelector("#backup-s3-enabled");o&&(o.checked=!!a.LocalEnabled),n&&(n.checked=!!a.S3Enabled);let r=t.querySelector("#backup-error-banner");if(r)if(a.last_error){let i=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";r.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${i}:</strong> ${y(a.last_error)}</p>
                    </div>`}else r.innerHTML="";let l=t.querySelector("#backup-list-wrap");l&&(l.innerHTML=Re(s))}catch(a){let s=t.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${y(a.message)}</p>`)}}function Wt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await m.put(`/sites/${e}/backup-repo`,s),c.success("Backup destinations saved")}catch(o){c.error(o.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await m.get(`/sites/${e}/backups`))?.length??0}catch{}try{await m.post(`/sites/${e}/backups`,{label:"manual"})}catch(r){c.error(r.message);return}$("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let o=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await m.get(`/sites/${e}/backups`))?.length??0)>s||Date.now()>o)&&(clearInterval(n),x(),await W(t,e),Date.now()<=o?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let o=s.target.closest(".backup-restore-btn");if(o){let l=o.dataset.id;if(!await L("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await m.post(`/sites/${e}/backups/${l}/restore`)}catch(k){c.error(k.message);return}$("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let d=Date.now(),u=Date.now()+900*1e3,b=setInterval(async()=>{try{let k=await m.get(`/sites/${e}/backups/restore-status`);(!k?.active||Date.now()>u)&&(clearInterval(b),x(),k?.active?c.error("Restore timed out"):c.success("Restore complete"),await W(t,e))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let l=n.dataset.id;if(!await L("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;$("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await m.delete(`/sites/${e}/backups/${l}`),x(),c.success("Snapshot deleted"),await W(t,e)}catch(d){x(),c.error(d.message)}}let r=s.target.closest(".backup-download-btn");if(r){let l=r.dataset.id,i=`${Date.now().toString(36)}${Math.random().toString(36).slice(2,10)}`,d=`kp_dl_${i}`;$("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let u=document.createElement("a");u.href=`/api/sites/${e}/backups/${l}/download?dl=${i}`,u.style.display="none",document.body.appendChild(u),u.click(),document.body.removeChild(u);let b=Date.now(),k=setInterval(()=>{!document.cookie.split(";").some(v=>v.trim().startsWith(`${d}=`))&&Date.now()-b<18e5||(clearInterval(k),document.cookie=`${d}=; Path=/; Max-Age=0`,x())},500)},300);return}});let a=t.querySelector("#import-backup-modal");a&&(UIkit.util.on(a,"beforeshow",async()=>{let s=a.querySelector("#import-target-site");try{let n=await m.get("/sites"),r=Number(e);s.innerHTML=n.map(l=>`<option value="${l.ID}"${Number(l.ID)===r?" selected":""}>${l.Name}</option>`).join("")}catch{s.innerHTML='<option value="">Failed to load sites</option>'}let o=a.querySelector("#import-sftp-list");try{let n=await m.get(`/sites/${e}/backups/import/files`);!n||n.length===0?o.innerHTML='<p class="kp-muted uk-text-small">No files found.</p>':o.innerHTML=n.map(r=>`
                    <div class="uk-flex uk-flex-middle uk-flex-between uk-margin-small-bottom">
                        <span class="kp-mono uk-text-small">${y(r)}</span>
                        <button class="uk-button kp-btn-primary kp-btn-sm import-sftp-btn" data-file="${y(r)}">
                            Restore
                        </button>
                    </div>`).join("")}catch(n){o.innerHTML=`<p class="kp-muted uk-text-small">Failed to list files: ${y(n.message)}</p>`}}),a.querySelector("#import-upload-btn")?.addEventListener("click",async()=>{let s=a.querySelector("#import-file-input"),o=a.querySelector("#import-target-site")?.value;if(!s?.files?.length){c.error("Select an archive file first");return}let n=s.files[0],r=new FormData;r.append("archive",n),r.append("target_site_id",o),UIkit.modal(a).hide(),$("Importing Backup","Uploading and restoring \u2014 this may take several minutes.");try{await fetch(`/api/sites/${e}/backups/import/upload`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:r,credentials:"same-origin"}).then(async l=>{if(!l.ok){let i=await l.json().catch(()=>({}));throw new Error(i.error||`HTTP ${l.status}`)}})}catch(l){x(),c.error(l.message);return}Ut(t,e)}),a.querySelector("#import-sftp-list")?.addEventListener("click",async s=>{let o=s.target.closest(".import-sftp-btn");if(!o)return;let n=o.dataset.file,r=a.querySelector("#import-target-site")?.value;UIkit.modal(a).hide(),$("Importing from SFTP","Restoring archive \u2014 this may take several minutes.");try{await m.post(`/sites/${e}/backups/import/sftp`,{filename:n,target_site_id:parseInt(r,10)})}catch(l){x(),c.error(l.message);return}Ut(t,e)}))}function yt(){return`
        ...
        </div>`}async function K(t){let e=document.getElementById("basicauth-panel");if(e)try{let[a,s]=await Promise.all([m.get(`/sites/${t}/basicauth`),m.get(`/sites/${t}/basicauth/users`)]),o=e.querySelector("#ba-enabled"),n=e.querySelector("#ba-realm");o&&(o.checked=!!a.Enabled),n&&(n.value=a.Realm??"Restricted"),He(e,s??[])}catch(a){c.error("Failed to load basic auth settings: "+a.message)}}function He(t,e){let a=t.querySelector("#ba-users-list");if(a){if(!e.length){a.innerHTML='<p class="kp-muted uk-text-small">No credentials configured.</p>';return}a.innerHTML=e.map(s=>`
        ...
        <div>
            ...
            <div class="kp-config-grid cfg-rows" data-type="${e}">
                ${s.map(([o,n])=>X(o,n)).join("")}
            </div>
        </div>`}function jt(t,e){let a=e?.enabled==="true",s=e?Object.entries(e).filter(([o])=>o!=="enabled"):[];return`
        <div>
    ...
    </div>`}function Ot(t,e,a){t.addEventListener("click",s=>{if(s.target.closest(".cfg-add-row")){let o=s.target.closest(".cfg-add-row");t.querySelector(`.cfg-rows[data-type="${o.dataset.type}"]`).insertAdjacentHTML("beforeend",X())}},{signal:a}),t.addEventListener("click",s=>{s.target.closest(".cfg-del-row")&&s.target.closest(".kp-config-row").remove()},{signal:a}),t.addEventListener("click",async s=>{let o=s.target.closest(".cfg-save");if(!o)return;let{type:n,site:r}=o.dataset,l=t.querySelectorAll(`.cfg-rows[data-type="${n}"] .kp-config-row`),i={};if(l.forEach(d=>{let u=d.querySelector(".cfg-key").value.trim(),b=d.querySelector(".cfg-val").value.trim();u&&(i[u]=b)}),n==="5"){let d=t.querySelector(".varnish-enabled-toggle");i.enabled=d?.checked?"true":"false"}try{await m.put(`/sites/${r}/configs/${n}`,i),c.success(`${J[n]} config saved`)}catch(d){c.error(d.message)}},{signal:a}),t.addEventListener("click",async s=>{let o=s.target.closest(".cfg-reset");if(!o)return;let{type:n,site:r}=o.dataset;if(await L("Reset Config",`Reset ${J[n]} config to defaults?`))try{let i=await m.post(`/sites/${r}/configs/${n}/reset`),d=t.querySelector(`.cfg-rows[data-type="${n}"]`);d.innerHTML=Object.entries(i).map(([u,b])=>X(u,b)).join(""),c.success(`${J[n]} reset to defaults`)}catch(i){c.error(i.message)}},{signal:a}),t.addEventListener("change",async s=>{let o=s.target.closest(".cfg-import-input");if(!o)return;let{type:n,site:r}=o.dataset,l=o.files[0];if(!l)return;let i=new FormData;i.append("file",l);try{let d=await fetch(`/api/sites/${r}/configs/${n}/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:i}),u=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(u?.error||`HTTP ${d.status}`);let b=t.querySelector(`.cfg-rows[data-type="${n}"]`);b.innerHTML=Object.entries(u).map(([k,p])=>X(k,p)).join(""),c.success(`${J[n]} config imported`)}catch(d){c.error(d.message)}finally{o.value=""}},{signal:a})}function Vt(t){return`
        ...
        <tr>
            ...
            <td>
        ...
        </div>`}async function rt(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await m.get(`/sites/${e}/crons`);a.innerHTML=Kt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${y(s.message)}</p>`}}function Jt(t,e){let a=[],s=t.querySelector("#cron-modal"),o=t.querySelector("#cron-modal-title"),n=t.querySelector("#cron-modal-id"),r=t.querySelector("#cron-modal-label"),l=t.querySelector("#cron-modal-command"),i=t.querySelector("#cron-modal-schedule"),d=t.querySelector("#cron-schedule-preview"),u=t.querySelector("#cron-modal-enabled");i?.addEventListener("input",()=>{d.textContent=zt(i.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{o.textContent="Add Cron Job",n.value="",r.value="",l.value="",i.value="",d.textContent="",u.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let b=l.value.trim(),k=i.value.trim();if(!b||!k){c.error("Command and schedule are required");return}let p={label:r.value.trim(),command:b,schedule:k,enabled:u.checked},v=n.value;try{v?(await m.put(`/sites/${e}/crons/${v}`,p),c.success("Cron job updated")):(await m.post(`/sites/${e}/crons`,p),c.success("Cron job created")),UIkit.modal(s).hide(),await rt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async b=>{let k=b.target.closest(".cron-detail-btn");if(k){let h=k.dataset.id,w=a.find(R=>String(R.ID)===h);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let S=document.getElementById("cron-detail-modal");UIkit.modal(S).show(),S.addEventListener("hidden",()=>S.remove(),{once:!0});return}let p=b.target.closest(".cron-edit-btn");if(p){let h=p.dataset.id,w=a.find(S=>String(S.ID)===h);if(!w)return;o.textContent="Edit Cron Job",n.value=w.ID,r.value=w.Label||"",l.value=w.Command,i.value=w.Schedule,d.textContent=zt(w.Schedule),u.checked=w.Enabled,UIkit.modal(s).show();return}let v=b.target.closest(".cron-delete-btn");if(v){let h=v.dataset.id;if(!await L("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await m.delete(`/sites/${e}/crons/${h}`),c.success("Cron job deleted"),await rt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(S){c.error(S.message)}return}let g=b.target.closest(".cron-run-btn");if(g){let h=g.dataset.id;try{await m.post(`/sites/${e}/crons/${h}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await m.get(`/sites/${e}/crons`)).find(H=>String(H.ID)===h)?.LastRun??null}catch{}let S=Date.now()+300*1e3,R=setInterval(async()=>{try{let I=await m.get(`/sites/${e}/crons`),H=I.find(Y=>String(Y.ID)===h);if(!H||H.LastRun!==w||Date.now()>S){clearInterval(R),x(),a=I??[];let Y=t.querySelector("#cron-list-wrap");Y&&(Y.innerHTML=Kt(I)),H?.LastError?c.error(`Job failed: ${H.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let p=k.dataset.id;try{await m.patch(`/sites/${e}/crons/${p}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(v){c.error(v.message),k.checked=!k.checked}}),m.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function zt(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,o,n,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&o==="*"&&n==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&o==="*"&&n==="*"&&r!=="*"){let l=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(d=>l[parseInt(d)]??d).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function Zt(t){return`
        ...
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            ...
            <tbody>${t.map(s=>{let o=j(P,s.name),n=s.is_dir,r=Ne(s.name,n),l=n?`<a href="#" class="fm-nav" data-path="${y(o)}"><span uk-icon="icon: ${Xt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</a>`:`<span><span uk-icon="icon: ${Xt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</span>`;return`
            <tr data-path="${y(o)}" data-name="${y(s.name)}" data-dir="${n?1:0}" data-mode="${y(s.mode)}">
                ...
                <td class="uk-text-right uk-text-nowrap">
                    ${r?`<button class="kp-fm-act fm-edit" data-path="${y(o)}" uk-tooltip="Edit"><span uk-icon="icon: pencil; ratio: 0.85"></span></button>`:""}
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Z(),a&&(a.innerHTML=Fe());try{let s=await m.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=We(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Qt(t,e){P=e||"",C(t)}function Q(t,e,a=""){return new Promise(s=>{let o="fm-prompt-modal";document.getElementById(o)?.remove();let n=document.createElement("div");n.id=o,n.setAttribute("uk-modal",""),n.innerHTML=`
            ...
            </div>`,document.body.appendChild(n),window.UIkit&&UIkit.icon(n);let r=UIkit.modal(n),l=n.querySelector("#fm-prompt-input"),i=!1,d=u=>{i||(i=!0,s(u),r.hide())};n.querySelector("#fm-prompt-ok").addEventListener("click",()=>d(l.value.trim()||null)),l.addEventListener("keydown",u=>{u.key==="Enter"&&(u.preventDefault(),d(l.value.trim()||null))}),UIkit.util.on(n,"hidden",()=>{i||(i=!0,s(null)),n.remove()}),r.show(),setTimeout(()=>l.focus(),50)})}async function je(t,e){let a=j(P,e.name),s=await fetch(`/api/sites/${t}/files/upload?path=${encodeURIComponent(a)}`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:e}),o=s.status===204?null:await s.json().catch(()=>null);if(!s.ok)throw new Error(o?.error||`HTTP ${s.status}`)}function te(t,e){P="",t.addEventListener("click",a=>{let s=a.target.closest("#fm-breadcrumb a");if(s){a.preventDefault(),Qt(e,s.dataset.path);return}let o=a.target.closest(".fm-nav");if(o){a.preventDefault(),Qt(e,o.dataset.path);return}let n=a.target.closest(".fm-edit");if(n){a.preventDefault(),Ve(e,n.dataset.path,n.dataset.path.split("/").pop());return}}),t.querySelector("#fm-new-file")?.addEventListener("click",async()=>{let a=await Q("New File","File name");if(a)try{await m.post(`/sites/${e}/files/file`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-new-dir")?.addEventListener("click",async()=>{let a=await Q("New Folder","Folder name");if(a)try{await m.post(`/sites/${e}/files/dir`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-upload")?.addEventListener("change",async a=>{let s=[...a.target.files];if(s.length)try{for(let o of s)await je(e,o);c.success(s.length===1?"File uploaded":`${s.length} files uploaded`),C(e)}catch(o){c.error(o.message)}finally{a.target.value=""}}),t.querySelector("#fm-refresh")?.addEventListener("click",()=>C(e)),t.addEventListener("click",async a=>{let s=a.target.closest("tr[data-path]");if(!s)return;let o=s.dataset.path,n=s.dataset.name;if(a.target.closest(".fm-chmod")){let r=await Q("Permissions",`Octal mode for "${n}"`,s.dataset.mode);if(!r)return;try{await m.patch(`/sites/${e}/files/chmod`,{path:o,mode:r}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-rename")){let r=await Q("Rename / Move","New path (relative to current folder)",n);if(!r||r===n)return;try{await m.post(`/sites/${e}/files/move`,{src:o,dst:j(P,r)}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-copy")){let r=await Q("Copy","Destination name",n+"-copy");if(!r)return;try{await m.post(`/sites/${e}/files/copy`,{src:o,dst:j(P,r)}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-delete")){if(!await L("Delete",`Delete "${n}"? This cannot be undone.`))return;try{await m.delete(`/sites/${e}/files?path=${encodeURIComponent(o)}`),C(e)}catch(l){c.error(l.message)}return}})}var ct=null;function Yt(t){if(document.querySelector(`link[href="${t}"]`))return;let e=document.createElement("link");e.rel="stylesheet",e.href=t,document.head.appendChild(e)}function xt(t){return new Promise((e,a)=>{let s=document.querySelector(`script[src="${t}"]`);if(s){if(s.dataset.loaded)return e();s.addEventListener("load",()=>e()),s.addEventListener("error",()=>a(new Error("failed to load "+t)));return}let o=document.createElement("script");o.src=t,o.addEventListener("load",()=>{o.dataset.loaded="1",e()}),o.addEventListener("error",()=>a(new Error("failed to load "+t))),document.head.appendChild(o)})}function Oe(){if(ct)return ct;let t="https://cdn.jsdelivr.net/npm/codemirror@5";return Yt(`${t}/lib/codemirror.css`),Yt(`${t}/theme/material-darker.css`),ct=xt(`${t}/lib/codemirror.js`).then(()=>Promise.all([xt(`${t}/mode/meta.js`),xt(`${t}/addon/mode/loadmode.js`)])).then(()=>{window.CodeMirror.modeURL=`${t}/mode/%N/%N.js`}),ct}function ze(t){let e=window.CodeMirror.findModeByFileName(t);return e?e.mode:null}async function Ve(t,e,a){let s;try{s=await m.get(`/sites/${t}/files/content?path=${encodeURIComponent(e)}`)}catch(k){let p=/too large/i.test(k.message)?"File is too large to edit \u2014 download it instead.":/binary/i.test(k.message)?"Binary file \u2014 download it instead of editing.":k.message;c.error(p);return}try{await Oe()}catch(k){c.error("Editor failed to load: "+k.message);return}let o="fm-editor-modal";document.getElementById(o)?.remove();let n=document.createElement("div");n.id=o,n.setAttribute("uk-modal",""),n.innerHTML=`
        ...
        </div>`}function ee(t,e){let a=null,s=!1,o=t.querySelector("#log-output"),n=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),l=t.querySelector("#log-clear"),i=t.querySelector("#log-autoscroll"),d=t.querySelector("#log-status");function u(p){for(p.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");g.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=v,o.appendChild(g)});o.childElementCount>Ke;)o.removeChild(o.firstChild);i.checked&&(o.scrollTop=o.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,n.disabled=!1,r.disabled=!0,d&&(d.textContent="Disconnected")}n.addEventListener("click",()=>{b();let p=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",h=p==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:p==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:p==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:`${g}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${v}`;a=new WebSocket(h),a.onopen=()=>{s=!0,n.disabled=!0,r.disabled=!1,d&&(d.textContent=`Connected \u2014 ${p}`)},a.onmessage=w=>u(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,n.disabled=!1,r.disabled=!0,d&&(d.textContent="Disconnected")}}),r.addEventListener("click",b),l.addEventListener("click",()=>{o.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),n.click())});let k=f.go.bind(f);f.go=function(p,v={}){return a&&b(),k(p,v)}}function Je(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ae(t,e){try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Je(a.status))}catch{}}function se(t){t.forEach(e=>ae(e.Domain,e.ID))}function ne(t,e,a,s=0,o=null){let n=t.SiteType!==3&&t.PMAPort>0;return`
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                        <tbody>
                            ...
                            ${o?`<tr><td class="kp-muted">Parent</td><td><a href="javascript:void(0)" data-action="manage" data-id="${s}" style="color:var(--kp-cyan)">${o}</a></td></tr>`:""}
        ...
        </div>`}async function $t(t){let e=document.getElementById("waf-tab-panel");if(!e)return;e.innerHTML=Xe();let a=document.getElementById("waf-export-btn");a&&(a.href=`/api/sites/${t}/waf/export`);try{let s=await m.get(`/sites/${t}/waf`),o=document.getElementById("waf-override"),n=document.getElementById("waf-site-exclusions");o&&(o.value=String(s.Override??0)),n&&(n.value=s.Exclusions??"");let r=document.getElementById("waf-plugins-list");if(r){let[l,i]=await Promise.all([m.get("/settings/waf/plugins"),m.get(`/sites/${t}/waf/plugins`)]),d=new Set(i??[]);!l||l.length===0?r.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':window.matchMedia("(max-width: 959px)").matches?r.innerHTML=`
        ...
        </div>`}function ke(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),o=t.querySelector("#wpcli-run"),n=t.querySelector("#wpcli-clear"),r=t.querySelector("#wpcli-status"),l=[],i=-1;function d(k,p=""){k.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");p?g.className=p:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function u(k){if(k=k.trim(),!k)return;l.unshift(k),i=-1,d(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,o.disabled=!0,r&&(r.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let h=g.data;if(h.trim()==="[done]"){v.close();return}if(h.startsWith("[info]")){d(h,"kp-muted");return}if(h.startsWith("[error]")){d(h,"kp-log-line-err");return}d(h)},v.onerror=()=>{d("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,o.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}o.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){u(s.value),s.value="",i=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),i<l.length-1&&(i++,s.value=l[i]);return}k.key==="ArrowDown"&&(k.preventDefault(),i>0?(i--,s.value=l[i]):(i=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let v=p.indexOf("''")+1;s.setSelectionRange(v,v);return}u(p)})}),n.addEventListener("click",()=>{a.innerHTML=""});let b=f.go.bind(f);f.go=function(k,p={}){return b(k,p)},s.focus()}var O=null,M=null;function be(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),o=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(l,i=!1){UIkit.switcher(a).show(l),e.querySelectorAll(":scope > li[data-pill]").forEach(d=>d.classList.remove("kp-pill-active")),i?(s?.classList.add("kp-pill-active"),o?.querySelectorAll("a[data-switcher]").forEach(d=>{d.classList.toggle("kp-dd-active",parseInt(d.dataset.switcher,10)===l)})):(s?.classList.remove("kp-pill-active"),o?.querySelectorAll("a[data-switcher]").forEach(d=>d.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(l=>{l.addEventListener("click",i=>{i.preventDefault();let d=parseInt(l.closest("li").dataset.pill,10);n(d,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",l=>{l.stopPropagation(),o.hidden=!o.hidden,s.classList.toggle("kp-pill-active",!o.hidden)}),o?.querySelectorAll("a[data-switcher]").forEach(l=>{l.addEventListener("click",i=>{i.preventDefault(),o.hidden=!0,n(parseInt(l.dataset.switcher,10),!0)})}),document.addEventListener("click",l=>{o&&!s.contains(l.target)&&(o.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(1)}function Qe(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}async function Ye(t){let e=document.getElementById("rp-routes-list");if(e)try{let a=await m.get(`/sites/${t}/rp-routes`);e.innerHTML=a.length?a.map(s=>Lt(s.Domain,s.Upstream,s.PassHost)).join(""):Lt()}catch(a){c.error("Failed to load routes: "+a.message)}}function Ze(t,e){t.addEventListener("click",async a=>{if(a.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",Lt());return}if(a.target.closest(".rp-remove-row")){a.target.closest(".rp-route-row").remove();return}if(!a.target.closest("#rp-save-btn"))return;let s=a.target.closest("#rp-save-btn"),o=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let n=[...document.querySelectorAll(".rp-route-row")].map(r=>({Domain:r.querySelector('[data-field="domain"]').value.trim(),Upstream:r.querySelector('[data-field="upstream"]').value.trim(),PassHost:r.querySelector('[data-field="pass_host"]').checked})).filter(r=>r.Domain&&r.Upstream);try{await m.put(`/sites/${e}/rp-routes`,n),c.success("Routes saved")}catch(r){c.error(r.message)}finally{s.disabled=!1,s.innerHTML=o}},{signal:O.signal})}function ta(t){return t.endsWith("-nginx")?"world":t.endsWith("-php")?"code":t.endsWith("-db")?"database":t.endsWith("-redis")?"server":t.endsWith("-varnish")?"grid":t.endsWith("-pma")?"table":t.endsWith("-app")?"laptop":"bolt"}function ve(t){let e=t.split("-").pop();return{nginx:"Nginx",php:"PHP-FPM",db:"MariaDB",redis:"Redis",varnish:"Varnish",pma:"phpMyAdmin",app:"App"}[e]??e}function Et(t){switch(t){case"healthy":return"var(--kp-success)";case"unhealthy":return"var(--kp-danger)";case"starting":return"var(--kp-warning)";default:return"var(--kp-text-dim)"}}function ea(t){return!t||!t.length?"":t.filter(e=>!e.name.endsWith("-infra")).map(e=>`
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${ne(a,s??[],o,a.ParentID??0,l.find(p=>p.ID===a.ParentID)?.Name??null)}</li>
            <li>${vt(e,a.SiteType)}</li>
            <li>${G(e,1,r[1])}</li>
            ${i?`<li>${G(e,2,r[2])}</li>`:""}
            <li>${G(e,3,r[3])}</li>
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a);let s=UIkit.modal("#kp-backup-codes-modal");s.show(),document.getElementById("kp-backup-copy-btn").addEventListener("click",()=>{let o=t.join(`
`),n=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(o).then(()=>{n.textContent="Copied!"});else{let r=document.createElement("textarea");r.value=o,r.style.cssText="position:fixed;opacity:0",document.body.appendChild(r),r.select();try{document.execCommand("copy"),n.textContent="Copied!"}catch{}r.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),f.go("users")})}function he(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let a=UIkit.modal("#kp-create-user-modal");a.show(),document.getElementById("create-user-form").addEventListener("submit",async s=>{s.preventDefault();let o=s.target.querySelector('[type="submit"]'),n=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let r=new FormData(s.target),l={fname:r.get("fname").trim(),lname:r.get("lname").trim(),uname:r.get("uname").trim(),email:r.get("email").trim(),phone:r.get("phone").trim(),password:r.get("password"),role:parseInt(r.get("role")),notify_email:r.get("notify_email")==="on",notify_sms:r.get("notify_sms")==="on"};try{let i=F(await m.post("/users",l));document.getElementById("users-table-body").insertAdjacentHTML("beforeend",Tt(i)),c.success(`User '${i.uname}' created`),document.getElementById("create-user-form").style.display="none",document.getElementById("cu-totp-section").style.display="",sa(i.id,a)}catch(i){c.error(i.message),o.disabled=!1,o.innerHTML=n}}),document.getElementById("kp-create-user-modal").addEventListener("hidden",()=>document.getElementById("kp-create-user-modal")?.remove())}function sa(t,e){let a=()=>{e.hide(),document.getElementById("kp-create-user-modal")?.remove(),f.go("users")};document.getElementById("cu-totp-skip-btn").addEventListener("click",a),document.getElementById("cu-totp-setup-btn").addEventListener("click",async()=>{let s=document.getElementById("cu-totp-setup-btn");s.disabled=!0,s.textContent="Setting up\u2026";try{let o=await m.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=o.secret,document.getElementById("totp-setup-area").style.display="",document.getElementById("cu-totp-skip-btn").style.display="none",await dt(o.uri)}catch(o){c.error(o.message),s.disabled=!1,s.textContent="Enable TOTP"}}),document.getElementById("cu-totp-confirm-btn").addEventListener("click",async()=>{let s=document.getElementById("totp-confirm-code").value.trim();if(s.length!==6){c.error("Enter a 6-digit code");return}let o=document.getElementById("cu-totp-confirm-btn");o.disabled=!0;try{let n=await m.post(`/users/${t}/totp/confirm`,{code:s});e.hide(),document.getElementById("kp-create-user-modal")?.remove(),c.success("TOTP enabled"),n.backup_codes?.length?ut(n.backup_codes):f.go("users")}catch(n){c.error(n.message),o.disabled=!1}})}async function fe(t,e){document.getElementById("kp-edit-user-modal")?.remove();let a;try{a=F(await m.get(`/users/${e}`))}catch(d){c.error(d.message);return}let s=window.KP?.user?.role===99,o=`
