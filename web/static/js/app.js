"use strict";(()=>{var d={async _req(e,t,s,n=6e4){let a=new AbortController,i=setTimeout(()=>a.abort(),n),l={method:e,headers:{"Content-Type":"application/json"},signal:a.signal};s!==void 0&&(l.body=JSON.stringify(s));try{let r=await fetch("/api"+t,l);clearTimeout(i);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(i),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e)};var K=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',S=e=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let s=UIkit.modal("#kp-edit-site-modal"),n=document.getElementById("es-site-type"),a=document.getElementById("es-php-version-wrap"),i=document.getElementById("es-node-version-wrap"),l=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),c=document.getElementById("es-wordpress-wrap");s.show();let p=u=>{a.classList.toggle("uk-hidden",u!==1&&u!==2),i.classList.toggle("uk-hidden",u!==4),l.classList.toggle("uk-hidden",u!==5),r.classList.toggle("uk-hidden",u!==4&&u!==5),c.classList.toggle("uk-hidden",u!==1)};p(e.SiteType),n.addEventListener("change",()=>p(parseInt(n.value))),document.getElementById("edit-site-form").addEventListener("submit",async u=>{u.preventDefault();let v=u.target.querySelector('[type="submit"]'),k=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let g=new FormData(u.target),b=parseInt(g.get("site_type")),h=null;b===4&&(h=parseInt(g.get("node_version"))),b===5&&(h=parseInt(g.get("dotnet_version")));let x={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:b,runtime_version:h,start_command:g.get("start_command")?.trim()||""},ge=b===1?g.get("install_wordpress")==="on":!1;try{await d.put(`/sites/${e.ID}`,x),s.hide(),document.getElementById("kp-edit-site-modal")?.remove(),y("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:ge}),f(),o.success("Site updated and pod recreated")}catch(j){f(),o.error("Site saved but pod recreate failed: "+j.message)}m.go("site-detail",{id:String(e.ID)})}catch(j){o.error(j.message),v.disabled=!1,v.innerHTML=k}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function H(){document.body.insertAdjacentHTML("beforeend",`
...
`).map(h=>h.trim()).filter(Boolean),install_wordpress:k===1?v.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove(),y("Creating Site",`Setting up '${b.name}' \u2014 pulling images and provisioning containers...`);try{await d.post("/sites",b),f(),o.success(`Site '${b.name}' created`),m.go("sites")}catch(h){f(),o.error(h.message),p.disabled=!1,p.innerHTML=u}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function Q(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        </table>`}async function B(e,t){try{let[s,n]=await Promise.all([d.get(`/sites/${t}/backup-repo`),d.get(`/sites/${t}/backups`)]),a=e.querySelector("#backup-local-enabled"),i=e.querySelector("#backup-s3-enabled");a&&(a.checked=!!s.LocalEnabled),i&&(i.checked=!!s.S3Enabled);let l=e.querySelector("#backup-error-banner");if(l)if(s.last_error){let c=s.last_error_at?` (${new Date(s.last_error_at).toLocaleString()})`:"";l.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${c}:</strong> ${s.last_error}</p>
                    </div>`}else l.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=fe(n))}catch(s){let n=e.querySelector("#backup-list-wrap");n&&(n.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${s.message}</p>`)}}function Z(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,s),o.success("Backup destinations saved")}catch(n){o.error(n.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(i){o.error(i.message);return}y("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let n=Date.now()+10*60*1e3,a=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>s||Date.now()>n)&&(clearInterval(a),f(),await B(e,t),Date.now()<=n&&o.success("Backup complete"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let n=s.target.closest(".backup-restore-btn");if(n){let l=n.dataset.id;if(!await w("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${l}/restore`)}catch(v){o.error(v.message);return}y("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let c=Date.now(),p=Date.now()+15*60*1e3,u=setInterval(async()=>{try{let v=await d.get(`/sites/${t}/backups/restore-status`);(!v?.active||Date.now()>p)&&(clearInterval(u),f(),v?.active?o.error("Restore timed out"):o.success("Restore complete"),await B(e,t))}catch{}},3e3);return}let a=s.target.closest(".backup-delete-btn");if(a){let l=a.dataset.id;if(!await w("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;y("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${l}`),f(),o.success("Snapshot deleted"),await B(e,t)}catch(c){f(),o.error(c.message)}}let i=s.target.closest(".backup-download-btn");if(i){let l=i.dataset.id;y("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${l}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{f()},5e3)},300);return}})}var A={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function C(e,t,s){let n=s?Object.entries(s):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
        ...
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
                ...
                <div class="uk-flex" style="gap:8px">
                    ...
                    <button class="uk-button kp-btn-secondary kp-btn-sm cfg-save" data-type="5" data-site="${e}">
                        <span uk-icon="check"></span> Save
                    </button>
                    <button class="uk-button kp-btn-ghost kp-btn-sm cfg-reset" data-type="5" data-site="${e}">
                        <span uk-icon="refresh"></span> Reset
                    </button>
    ...
    </div>`}function te(e,t){e.addEventListener("click",s=>{if(s.target.closest(".cfg-add-row")){let n=s.target.closest(".cfg-add-row");e.querySelector(`.cfg-rows[data-type="${n.dataset.type}"]`).insertAdjacentHTML("beforeend",I())}}),e.addEventListener("click",s=>{s.target.closest(".cfg-del-row")&&s.target.closest(".kp-config-row").remove()}),e.addEventListener("click",async s=>{let n=s.target.closest(".cfg-save");if(!n)return;let{type:a,site:i}=n.dataset,l=e.querySelectorAll(`.cfg-rows[data-type="${a}"] .kp-config-row`),r={};if(l.forEach(c=>{let p=c.querySelector(".cfg-key").value.trim(),u=c.querySelector(".cfg-val").value.trim();p&&(r[p]=u)}),a==="5"){let c=e.querySelector(".varnish-enabled-toggle");r.enabled=c?.checked?"true":"false"}try{await d.put(`/sites/${i}/configs/${a}`,r),o.success(`${A[a]} config saved`)}catch(c){o.error(c.message)}}),e.addEventListener("click",async s=>{let n=s.target.closest(".cfg-reset");if(!n)return;let{type:a,site:i}=n.dataset;if(await w("Reset Config",`Reset ${A[a]} config to defaults?`))try{let r=await d.post(`/sites/${i}/configs/${a}/reset`),c=e.querySelector(`.cfg-rows[data-type="${a}"]`);c.innerHTML=Object.entries(r).map(([p,u])=>I(p,u)).join(""),o.success(`${A[a]} reset to defaults`)}catch(r){o.error(r.message)}}),e.addEventListener("change",async s=>{let n=s.target.closest(".cfg-import-input");if(!n)return;let{type:a,site:i}=n.dataset,l=n.files[0];if(!l)return;let r=new FormData;r.append("file",l);try{let c=await fetch(`/api/sites/${i}/configs/${a}/import`,{method:"POST",body:r}),p=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(p?.error||`HTTP ${c.status}`);let u=e.querySelector(`.cfg-rows[data-type="${a}"]`);u.innerHTML=Object.entries(p).map(([v,k])=>I(v,k)).join(""),o.success(`${A[a]} config imported`)}catch(c){o.error(c.message)}finally{n.value=""}})}function se(e,t){return`
        <div>
...
`).forEach(b=>{if(!b)return;let h=document.createElement("div");g?h.className=g:h.className=b.match(/error|fatal|critical/i)?"kp-log-line-err":b.match(/warning|warn/i)?"kp-log-line-warn":b.match(/success|done\]/i)?"kp-log-line-info":"",h.textContent=b,s.appendChild(h)}),s.scrollTop=s.scrollHeight}function u(k){if(k=k.trim(),!k)return;r.unshift(k),c=-1,p(`wp> ${k}`,"kp-log-line-info"),n.disabled=!0,a.disabled=!0,l&&(l.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",b=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);b.onopen=()=>{b.send(JSON.stringify({command:k}))},b.onmessage=h=>{let x=h.data;if(x.trim()==="[done]"){b.close();return}if(x.startsWith("[info]")){p(x,"kp-muted");return}if(x.startsWith("[error]")){p(x,"kp-log-line-err");return}p(x)},b.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},b.onclose=()=>{n.disabled=!1,a.disabled=!1,l&&(l.textContent="Ready"),n.focus()}}a.addEventListener("click",()=>{u(n.value),n.value=""}),n.addEventListener("keydown",k=>{if(k.key==="Enter"){u(n.value),n.value="",c=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),c<r.length-1&&(c++,n.value=r[c]);return}k.key==="ArrowDown"&&(k.preventDefault(),c>0?(c--,n.value=r[c]):(c=-1,n.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let g=k.dataset.cmd;if(g.startsWith("search-replace")){n.value=g,n.focus();let b=g.indexOf("''")+1;n.setSelectionRange(b,b);return}u(g)})}),i.addEventListener("click",()=>{s.innerHTML=""});let v=m.go.bind(m);m.go=function(k,g={}){return v(k,g)},n.focus()}async function ue(e,{id:t}){let{site:s,domains:n,sftp:a}=await d.get(`/sites/${t}`),i=await d.get(`/sites/${t}/configs`),l=s.SiteType===1||s.SiteType===2;e.innerHTML=`
        ...
        <ul class="uk-switcher">
            ...
            <li>${X(t)}</li>
        </ul>`,document.getElementById("sd-back").addEventListener("click",()=>m.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>q(s)),document.getElementById("sd-recreate").addEventListener("click",async()=>{y("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),f(),o.success("Pod recreated"),m.go("site-detail",{id:t})}catch(r){f(),o.error(r.message)}}),e.querySelectorAll("[data-action]").forEach(r=>{r.addEventListener("click",async()=>{let c=r.dataset.action;if(c==="flush"){try{await d.post(`/sites/${t}/flush`),o.success("Caches flushed")}catch(u){o.error(u.message)}return}y(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[c]??c} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${c}`),f(),o.success(`Site ${c} successful`),m.go("site-detail",{id:t})}catch(u){f(),o.error(u.message)}})}),te(e,t),re(e,t),ae(e,t),N(e),s.SiteType===1&&pe(e,t),L(e),ce(e,t),Z(e,t),B(e,t),ie(n??[])}async function U(e){let t=document.getElementById("totp-qr-img"),s=document.getElementById("totp-qr-wrap");if(!t||!s)return;if(s.querySelectorAll(".totp-uri-text").forEach(a=>a.remove()),typeof QRCode<"u")try{let a=await new Promise((i,l)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,c)=>{r?l(r):i(c)})});t.src=a,t.style.display="";return}catch{}let n=document.createElement("p");n.className="totp-uri-text kp-muted uk-text-small",n.style.wordBreak="break-all",n.textContent=e,s.appendChild(n)}function V(e){document.getElementById("kp-backup-codes-modal")?.remove();let s=`
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(a).then(()=>{i.textContent="Copied!"});else{let l=document.createElement("textarea");l.value=a,l.style.cssText="position:fixed;opacity:0",document.body.appendChild(l),l.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}l.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{n.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),m.go("users")})}function me(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a);let i=UIkit.modal("#kp-edit-user-modal");i.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let u=p.target.querySelector('[type="submit"]'),v=u.innerHTML;u.disabled=!0,u.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(p.target),g={fname:k.get("fname").trim(),lname:k.get("lname").trim(),email:k.get("email").trim(),phone:k.get("phone").trim()};if(n){g.role=parseInt(k.get("role"));let h=k.get("uname");h&&(g.uname=h.trim())}let b=k.get("password");b&&(g.password=b);try{await d.put(`/users/${t}`,g),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),o.success("User updated"),m.go("users")}catch(h){o.error(h.message),u.disabled=!1,u.innerHTML=v}});let l=document.getElementById("totp-setup-btn");l&&l.addEventListener("click",async()=>{l.disabled=!0,l.textContent="Setting up\u2026";try{let p=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await U(p.uri)}catch(p){o.error(p.message),l.disabled=!1,l.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){o.error("Enter a 6-digit code");return}r.disabled=!0;try{let u=await d.post(`/users/${t}/totp/confirm`,{code:p});i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),o.success("TOTP enabled"),u.backup_codes?.length?V(u.backup_codes):m.go("users")}catch(u){o.error(u.message),r.disabled=!1}});let c=document.getElementById("totp-disable-btn");c&&c.addEventListener("click",async()=>{c.disabled=!0;try{await d.delete(`/users/${t}/totp`),o.success("TOTP disabled"),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),m.go("users")}catch(p){o.error(p.message),c.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function be(e){if(!E()){e.innerHTML=S("Access denied");return}let t=await d.get("/users");e.innerHTML=`
        <div class="kp-view-header">
