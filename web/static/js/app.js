"use strict";(()=>{var d={async _req(e,t,a,s=6e4){let n=new AbortController,o=setTimeout(()=>n.abort(),s),i={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};a!==void 0&&(i.body=JSON.stringify(a));try{let r=await fetch("/api"+t,i);clearTimeout(o);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(o),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e),patch:(e,t)=>d._req("PATCH",e,t)};var se=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=e=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a),UIkit.modal("#kp-progress-modal").show()}function f(){let e=document.getElementById("kp-progress-modal");e&&(UIkit.modal(e).hide(),setTimeout(()=>e.remove(),300))}function U(e){return new Promise(t=>{let a="kp-clone-modal",s=`
            <div id="${a}" uk-modal>
...
`).map(S=>S.trim()).filter(Boolean),install_wordpress:k===1?g.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=k===6?`Setting up '${y.name}' as a reverse proxy...`:`Setting up '${y.name}' \u2014 pulling images and provisioning containers...`;x("Creating Site",w);try{await d.post("/sites",y),f(),l.success(`Site '${y.name}' created`),v.go("sites")}catch(S){f(),l.error(S.message),m.disabled=!1,m.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function ne(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        ${t.length===0?F("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>W())}function qe(e,t=[]){let a=e.Domains?.[0]??null,s=e.SiteType===6,n=e.ParentID>0?t.find(o=>o.ID===e.ParentID)??null:null;return`
            ...
            <div class="kp-site-card-header">
                <div>
                    ...
                    <div class="kp-site-meta">
                        ...
                        <span class="kp-site-meta-item"><span uk-icon="icon: code; ratio: 0.75"></span> ${q(e.SiteType)}${I(e)?" / "+I(e):""}</span>
        ...
        ${M(null)}`,O(e),T(e)}function Me(e){switch(e){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function re(e){let t=document.getElementById("admin-domain-ssl");if(!(!t||!e))try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`);t.outerHTML=Me(a.status)}catch{}}async function ce(e){if(!B()){e.innerHTML=L("Access denied");return}let[t,a,s,n]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf"),d.get("/settings/trusted-proxies")]);e.innerHTML=`
    <div class="kp-view-header">
    ...
    `,t.admin_domain&&re(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await d.put("/settings",p),l.success("Settings saved"),re(p.admin_domain)}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api/settings/import",{method:"POST",body:r}),p=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(p?.error||`HTTP ${c.status}`);l.success("Settings imported")}catch(c){l.error(c.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),p={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",p),l.success("Backup settings saved")}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),p={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},u=c.get("s3_secret_key").trim();u&&(p.s3_secret_key=u);try{await d.put("/settings/backup",p),l.success("S3 settings saved")}catch(m){l.error(m.message)}finally{i.disabled=!1,i.innerHTML=r}})}function de(e){return`
        ...
        </table>`}async function H(e,t){try{let[a,s]=await Promise.all([d.get(`/sites/${t}/backup-repo`),d.get(`/sites/${t}/backups`)]),n=e.querySelector("#backup-local-enabled"),o=e.querySelector("#backup-s3-enabled");n&&(n.checked=!!a.LocalEnabled),o&&(o.checked=!!a.S3Enabled);let i=e.querySelector("#backup-error-banner");if(i)if(a.last_error){let c=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";i.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${c}:</strong> ${a.last_error}</p>
                    </div>`}else i.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=He(s))}catch(a){let s=e.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function ue(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let a={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,a),l.success("Backup destinations saved")}catch(s){l.error(s.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let a=0;try{a=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(o){l.error(o.message);return}x("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let s=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>a||Date.now()>s)&&(clearInterval(n),f(),await H(e,t),Date.now()<=s?l.success("Backup complete"):l.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async a=>{let s=a.target.closest(".backup-restore-btn");if(s){let i=s.dataset.id;if(!await $("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${i}/restore`)}catch(m){l.error(m.message);return}x("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let c=Date.now(),p=Date.now()+900*1e3,u=setInterval(async()=>{try{let m=await d.get(`/sites/${t}/backups/restore-status`);(!m?.active||Date.now()>p)&&(clearInterval(u),f(),m?.active?l.error("Restore timed out"):l.success("Restore complete"),await H(e,t))}catch{}},3e3);return}let n=a.target.closest(".backup-delete-btn");if(n){let i=n.dataset.id;if(!await $("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;x("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${i}`),f(),l.success("Snapshot deleted"),await H(e,t)}catch(c){f(),l.error(c.message)}}let o=a.target.closest(".backup-download-btn");if(o){let i=o.dataset.id;x("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${i}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{f()},5e3)},300);return}})}var V={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function _(e,t,a){let s=a?Object.entries(a):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
        ...
        </div>`}function R(e="",t=""){return`<div class="kp-config-row">
        ...
        <tr>
            ...
            <td>
                ...
                </div>`);let S=document.getElementById("cron-detail-modal");UIkit.modal(S).show(),S.addEventListener("hidden",()=>S.remove(),{once:!0});return}let g=m.target.closest(".cron-edit-btn");if(g){let y=g.dataset.id,w=a.find(S=>String(S.ID)===y);if(!w)return;n.textContent="Edit Cron Job",o.value=w.ID,i.value=w.Label||"",r.value=w.Command,c.value=w.Schedule,p.textContent=ke(w.Schedule),u.checked=w.Enabled,UIkit.modal(s).show();return}let k=m.target.closest(".cron-delete-btn");if(k){let y=k.dataset.id;if(!await $("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await d.delete(`/sites/${t}/crons/${y}`),l.success("Cron job deleted"),await z(e,t),a=await d.get(`/sites/${t}/crons`)}catch(S){l.error(S.message)}return}let h=m.target.closest(".cron-run-btn");if(h){let y=h.dataset.id;try{await d.post(`/sites/${t}/crons/${y}/run`)}catch(E){l.error(E.message);return}x("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await d.get(`/sites/${t}/crons`)).find(C=>String(C.ID)===y)?.LastRun??null}catch{}let S=Date.now()+300*1e3,G=setInterval(async()=>{try{let E=await d.get(`/sites/${t}/crons`),C=E.find(N=>String(N.ID)===y);if(!C||C.LastRun!==w||Date.now()>S){clearInterval(G),f(),a=E??[];let N=e.querySelector("#cron-list-wrap");N&&(N.innerHTML=ge(E)),C?.LastError?l.error(`Job failed: ${C.LastError}`):l.success("Cron job complete")}}catch{}},2e3);return}}),e.querySelector("#cron-list-wrap")?.addEventListener("change",async m=>{let b=m.target.closest(".cron-toggle");if(!b)return;let g=b.dataset.id;try{await d.patch(`/sites/${t}/crons/${g}/toggle`,{enabled:b.checked}),l.success(b.checked?"Cron job enabled":"Cron job disabled")}catch(k){l.error(k.message),b.checked=!b.checked}}),d.get(`/sites/${t}/crons`).then(m=>{a=m??[]}).catch(()=>{})}function Z(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ke(e){if(!e)return"";let t=e.trim().split(/\s+/);if(t.length!==5)return"invalid expression";let[a,s,n,o,i]=t;if(e==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(p=>r[parseInt(p)]??p).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:e}function ee(e,t){return`
        <div>
        ...
        </div>`}function he(e,t){let a=null,s=!1,n=e.querySelector("#log-output"),o=e.querySelector("#log-connect"),i=e.querySelector("#log-disconnect"),r=e.querySelector("#log-clear"),c=e.querySelector("#log-autoscroll"),p=e.querySelector("#log-status");function u(g){g.split(`
`).forEach(k=>{if(!k)return;let h=document.createElement("div");h.className=k.match(/WAF BLOCK/i)?"kp-log-line-err":k.match(/WAF DETECT/i)?"kp-log-line-warn":k.match(/error|crit|emerg/i)?"kp-log-line-err":k.match(/warn/i)?"kp-log-line-warn":k.match(/info|notice/i)?"kp-log-line-info":"",h.textContent=k,n.appendChild(h)}),c.checked&&(n.scrollTop=n.scrollHeight)}function m(){a&&(a.close(),a=null),s=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}o.addEventListener("click",()=>{m();let g=e.querySelector("#log-container").value,k=e.querySelector("#log-tail").value,h=location.protocol==="https:"?"wss":"ws",y=g==="waf"?`${h}://${location.host}/api/sites/${t}/logs/waf?tail=${k}`:`${h}://${location.host}/api/sites/${t}/logs?container=${g}&tail=${k}`;a=new WebSocket(y),a.onopen=()=>{s=!0,o.disabled=!0,i.disabled=!1,p&&(p.textContent=`Connected \u2014 ${g}`)},a.onmessage=w=>u(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}}),i.addEventListener("click",m),r.addEventListener("click",()=>{n.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(m(),o.click())});let b=v.go.bind(v);v.go=function(g,k={}){return a&&m(),b(g,k)}}function Re(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function fe(e,t){try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),s=document.getElementById(`ssl-icon-${t}`);s&&(s.outerHTML=Re(a.status))}catch{}}function ye(e){e.forEach(t=>fe(t.Domain,t.ID))}function we(e,t,a,s=0,n=null){let o=e.SiteType!==3&&e.PMAPort>0;return`
...
`).forEach(k=>{if(!k)return;let h=document.createElement("div");g?h.className=g:h.className=k.match(/error|fatal|critical/i)?"kp-log-line-err":k.match(/warning|warn/i)?"kp-log-line-warn":k.match(/success|done\]/i)?"kp-log-line-info":"",h.textContent=k,a.appendChild(h)}),a.scrollTop=a.scrollHeight}function u(b){if(b=b.trim(),!b)return;r.unshift(b),c=-1,p(`wp> ${b}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,i&&(i.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",k=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);k.onopen=()=>{k.send(JSON.stringify({command:b}))},k.onmessage=h=>{let y=h.data;if(y.trim()==="[done]"){k.close();return}if(y.startsWith("[info]")){p(y,"kp-muted");return}if(y.startsWith("[error]")){p(y,"kp-log-line-err");return}p(y)},k.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},k.onclose=()=>{s.disabled=!1,n.disabled=!1,i&&(i.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",b=>{if(b.key==="Enter"){u(s.value),s.value="",c=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),c<r.length-1&&(c++,s.value=r[c]);return}b.key==="ArrowDown"&&(b.preventDefault(),c>0?(c--,s.value=r[c]):(c=-1,s.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let g=b.dataset.cmd;if(g.startsWith("search-replace")){s.value=g,s.focus();let k=g.indexOf("''")+1;s.setSelectionRange(k,k);return}u(g)})}),o.addEventListener("click",()=>{a.innerHTML=""});let m=v.go.bind(v);v.go=function(b,g={}){return m(b,g)},s.focus()}var A=null;function Ae(){return`
        ...
        </div>`}async function je(e){let t=document.getElementById("rp-routes-list");if(t)try{let a=await d.get(`/sites/${e}/rp-routes`);t.innerHTML=a.length?a.map(s=>te(s.Domain,s.Upstream)).join(""):te()}catch(a){l.error("Failed to load routes: "+a.message)}}function We(e,t){e.addEventListener("click",async a=>{if(a.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",te());return}if(a.target.closest(".rp-remove-row")){a.target.closest(".rp-route-row").remove();return}if(!a.target.closest("#rp-save-btn"))return;let s=a.target.closest("#rp-save-btn"),n=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=[...document.querySelectorAll(".rp-route-row")].map(i=>({Domain:i.querySelector('[data-field="domain"]').value.trim(),Upstream:i.querySelector('[data-field="upstream"]').value.trim()})).filter(i=>i.Domain&&i.Upstream);try{await d.put(`/sites/${t}/rp-routes`,o),l.success("Routes saved")}catch(i){l.error(i.message)}finally{s.disabled=!1,s.innerHTML=n}},{signal:A.signal})}async function Pe(e,{id:t}){let[{site:a,domains:s,sftp:n},o,i]=await Promise.all([d.get(`/sites/${t}`),d.get("/sites"),d.get(`/sites/${t}/configs`)]),r=Array.isArray(o)?o:[],c=a.SiteType===1||a.SiteType===2,p=a.SiteType===6,u=[1,2,4,5].includes(a.SiteType);if(e.innerHTML=`
        <div class="kp-view-header">
        ...
        <ul class="uk-switcher">
            ...
            <li>${de(t)}</li>
            ${u?`<li>${be(t)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>v.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>j(a)),document.getElementById("sd-site-nav")?.addEventListener("change",m=>{v.go("site-detail",{id:m.target.value})}),O(e),T(e),he(e,t),Fe(e,t),Te(t),p){We(e,t),je(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{x("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),f(),l.success("Pod recreated"),v.go("site-detail",{id:t})}catch(m){f(),l.error(m.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let m=await U(a.Name);if(m){x("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${t}/clone`,{name:m}),f(),l.success(`Site cloned as '${m}'`),v.go("sites")}catch(b){f(),l.error(b.message)}}}),e.querySelectorAll("[data-action]").forEach(m=>{m.addEventListener("click",async()=>{let b=m.dataset.action;if(b==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(k){l.error(k.message)}return}x(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${b}`),f(),l.success(`Site ${b} successful`),v.go("site-detail",{id:t})}catch(k){f(),l.error(k.message)}})}),me(e,t),Se(e,t),a.SiteType===1&&Le(e,t),$e(e,t,a),ue(e,t),H(e,t),u&&(ve(e,t),z(e,t)),Ne(e),ye(s??[])}async function J(e){let t=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!t||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((o,i)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,c)=>{r?i(r):o(c)})});t.src=n,t.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=e,a.appendChild(s)}function K(e){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=n,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),v.go("users")})}function Ie(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>Ie(e)),Ve(e)}function ae(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
    ...
    </tr>`}function Ve(e){e.addEventListener("click",async t=>{let a=t.target.closest('[data-action="delete-user"]');if(!(!a||!await $("Delete User","Delete this user? This cannot be undone.")))try{await d.delete(`/users/${a.dataset.uid}`),a.closest("tr").remove(),l.success("User deleted")}catch(n){l.error(n.message)}}),e.addEventListener("click",async t=>{let a=t.target.closest('[data-action="edit-user"]');a&&Ce(e,a.dataset.uid)})}v.register("dashboard",e=>ie(e));v.register("sites",e=>ne(e));v.register("site-detail",(e,t)=>Pe(e,t));v.register("users",e=>Be(e));v.register("settings",e=>ce(e));v.register("security",e=>le(e));document.addEventListener("click",e=>{let t=e.target.closest("[data-view]");if(!t)return;e.preventDefault(),v.go(t.dataset.view);let a=document.getElementById("kp-offcanvas");a&&UIkit.offcanvas(a).hide()});document.addEventListener("click",async e=>{let t=e.target.closest("[data-action]");if(!t)return;e.stopPropagation();let{action:a,id:s}=t.dataset;switch(a){case"manage":v.go("site-detail",{id:s});break;case"start":await Q(s,"start","Starting Site","Starting all containers - please wait...");break;case"stop":await Q(s,"stop","Stopping Site","Gracefully stopping all containers - please wait...");break;case"restart":await Q(s,"restart","Restarting Site","Restarting all containers - please wait...");break;case"flush":await Q(s,"flush","Flushing Caches","Clearing container caches - please wait...");break;case"edit":{let n=await d.get(`/sites/${s}`);j(n.site);break}case"clone":{let n=await U(t.dataset.name??s);if(!n)break;x("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{let o=await d.post(`/sites/${s}/clone`,{name:n}),i=!1,r=0;for(;!i&&r<60;)await new Promise(p=>setTimeout(p,3e3)),i=(await d.get("/sites")).some(p=>p.ID===o.id&&p.SiteStatus===1),r++;f(),i?(l.success(`Site cloned as '${n}'`),v.go("sites")):l.error("Clone timed out \u2014 check container logs")}catch(o){f(),l.error(o.message)}break}case"delete":await ze(s);break;case"recreate":x("Recreating Pod","Recreating containers for this site - this may take a few minutes...");try{await d.post(`/sites/${s}/recreate`),f(),l.success("Pod recreated")}catch(n){f(),l.error(n.message)}break}});async function Q(e,t,a,s){x(a,s);try{await d.post(`/sites/${e}/${t}`),f(),l.success(a+" complete")}catch(n){f(),l.error(n.message)}}async function ze(e){if(!await $("Delete Site","This will stop and permanently remove the pod and all its data. Are you sure?"))return;x("Deleting Site","Stopping containers and removing the pod - please wait...");try{await d.delete(`/sites/${e}`)}catch{}let a=!1,s=0;for(;!a&&s<10;){try{await new Promise(o=>setTimeout(o,2e3)),a=!(await d.get("/sites")).find(o=>o.ID===parseInt(e))}catch{}s++}f(),a?(l.success("Site deleted"),v.go("sites")):l.error("Delete failed - site still exists after 20s")}window.addEventListener("hashchange",()=>{if(v._ownHashChange)return;let{view:e,params:t}=X();v.go(e,t)});var{view:Je,params:Ke}=X();v.go(Je,Ke);})();
