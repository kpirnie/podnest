...
"use strict";(()=>{var u={async _req(t,e,a,s=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:n.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let o=await fetch("/api"+e,r);clearTimeout(i);let l=o.status===204?null:await o.json().catch(()=>null);if(o.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!o.ok)throw new Error(l?.error||`HTTP ${o.status}`);return l}catch(o){throw clearTimeout(i),o}},get:t=>u._req("GET",t),post:(t,e,a)=>u._req("POST",t,e,a),put:(t,e,a)=>u._req("PUT",t,e,a),delete:t=>u._req("DELETE",t),patch:(t,e)=>u._req("PATCH",t,e)};var $=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),Y=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',T=t=>`<div class="kp-empty">
        ...
        </div>`}function Se(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),n=t.querySelector("#admin-log-connect"),i=t.querySelector("#admin-log-disconnect"),r=t.querySelector("#admin-log-clear"),o=t.querySelector("#admin-log-autoscroll"),l=t.querySelector("#admin-log-status");function m(k){for(k.split(`
...
`).map(x=>x.trim()).filter(Boolean),install_wordpress:g===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=g===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;S("Creating Site",w);try{await u.post("/sites",f,6e5),y(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){y(),c.error(x.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function B(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}function _e(t){return`${t.toFixed(1)}%`}var nt=null;function bt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function gt(t,e){return`
        ...
        ${O(null)}`,ot(t),q(t)}function De(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Ht(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await u.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Rt(t){if(!I()){t.innerHTML=T("Access denied");return}let[e,a,s,n,i,r]=await Promise.all([u.get("/settings"),u.get("/settings/backup"),u.get("/settings/waf"),u.get("/settings/trusted-proxies"),u.get("/settings/notifications"),u.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Ht(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await u.put("/settings",b),c.success("Settings saved"),Ht(b.admin_domain)}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("settings-import").addEventListener("change",async o=>{let l=o.target.files[0];if(!l)return;let m=new FormData;m.append("file",l);try{let d=await fetch("/api/settings/import",{method:"POST",body:m}),b=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(b?.error||`HTTP ${d.status}`);c.success("Settings imported")}catch(d){c.error(d.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={backup_schedule:d.get("backup_schedule").trim(),backup_retain_days:d.get("backup_retain_days").trim()};try{await u.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={s3_endpoint:d.get("s3_endpoint").trim(),s3_bucket:d.get("s3_bucket").trim(),s3_region:d.get("s3_region").trim(),s3_access_key:d.get("s3_access_key").trim()},k=d.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await u.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("smtp-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={smtp_host:d.get("smtp_host").trim(),smtp_port:d.get("smtp_port").trim(),smtp_username:d.get("smtp_username").trim(),smtp_from:d.get("smtp_from").trim(),smtp_tls:d.get("smtp_tls")?"true":"false"},k=d.get("smtp_password").trim();k&&(b.smtp_password=k);try{await u.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("sns-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={aws_access_key:d.get("aws_access_key").trim(),aws_region:d.get("aws_region").trim(),aws_sns_sender_id:d.get("aws_sns_sender_id").trim()},k=d.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await u.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("resource-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={resource_ram_reserve_gb:d.get("resource_ram_reserve_gb").trim(),resource_poll_interval:d.get("resource_poll_interval").trim(),resource_throttle_pct:d.get("resource_throttle_pct").trim(),resource_webhook_url:d.get("resource_webhook_url").trim()};try{await u.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=m}})}function Ut(t){return`
        ...
        </div>`}async function z(t){let e=document.getElementById("basicauth-panel");if(e)try{let[a,s]=await Promise.all([u.get(`/sites/${t}/basicauth`),u.get(`/sites/${t}/basicauth/users`)]),n=e.querySelector("#ba-enabled"),i=e.querySelector("#ba-realm");n&&(n.checked=!!a.Enabled),i&&(i.value=a.Realm??"Restricted"),He(e,s??[])}catch(a){c.error("Failed to load basic auth settings: "+a.message)}}function He(t,e){let a=t.querySelector("#ba-users-list");if(a){if(!e.length){a.innerHTML='<p class="kp-muted uk-text-small">No credentials configured.</p>';return}a.innerHTML=e.map(s=>`
        ...
        <div class="kp-card uk-padding uk-margin-top" id="fm-root" data-site="${t}">
            ...
            <div id="fm-list"></div>
        </div>`}function Re(){let t=C?C.split("/"):[],e="",a=['<a href="#" data-path="">html</a>'];for(let s of t)e=e?e+"/"+s:s,a.push(`<span class="kp-fm-sep">/</span><a href="#" data-path="${$(e)}">${$(s)}</a>`);return a.join("")}function Fe(t,e){if(e)return"\u2014";let a=["B","KB","MB","GB","TB"],s=t,n=0;for(;s>=1024&&n<a.length-1;)s/=1024,n++;return`${n===0?s:s.toFixed(1)} ${a[n]}`}function Ue(t){let e=new Date(t);return isNaN(e)?"":e.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Jt(t){return t==="d"?"folder":t==="l"?"link":"file-text"}function j(t,e){return t?t+"/"+e:e}var Gt=new Set(["php","js","jsx","ts","tsx","css","scss","sass","less","html","htm","xml","json","txt","md","markdown","yml","yaml","ini","conf","cnf","toml","env","sh","bash","sql","log","csv","tsv","svg","htaccess","gitignore","lock","map"]);function Ne(t,e){if(e)return!1;let a=t.lastIndexOf(".");return t.startsWith(".")&&a===0?Gt.has(t.slice(1).toLowerCase()):a>=0&&Gt.has(t.slice(a+1).toLowerCase())}function je(t){if(!t||!t.length)return R("folder","This folder is empty");let e=document.getElementById("fm-root").dataset.site;return`
...
`).forEach(g=>{if(!g)return;let v=document.createElement("div");v.className=g.match(/WAF BLOCK/i)?"kp-log-line-err":g.match(/WAF DETECT/i)?"kp-log-line-warn":g.match(/error|crit|emerg/i)?"kp-log-line-err":g.match(/warn/i)?"kp-log-line-warn":g.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=g,n.appendChild(v)});n.childElementCount>Ke;)n.removeChild(n.firstChild);l.checked&&(n.scrollTop=n.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,i.disabled=!1,r.disabled=!0,m&&(m.textContent="Disconnected")}i.addEventListener("click",()=>{b();let p=t.querySelector("#log-container").value,g=t.querySelector("#log-tail").value,v=location.protocol==="https:"?"wss":"ws",f=p==="waf"?`${v}://${location.host}/api/sites/${e}/logs/waf?tail=${g}`:p==="proxy"?`${v}://${location.host}/api/sites/${e}/logs/proxy?tail=${g}`:p==="access"?`${v}://${location.host}/api/sites/${e}/logs/proxy?tail=${g}`:`${v}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${g}`;a=new WebSocket(f),a.onopen=()=>{s=!0,i.disabled=!0,r.disabled=!1,m&&(m.textContent=`Connected \u2014 ${p}`)},a.onmessage=w=>d(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,i.disabled=!1,r.disabled=!0,m&&(m.textContent="Disconnected")}}),r.addEventListener("click",b),o.addEventListener("click",()=>{n.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),i.click())});let k=h.go.bind(h);h.go=function(p,g={}){return a&&b(),k(p,g)}}function Je(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ee(t,e){try{let a=await u.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Je(a.status))}catch{}}function ae(t){t.forEach(e=>ee(e.Domain,e.ID))}function se(t,e,a,s=0,n=null){let i=t.SiteType!==3&&t.PMAPort>0;return`
        ...
        </div>`}function ue(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),n=t.querySelector("#wpcli-run"),i=t.querySelector("#wpcli-clear"),r=t.querySelector("#wpcli-status"),o=[],l=-1;function m(k,p=""){k.split(`
