...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let r=new AbortController,n=setTimeout(()=>r.abort(),s),l={method:t,headers:{"Content-Type":"application/json"},signal:r.signal};t!=="GET"&&t!=="HEAD"&&(l.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(l.body=JSON.stringify(a));try{let o=await fetch("/api"+e,l);clearTimeout(n);let i=o.status===204?null:await o.json().catch(()=>null);if(o.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!o.ok)throw new Error(i?.error||`HTTP ${o.status}`);return i}catch(o){throw clearTimeout(n),o}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var y=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function E(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let r=UIkit.modal(`#${a}`),n=document.getElementById("kp-clone-name"),l=document.getElementById("kp-clone-ok"),o=document.getElementById("kp-clone-cancel"),i=d=>{r.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(d)};l.addEventListener("click",()=>i(n.value.trim()||null),{once:!0}),o.addEventListener("click",()=>i(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>i(null),{once:!0}),r.show(),setTimeout(()=>n.focus(),150),n.addEventListener("keydown",d=>{d.key==="Enter"&&l.click()})})}function Tt(t){return new Promise(e=>{let a="kp-rename-modal",s=`
            <div id="${a}" uk-modal>
                <div class="uk-modal-dialog kp-modal uk-modal-body" style="max-width:420px">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let r=UIkit.modal(`#${a}`),n=document.getElementById("kp-rename-name"),l=document.getElementById("kp-rename-ok"),o=document.getElementById("kp-rename-cancel"),i=d=>{r.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(d)};l.addEventListener("click",()=>i(n.value.trim()||null),{once:!0}),o.addEventListener("click",()=>i(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>i(null),{once:!0}),r.show(),setTimeout(()=>n.focus(),150),n.addEventListener("keydown",d=>{d.key==="Enter"&&l.click()})})}function ut(t,e,a){return new Promise(s=>{let r="kp-sync-modal",n=t==="pull",l=n?"Pull From Parent":"Push To Parent",o=n?"cloud-download":"cloud-upload",i=n?a:e,d=n?e:a,u=`
            <div id="${r}" uk-modal>
                <div class="uk-modal-dialog kp-modal uk-modal-body" style="max-width:460px">
                    ...
                    <p class="kp-muted uk-text-small uk-margin-small-bottom">
                        ...
                        <strong>${d}</strong> with data from <strong>${i}</strong>.
                    ...
                    <div class="uk-flex uk-flex-right uk-margin-top" style="gap:8px">
                        ...
                        <button class="uk-button kp-btn-primary" id="kp-sync-ok">
                            <span uk-icon="${o}"></span> ${l}
                        </button>
                    </div>
                </div>
            </div>`;document.body.insertAdjacentHTML("beforeend",u);let b=UIkit.modal(`#${r}`),k=document.getElementById("kp-sync-ok"),p=document.getElementById("kp-sync-cancel"),v=g=>{b.hide(),setTimeout(()=>document.getElementById(r)?.remove(),300),s(g)};k.addEventListener("click",()=>v(!0),{once:!0}),p.addEventListener("click",()=>v(!1),{once:!0}),document.getElementById(r).addEventListener("hidden",()=>v(!1),{once:!0}),b.show()})}var f={routes:{},_ownHashChange:!1,register(t,e){this.routes[t]=e},async go(t,e={}){let a=Object.keys(e).length?t+"/"+Object.values(e).join("/"):t;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)}),document.querySelectorAll(".kp-bn-item[data-view]").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)});let s=this.routes[t];if(!s)return;let r=document.getElementById("kp-view");r.innerHTML=Z();try{await s(r,e)}catch(n){r.innerHTML=_(n.message)}}};function pt(){let e=(window.location.hash.replace("#","")||"dashboard").split("/"),a=e[0],s={};return a==="site-detail"&&e[1]&&(s.id=e[1]),{view:a,params:s}}var c={show(t,e="info",a=7e3){let s={success:"check",error:"warning",info:"info"},r=document.createElement("div");r.className=`kp-toast kp-toast-${e}`,r.innerHTML=`<span uk-icon="${s[e]||"info"}"></span><span>${y(t)}</span>`,document.getElementById("kp-toasts").appendChild(r),UIkit.icon(r.querySelector("[uk-icon]")),setTimeout(()=>r.remove(),a)},success:t=>c.show(t,"success"),error:t=>c.show(t,"error"),info:t=>c.show(t,"info")};var Se=2e3;function $e(){return`
        ...
        </div>`}function Le(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),r=t.querySelector("#admin-log-connect"),n=t.querySelector("#admin-log-disconnect"),l=t.querySelector("#admin-log-clear"),o=t.querySelector("#admin-log-autoscroll"),i=t.querySelector("#admin-log-status");function d(k){for(k.split(`
`).forEach(p=>{if(!p)return;let v=document.createElement("div");v.className=p.match(/WAF BLOCK/i)?"kp-log-line-err":p.match(/WAF DETECT/i)?"kp-log-line-warn":p.match(/error|crit|emerg/i)?"kp-log-line-err":p.match(/warn/i)?"kp-log-line-warn":p.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=p,s.appendChild(v)});s.childElementCount>Se;)s.removeChild(s.firstChild);o.checked&&(s.scrollTop=s.scrollHeight)}function u(){e&&(e.close(),e=null),a=!1,r.disabled=!1,n.disabled=!0,i&&(i.textContent="Disconnected")}r.addEventListener("click",()=>{u();let k=t.querySelector("#admin-log-source").value,p=t.querySelector("#admin-log-tail").value,v=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${v}://${location.host}/api/logs/waf?tail=${p}`:`${v}://${location.host}/api/logs/proxy?tail=${p}`;e=new WebSocket(g),e.onopen=()=>{a=!0,r.disabled=!0,n.disabled=!1,i&&(i.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=h=>d(h.data),e.onerror=()=>{},e.onclose=()=>{a=!1,r.disabled=!1,n.disabled=!0,i&&(i.textContent="Disconnected")}}),n.addEventListener("click",u),l.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(u(),r.click())});let b=f.go.bind(f);f.go=function(k,p={}){return e&&u(),b(k,p)}}function _t(t){t.innerHTML=$e(),Le(t)}var at=50;function Ee(t,e,a){let s=Math.max(1,Math.ceil(t.total/at)),r=(t.entries??[]).map(Ct).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        ...
        </div>`}function Ct(t){let e=new Date(t.ts).toLocaleString(),a=t.username?`<span style="font-family:monospace">${T(t.username)}</span>`:'<span style="color:var(--kp-text-dim)">\u2014</span>',s=Te(t.status),n=t.prior_state||t.new_state?`<button class="uk-button kp-btn-ghost kp-btn-sm al-diff-btn"
        ...
        ${s}
    </div>`}function Te(t){return`<span class="kp-badge ${t>=500?"kp-badge-error":t>=400?"kp-badge-warn":t>=300?"kp-badge-info":"kp-badge-ok"}">${t}</span>`}var T=t=>y(t??"");async function It(t,e){let a=new URLSearchParams({page:e,page_size:at});return t.username&&a.set("username",t.username),t.action&&a.set("action",t.action),t.target_type&&a.set("target_type",t.target_type),t.date_from&&a.set("date_from",t.date_from),t.date_to&&a.set("date_to",t.date_to),t.auth!==""&&a.set("auth",t.auth),m.get(`/audit?${a}`)}function _e(t){return{username:t.querySelector("#al-filter-user").value.trim(),action:t.querySelector("#al-filter-action").value.trim(),target_type:t.querySelector("#al-filter-target").value.trim(),date_from:t.querySelector("#al-filter-date-from").value,date_to:t.querySelector("#al-filter-date-to").value,auth:t.querySelector("#al-filter-auth").value}}async function Ce(t,e,a){async function s(l,o){let i=await It(l,o);t.querySelector("#al-table-body").innerHTML=(i.entries??[]).map(Ct).join("")||'<tr><td colspan="8" class="uk-text-center kp-text-dim">No records found</td></tr>';let d=Math.max(1,Math.ceil(i.total/at)),u=t.querySelector("#al-pager");u&&(u.innerHTML=d>1?Pt(o,d):""),t.querySelector("#al-record-count").textContent=`${i.total} record${i.total!==1?"s":""}`,r(t,l,o,d),e=l,a=o}function r(l,o,i,d){l.querySelector("#al-prev")?.addEventListener("click",()=>s(o,i-1)),l.querySelector("#al-next")?.addEventListener("click",()=>s(o,i+1))}t.querySelector("#al-filter-apply")?.addEventListener("click",()=>{s(_e(t),1)}),t.querySelector("#al-filter-clear")?.addEventListener("click",()=>{["al-filter-user","al-filter-action","al-filter-target","al-filter-date-from","al-filter-date-to"].forEach(l=>{let o=t.querySelector(`#${l}`);o&&(o.value="")}),t.querySelector("#al-filter-auth").value="",s({username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},1)});let n=Math.max(1,Math.ceil(parseInt(t.querySelector("#al-record-count")?.textContent??"0")/at));r(t,e,a,n),t.querySelector("#audit-log-panel")?.addEventListener("click",l=>{let o=l.target.closest(".al-diff-btn");if(!o)return;l.preventDefault(),l.stopPropagation();let i=o.dataset.prior??"",d=o.dataset.new??"",u="";i&&d?u=`=== BEFORE ===
`+et(i)+`

=== AFTER ===
`+et(d):d?u=et(d):u=et(i),document.body.insertAdjacentHTML("beforeend",`
            ...
            </div>`);let b=document.getElementById("al-diff-modal-inst");UIkit.modal(b).show(),b.addEventListener("hidden",()=>b.remove(),{once:!0})})}function et(t){try{return JSON.stringify(JSON.parse(t),null,2)}catch{return t}}async function qt(t){if(!q()){t.innerHTML=_("Access denied");return}let e={username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},a=await It(e,1);t.innerHTML=Ee(a,e,1),Ce(t,e,1)}function st(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),r=document.getElementById("cs-node-version-wrap"),n=document.getElementById("cs-dotnet-version-wrap"),l=document.getElementById("cs-start-command-wrap"),o=document.getElementById("cs-wordpress-wrap");e.show();let i=document.getElementById("cs-domains-wrap"),d=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let u=parseInt(a.value);s.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),r.classList.toggle("uk-hidden",u!==4),n.classList.toggle("uk-hidden",u!==5),l.classList.toggle("uk-hidden",u!==4&&u!==5),o.classList.toggle("uk-hidden",u!==1||u===6),i.classList.toggle("uk-hidden",u===6),d.classList.toggle("uk-hidden",u!==6)}),document.getElementById("create-site-form").addEventListener("submit",async u=>{u.preventDefault();let b=u.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(u.target),v=parseInt(p.get("site_type")),g=null;v===4&&(g=parseInt(p.get("node_version"))),v===5&&(g=parseInt(p.get("dotnet_version")));let h={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:v,runtime_version:g,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(S=>S.trim()).filter(Boolean),install_wordpress:v===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${h.name}' as a reverse proxy...`:`Setting up '${h.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",w);try{await m.post("/sites",h,6e5),x(),c.success(`Site '${h.name}' created`),f.go("sites")}catch(S){x(),c.error(S.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function Pe(t){return`${t.toFixed(1)}%`}var nt=null;function mt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function kt(t,e){return`
        ...
        </table>`}function Be(t,e,a,s,r){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let n=[...t].sort((b,k)=>{let p,v;switch(a){case"time":p=b.time,v=k.time;break;case"method":p=b.method,v=k.method;break;case"site":p=b.site_name,v=k.site_name;break;case"ip":p=b.client_ip,v=k.client_ip;break;default:p=b.status,v=k.status;break}return p<v?s?1:-1:p>v?s?-1:1:0}),l=50,o=Math.ceil(n.length/l),d=n.slice(e*l,(e+1)*l).map(b=>{let k=y(b.ua),p=b.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
                ...
                <td><span class="kp-badge ${p}">${b.status}</span>${b.reason?` <span class="kp-badge kp-badge-danger" style="font-size:0.65rem" uk-tooltip="Blocked by security rule">${y(b.reason)}</span>`:""}</td>
        ...
        ${u}`}async function bt(t,e,a,s=!1){let r=document.getElementById("stats-drilldown-modal"),n=document.getElementById("stats-drilldown-title"),l=document.getElementById("stats-drilldown-body");if(!r||!l)return;n.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,l.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(r).show();let o=[],i=0,d="time",u=!0;function b(){console.log(s),l.innerHTML=Be(o,i,d,u,s),l.querySelectorAll("th[data-dd-col]").forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.ddCol;d===p?u=!u:(d=p,u=!0),i=0,b()})}),l.querySelectorAll("[data-dd-page]").forEach(k=>{k.addEventListener("click",()=>{i=parseInt(k.dataset.ddPage,10),b()})})}try{o=await m.get(`${t}?hour=${encodeURIComponent(e)}&status=${a}`)}catch(k){l.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${y(k.message)}</p>`;return}b()}function vt(t,e,a){let s=a===6,r=null;function n(){if(s)return;let i=t.querySelector("#stats-pod-indicator"),d=t.querySelector("#stats-pod-table-wrap");if(!d)return;let u=location.protocol==="https:"?"wss":"ws";r=new WebSocket(`${u}://${location.host}/api/sites/${e}/stats/pod`),r.onopen=()=>{i&&(i.className="kp-status kp-status-running",i.textContent="Live")},r.onmessage=b=>{try{let k=JSON.parse(b.data);d.innerHTML=qe(k.containers??[]),d.querySelectorAll(".kp-stats-pod-role-btn").forEach(p=>{p.addEventListener("click",async()=>{let v=p.style.color;p.style.color="var(--kp-warning)";let g=p.dataset.container.split("-").pop();try{await m.post(`/sites/${e}/containers/${g}/restart`),c.success(`${g} restarted`)}catch(h){p.style.color=v,c.error(h.message)}})})}catch{}},r.onerror=()=>{i&&(i.className="kp-status kp-status-error",i.textContent="Error")},r.onclose=()=>{i&&i.textContent==="Live"&&(i.className="kp-status kp-status-stopped",i.textContent="Disconnected")}}function l(){r&&r.readyState===WebSocket.OPEN&&r.close(),r=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{Bt(e)}),n();let o=new MutationObserver(()=>{document.getElementById("stats-panel")||(l(),o.disconnect())});o.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function gt(t,e){let a=e===6;await Ie(t),a||await Bt(t)}async function Mt(t){let e=await m.get("/sites")??[];t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${e.length===0?tt("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>st()),Ae()}function Me(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,r=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
            ...
            <div class="kp-site-card-header">
                <div>
                    ...
                    <div class="kp-site-meta">
                        ...
                        <span class="kp-site-meta-item"><span uk-icon="icon: code; ratio: 0.75"></span> ${z(t.SiteType)}${R(t)?" / "+R(t):""}</span>
        ...
        </div>`}function Ae(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),r=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let n=null,l=!0,o=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],i=()=>{let v=o().length;e.textContent=`${v} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(w=>{let S=document.getElementById(w);S&&(S.disabled=v===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=v===0);let h=document.querySelectorAll(".kp-site-row-check");a.indeterminate=v>0&&v<h.length,a.checked=h.length>0&&v===h.length},d=()=>{let p=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(v=>{let g=v.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",h=v.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";v.style.display=!p||g.includes(p)||h.includes(p)?"":"none"})},u=p=>{n===p?l=!l:(n=p,l=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===p?l?" \u2191":" \u2193":" \u2195"});let v=[...r.querySelectorAll("tr")];v.sort((g,h)=>{let w="",S="";return p==="name"?(w=g.querySelector(".kp-site-row-name")?.textContent??"",S=h.querySelector(".kp-site-row-name")?.textContent??""):p==="status"?(w=g.dataset.status??"",S=h.dataset.status??""):p==="type"?(w=g.dataset.type??"",S=h.dataset.type??""):p==="domain"&&(w=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",S=h.querySelector("td:nth-child(6)")?.textContent.trim()??""),l?w.localeCompare(S):S.localeCompare(w)}),v.forEach(g=>r.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(p=>{p.checked=a.checked}),i()}),r?.addEventListener("change",p=>{p.target.classList.contains("kp-site-row-check")&&i()}),s?.addEventListener("input",d),document.querySelectorAll(".kp-sortable").forEach(p=>{p.addEventListener("click",()=>u(p.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(p=>{let v=p.replace("bulk-","");document.getElementById(p)?.addEventListener("click",()=>{let g=o().filter(h=>h.dataset.siteType!=="6").map(h=>h.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:v,ids:g}}))})});let b=document.getElementById("kp-bulk-mobile-pill"),k=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",p=>{p.stopPropagation(),k.hidden=!k.hidden}),document.addEventListener("click",p=>{k&&!b?.contains(p.target)&&(k.hidden=!0)},{capture:!0}),["start","stop","restart","flush","recreate"].forEach(p=>{document.getElementById(`bulk-mobile-${p}`)?.addEventListener("click",v=>{v.preventDefault(),k.hidden=!0;let g=o().filter(h=>h.dataset.siteType!=="6").map(h=>h.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:p,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(p=>{p.textContent=" \u2195"}),i()}var N=null;async function Rt(t){let[e,a,s]=await Promise.all([m.get("/sites").catch(()=>[]),m.get("/stats/traffic").catch(()=>null),m.get("/stats/pod").catch(()=>null)]),r=e.filter(o=>o.SiteType!==6&&o.SiteStatus===1).length,n=e.filter(o=>o.SiteType===6).length,l=e.filter(o=>o.SiteType!==6&&o.SiteStatus===4).length;if(t.innerHTML=`
        ...
        <div class="uk-grid-small uk-child-width-1-1 uk-child-width-1-2@m uk-margin-medium-bottom" uk-grid>
            <div>
                ...
                <div class="">
                    ${e.length===0?emptyState("world","No sites yet"):e.slice(-3).reverse().map(o=>At(o,e)).join("")}
                </div>
                    ...
                    <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                        ...
                        <tbody>
                            ${(a?.top_sites??[]).length===0?'<tr><td colspan="2" class="kp-muted uk-text-small">No traffic data</td></tr>':(a?.top_sites??[]).map(o=>`
                                    <tr>
                                        <td class="kp-mono" style="font-size:0.8rem">${y(o.name)}</td>
        ...
        `,a?.hits_per_hour?.length){await mt();let o=document.getElementById("dash-traffic-chart");o&&window.Chart&&(N&&(N.destroy(),N=null),N=new window.Chart(o,{type:"bar",data:{labels:a.hits_per_hour.map(i=>new Date(i.hour).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),datasets:[{label:"2xx",data:a.hits_per_hour.map(i=>i["2xx"]),backgroundColor:"rgba(39,174,96,0.75)",borderColor:"rgba(39,174,96,1)",borderWidth:1,borderRadius:3},{label:"3xx",data:a.hits_per_hour.map(i=>i["3xx"]),backgroundColor:"rgba(43,142,255,0.75)",borderColor:"rgba(43,142,255,1)",borderWidth:1,borderRadius:3},{label:"4xx",data:a.hits_per_hour.map(i=>i["4xx"]),backgroundColor:"rgba(255,171,0,0.75)",borderColor:"rgba(255,171,0,1)",borderWidth:1,borderRadius:3},{label:"5xx",data:a.hits_per_hour.map(i=>i["5xx"]),backgroundColor:"rgba(235,59,90,0.75)",borderColor:"rgba(235,59,90,1)",borderWidth:1,borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,onClick:(i,d)=>{if(!d||!d.length)return;let u=d[0].datasetIndex,b=N.data.datasets[u].label;if(b!=="4xx"&&b!=="5xx")return;let k=a.hits_per_hour[d[0].index]?.hour;k&&bt("/stats/drilldown",k,b,!0)},onHover:(i,d)=>{if(!d||!d.length){i.native.target.style.cursor="default";return}let u=N.data.datasets[d[0].datasetIndex].label;i.native.target.style.cursor=u==="4xx"||u==="5xx"?"pointer":"default"},plugins:{legend:{display:!0,labels:{color:"#6b8cae",font:{size:11}},onHover:i=>{i.native.target.style.cursor="pointer"},onLeave:i=>{i.native.target.style.cursor="default"}},tooltip:{mode:"index",backgroundColor:"#0c1530",borderColor:"#1a2a4a",borderWidth:1,titleColor:"#dde8f5",bodyColor:"#6b8cae"}},scales:{x:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10},maxRotation:45},grid:{color:"rgba(26,42,74,0.6)"}},y:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10}},grid:{color:"rgba(26,42,74,0.6)"},beginAtZero:!0}}}}))}document.getElementById("dash-new-site")?.addEventListener("click",()=>st())}function V(t=null){let e=t?`/sites/${t}/security/ip`:"/security/ip",a=t?`/sites/${t}/security/ua`:"/security/ua",s=t?`/sites/${t}/waf`:"/settings/waf",r=t?`/sites/${t}/security/country`:"/security/country",n=t?`/sites/${t}/security/asn`:"/security/asn";return`
        ...
        </div>`}async function B(t){let e=t.querySelector("#security-panel");if(!e)return;let a=e.dataset.ipBase,s=e.dataset.uaBase,r=e.dataset.wafBase;try{let n=e.dataset.geoBase,l=e.dataset.asnBase,o=[m.get(a),m.get(s),m.get(n),m.get(l)];e.dataset.siteId||o.push(m.get(r),m.get("/settings/trusted-proxies"),m.get("/security/bypass"));let[i,d,u,b,k,p,v]=await Promise.all(o);if(!t.querySelector("#sec-ip-whitelist"))return;if(t.querySelector("#sec-ip-whitelist").value=i.whitelist??"",t.querySelector("#sec-ip-blacklist").value=i.blacklist??"",t.querySelector("#sec-ua-whitelist").value=d.whitelist??"",t.querySelector("#sec-ua-blacklist").value=d.blacklist??"",t.querySelector("#sec-geo-whitelist").value=u.whitelist??"",t.querySelector("#sec-geo-blacklist").value=u.blacklist??"",t.querySelector("#sec-asn-whitelist").value=b.whitelist??"",t.querySelector("#sec-asn-blacklist").value=b.blacklist??"",k){let g=t.querySelector("#sec-waf-enabled"),h=t.querySelector("#sec-waf-audit"),w=t.querySelector("#sec-waf-mode"),S=t.querySelector("#sec-waf-paranoia"),H=t.querySelector("#sec-waf-exclusions");g&&(g.checked=!!k.Enabled),h&&(h.checked=!!k.AuditLog),w&&(w.value=String(k.Mode??0)),S&&(S.value=String(k.ParanoiaLevel??1)),H&&(H.value=k.Exclusions??"")}if(p){let g=t.querySelector("#sec-tp-cidrs");g&&(g.value=p.trusted_proxies_custom??"")}if(v){let g=t.querySelector("#sec-bypass-cidrs");g&&(g.value=v.bypass??"")}}catch(n){c.error("Failed to load security rules: "+n.message)}}function it(t){let e=t.querySelector("#security-panel");if(!e)return;let a=e.dataset.ipBase,s=e.dataset.uaBase,r=e.dataset.geoBase;t.querySelector("#sec-ip-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-ip-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put(a,{whitelist:t.querySelector("#sec-ip-whitelist").value,blacklist:t.querySelector("#sec-ip-blacklist").value}),c.success("IP rules saved")}catch(o){c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-ua-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-ua-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put(s,{whitelist:t.querySelector("#sec-ua-whitelist").value,blacklist:t.querySelector("#sec-ua-blacklist").value}),c.success("UA rules saved")}catch(o){c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-geo-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-geo-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{let o={whitelist:t.querySelector("#sec-geo-whitelist").value,blacklist:t.querySelector("#sec-geo-blacklist").value},i=await m.put(r,o);i?.status==="confirm"&&(await UIkit.modal.confirm(`${i.reason}. Save anyway?`),i=await m.put(r,{...o,confirm:!0})),c.success("Country rules saved")}catch(o){o instanceof Error&&c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-asn-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-asn-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{let o=t.querySelector("#security-panel").dataset.asnBase,i={whitelist:t.querySelector("#sec-asn-whitelist").value,blacklist:t.querySelector("#sec-asn-blacklist").value},d=await m.put(o,i);d?.status==="confirm"&&(await UIkit.modal.confirm(`${d.reason}. Save anyway?`),d=await m.put(o,{...i,confirm:!0})),c.success("ASN rules saved")}catch(o){o instanceof Error&&c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-asn-lookup")?.addEventListener("click",()=>Re(t)),t.querySelector("#sec-tp-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-tp-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put("/settings/trusted-proxies",{trusted_proxies_custom:t.querySelector("#sec-tp-cidrs").value.trim()}),c.success("Trusted proxy ranges saved")}catch(o){c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-bypass-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-bypass-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put("/security/bypass",{bypass:t.querySelector("#sec-bypass-cidrs").value.trim()}),c.success("Bypass rules saved")}catch(o){c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-tp-import")?.addEventListener("change",async n=>{let l=n.target.files[0];if(!l)return;let o=new FormData;o.append("file",l);try{let i=await fetch("/api/settings/trusted-proxies/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:o}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("Trusted proxies imported")}catch(i){c.error(i.message)}finally{n.target.value=""}}),t.querySelector("#sec-waf-save")?.addEventListener("click",async()=>{let n=t.querySelector("#sec-waf-save"),l=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await m.put(e.dataset.wafBase,{enabled:t.querySelector("#sec-waf-enabled").checked,mode:parseInt(t.querySelector("#sec-waf-mode").value,10),paranoia_level:parseInt(t.querySelector("#sec-waf-paranoia").value,10),audit_log:t.querySelector("#sec-waf-audit").checked,exclusions:t.querySelector("#sec-waf-exclusions").value.trim()}),c.success("WAF settings saved \u2014 engine recompiling in background")}catch(o){c.error(o.message)}finally{n.disabled=!1,n.innerHTML=l}}),t.querySelector("#sec-ip-import")?.addEventListener("change",async n=>{let l=n.target.files[0];if(!l)return;let o=new FormData;o.append("file",l);try{let i=await fetch("/api"+a+"/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:o}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("IP rules imported")}catch(i){c.error(i.message)}finally{n.target.value=""}}),t.querySelector("#sec-ua-import")?.addEventListener("change",async n=>{let l=n.target.files[0];if(!l)return;let o=new FormData;o.append("file",l);try{let i=await fetch("/api"+s+"/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:o}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("UA rules imported")}catch(i){c.error(i.message)}finally{n.target.value=""}}),t.querySelector("#sec-waf-import")?.addEventListener("change",async n=>{let l=n.target.files[0];if(!l)return;let o=new FormData;o.append("file",l);try{let i=await fetch("/api/settings/waf/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:o}),d=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(d?.error||`HTTP ${i.status}`);await B(t),c.success("WAF settings imported")}catch(i){c.error(i.message)}finally{n.target.value=""}})}function Re(t){document.getElementById("kp-asn-lookup-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
        <div id="kp-asn-lookup-modal" uk-modal>
            <div class="uk-modal-dialog kp-modal uk-modal-body">
                ...
                </button>`,n.querySelector("#asn-lookup-ip").textContent=l.ip,n.querySelector("#asn-lookup-org").textContent=l.org||"",n.querySelector("#asn-lookup-add").addEventListener("click",()=>{let o=t.querySelector("#sec-asn-blacklist"),i=`AS${l.asn}`;o.value.split(`
`).some(d=>d.trim().toUpperCase()===i)||(o.value=o.value.trim()?`${o.value.replace(/\s+$/,"")}
${i}`:i),a.hide(),c.success(`${i} added to blacklist \u2014 save to apply`)})}catch(l){n.innerHTML="",c.error(l.message)}};document.getElementById("asn-lookup-go").addEventListener("click",s),document.getElementById("asn-lookup-q").addEventListener("keydown",r=>{r.key==="Enter"&&s()})}async function Ht(t){if(!q()){t.innerHTML=_("Access denied");return}t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${V(null)}`,it(t),B(t)}function He(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';case"expired":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Expired certificate"></span>';case"mismatch":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Certificate does not match this domain"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Dt(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=He(a.status)}catch{}}async function Ft(t){if(!q()){t.innerHTML=_("Access denied");return}let[e,a,s,r,n,l]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Dt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await m.put("/settings",b),c.success("Settings saved"),Dt(b.admin_domain)}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let d=new FormData;d.append("file",i);try{let u=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:d}),b=u.status===204?null:await u.json().catch(()=>null);if(!u.ok)throw new Error(b?.error||`HTTP ${u.status}`);c.success("Settings imported")}catch(u){c.error(u.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(o.target),b={backup_schedule:u.get("backup_schedule").trim(),backup_retain_days:u.get("backup_retain_days").trim()};try{await m.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(o.target),b={s3_endpoint:u.get("s3_endpoint").trim(),s3_bucket:u.get("s3_bucket").trim(),s3_region:u.get("s3_region").trim(),s3_access_key:u.get("s3_access_key").trim()},k=u.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await m.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("smtp-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(o.target),b={smtp_host:u.get("smtp_host").trim(),smtp_port:u.get("smtp_port").trim(),smtp_username:u.get("smtp_username").trim(),smtp_from:u.get("smtp_from").trim(),smtp_tls:u.get("smtp_tls")?"true":"false"},k=u.get("smtp_password").trim();k&&(b.smtp_password=k);try{await m.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("sns-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(o.target),b={aws_access_key:u.get("aws_access_key").trim(),aws_region:u.get("aws_region").trim(),aws_sns_sender_id:u.get("aws_sns_sender_id").trim()},k=u.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await m.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{i.disabled=!1,i.innerHTML=d}}),document.getElementById("resource-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),d=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(o.target),b={resource_ram_reserve_gb:u.get("resource_ram_reserve_gb").trim(),resource_poll_interval:u.get("resource_poll_interval").trim(),resource_throttle_pct:u.get("resource_throttle_pct").trim(),resource_webhook_url:u.get("resource_webhook_url").trim()};try{await m.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=d}})}async function Ut(t){let e=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",e);let a=UIkit.modal("#kp-edit-site-modal"),s=document.getElementById("es-site-type"),r=document.getElementById("es-php-version-wrap"),n=document.getElementById("es-node-version-wrap"),l=document.getElementById("es-dotnet-version-wrap"),o=document.getElementById("es-start-command-wrap"),i=document.getElementById("es-wordpress-wrap");a.show();let d=u=>{r.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),n.classList.toggle("uk-hidden",u!==4),l.classList.toggle("uk-hidden",u!==5),o.classList.toggle("uk-hidden",u!==4&&u!==5),i.classList.toggle("uk-hidden",u!==1)};d(t.SiteType),s.addEventListener("change",()=>d(parseInt(s.value))),document.getElementById("edit-site-form").addEventListener("submit",async u=>{u.preventDefault();let b=u.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p=new FormData(u.target),v=parseInt(p.get("site_type")),g=null;v===4&&(g=parseInt(p.get("node_version"))),v===5&&(g=parseInt(p.get("dotnet_version")));let h={php_version:parseInt(p.get("php_version"))||3,site_type:v,runtime_version:g,start_command:p.get("start_command")?.trim()||""},w=v===1?p.get("install_wordpress")==="on":!1;try{if(await m.put(`/sites/${t.ID}`,h),a.hide(),document.getElementById("kp-edit-site-modal")?.remove(),v!==6){$("Applying Changes","Saving changes and recreating pod...");try{await m.post(`/sites/${t.ID}/recreate`,{install_wordpress:w}),x(),c.success("Site updated and pod recreated")}catch(S){x(),c.error("Site saved but pod recreate failed: "+S.message)}}else c.success("Site updated");f.go("site-detail",{id:String(t.ID)})}catch(S){c.error(S.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function Wt(t){return`
        ...
        </div>`}function Nt(t,e){let a=Date.now()+18e5,s=setInterval(async()=>{try{let r=await m.get(`/sites/${e}/backups/restore-status`);(!r?.active||Date.now()>a)&&(clearInterval(s),x(),r?.active?c.error("Import timed out \u2014 check server logs"):c.success("Import complete"),await W(t,e))}catch{}},3e3)}async function W(t,e){try{let[a,s]=await Promise.all([m.get(`/sites/${e}/backup-repo`),m.get(`/sites/${e}/backups`)]),r=t.querySelector("#backup-local-enabled"),n=t.querySelector("#backup-s3-enabled");r&&(r.checked=!!a.LocalEnabled),n&&(n.checked=!!a.S3Enabled);let l=t.querySelector("#backup-error-banner");if(l)if(a.last_error){let i=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";l.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${i}:</strong> ${y(a.last_error)}</p>
                    </div>`}else l.innerHTML="";let o=t.querySelector("#backup-list-wrap");o&&(o.innerHTML=De(s))}catch(a){let s=t.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${y(a.message)}</p>`)}}function jt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await m.put(`/sites/${e}/backup-repo`,s),c.success("Backup destinations saved")}catch(r){c.error(r.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await m.get(`/sites/${e}/backups`))?.length??0}catch{}try{await m.post(`/sites/${e}/backups`,{label:"manual"})}catch(l){c.error(l.message);return}$("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let r=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await m.get(`/sites/${e}/backups`))?.length??0)>s||Date.now()>r)&&(clearInterval(n),x(),await W(t,e),Date.now()<=r?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let r=s.target.closest(".backup-restore-btn");if(r){let o=r.dataset.id;if(!await L("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await m.post(`/sites/${e}/backups/${o}/restore`)}catch(k){c.error(k.message);return}$("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let d=Date.now(),u=Date.now()+900*1e3,b=setInterval(async()=>{try{let k=await m.get(`/sites/${e}/backups/restore-status`);(!k?.active||Date.now()>u)&&(clearInterval(b),x(),k?.active?c.error("Restore timed out"):c.success("Restore complete"),await W(t,e))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let o=n.dataset.id;if(!await L("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;$("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await m.delete(`/sites/${e}/backups/${o}`),x(),c.success("Snapshot deleted"),await W(t,e)}catch(d){x(),c.error(d.message)}}let l=s.target.closest(".backup-download-btn");if(l){let o=l.dataset.id,i=`${Date.now().toString(36)}${Math.random().toString(36).slice(2,10)}`,d=`kp_dl_${i}`;$("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let u=document.createElement("a");u.href=`/api/sites/${e}/backups/${o}/download?dl=${i}`,u.style.display="none",document.body.appendChild(u),u.click(),document.body.removeChild(u);let b=Date.now(),k=setInterval(()=>{!document.cookie.split(";").some(v=>v.trim().startsWith(`${d}=`))&&Date.now()-b<18e5||(clearInterval(k),document.cookie=`${d}=; Path=/; Max-Age=0`,x())},500)},300);return}});let a=t.querySelector("#import-backup-modal");a&&(UIkit.util.on(a,"beforeshow",async()=>{let s=a.querySelector("#import-target-site");try{let n=await m.get("/sites"),l=Number(e);s.innerHTML=n.map(o=>`<option value="${o.ID}"${Number(o.ID)===l?" selected":""}>${o.Name}</option>`).join("")}catch{s.innerHTML='<option value="">Failed to load sites</option>'}let r=a.querySelector("#import-sftp-list");try{let n=await m.get(`/sites/${e}/backups/import/files`);!n||n.length===0?r.innerHTML='<p class="kp-muted uk-text-small">No files found.</p>':r.innerHTML=n.map(l=>`
    ...
    </div>`}function zt(t,e,a){t.addEventListener("click",s=>{if(s.target.closest(".cfg-add-row")){let r=s.target.closest(".cfg-add-row");t.querySelector(`.cfg-rows[data-type="${r.dataset.type}"]`).insertAdjacentHTML("beforeend",X())}},{signal:a}),t.addEventListener("click",s=>{s.target.closest(".cfg-del-row")&&s.target.closest(".kp-config-row").remove()},{signal:a}),t.addEventListener("click",async s=>{let r=s.target.closest(".cfg-save");if(!r)return;let{type:n,site:l}=r.dataset,o=t.querySelectorAll(`.cfg-rows[data-type="${n}"] .kp-config-row`),i={};if(o.forEach(d=>{let u=d.querySelector(".cfg-key").value.trim(),b=d.querySelector(".cfg-val").value.trim();u&&(i[u]=b)}),n==="5"){let d=t.querySelector(".varnish-enabled-toggle");i.enabled=d?.checked?"true":"false"}try{await m.put(`/sites/${l}/configs/${n}`,i),c.success(`${J[n]} config saved`)}catch(d){c.error(d.message)}},{signal:a}),t.addEventListener("click",async s=>{let r=s.target.closest(".cfg-reset");if(!r)return;let{type:n,site:l}=r.dataset;if(await L("Reset Config",`Reset ${J[n]} config to defaults?`))try{let i=await m.post(`/sites/${l}/configs/${n}/reset`),d=t.querySelector(`.cfg-rows[data-type="${n}"]`);d.innerHTML=Object.entries(i).map(([u,b])=>X(u,b)).join(""),c.success(`${J[n]} reset to defaults`)}catch(i){c.error(i.message)}},{signal:a}),t.addEventListener("change",async s=>{let r=s.target.closest(".cfg-import-input");if(!r)return;let{type:n,site:l}=r.dataset,o=r.files[0];if(!o)return;let i=new FormData;i.append("file",o);try{let d=await fetch(`/api/sites/${l}/configs/${n}/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:i}),u=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(u?.error||`HTTP ${d.status}`);let b=t.querySelector(`.cfg-rows[data-type="${n}"]`);b.innerHTML=Object.entries(u).map(([k,p])=>X(k,p)).join(""),c.success(`${J[n]} config imported`)}catch(d){c.error(d.message)}finally{r.value=""}},{signal:a})}function Kt(t){return`
        ...
        <tr>
            ...
            <td>
        ...
        </div>`}async function ot(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await m.get(`/sites/${e}/crons`);a.innerHTML=Jt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${y(s.message)}</p>`}}function Xt(t,e){let a=[],s=t.querySelector("#cron-modal"),r=t.querySelector("#cron-modal-title"),n=t.querySelector("#cron-modal-id"),l=t.querySelector("#cron-modal-label"),o=t.querySelector("#cron-modal-command"),i=t.querySelector("#cron-modal-schedule"),d=t.querySelector("#cron-schedule-preview"),u=t.querySelector("#cron-modal-enabled");i?.addEventListener("input",()=>{d.textContent=Vt(i.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{r.textContent="Add Cron Job",n.value="",l.value="",o.value="",i.value="",d.textContent="",u.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let b=o.value.trim(),k=i.value.trim();if(!b||!k){c.error("Command and schedule are required");return}let p={label:l.value.trim(),command:b,schedule:k,enabled:u.checked},v=n.value;try{v?(await m.put(`/sites/${e}/crons/${v}`,p),c.success("Cron job updated")):(await m.post(`/sites/${e}/crons`,p),c.success("Cron job created")),UIkit.modal(s).hide(),await ot(t,e),a=await m.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async b=>{let k=b.target.closest(".cron-detail-btn");if(k){let h=k.dataset.id,w=a.find(H=>String(H.ID)===h);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let S=document.getElementById("cron-detail-modal");UIkit.modal(S).show(),S.addEventListener("hidden",()=>S.remove(),{once:!0});return}let p=b.target.closest(".cron-edit-btn");if(p){let h=p.dataset.id,w=a.find(S=>String(S.ID)===h);if(!w)return;r.textContent="Edit Cron Job",n.value=w.ID,l.value=w.Label||"",o.value=w.Command,i.value=w.Schedule,d.textContent=Vt(w.Schedule),u.checked=w.Enabled,UIkit.modal(s).show();return}let v=b.target.closest(".cron-delete-btn");if(v){let h=v.dataset.id;if(!await L("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await m.delete(`/sites/${e}/crons/${h}`),c.success("Cron job deleted"),await ot(t,e),a=await m.get(`/sites/${e}/crons`)}catch(S){c.error(S.message)}return}let g=b.target.closest(".cron-run-btn");if(g){let h=g.dataset.id;try{await m.post(`/sites/${e}/crons/${h}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await m.get(`/sites/${e}/crons`)).find(D=>String(D.ID)===h)?.LastRun??null}catch{}let S=Date.now()+300*1e3,H=setInterval(async()=>{try{let I=await m.get(`/sites/${e}/crons`),D=I.find(Y=>String(Y.ID)===h);if(!D||D.LastRun!==w||Date.now()>S){clearInterval(H),x(),a=I??[];let Y=t.querySelector("#cron-list-wrap");Y&&(Y.innerHTML=Jt(I)),D?.LastError?c.error(`Job failed: ${D.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let p=k.dataset.id;try{await m.patch(`/sites/${e}/crons/${p}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(v){c.error(v.message),k.checked=!k.checked}}),m.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function Vt(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,r,n,l]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&r==="*"&&n==="*"&&l==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&r==="*"&&n==="*"&&l!=="*"){let o=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${l.split(",").map(d=>o[parseInt(d)]??d).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function te(t){return`
        <div class="kp-card uk-padding uk-margin-top" id="fm-root" data-site="${t}">
            ...
            <div id="fm-list"></div>
        </div>`}function Ue(){let t=P?P.split("/"):[],e="",a=['<a href="#" data-path="">html</a>'];for(let s of t)e=e?e+"/"+s:s,a.push(`<span class="kp-fm-sep">/</span><a href="#" data-path="${y(e)}">${y(s)}</a>`);return a.join("")}function Ne(t){let e=new Date(t);return isNaN(e)?"":e.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Gt(t){return t==="d"?"folder":t==="l"?"link":"file-text"}function j(t,e){return t?t+"/"+e:e}var Qt=new Set(["php","js","jsx","ts","tsx","css","scss","sass","less","html","htm","xml","json","txt","md","markdown","yml","yaml","ini","conf","cnf","toml","env","sh","bash","sql","log","csv","tsv","svg","htaccess","gitignore","lock","map"]);function We(t,e){if(e)return!1;let a=t.lastIndexOf(".");return t.startsWith(".")&&a===0?Qt.has(t.slice(1).toLowerCase()):a>=0&&Qt.has(t.slice(a+1).toLowerCase())}function je(t){if(!t||!t.length)return tt("folder","This folder is empty");let e=document.getElementById("fm-root").dataset.site;return`
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            <thead>
                <tr>
            ...
            <tbody>${t.map(s=>{let r=j(P,s.name),n=s.is_dir,l=We(s.name,n),o=n?`<a href="#" class="fm-nav" data-path="${y(r)}"><span uk-icon="icon: ${Gt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</a>`:`<span><span uk-icon="icon: ${Gt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</span>`;return`
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Z(),a&&(a.innerHTML=Ue());try{let s=await m.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=je(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Yt(t,e){P=e||"",C(t)}function Q(t,e,a=""){return new Promise(s=>{let r="fm-prompt-modal";document.getElementById(r)?.remove();let n=document.createElement("div");n.id=r,n.setAttribute("uk-modal",""),n.innerHTML=`
        ...
        </div>`,document.body.appendChild(n),window.UIkit&&UIkit.icon(n);let l=UIkit.modal(n,{bgClose:!1,escClose:!0}),o=n.querySelector("#fm-ed-dirty"),i=null,d=!0,u=k=>{d=!k,o.hidden=!k};UIkit.util.on(n,"shown",()=>{if(i)return;i=window.CodeMirror.fromTextArea(n.querySelector("#fm-ed-area"),{value:s.content,lineNumbers:!0,theme:"material-darker",indentUnit:4,lineWrapping:!1,extraKeys:{"Ctrl-S":b,"Cmd-S":b,"Ctrl-F":"findPersistent","Ctrl-/":"toggleComment"}}),i.setValue(s.content),i.on("change",()=>u(!0));let k=Ve(a);k&&(i.setOption("mode",k),window.CodeMirror.autoLoadMode(i,k)),setTimeout(()=>i.refresh(),30)}),UIkit.util.on(n,"hidden",()=>n.remove());async function b(){if(!i)return;let k=n.querySelector("#fm-ed-save"),p=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div>';try{await m.put(`/sites/${t}/files/content`,{path:e,content:i.getValue()}),u(!1),c.success("Saved"),C(t)}catch(v){c.error(v.message)}finally{k.disabled=!1,k.innerHTML=p}}n.querySelector("#fm-ed-save").addEventListener("click",b),UIkit.util.on(n,"beforehide",k=>{!d&&!window.confirm("Discard unsaved changes?")&&k.preventDefault()}),l.show()}var Je=2e3;function wt(t,e){return`
        <div>
            <div class="kp-log-controls">
        ...
        </div>`}function ae(t,e){let a=null,s=!1,r=t.querySelector("#log-output"),n=t.querySelector("#log-connect"),l=t.querySelector("#log-disconnect"),o=t.querySelector("#log-clear"),i=t.querySelector("#log-autoscroll"),d=t.querySelector("#log-status");function u(p){for(p.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");g.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=v,r.appendChild(g)});r.childElementCount>Je;)r.removeChild(r.firstChild);i.checked&&(r.scrollTop=r.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,n.disabled=!1,l.disabled=!0,d&&(d.textContent="Disconnected")}n.addEventListener("click",()=>{b();let p=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",h=p==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:p==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:p==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:`${g}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${v}`;a=new WebSocket(h),a.onopen=()=>{s=!0,n.disabled=!0,l.disabled=!1,d&&(d.textContent=`Connected \u2014 ${p}`)},a.onmessage=w=>u(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,n.disabled=!1,l.disabled=!0,d&&(d.textContent="Disconnected")}}),l.addEventListener("click",b),o.addEventListener("click",()=>{r.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),n.click())});let k=f.go.bind(f);f.go=function(p,v={}){return a&&b(),k(p,v)}}function Xe(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';case"expired":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Expired certificate"></span>';case"mismatch":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Certificate does not match this domain"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function se(t,e){try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Xe(a.status))}catch{}}function ne(t){t.forEach(e=>se(e.Domain,e.ID))}function ie(t,e,a,s=0,r=null){let n=t.SiteType!==3&&t.PMAPort>0;return`
        <div class="uk-grid-medium" uk-grid>
            ...
            <div class="uk-width-1-2@m">
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <div id="domain-list">
                        ${e.length?e.map(oe).join(""):'<p class="kp-muted uk-text-small">No domains configured</p>'}
                    </div>
        ...
        </div>`}function oe(t){return`<div class="uk-flex uk-flex-between uk-flex-middle kp-config-row" data-domain-id="${t.ID}">
            ...
            <form id="waf-override-form" class="uk-form-stacked">
                ...
                <div class="uk-flex uk-flex-right uk-margin-top" style="gap:8px">
        ...
        </div>`}async function xt(t){let e=document.getElementById("waf-tab-panel");if(!e)return;e.innerHTML=Ge();let a=document.getElementById("waf-export-btn");a&&(a.href=`/api/sites/${t}/waf/export`);try{let s=await m.get(`/sites/${t}/waf`),r=document.getElementById("waf-override"),n=document.getElementById("waf-site-exclusions");r&&(r.value=String(s.Override??0)),n&&(n.value=s.Exclusions??"");let l=document.getElementById("waf-plugins-list");if(l){let[o,i]=await Promise.all([m.get("/settings/waf/plugins"),m.get(`/sites/${t}/waf/plugins`)]),d=new Set(i??[]);!o||o.length===0?l.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':window.matchMedia("(max-width: 959px)").matches?l.innerHTML=`
                    <select multiple class="uk-select kp-select waf-plugin-select" size="${Math.min(o.length,8)}">
                        ${o.map(u=>`
                    ...
                    <div class="waf-plugin-pills">
                        ...
                        `).join("")}
                    </div>`,l.querySelectorAll(".waf-plugin-pill").forEach(u=>{u.addEventListener("click",()=>u.classList.toggle("active"))}))}}catch(s){c.error("Failed to load WAF settings: "+s.message)}}function me(t,e,a){t.addEventListener("submit",async s=>{if(s.target.id!=="waf-override-form")return;s.preventDefault();let r=s.target.querySelector('[type="submit"]'),n=r.innerHTML;r.disabled=!0,r.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(s.target),o={override:parseInt(l.get("override"),10),exclusions:l.get("exclusions").trim()};try{await m.put(`/sites/${e}/waf`,o);let i=document.querySelector(".waf-plugin-select"),d=i?[...i.selectedOptions].map(u=>u.value):[...document.querySelectorAll(".waf-plugin-pill.active")].map(u=>u.dataset.plugin);await m.put(`/sites/${e}/waf/plugins`,d),c.success("WAF override saved \u2014 engine recompiling in background")}catch(i){c.error(i.message)}finally{r.disabled=!1,r.innerHTML=n}},{signal:a}),t.querySelector("#waf-import")?.addEventListener("change",async s=>{let r=s.target.files[0];if(!r)return;let n=new FormData;n.append("file",r);try{let l=await fetch(`/api/sites/${e}/waf/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:n}),o=l.status===204?null:await l.json().catch(()=>null);if(!l.ok)throw new Error(o?.error||`HTTP ${l.status}`);await xt(e),c.success("WAF settings imported")}catch(l){c.error(l.message)}finally{s.target.value=""}})}var Qe=[{label:"Cache Flush",cmd:"cache flush"},{label:"Plugin List",cmd:"plugin list"},{label:"Theme List",cmd:"theme list"},{label:"User List",cmd:"user list"},{label:"Core Check",cmd:"core check-update"},{label:"Core Update",cmd:"core update"},{label:"Plugin Updates",cmd:"plugin update --all"},{label:"Theme Updates",cmd:"theme update --all"},{label:"Rewrite Flush",cmd:"rewrite flush"},{label:"Transient Delete",cmd:"transient delete --all"},{label:"Search Replace",cmd:"search-replace '' ''"}];function ke(t){return`
        <div class="kp-wpcli">
        ...
        </div>`}function be(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),r=t.querySelector("#wpcli-run"),n=t.querySelector("#wpcli-clear"),l=t.querySelector("#wpcli-status"),o=[],i=-1;function d(k,p=""){k.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");p?g.className=p:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function u(k){if(k=k.trim(),!k)return;o.unshift(k),i=-1,d(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,r.disabled=!0,l&&(l.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let h=g.data;if(h.trim()==="[done]"){v.close();return}if(h.startsWith("[info]")){d(h,"kp-muted");return}if(h.startsWith("[error]")){d(h,"kp-log-line-err");return}d(h)},v.onerror=()=>{d("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,r.disabled=!1,l&&(l.textContent="Ready"),s.focus()}}r.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){u(s.value),s.value="",i=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),i<o.length-1&&(i++,s.value=o[i]);return}k.key==="ArrowDown"&&(k.preventDefault(),i>0?(i--,s.value=o[i]):(i=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let v=p.indexOf("''")+1;s.setSelectionRange(v,v);return}u(p)})}),n.addEventListener("click",()=>{a.innerHTML=""});let b=f.go.bind(f);f.go=function(k,p={}){return b(k,p)},s.focus()}var O=null,M=null;function ve(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),r=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(o,i=!1){UIkit.switcher(a).show(o),e.querySelectorAll(":scope > li[data-pill]").forEach(d=>d.classList.remove("kp-pill-active")),i?(s?.classList.add("kp-pill-active"),r?.querySelectorAll("a[data-switcher]").forEach(d=>{d.classList.toggle("kp-dd-active",parseInt(d.dataset.switcher,10)===o)})):(s?.classList.remove("kp-pill-active"),r?.querySelectorAll("a[data-switcher]").forEach(d=>d.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault();let d=parseInt(o.closest("li").dataset.pill,10);n(d,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",o=>{o.stopPropagation(),r.hidden=!r.hidden,s.classList.toggle("kp-pill-active",!r.hidden)}),r?.querySelectorAll("a[data-switcher]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),r.hidden=!0,n(parseInt(o.dataset.switcher,10),!0)})}),document.addEventListener("click",o=>{r&&!s.contains(o.target)&&(r.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(1)}function Ye(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        <div class="rp-route-row uk-flex uk-flex-middle uk-margin-small-bottom" style="gap:8px">
            ...
            <span class="kp-health-badge"
                ...
                <span class="kp-health-badge-label">${ge(e.name)}</span>
            </span>
        `).join("")}function sa(t,e){M&&(M.close(),M=null);let a=document.getElementById("sd-health-badges");if(!a)return;let s=location.protocol==="https:"?"wss":"ws";M=new WebSocket(`${s}://${location.host}/api/sites/${e}/health/stream`),M.onmessage=r=>{try{let n=JSON.parse(r.data);a.innerHTML=aa(n),a.querySelectorAll(".kp-health-badge").forEach(l=>{l.addEventListener("click",async()=>{l.style.color=$t("starting");let o=l.dataset.container,i=o.split("-").pop();try{await m.post(`/sites/${e}/containers/${i}/restart`),c.success(`${ge(o)} restarted`)}catch(d){l.style.color=$t("none"),c.error(d.message)}})})}catch{}},M.onerror=()=>{},M.onclose=()=>{M=null}}async function he(t,{id:e}){let[{site:a,domains:s,sftp:r},n,l]=await Promise.all([m.get(`/sites/${e}`),m.get("/sites"),m.get(`/sites/${e}/configs`)]),o=Array.isArray(n)?n:[],i=a.SiteType===1||a.SiteType===2,d=a.SiteType===6,u=[1,2,4,5].includes(a.SiteType);if(O&&O.abort(),O=new AbortController,t.innerHTML=`
        <div class="kp-view-header">
        ...
        <ul class="kp-tab-pills" id="kp-site-pills">
            ...
            <li id="kp-manage-pill">
                ...
                <div class="kp-pill-dropdown" id="kp-manage-dropdown" hidden>
                    ...
                    <a href="#" data-switcher="${i?4:3}"><span uk-icon="icon: database; ratio: 0.85"></span> MariaDB</a>
                    <a href="#" data-switcher="${i?5:4}"><span uk-icon="icon: server; ratio: 0.85"></span> Redis</a>
                    <a href="#" data-switcher="${i?6:5}"><span uk-icon="icon: world; ratio: 0.85"></span> Varnish</a>
                    <hr>
                    <div class="kp-pill-dropdown-section">Security</div>
                    <a href="#" data-switcher="${i?8:7}"><span uk-icon="icon: lock; ratio: 0.85"></span> Security</a>
                    <a href="#" data-switcher="${i?9:8}"><span uk-icon="icon: lifesaver; ratio: 0.85"></span> WAF</a>
                    <a href="#" data-switcher="${i?10:9}"><span uk-icon="icon: user; ratio: 0.85"></span> Basic Auth</a>
                    <hr>
                    <div class="kp-pill-dropdown-section">Tools</div>
                    ${a.SiteType===1?`<a href="#" data-switcher="${i?11:10}"><span uk-icon="icon: file-text; ratio: 0.85"></span> WP-CLI</a>`:""}
                    <a href="#" data-switcher="${a.SiteType===1?i?12:11:i?11:10}"><span uk-icon="icon: history; ratio: 0.85"></span> Backups</a>
                    ${u?`<a href="#" data-switcher="${a.SiteType===1?i?13:12:i?12:11}"><span uk-icon="icon: clock; ratio: 0.85"></span> Crons</a>`:""}
                    <a href="#" data-switcher="${a.SiteType===1?i?14:13:i?13:12}"><span uk-icon="icon: forward; ratio: 0.85"></span> Redirects</a>
                    <a href="#" data-switcher="files"><span uk-icon="icon: folder; ratio: 0.85"></span> Files</a>
                </div>
            </li>
            <li data-pill="1"><a href="#">Stats</a></li>
            <li data-pill="${i?7:6}"><a href="#">Logs</a></li>
        </ul>

        <!-- switcher panels (driven by pills above) -->
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${ie(a,s??[],r,a.ParentID??0,o.find(p=>p.ID===a.ParentID)?.Name??null)}</li>
            <li>${kt(e,a.SiteType)}</li>
            <li>${G(e,1,l[1])}</li>
            ${i?`<li>${G(e,2,l[2])}</li>`:""}
            <li>${G(e,3,l[3])}</li>
            ...
            <li>${ce()}</li>
            <li>${te(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>f.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>Ut(a)),document.getElementById("sd-rename").addEventListener("click",async()=>{let p=await Tt(a.Name);if(!(!p||p===a.Name)){$("Renaming Site","Moving the database, files, and pod \u2014 this may take a few minutes...");try{await m.post(`/sites/${e}/rename`,{name:p},18e5),x(),c.success(`Site renamed to '${p}'`),f.go("site-detail",{id:e})}catch(v){x(),c.error(v.message)}}}),document.getElementById("sd-site-nav")?.addEventListener("change",p=>{f.go("site-detail",{id:p.target.value})}),it(t),B(t),ae(t,e),me(t,e,O.signal),xt(e),d){ta(t,e),Ze(e),vt(t,e,a.SiteType),gt(e,a.SiteType),ft(t,e),K(e),ve(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{$("Recreating Pod","Recreating containers for this site...");try{await m.post(`/sites/${e}/recreate`),x(),c.success("Pod recreated"),f.go("site-detail",{id:e})}catch(p){x(),c.error(p.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let p=await Et(a.Name);if(p){$("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await m.post(`/sites/${e}/clone`,{name:p},6e5),x(),c.success(`Site cloned as '${p}'`),f.go("sites")}catch(v){x(),c.error(v.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(p=>{p.addEventListener("click",async()=>{let v=p.dataset.action;if(v==="flush"){try{await m.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(h){c.error(h.message)}return}$(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[v]??v} Pod`,"Please wait...");try{await m.post(`/sites/${e}/${v}`),x(),c.success(`Site ${v} successful`),f.go("site-detail",{id:e})}catch(h){x(),c.error(h.message)}})}),zt(t,e,O.signal),le(t,e),a.SiteType===1&&be(t,e),re(t,e,a),jt(t,e),W(t,e),u&&(Xt(t,e),ot(t,e)),pe(t,e),ue(e);let b=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');b&&k&&(k.dataset.switcher=String(b.children.length-1)),ee(t,e),C(e),sa(t,e),vt(t,e,a.SiteType),gt(e,a.SiteType),ve(t),ft(t,e),K(e),ne(s??[])}async function rt(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(r=>r.remove()),typeof QRCode<"u")try{let r=await new Promise((n,l)=>{QRCode.toDataURL(t,{width:220,margin:2},(o,i)=>{o?l(o):n(i)})});e.src=r,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function ct(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),n=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(r).then(()=>{n.textContent="Copied!"});else{let l=document.createElement("textarea");l.value=r,l.style.cssText="position:fixed;opacity:0",document.body.appendChild(l),l.select();try{document.execCommand("copy"),n.textContent="Copied!"}catch{}l.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),f.go("users")})}function fe(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",r);let n=UIkit.modal("#kp-edit-user-modal");n.show(),document.getElementById("edit-user-form").addEventListener("submit",async d=>{d.preventDefault();let u=d.target.querySelector('[type="submit"]'),b=u.innerHTML;u.disabled=!0,u.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(d.target),p={fname:k.get("fname").trim(),lname:k.get("lname").trim(),email:k.get("email").trim(),phone:k.get("phone").trim(),notify_email:k.get("notify_email")==="on",notify_sms:k.get("notify_sms")==="on"};if(s){p.role=parseInt(k.get("role"));let g=k.get("uname");g&&(p.uname=g.trim())}let v=k.get("password");v&&(p.password=v);try{await m.put(`/users/${e}`,p),n.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("User updated"),f.go("users")}catch(g){c.error(g.message),u.disabled=!1,u.innerHTML=b}});let l=document.getElementById("totp-setup-btn");l&&l.addEventListener("click",async()=>{l.disabled=!0,l.textContent="Setting up\u2026";try{let d=await m.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=d.secret,document.getElementById("totp-setup-area").style.display="",await rt(d.uri)}catch(d){c.error(d.message),l.disabled=!1,l.textContent="Enable TOTP"}});let o=document.getElementById("totp-confirm-btn");o&&o.addEventListener("click",async()=>{let d=document.getElementById("totp-confirm-code").value.trim();if(d.length!==6){c.error("Enter a 6-digit code");return}o.disabled=!0;try{let u=await m.post(`/users/${e}/totp/confirm`,{code:d});n.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("TOTP enabled"),u.backup_codes?.length?ct(u.backup_codes):f.go("users")}catch(u){c.error(u.message),o.disabled=!1}});let i=document.getElementById("totp-disable-btn");i&&i.addEventListener("click",async()=>{i.disabled=!0;try{await m.delete(`/users/${e}/totp`),c.success("TOTP disabled"),n.hide(),document.getElementById("kp-edit-user-modal")?.remove(),f.go("users")}catch(d){c.error(d.message),i.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function we(t){if(!q()){t.innerHTML=_("Access denied");return}let e=await m.get("/users");t.innerHTML=`
        <div class="kp-view-header">
