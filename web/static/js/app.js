...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let r=new AbortController,n=setTimeout(()=>r.abort(),s),o={method:t,headers:{"Content-Type":"application/json"},signal:r.signal};t!=="GET"&&t!=="HEAD"&&(o.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(o.body=JSON.stringify(a));try{let i=await fetch("/api"+e,o);clearTimeout(n);let l=i.status===204?null:await i.json().catch(()=>null);if(i.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!i.ok)throw new Error(l?.error||`HTTP ${i.status}`);return l}catch(i){throw clearTimeout(n),i}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var y=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function E(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let r=UIkit.modal(`#${a}`),n=document.getElementById("kp-clone-name"),o=document.getElementById("kp-clone-ok"),i=document.getElementById("kp-clone-cancel"),l=d=>{r.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(d)};o.addEventListener("click",()=>l(n.value.trim()||null),{once:!0}),i.addEventListener("click",()=>l(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>l(null),{once:!0}),r.show(),setTimeout(()=>n.focus(),150),n.addEventListener("keydown",d=>{d.key==="Enter"&&o.click()})})}function mt(t,e,a){return new Promise(s=>{let r="kp-sync-modal",n=t==="pull",o=n?"Pull From Parent":"Push To Parent",i=n?"cloud-download":"cloud-upload",l=n?a:e,d=n?e:a,u=`
            <div id="${r}" uk-modal>
...
`).forEach(p=>{if(!p)return;let v=document.createElement("div");v.className=p.match(/WAF BLOCK/i)?"kp-log-line-err":p.match(/WAF DETECT/i)?"kp-log-line-warn":p.match(/error|crit|emerg/i)?"kp-log-line-err":p.match(/warn/i)?"kp-log-line-warn":p.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=p,s.appendChild(v)});s.childElementCount>xe;)s.removeChild(s.firstChild);i.checked&&(s.scrollTop=s.scrollHeight)}function u(){e&&(e.close(),e=null),a=!1,r.disabled=!1,n.disabled=!0,l&&(l.textContent="Disconnected")}r.addEventListener("click",()=>{u();let k=t.querySelector("#admin-log-source").value,p=t.querySelector("#admin-log-tail").value,v=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${v}://${location.host}/api/logs/waf?tail=${p}`:`${v}://${location.host}/api/logs/proxy?tail=${p}`;e=new WebSocket(g),e.onopen=()=>{a=!0,r.disabled=!0,n.disabled=!1,l&&(l.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=h=>d(h.data),e.onerror=()=>{},e.onclose=()=>{a=!1,r.disabled=!1,n.disabled=!0,l&&(l.textContent="Disconnected")}}),n.addEventListener("click",u),o.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(u(),r.click())});let b=f.go.bind(f);f.go=function(k,p={}){return e&&u(),b(k,p)}}function _t(t){t.innerHTML=Se(),$e(t)}var nt=50;function Le(t,e,a){let s=Math.max(1,Math.ceil(t.total/nt)),r=(t.entries??[]).map(Ct).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        ...
        ${s}
    </div>`}function Ee(t){return`<span class="kp-badge ${t>=500?"kp-badge-error":t>=400?"kp-badge-warn":t>=300?"kp-badge-info":"kp-badge-ok"}">${t}</span>`}var T=t=>y(t??"");async function It(t,e){let a=new URLSearchParams({page:e,page_size:nt});return t.username&&a.set("username",t.username),t.action&&a.set("action",t.action),t.target_type&&a.set("target_type",t.target_type),t.date_from&&a.set("date_from",t.date_from),t.date_to&&a.set("date_to",t.date_to),t.auth!==""&&a.set("auth",t.auth),m.get(`/audit?${a}`)}function Te(t){return{username:t.querySelector("#al-filter-user").value.trim(),action:t.querySelector("#al-filter-action").value.trim(),target_type:t.querySelector("#al-filter-target").value.trim(),date_from:t.querySelector("#al-filter-date-from").value,date_to:t.querySelector("#al-filter-date-to").value,auth:t.querySelector("#al-filter-auth").value}}async function _e(t,e,a){async function s(o,i){let l=await It(o,i);t.querySelector("#al-table-body").innerHTML=(l.entries??[]).map(Ct).join("")||'<tr><td colspan="8" class="uk-text-center kp-text-dim">No records found</td></tr>';let d=Math.max(1,Math.ceil(l.total/nt)),u=t.querySelector("#al-pager");u&&(u.innerHTML=d>1?Pt(i,d):""),t.querySelector("#al-record-count").textContent=`${l.total} record${l.total!==1?"s":""}`,r(t,o,i,d),e=o,a=i}function r(o,i,l,d){o.querySelector("#al-prev")?.addEventListener("click",()=>s(i,l-1)),o.querySelector("#al-next")?.addEventListener("click",()=>s(i,l+1))}t.querySelector("#al-filter-apply")?.addEventListener("click",()=>{s(Te(t),1)}),t.querySelector("#al-filter-clear")?.addEventListener("click",()=>{["al-filter-user","al-filter-action","al-filter-target","al-filter-date-from","al-filter-date-to"].forEach(o=>{let i=t.querySelector(`#${o}`);i&&(i.value="")}),t.querySelector("#al-filter-auth").value="",s({username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},1)});let n=Math.max(1,Math.ceil(parseInt(t.querySelector("#al-record-count")?.textContent??"0")/nt));r(t,e,a,n),t.querySelector("#audit-log-panel")?.addEventListener("click",o=>{let i=o.target.closest(".al-diff-btn");if(!i)return;o.preventDefault(),o.stopPropagation();let l=i.dataset.prior??"",d=i.dataset.new??"",u="";l&&d?u=`=== BEFORE ===
`+st(l)+`
...
`+st(d):d?u=st(d):u=st(l),document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),r=document.getElementById("cs-node-version-wrap"),n=document.getElementById("cs-dotnet-version-wrap"),o=document.getElementById("cs-start-command-wrap"),i=document.getElementById("cs-wordpress-wrap");e.show();let l=document.getElementById("cs-domains-wrap"),d=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let u=parseInt(a.value);s.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),r.classList.toggle("uk-hidden",u!==4),n.classList.toggle("uk-hidden",u!==5),o.classList.toggle("uk-hidden",u!==4&&u!==5),i.classList.toggle("uk-hidden",u!==1||u===6),l.classList.toggle("uk-hidden",u===6),d.classList.toggle("uk-hidden",u!==6)}),document.getElementById("create-site-form").addEventListener("submit",async u=>{u.preventDefault();let b=u.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(u.target),v=parseInt(p.get("site_type")),g=null;v===4&&(g=parseInt(p.get("node_version"))),v===5&&(g=parseInt(p.get("dotnet_version")));let h={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:v,runtime_version:g,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(S=>S.trim()).filter(Boolean),install_wordpress:v===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${h.name}' as a reverse proxy...`:`Setting up '${h.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",w);try{await m.post("/sites",h,6e5),x(),c.success(`Site '${h.name}' created`),f.go("sites")}catch(S){x(),c.error(S.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function Ce(t){return`${t.toFixed(1)}%`}var ot=null;function bt(){return ot||(ot=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),ot)}function vt(t,e){return`
        ...
        </table>`}function qe(t,e,a,s,r){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let n=[...t].sort((b,k)=>{let p,v;switch(a){case"time":p=b.time,v=k.time;break;case"method":p=b.method,v=k.method;break;case"site":p=b.site_name,v=k.site_name;break;case"ip":p=b.client_ip,v=k.client_ip;break;default:p=b.status,v=k.status;break}return p<v?s?1:-1:p>v?s?-1:1:0}),o=50,i=Math.ceil(n.length/o),d=n.slice(e*o,(e+1)*o).map(b=>{let k=y(b.ua),p=b.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
        ...
        ${u}`}async function gt(t,e,a,s=!1){let r=document.getElementById("stats-drilldown-modal"),n=document.getElementById("stats-drilldown-title"),o=document.getElementById("stats-drilldown-body");if(!r||!o)return;n.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,o.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(r).show();let i=[],l=0,d="time",u=!0;function b(){console.log(s),o.innerHTML=qe(i,l,d,u,s),o.querySelectorAll("th[data-dd-col]").forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.ddCol;d===p?u=!u:(d=p,u=!0),l=0,b()})}),o.querySelectorAll("[data-dd-page]").forEach(k=>{k.addEventListener("click",()=>{l=parseInt(k.dataset.ddPage,10),b()})})}try{i=await m.get(`${t}?hour=${encodeURIComponent(e)}&status=${a}`)}catch(k){o.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${y(k.message)}</p>`;return}b()}function ht(t,e,a){let s=a===6,r=null;function n(){if(s)return;let l=t.querySelector("#stats-pod-indicator"),d=t.querySelector("#stats-pod-table-wrap");if(!d)return;let u=location.protocol==="https:"?"wss":"ws";r=new WebSocket(`${u}://${location.host}/api/sites/${e}/stats/pod`),r.onopen=()=>{l&&(l.className="kp-status kp-status-running",l.textContent="Live")},r.onmessage=b=>{try{let k=JSON.parse(b.data);d.innerHTML=Ie(k.containers??[]),d.querySelectorAll(".kp-stats-pod-role-btn").forEach(p=>{p.addEventListener("click",async()=>{let v=p.style.color;p.style.color="var(--kp-warning)";let g=p.dataset.container.split("-").pop();try{await m.post(`/sites/${e}/containers/${g}/restart`),c.success(`${g} restarted`)}catch(h){p.style.color=v,c.error(h.message)}})})}catch{}},r.onerror=()=>{l&&(l.className="kp-status kp-status-error",l.textContent="Error")},r.onclose=()=>{l&&l.textContent==="Live"&&(l.className="kp-status kp-status-stopped",l.textContent="Disconnected")}}function o(){r&&r.readyState===WebSocket.OPEN&&r.close(),r=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{Bt(e)}),n();let i=new MutationObserver(()=>{document.getElementById("stats-panel")||(o(),i.disconnect())});i.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ft(t,e){let a=e===6;await Pe(t),a||await Bt(t)}async function Mt(t){let e=await m.get("/sites")??[];t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${e.length===0?tt("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>it()),Me()}function Be(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,r=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
        ...
        <div id="kp-asn-lookup-modal" uk-modal>
            <div class="uk-modal-dialog kp-modal uk-modal-body">
        ...
        </div>`);let a=UIkit.modal("#kp-asn-lookup-modal");a.show();let s=async()=>{let r=document.getElementById("asn-lookup-q").value.trim();if(!r)return;let n=document.getElementById("asn-lookup-result");n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{let o=await m.get(`/security/asn/lookup?q=${encodeURIComponent(r)}`);if(!o?.asn){n.innerHTML='<p class="kp-muted uk-text-small">No ASN found for <span class="kp-mono"></span>.</p>',n.querySelector(".kp-mono").textContent=o?.ip||r;return}n.innerHTML=`
                ...
                </button>`,n.querySelector("#asn-lookup-ip").textContent=o.ip,n.querySelector("#asn-lookup-org").textContent=o.org||"",n.querySelector("#asn-lookup-add").addEventListener("click",()=>{let i=t.querySelector("#sec-asn-blacklist"),l=`AS${o.asn}`;i.value.split(`
`).some(d=>d.trim().toUpperCase()===l)||(i.value=i.value.trim()?`${i.value.replace(/\s+$/,"")}
${l}`:l),a.hide(),c.success(`${l} added to blacklist \u2014 save to apply`)})}catch(o){n.innerHTML="",c.error(o.message)}};document.getElementById("asn-lookup-go").addEventListener("click",s),document.getElementById("asn-lookup-q").addEventListener("keydown",r=>{r.key==="Enter"&&s()})}async function Ht(t){if(!q()){t.innerHTML=_("Access denied");return}t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${V(null)}`,lt(t),B(t)}function De(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';case"expired":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Expired certificate"></span>';case"mismatch":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Certificate does not match this domain"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Rt(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Ft(t){if(!q()){t.innerHTML=_("Access denied");return}let[e,a,s,r,n,o]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
                ...
                <form id="resource-form" class="uk-form-stacked">
                    <div class="uk-margin">
                        ...
                        <input class="uk-input kp-input" id="resource-ram-reserve" name="resource_ram_reserve_gb" type="number"
                            ...
                            value="${o.resource_ram_reserve_gb??"2"}">
                        <p class="kp-muted uk-text-small uk-margin-small-top">
                    ...
                    <div class="uk-margin">
                        ...
                        <input class="uk-input kp-input" id="resource-poll-interval" name="resource_poll_interval" type="number"
                            ...
                            value="${o.resource_poll_interval??"30"}">
                    </div>
                    <div class="uk-margin">
                        ...
                        <input class="uk-input kp-input" id="resource-throttle-pct" name="resource_throttle_pct" type="number"
                            ...
                            value="${o.resource_throttle_pct??"50"}">
                        <p class="kp-muted uk-text-small uk-margin-small-top">
    ...
    `,e.admin_domain&&Rt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async i=>{i.preventDefault();let l=i.target.querySelector('[type="submit"]'),d=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(i.target).get("admin_domain").trim()};try{await m.put("/settings",b),c.success("Settings saved"),Rt(b.admin_domain)}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=d}}),document.getElementById("settings-import").addEventListener("change",async i=>{let l=i.target.files[0];if(!l)return;let d=new FormData;d.append("file",l);try{let u=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:d}),b=u.status===204?null:await u.json().catch(()=>null);if(!u.ok)throw new Error(b?.error||`HTTP ${u.status}`);c.success("Settings imported")}catch(u){c.error(u.message)}finally{i.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async i=>{i.preventDefault();let l=i.target.querySelector('[type="submit"]'),d=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={backup_schedule:u.get("backup_schedule").trim(),backup_retain_days:u.get("backup_retain_days").trim()};try{await m.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=d}}),document.getElementById("s3-form").addEventListener("submit",async i=>{i.preventDefault();let l=i.target.querySelector('[type="submit"]'),d=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={s3_endpoint:u.get("s3_endpoint").trim(),s3_bucket:u.get("s3_bucket").trim(),s3_region:u.get("s3_region").trim(),s3_access_key:u.get("s3_access_key").trim()},k=u.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await m.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=d}}),document.getElementById("smtp-form").addEventListener("submit",async i=>{i.preventDefault();let l=i.target.querySelector('[type="submit"]'),d=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={smtp_host:u.get("smtp_host").trim(),smtp_port:u.get("smtp_port").trim(),smtp_username:u.get("smtp_username").trim(),smtp_from:u.get("smtp_from").trim(),smtp_tls:u.get("smtp_tls")?"true":"false"},k=u.get("smtp_password").trim();k&&(b.smtp_password=k);try{await m.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=d}}),document.getElementById("sns-form").addEventListener("submit",async i=>{i.preventDefault();let l=i.target.querySelector('[type="submit"]'),d=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={aws_access_key:u.get("aws_access_key").trim(),aws_region:u.get("aws_region").trim(),aws_sns_sender_id:u.get("aws_sns_sender_id").trim()},k=u.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await m.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=d}}),document.getElementById("resource-form").addEventListener("submit",async i=>{i.preventDefault();let l=i.target.querySelector('[type="submit"]'),d=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={resource_ram_reserve_gb:u.get("resource_ram_reserve_gb").trim(),resource_poll_interval:u.get("resource_poll_interval").trim(),resource_throttle_pct:u.get("resource_throttle_pct").trim(),resource_webhook_url:u.get("resource_webhook_url").trim()};try{await m.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=d}})}function Nt(t){return`
        ...
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            ...
            <tbody>${t.map(s=>{let r=j(P,s.name),n=s.is_dir,o=Ne(s.name,n),i=n?`<a href="#" class="fm-nav" data-path="${y(r)}"><span uk-icon="icon: ${Xt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</a>`:`<span><span uk-icon="icon: ${Xt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</span>`;return`
            <tr data-path="${y(r)}" data-name="${y(s.name)}" data-dir="${n?1:0}" data-mode="${y(s.mode)}">
                ...
                <td class="uk-text-right uk-text-nowrap">
                    ${o?`<button class="kp-fm-act fm-edit" data-path="${y(r)}" uk-tooltip="Edit"><span uk-icon="icon: pencil; ratio: 0.85"></span></button>`:""}
...
`).forEach(v=>{if(!v)return;let g=document.createElement("div");g.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=v,r.appendChild(g)});r.childElementCount>Ke;)r.removeChild(r.firstChild);l.checked&&(r.scrollTop=r.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,n.disabled=!1,o.disabled=!0,d&&(d.textContent="Disconnected")}n.addEventListener("click",()=>{b();let p=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",h=p==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:p==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:p==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:`${g}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${v}`;a=new WebSocket(h),a.onopen=()=>{s=!0,n.disabled=!0,o.disabled=!1,d&&(d.textContent=`Connected \u2014 ${p}`)},a.onmessage=w=>u(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,n.disabled=!1,o.disabled=!0,d&&(d.textContent="Disconnected")}}),o.addEventListener("click",b),i.addEventListener("click",()=>{r.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),n.click())});let k=f.go.bind(f);f.go=function(p,v={}){return a&&b(),k(p,v)}}function Je(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';case"expired":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Expired certificate"></span>';case"mismatch":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Certificate does not match this domain"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ae(t,e){try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Je(a.status))}catch{}}function se(t){t.forEach(e=>ae(e.Domain,e.ID))}function ne(t,e,a,s=0,r=null){let n=t.SiteType!==3&&t.PMAPort>0;return`
            ...
            <form id="waf-override-form" class="uk-form-stacked">
                ...
                <div class="uk-flex uk-flex-right uk-margin-top" style="gap:8px">
        ...
        </div>`}async function $t(t){let e=document.getElementById("waf-tab-panel");if(!e)return;e.innerHTML=Xe();let a=document.getElementById("waf-export-btn");a&&(a.href=`/api/sites/${t}/waf/export`);try{let s=await m.get(`/sites/${t}/waf`),r=document.getElementById("waf-override"),n=document.getElementById("waf-site-exclusions");r&&(r.value=String(s.Override??0)),n&&(n.value=s.Exclusions??"");let o=document.getElementById("waf-plugins-list");if(o){let[i,l]=await Promise.all([m.get("/settings/waf/plugins"),m.get(`/sites/${t}/waf/plugins`)]),d=new Set(l??[]);!i||i.length===0?o.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':window.matchMedia("(max-width: 959px)").matches?o.innerHTML=`
                    ...
                    <div class="waf-plugin-pills">
                        ...
                        `).join("")}
                    </div>`,o.querySelectorAll(".waf-plugin-pill").forEach(u=>{u.addEventListener("click",()=>u.classList.toggle("active"))}))}}catch(s){c.error("Failed to load WAF settings: "+s.message)}}function pe(t,e,a){t.addEventListener("submit",async s=>{if(s.target.id!=="waf-override-form")return;s.preventDefault();let r=s.target.querySelector('[type="submit"]'),n=r.innerHTML;r.disabled=!0,r.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=new FormData(s.target),i={override:parseInt(o.get("override"),10),exclusions:o.get("exclusions").trim()};try{await m.put(`/sites/${e}/waf`,i);let l=document.querySelector(".waf-plugin-select"),d=l?[...l.selectedOptions].map(u=>u.value):[...document.querySelectorAll(".waf-plugin-pill.active")].map(u=>u.dataset.plugin);await m.put(`/sites/${e}/waf/plugins`,d),c.success("WAF override saved \u2014 engine recompiling in background")}catch(l){c.error(l.message)}finally{r.disabled=!1,r.innerHTML=n}},{signal:a}),t.querySelector("#waf-import")?.addEventListener("change",async s=>{let r=s.target.files[0];if(!r)return;let n=new FormData;n.append("file",r);try{let o=await fetch(`/api/sites/${e}/waf/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:n}),i=o.status===204?null:await o.json().catch(()=>null);if(!o.ok)throw new Error(i?.error||`HTTP ${o.status}`);await $t(e),c.success("WAF settings imported")}catch(o){c.error(o.message)}finally{s.target.value=""}})}var Ge=[{label:"Cache Flush",cmd:"cache flush"},{label:"Plugin List",cmd:"plugin list"},{label:"Theme List",cmd:"theme list"},{label:"User List",cmd:"user list"},{label:"Core Check",cmd:"core check-update"},{label:"Core Update",cmd:"core update"},{label:"Plugin Updates",cmd:"plugin update --all"},{label:"Theme Updates",cmd:"theme update --all"},{label:"Rewrite Flush",cmd:"rewrite flush"},{label:"Transient Delete",cmd:"transient delete --all"},{label:"Search Replace",cmd:"search-replace '' ''"}];function me(t){return`
        <div class="kp-wpcli">
        ...
        </div>`}function ke(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),r=t.querySelector("#wpcli-run"),n=t.querySelector("#wpcli-clear"),o=t.querySelector("#wpcli-status"),i=[],l=-1;function d(k,p=""){k.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");p?g.className=p:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function u(k){if(k=k.trim(),!k)return;i.unshift(k),l=-1,d(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,r.disabled=!0,o&&(o.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let h=g.data;if(h.trim()==="[done]"){v.close();return}if(h.startsWith("[info]")){d(h,"kp-muted");return}if(h.startsWith("[error]")){d(h,"kp-log-line-err");return}d(h)},v.onerror=()=>{d("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,r.disabled=!1,o&&(o.textContent="Ready"),s.focus()}}r.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){u(s.value),s.value="",l=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),l<i.length-1&&(l++,s.value=i[l]);return}k.key==="ArrowDown"&&(k.preventDefault(),l>0?(l--,s.value=i[l]):(l=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let v=p.indexOf("''")+1;s.setSelectionRange(v,v);return}u(p)})}),n.addEventListener("click",()=>{a.innerHTML=""});let b=f.go.bind(f);f.go=function(k,p={}){return b(k,p)},s.focus()}var O=null,M=null;function be(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),r=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(i,l=!1){UIkit.switcher(a).show(i),e.querySelectorAll(":scope > li[data-pill]").forEach(d=>d.classList.remove("kp-pill-active")),l?(s?.classList.add("kp-pill-active"),r?.querySelectorAll("a[data-switcher]").forEach(d=>{d.classList.toggle("kp-dd-active",parseInt(d.dataset.switcher,10)===i)})):(s?.classList.remove("kp-pill-active"),r?.querySelectorAll("a[data-switcher]").forEach(d=>d.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(i=>{i.addEventListener("click",l=>{l.preventDefault();let d=parseInt(i.closest("li").dataset.pill,10);n(d,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",i=>{i.stopPropagation(),r.hidden=!r.hidden,s.classList.toggle("kp-pill-active",!r.hidden)}),r?.querySelectorAll("a[data-switcher]").forEach(i=>{i.addEventListener("click",l=>{l.preventDefault(),r.hidden=!0,n(parseInt(i.dataset.switcher,10),!0)})}),document.addEventListener("click",i=>{r&&!s.contains(i.target)&&(r.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(1)}function Qe(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        `).join("")}function aa(t,e){M&&(M.close(),M=null);let a=document.getElementById("sd-health-badges");if(!a)return;let s=location.protocol==="https:"?"wss":"ws";M=new WebSocket(`${s}://${location.host}/api/sites/${e}/health/stream`),M.onmessage=r=>{try{let n=JSON.parse(r.data);a.innerHTML=ea(n),a.querySelectorAll(".kp-health-badge").forEach(o=>{o.addEventListener("click",async()=>{o.style.color=Et("starting");let i=o.dataset.container,l=i.split("-").pop();try{await m.post(`/sites/${e}/containers/${l}/restart`),c.success(`${ve(i)} restarted`)}catch(d){o.style.color=Et("none"),c.error(d.message)}})})}catch{}},M.onerror=()=>{},M.onclose=()=>{M=null}}async function ge(t,{id:e}){let[{site:a,domains:s,sftp:r},n,o]=await Promise.all([m.get(`/sites/${e}`),m.get("/sites"),m.get(`/sites/${e}/configs`)]),i=Array.isArray(n)?n:[],l=a.SiteType===1||a.SiteType===2,d=a.SiteType===6,u=[1,2,4,5].includes(a.SiteType);if(O&&O.abort(),O=new AbortController,t.innerHTML=`
        <div class="kp-view-header">
