"use strict";(()=>{var d={async _req(e,t,s,n=6e4){let a=new AbortController,i=setTimeout(()=>a.abort(),n),r={method:e,headers:{"Content-Type":"application/json"},signal:a.signal};s!==void 0&&(r.body=JSON.stringify(s));try{let c=await fetch("/api"+t,r);clearTimeout(i);let o=c.status===204?null:await c.json().catch(()=>null);if(c.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!c.ok)throw new Error(o?.error||`HTTP ${c.status}`);return o}catch(c){throw clearTimeout(i),c}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e)};var K=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',S=e=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",s),UIkit.modal("#kp-progress-modal").show()}function h(){let e=document.getElementById("kp-progress-modal");e&&(UIkit.modal(e).hide(),setTimeout(()=>e.remove(),300))}var m={routes:{},register(e,t){this.routes[e]=t},async go(e,t={}){let s=Object.keys(t).length?e+"/"+Object.values(t).join("/"):e;window.location.hash=s,document.querySelectorAll(".kp-nav-link").forEach(i=>{i.classList.toggle("kp-active",i.dataset.view===e)});let n=this.routes[e];if(!n)return;let a=document.getElementById("kp-view");a.innerHTML=K();try{await n(a,t)}catch(i){a.innerHTML=S(i.message)}}};function j(){let t=(window.location.hash.replace("#","")||"dashboard").split("/"),s=t[0],n={};return s==="site-detail"&&t[1]&&(n.id=t[1]),{view:s,params:n}}var l={show(e,t="info",s=7e3){let n={success:"check",error:"warning",info:"info"},a=document.createElement("div");a.className=`kp-toast kp-toast-${t}`,a.innerHTML=`<span uk-icon="${n[t]||"info"}"></span><span>${e}</span>`,document.getElementById("kp-toasts").appendChild(a),UIkit.icon(a.querySelector("[uk-icon]")),setTimeout(()=>a.remove(),s)},success:e=>l.show(e,"success"),error:e=>l.show(e,"error"),info:e=>l.show(e,"info")};async function _(e){let t=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let s=UIkit.modal("#kp-edit-site-modal"),n=document.getElementById("es-site-type"),a=document.getElementById("es-php-version-wrap"),i=document.getElementById("es-node-version-wrap"),r=document.getElementById("es-dotnet-version-wrap"),c=document.getElementById("es-start-command-wrap"),o=document.getElementById("es-wordpress-wrap");s.show();let p=u=>{a.classList.toggle("uk-hidden",u!==1&&u!==2),i.classList.toggle("uk-hidden",u!==4),r.classList.toggle("uk-hidden",u!==5),c.classList.toggle("uk-hidden",u!==4&&u!==5),o.classList.toggle("uk-hidden",u!==1)};p(e.SiteType),n.addEventListener("change",()=>p(parseInt(n.value))),document.getElementById("edit-site-form").addEventListener("submit",async u=>{u.preventDefault();let v=u.target.querySelector('[type="submit"]'),k=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let g=new FormData(u.target),b=parseInt(g.get("site_type")),f=null;b===4&&(f=parseInt(g.get("node_version"))),b===5&&(f=parseInt(g.get("dotnet_version")));let x={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:b,runtime_version:f,start_command:g.get("start_command")?.trim()||""},W=b===1?g.get("install_wordpress")==="on":!1;try{await d.put(`/sites/${e.ID}`,x),s.hide(),document.getElementById("kp-edit-site-modal")?.remove(),y("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:W}),h(),l.success("Site updated and pod recreated")}catch(O){h(),l.error("Site saved but pod recreate failed: "+O.message)}m.go("site-detail",{id:String(e.ID)})}catch(O){l.error(O.message),v.disabled=!1,v.innerHTML=k}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function H(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let t=UIkit.modal("#kp-create-site-modal"),s=document.getElementById("cs-site-type"),n=document.getElementById("cs-php-version-wrap"),a=document.getElementById("cs-node-version-wrap"),i=document.getElementById("cs-dotnet-version-wrap"),r=document.getElementById("cs-start-command-wrap"),c=document.getElementById("cs-wordpress-wrap");t.show(),s.addEventListener("change",()=>{let o=parseInt(s.value);n.classList.toggle("uk-hidden",o!==1&&o!==2),a.classList.toggle("uk-hidden",o!==4),i.classList.toggle("uk-hidden",o!==5),r.classList.toggle("uk-hidden",o!==4&&o!==5),c.classList.toggle("uk-hidden",o!==1)}),document.getElementById("create-site-form").addEventListener("submit",async o=>{o.preventDefault();let p=o.target.querySelector('[type="submit"]'),u=p.innerHTML;p.disabled=!0,p.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let v=new FormData(o.target),k=parseInt(v.get("site_type")),g=null;k===4&&(g=parseInt(v.get("node_version"))),k===5&&(g=parseInt(v.get("dotnet_version")));let b={name:v.get("name").trim(),php_version:parseInt(v.get("php_version"))||3,site_type:k,runtime_version:g,start_command:v.get("start_command")?.trim()||"",domains:v.get("domains").split(`
`).map(f=>f.trim()).filter(Boolean),install_wordpress:k===1?v.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove(),y("Creating Site",`Setting up '${b.name}' \u2014 pulling images and provisioning containers...`);try{await d.post("/sites",b),h(),l.success(`Site '${b.name}' created`),m.go("sites")}catch(f){h(),l.error(f.message),p.disabled=!1,p.innerHTML=u}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function Q(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        <div class="kp-site-grid">
            ${t.length===0?M("world","No sites yet \u2014 create one to get started"):t.map(V).join("")}
            ...
            <div class="kp-site-card-header">
                <div>
                    ...
                    <div class="kp-site-meta">
                        ...
                        <span class="kp-site-meta-item"><span uk-icon="icon: code; ratio: 0.75"></span> ${q(e.SiteType)}${P(e)?" / "+P(e):""}</span>
        ...
        <div class="kp-site-grid">
            ${t.length===0?M("world","No sites yet"):t.slice(0,6).map(V).join("")}
        </div>`,document.getElementById("dash-new-site")?.addEventListener("click",()=>H())}function R(e=null){let t=e?`/sites/${e}/security/ip`:"/security/ip",s=e?`/sites/${e}/security/ua`:"/security/ua",n=e?`/sites/${e}/waf`:"/settings/waf";return`
        ...
        </div>`}async function T(e){let t=e.querySelector("#security-panel");if(!t)return;let s=t.dataset.ipBase,n=t.dataset.uaBase,a=t.dataset.wafBase;try{let i=[d.get(s),d.get(n)];t.dataset.siteId||i.push(d.get(a));let[r,c,o]=await Promise.all(i);if(e.querySelector("#sec-ip-whitelist").value=r.whitelist??"",e.querySelector("#sec-ip-blacklist").value=r.blacklist??"",e.querySelector("#sec-ua-whitelist").value=c.whitelist??"",e.querySelector("#sec-ua-blacklist").value=c.blacklist??"",o){let p=e.querySelector("#sec-waf-enabled"),u=e.querySelector("#sec-waf-audit"),v=e.querySelector("#sec-waf-mode"),k=e.querySelector("#sec-waf-paranoia"),g=e.querySelector("#sec-waf-exclusions");p&&(p.checked=!!o.Enabled),u&&(u.checked=!!o.AuditLog),v&&(v.value=String(o.Mode??0)),k&&(k.value=String(o.ParanoiaLevel??1)),g&&(g.value=o.Exclusions??"")}}catch(i){l.error("Failed to load security rules: "+i.message)}}function A(e){let t=e.querySelector("#security-panel");if(!t)return;let s=t.dataset.ipBase,n=t.dataset.uaBase;e.querySelector("#sec-ip-save")?.addEventListener("click",async()=>{let a=e.querySelector("#sec-ip-save"),i=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put(s,{whitelist:e.querySelector("#sec-ip-whitelist").value,blacklist:e.querySelector("#sec-ip-blacklist").value}),l.success("IP rules saved")}catch(r){l.error(r.message)}finally{a.disabled=!1,a.innerHTML=i}}),e.querySelector("#sec-ua-save")?.addEventListener("click",async()=>{let a=e.querySelector("#sec-ua-save"),i=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put(n,{whitelist:e.querySelector("#sec-ua-whitelist").value,blacklist:e.querySelector("#sec-ua-blacklist").value}),l.success("UA rules saved")}catch(r){l.error(r.message)}finally{a.disabled=!1,a.innerHTML=i}}),e.querySelector("#sec-waf-save")?.addEventListener("click",async()=>{let a=e.querySelector("#sec-waf-save"),i=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put(t.dataset.wafBase,{enabled:e.querySelector("#sec-waf-enabled").checked,mode:parseInt(e.querySelector("#sec-waf-mode").value,10),paranoia_level:parseInt(e.querySelector("#sec-waf-paranoia").value,10),audit_log:e.querySelector("#sec-waf-audit").checked,exclusions:e.querySelector("#sec-waf-exclusions").value.trim()}),l.success("WAF settings saved \u2014 engine recompiling in background")}catch(r){l.error(r.message)}finally{a.disabled=!1,a.innerHTML=i}}),e.querySelector("#sec-ip-import")?.addEventListener("change",async a=>{let i=a.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api"+s+"/import",{method:"POST",body:r}),o=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(o?.error||`HTTP ${c.status}`);await T(e),l.success("IP rules imported")}catch(c){l.error(c.message)}finally{a.target.value=""}}),e.querySelector("#sec-ua-import")?.addEventListener("change",async a=>{let i=a.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api"+n+"/import",{method:"POST",body:r}),o=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(o?.error||`HTTP ${c.status}`);await T(e),l.success("UA rules imported")}catch(c){l.error(c.message)}finally{a.target.value=""}})}async function Y(e){if(!E()){e.innerHTML=S("Access denied");return}e.innerHTML=`
        <div class="kp-view-header">
        ...
        ${R(null)}`,A(e),T(e)}function fe(e){switch(e){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function J(e){let t=document.getElementById("admin-domain-ssl");if(!(!t||!e))try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`);t.outerHTML=fe(s.status)}catch{}}async function X(e){if(!E()){e.innerHTML=S("Access denied");return}let[t,s,n]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf")]);e.innerHTML=`
    <div class="kp-view-header">
    ...
    `,t.admin_domain&&J(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async a=>{a.preventDefault();let i=a.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o={admin_domain:new FormData(a.target).get("admin_domain").trim()};try{await d.put("/settings",o),l.success("Settings saved"),J(o.admin_domain)}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("backup-form").addEventListener("submit",async a=>{a.preventDefault();let i=a.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(a.target),o={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",o),l.success("Backup settings saved")}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async a=>{a.preventDefault();let i=a.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(a.target),o={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},p=c.get("s3_secret_key").trim();p&&(o.s3_secret_key=p);try{await d.put("/settings/backup",o),l.success("S3 settings saved")}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}})}function Z(e){return`
        ...
        </div>`}function he(e){if(!e||e.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No snapshots yet.</p>';let t=a=>a===2?'<span class="kp-mono" style="color:var(--kp-cyan)">S3</span>':'<span class="kp-mono" style="color:var(--kp-blue)">Local</span>',s=a=>a<1024?`${a} B`:a<1048576?`${(a/1024).toFixed(1)} KB`:a<1073741824?`${(a/1048576).toFixed(1)} MB`:`${(a/1073741824).toFixed(2)} GB`;return`
        ...
        </table>`}async function B(e,t){try{let[s,n]=await Promise.all([d.get(`/sites/${t}/backup-repo`),d.get(`/sites/${t}/backups`)]),a=e.querySelector("#backup-local-enabled"),i=e.querySelector("#backup-s3-enabled");a&&(a.checked=!!s.LocalEnabled),i&&(i.checked=!!s.S3Enabled);let r=e.querySelector("#backup-error-banner");if(r)if(s.last_error){let o=s.last_error_at?` (${new Date(s.last_error_at).toLocaleString()})`:"";r.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${o}:</strong> ${s.last_error}</p>
                    </div>`}else r.innerHTML="";let c=e.querySelector("#backup-list-wrap");c&&(c.innerHTML=he(n))}catch(s){let n=e.querySelector("#backup-list-wrap");n&&(n.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${s.message}</p>`)}}function ee(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,s),l.success("Backup destinations saved")}catch(n){l.error(n.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(i){l.error(i.message);return}y("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let n=Date.now()+600*1e3,a=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>s||Date.now()>n)&&(clearInterval(a),h(),await B(e,t),Date.now()<=n&&l.success("Backup complete"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let n=s.target.closest(".backup-restore-btn");if(n){let r=n.dataset.id;if(!await w("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${r}/restore`)}catch(v){l.error(v.message);return}y("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let o=Date.now(),p=Date.now()+900*1e3,u=setInterval(async()=>{try{let v=await d.get(`/sites/${t}/backups/restore-status`);(!v?.active||Date.now()>p)&&(clearInterval(u),h(),v?.active?l.error("Restore timed out"):l.success("Restore complete"),await B(e,t))}catch{}},3e3);return}let a=s.target.closest(".backup-delete-btn");if(a){let r=a.dataset.id;if(!await w("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;y("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${r}`),h(),l.success("Snapshot deleted"),await B(e,t)}catch(o){h(),l.error(o.message)}}let i=s.target.closest(".backup-download-btn");if(i){let r=i.dataset.id;y("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let c=document.createElement("a");c.href=`/api/sites/${t}/backups/${r}/download`,c.style.display="none",document.body.appendChild(c),c.click(),document.body.removeChild(c),setTimeout(()=>{h()},5e3)},300);return}})}var N={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function C(e,t,s){let n=s?Object.entries(s):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
    ...
    </div>`}function se(e,t){e.addEventListener("click",s=>{if(s.target.closest(".cfg-add-row")){let n=s.target.closest(".cfg-add-row");e.querySelector(`.cfg-rows[data-type="${n.dataset.type}"]`).insertAdjacentHTML("beforeend",I())}}),e.addEventListener("click",s=>{s.target.closest(".cfg-del-row")&&s.target.closest(".kp-config-row").remove()}),e.addEventListener("click",async s=>{let n=s.target.closest(".cfg-save");if(!n)return;let{type:a,site:i}=n.dataset,r=e.querySelectorAll(`.cfg-rows[data-type="${a}"] .kp-config-row`),c={};if(r.forEach(o=>{let p=o.querySelector(".cfg-key").value.trim(),u=o.querySelector(".cfg-val").value.trim();p&&(c[p]=u)}),a==="5"){let o=e.querySelector(".varnish-enabled-toggle");c.enabled=o?.checked?"true":"false"}try{await d.put(`/sites/${i}/configs/${a}`,c),l.success(`${N[a]} config saved`)}catch(o){l.error(o.message)}}),e.addEventListener("click",async s=>{let n=s.target.closest(".cfg-reset");if(!n)return;let{type:a,site:i}=n.dataset;if(await w("Reset Config",`Reset ${N[a]} config to defaults?`))try{let c=await d.post(`/sites/${i}/configs/${a}/reset`),o=e.querySelector(`.cfg-rows[data-type="${a}"]`);o.innerHTML=Object.entries(c).map(([p,u])=>I(p,u)).join(""),l.success(`${N[a]} reset to defaults`)}catch(c){l.error(c.message)}}),e.addEventListener("change",async s=>{let n=s.target.closest(".cfg-import-input");if(!n)return;let{type:a,site:i}=n.dataset,r=n.files[0];if(!r)return;let c=new FormData;c.append("file",r);try{let o=await fetch(`/api/sites/${i}/configs/${a}/import`,{method:"POST",body:c}),p=o.status===204?null:await o.json().catch(()=>null);if(!o.ok)throw new Error(p?.error||`HTTP ${o.status}`);let u=e.querySelector(`.cfg-rows[data-type="${a}"]`);u.innerHTML=Object.entries(p).map(([v,k])=>I(v,k)).join(""),l.success(`${N[a]} config imported`)}catch(o){l.error(o.message)}finally{n.value=""}})}function ae(e,t){return`
        <div>
        ...
        </div>`}function ne(e,t){let s=null,n=!1,a=e.querySelector("#log-output"),i=e.querySelector("#log-connect"),r=e.querySelector("#log-disconnect"),c=e.querySelector("#log-clear"),o=e.querySelector("#log-autoscroll"),p=e.querySelector("#log-status");function u(g){g.split(`
`).forEach(b=>{if(!b)return;let f=document.createElement("div");f.className=b.match(/WAF BLOCK/i)?"kp-log-line-err":b.match(/WAF DETECT/i)?"kp-log-line-warn":b.match(/error|crit|emerg/i)?"kp-log-line-err":b.match(/warn/i)?"kp-log-line-warn":b.match(/info|notice/i)?"kp-log-line-info":"",f.textContent=b,a.appendChild(f)}),o.checked&&(a.scrollTop=a.scrollHeight)}function v(){s&&(s.close(),s=null),n=!1,i.disabled=!1,r.disabled=!0,p&&(p.textContent="Disconnected")}i.addEventListener("click",()=>{v();let g=e.querySelector("#log-container").value,b=e.querySelector("#log-tail").value,f=location.protocol==="https:"?"wss":"ws",x=g==="waf"?`${f}://${location.host}/api/sites/${t}/logs/waf?tail=${b}`:`${f}://${location.host}/api/sites/${t}/logs?container=${g}&tail=${b}`;s=new WebSocket(x),s.onopen=()=>{n=!0,i.disabled=!0,r.disabled=!1,p&&(p.textContent=`Connected \u2014 ${g}`)},s.onmessage=W=>u(W.data),s.onerror=()=>{},s.onclose=()=>{n=!1,i.disabled=!1,r.disabled=!0,p&&(p.textContent="Disconnected")}}),r.addEventListener("click",v),c.addEventListener("click",()=>{a.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{s&&s.readyState===WebSocket.OPEN&&(v(),i.click())});let k=m.go.bind(m);m.go=function(g,b={}){return s&&v(),k(g,b)}}function ye(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ie(e,t){try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),n=document.getElementById(`ssl-icon-${t}`);n&&(n.outerHTML=ye(s.status))}catch{}}function oe(e){e.forEach(t=>ie(t.Domain,t.ID))}function le(e,t,s){let n=e.SiteType!==3&&e.PMAPort>0;return`
    ...
    </div>`}function ce(e,t){e.querySelector("#domain-add-btn")?.addEventListener("click",()=>{e.querySelector("#domain-add-form").classList.remove("uk-hidden")}),e.querySelector("#domain-cancel-btn")?.addEventListener("click",()=>{e.querySelector("#domain-add-form").classList.add("uk-hidden")}),e.querySelector("#domain-save-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#domain-add-input").value.trim();if(s)try{let n=await d.post(`/sites/${t}/domains`,{domain:s});e.querySelector("#domain-list").insertAdjacentHTML("beforeend",re(n)),ie(n.Domain,n.ID),e.querySelector("#domain-add-form").classList.add("uk-hidden"),e.querySelector("#domain-add-input").value="",l.success("Domain added")}catch(n){l.error(n.message)}}),e.querySelector("#domain-list")?.addEventListener("click",async s=>{let n=s.target.closest('[data-action="delete-domain"]');if(!(!n||!await w("Remove Domain","Remove this domain from the site?")))try{await d.delete(`/sites/${t}/domains/${n.dataset.did}`),n.closest("[data-domain-id]").remove(),l.success("Domain removed")}catch(i){l.error(i.message)}})}function de(e,t){e.querySelector("#sftp-regen-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#sftp-regen-btn"),n=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.post(`/sites/${t}/sftp-regen`),l.success("SFTP password regenerated"),m.go("site-detail",{id:String(t)})}catch(a){l.error(a.message),s.disabled=!1,s.innerHTML=n}}),e.querySelector("#sftp-copy-btn")?.addEventListener("click",()=>{let s=e.querySelector("#sftp-pass-display")?.textContent;if(s)if(navigator.clipboard)navigator.clipboard.writeText(s).then(()=>l.success("Password copied to clipboard")).catch(()=>l.error("Failed to copy password"));else{let n=document.createElement("textarea");n.value=s,n.style.cssText="position:fixed;opacity:0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n),l.success("Password copied to clipboard")}}),e.querySelector("#pma-open-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#pma-open-btn"),n=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div> Opening...';try{let a=await d.post(`/sites/${t}/pma-token`);window.open(a.url,"_blank")}catch(a){l.error(a.message)}finally{s.disabled=!1,s.innerHTML=n}})}var we=[{label:"Cache Flush",cmd:"cache flush"},{label:"Plugin List",cmd:"plugin list"},{label:"Theme List",cmd:"theme list"},{label:"User List",cmd:"user list"},{label:"Core Check",cmd:"core check-update"},{label:"Core Update",cmd:"core update"},{label:"Plugin Updates",cmd:"plugin update --all"},{label:"Theme Updates",cmd:"theme update --all"},{label:"Rewrite Flush",cmd:"rewrite flush"},{label:"Transient Delete",cmd:"transient delete --all"},{label:"Search Replace",cmd:"search-replace '' ''"}];function pe(e){return`
        <div>
        ...
        </div>`}function ue(e,t){let s=e.querySelector("#wpcli-output"),n=e.querySelector("#wpcli-input"),a=e.querySelector("#wpcli-run"),i=e.querySelector("#wpcli-clear"),r=e.querySelector("#wpcli-status"),c=[],o=-1;function p(k,g=""){k.split(`
`).forEach(b=>{if(!b)return;let f=document.createElement("div");g?f.className=g:f.className=b.match(/error|fatal|critical/i)?"kp-log-line-err":b.match(/warning|warn/i)?"kp-log-line-warn":b.match(/success|done\]/i)?"kp-log-line-info":"",f.textContent=b,s.appendChild(f)}),s.scrollTop=s.scrollHeight}function u(k){if(k=k.trim(),!k)return;c.unshift(k),o=-1,p(`wp> ${k}`,"kp-log-line-info"),n.disabled=!0,a.disabled=!0,r&&(r.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",b=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);b.onopen=()=>{b.send(JSON.stringify({command:k}))},b.onmessage=f=>{let x=f.data;if(x.trim()==="[done]"){b.close();return}if(x.startsWith("[info]")){p(x,"kp-muted");return}if(x.startsWith("[error]")){p(x,"kp-log-line-err");return}p(x)},b.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},b.onclose=()=>{n.disabled=!1,a.disabled=!1,r&&(r.textContent="Ready"),n.focus()}}a.addEventListener("click",()=>{u(n.value),n.value=""}),n.addEventListener("keydown",k=>{if(k.key==="Enter"){u(n.value),n.value="",o=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),o<c.length-1&&(o++,n.value=c[o]);return}k.key==="ArrowDown"&&(k.preventDefault(),o>0?(o--,n.value=c[o]):(o=-1,n.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let g=k.dataset.cmd;if(g.startsWith("search-replace")){n.value=g,n.focus();let b=g.indexOf("''")+1;n.setSelectionRange(b,b);return}u(g)})}),i.addEventListener("click",()=>{s.innerHTML=""});let v=m.go.bind(m);m.go=function(k,g={}){return v(k,g)},n.focus()}function xe(){return`
        ...
        </div>`}async function Se(e){let t=document.getElementById("waf-tab-panel");if(t){t.innerHTML=xe();try{let s=await d.get(`/sites/${e}/waf`),n=document.getElementById("waf-override"),a=document.getElementById("waf-site-exclusions");n&&(n.value=String(s.Override??0)),a&&(a.value=s.Exclusions??"");let i=document.getElementById("waf-plugins-list");if(i){let[r,c]=await Promise.all([d.get("/settings/waf/plugins"),d.get(`/sites/${e}/waf/plugins`)]),o=new Set(c??[]);!r||r.length===0?i.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':(i.innerHTML=`
                    <div class="waf-plugin-pills">
                        ${r.map(p=>`
                        <span class="waf-plugin-pill ${o.has(p)?"active":""}"
                            data-plugin="${p}">${p}</span>
                        `).join("")}
                    </div>`,i.querySelectorAll(".waf-plugin-pill").forEach(p=>{p.addEventListener("click",()=>p.classList.toggle("active"))}))}}catch(s){l.error("Failed to load WAF settings: "+s.message)}}}function $e(e,t){e.addEventListener("submit",async s=>{if(s.target.id!=="waf-override-form")return;s.preventDefault();let n=s.target.querySelector('[type="submit"]'),a=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let i=new FormData(s.target),r={override:parseInt(i.get("override"),10),exclusions:i.get("exclusions").trim()};try{await d.put(`/sites/${t}/waf`,r);let c=[...document.querySelectorAll(".waf-plugin-pill.active")].map(o=>o.dataset.plugin);await d.put(`/sites/${t}/waf/plugins`,c),l.success("WAF override saved \u2014 engine recompiling in background")}catch(c){l.error(c.message)}finally{n.disabled=!1,n.innerHTML=a}})}async function me(e,{id:t}){let[{site:s,domains:n,sftp:a},i,r]=await Promise.all([d.get(`/sites/${t}`),d.get("/sites"),d.get(`/sites/${t}/configs`)]),c=s.SiteType===1||s.SiteType===2;e.innerHTML=`
        <div class="kp-view-header">
        ...
        <ul class="uk-switcher">
            ...
            <li>${Z(t)}</li>
        </ul>`,document.getElementById("sd-back").addEventListener("click",()=>m.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>_(s)),document.getElementById("sd-recreate").addEventListener("click",async()=>{y("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),h(),l.success("Pod recreated"),m.go("site-detail",{id:t})}catch(o){h(),l.error(o.message)}}),document.getElementById("sd-site-nav")?.addEventListener("change",o=>{m.go("site-detail",{id:o.target.value})}),e.querySelectorAll("[data-action]").forEach(o=>{o.addEventListener("click",async()=>{let p=o.dataset.action;if(p==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(v){l.error(v.message)}return}y(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[p]??p} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${p}`),h(),l.success(`Site ${p} successful`),m.go("site-detail",{id:t})}catch(v){h(),l.error(v.message)}})}),se(e,t),ce(e,t),ne(e,t),A(e),s.SiteType===1&&ue(e,t),T(e),$e(e,t),Se(t),de(e,t),ee(e,t),B(e,t),oe(n??[])}async function U(e){let t=document.getElementById("totp-qr-img"),s=document.getElementById("totp-qr-wrap");if(!t||!s)return;if(s.querySelectorAll(".totp-uri-text").forEach(a=>a.remove()),typeof QRCode<"u")try{let a=await new Promise((i,r)=>{QRCode.toDataURL(e,{width:220,margin:2},(c,o)=>{c?r(c):i(o)})});t.src=a,t.style.display="";return}catch{}let n=document.createElement("p");n.className="totp-uri-text kp-muted uk-text-small",n.style.wordBreak="break-all",n.textContent=e,s.appendChild(n)}function F(e){document.getElementById("kp-backup-codes-modal")?.remove();let s=`
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(a).then(()=>{i.textContent="Copied!"});else{let r=document.createElement("textarea");r.value=a,r.style.cssText="position:fixed;opacity:0",document.body.appendChild(r),r.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}r.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{n.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),m.go("users")})}function ke(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let s=UIkit.modal("#kp-create-user-modal");s.show(),document.getElementById("create-user-form").addEventListener("submit",async n=>{n.preventDefault();let a=n.target.querySelector('[type="submit"]'),i=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let r=new FormData(n.target),c={fname:r.get("fname").trim(),lname:r.get("lname").trim(),uname:r.get("uname").trim(),email:r.get("email").trim(),phone:r.get("phone").trim(),password:r.get("password"),role:parseInt(r.get("role"))};try{let o=L(await d.post("/users",c));document.getElementById("users-table-body").insertAdjacentHTML("beforeend",z(o)),l.success(`User '${o.uname}' created`),document.getElementById("create-user-form").style.display="none",document.getElementById("cu-totp-section").style.display="",Ee(o.id,s)}catch(o){l.error(o.message),a.disabled=!1,a.innerHTML=i}}),document.getElementById("kp-create-user-modal").addEventListener("hidden",()=>document.getElementById("kp-create-user-modal")?.remove())}function Ee(e,t){let s=()=>{t.hide(),document.getElementById("kp-create-user-modal")?.remove(),m.go("users")};document.getElementById("cu-totp-skip-btn").addEventListener("click",s),document.getElementById("cu-totp-setup-btn").addEventListener("click",async()=>{let n=document.getElementById("cu-totp-setup-btn");n.disabled=!0,n.textContent="Setting up\u2026";try{let a=await d.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=a.secret,document.getElementById("totp-setup-area").style.display="",document.getElementById("cu-totp-skip-btn").style.display="none",await U(a.uri)}catch(a){l.error(a.message),n.disabled=!1,n.textContent="Enable TOTP"}}),document.getElementById("cu-totp-confirm-btn").addEventListener("click",async()=>{let n=document.getElementById("totp-confirm-code").value.trim();if(n.length!==6){l.error("Enter a 6-digit code");return}let a=document.getElementById("cu-totp-confirm-btn");a.disabled=!0;try{let i=await d.post(`/users/${e}/totp/confirm`,{code:n});t.hide(),document.getElementById("kp-create-user-modal")?.remove(),l.success("TOTP enabled"),i.backup_codes?.length?F(i.backup_codes):m.go("users")}catch(i){l.error(i.message),a.disabled=!1}})}async function be(e,t){document.getElementById("kp-edit-user-modal")?.remove();let s;try{s=L(await d.get(`/users/${t}`))}catch(p){l.error(p.message);return}let n=window.KP?.user?.role===99,a=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a);let i=UIkit.modal("#kp-edit-user-modal");i.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let u=p.target.querySelector('[type="submit"]'),v=u.innerHTML;u.disabled=!0,u.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(p.target),g={fname:k.get("fname").trim(),lname:k.get("lname").trim(),email:k.get("email").trim(),phone:k.get("phone").trim()};if(n){g.role=parseInt(k.get("role"));let f=k.get("uname");f&&(g.uname=f.trim())}let b=k.get("password");b&&(g.password=b);try{await d.put(`/users/${t}`,g),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("User updated"),m.go("users")}catch(f){l.error(f.message),u.disabled=!1,u.innerHTML=v}});let r=document.getElementById("totp-setup-btn");r&&r.addEventListener("click",async()=>{r.disabled=!0,r.textContent="Setting up\u2026";try{let p=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await U(p.uri)}catch(p){l.error(p.message),r.disabled=!1,r.textContent="Enable TOTP"}});let c=document.getElementById("totp-confirm-btn");c&&c.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){l.error("Enter a 6-digit code");return}c.disabled=!0;try{let u=await d.post(`/users/${t}/totp/confirm`,{code:p});i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("TOTP enabled"),u.backup_codes?.length?F(u.backup_codes):m.go("users")}catch(u){l.error(u.message),c.disabled=!1}});let o=document.getElementById("totp-disable-btn");o&&o.addEventListener("click",async()=>{o.disabled=!0;try{await d.delete(`/users/${t}/totp`),l.success("TOTP disabled"),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),m.go("users")}catch(p){l.error(p.message),o.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function ge(e){if(!E()){e.innerHTML=S("Access denied");return}let t=await d.get("/users");e.innerHTML=`
        <div class="kp-view-header">
    ...
    </tr>`}function Le(e){e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="delete-user"]');if(!(!s||!await w("Delete User","Delete this user? This cannot be undone.")))try{await d.delete(`/users/${s.dataset.uid}`),s.closest("tr").remove(),l.success("User deleted")}catch(a){l.error(a.message)}}),e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="edit-user"]');s&&be(e,s.dataset.uid)})}m.register("dashboard",e=>G(e));m.register("sites",e=>Q(e));m.register("site-detail",(e,t)=>me(e,t));m.register("users",e=>ge(e));m.register("settings",e=>X(e));m.register("security",e=>Y(e));document.addEventListener("click",e=>{let t=e.target.closest("[data-view]");if(!t)return;e.preventDefault(),m.go(t.dataset.view);let s=document.getElementById("kp-offcanvas");s&&UIkit.offcanvas(s).hide()});document.addEventListener("click",async e=>{let t=e.target.closest("[data-action]");if(!t)return;e.stopPropagation();let{action:s,id:n}=t.dataset;switch(s){case"manage":m.go("site-detail",{id:n});break;case"start":await D(n,"start","Starting Site","Starting all containers - please wait...");break;case"stop":await D(n,"stop","Stopping Site","Gracefully stopping all containers - please wait...");break;case"restart":await D(n,"restart","Restarting Site","Restarting all containers - please wait...");break;case"flush":await D(n,"flush","Flushing Caches","Clearing container caches - please wait...");break;case"edit":{let a=await d.get(`/sites/${n}`);_(a.site);break}case"delete":await Te(n);break;case"update":await D(n,"update","Updating Images","Pulling latest container images - this may take a few minutes...");break;case"recreate":y("Recreating Pod","Recreating containers for this site - this may take a few minutes...");try{await d.post(`/sites/${n}/recreate`),h(),l.success("Pod recreated"),m.go("site-detail",{id:n})}catch(a){h(),l.error(a.message)}break}});async function D(e,t,s,n){y(s,n);try{await d.post(`/sites/${e}/${t}`),h(),l.success(s+" complete"),["start","stop","restart"].includes(t)&&m.go("sites")}catch(a){h(),l.error(a.message)}}async function Te(e){if(!await w("Delete Site","This will stop and permanently remove the pod and all its data. Are you sure?"))return;y("Deleting Site","Stopping containers and removing the pod - please wait...");try{await d.delete(`/sites/${e}`)}catch{}let s=!1,n=0;for(;!s&&n<10;){try{await new Promise(i=>setTimeout(i,2e3)),s=!(await d.get("/sites")).find(i=>i.ID===parseInt(e))}catch{}n++}h(),s?(l.success("Site deleted"),m.go("sites")):l.error("Delete failed - site still exists after 20s")}window.addEventListener("hashchange",()=>{let{view:e,params:t}=j();m.go(e,t)});var{view:Pe,params:Be}=j();m.go(Pe,Be);})();
