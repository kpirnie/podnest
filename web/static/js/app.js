"use strict";(()=>{var d={async _req(e,t,s,a=6e4){let n=new AbortController,o=setTimeout(()=>n.abort(),a),i={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};s!==void 0&&(i.body=JSON.stringify(s));try{let r=await fetch("/api"+t,i);clearTimeout(o);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(o),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e),patch:(e,t)=>d._req("PATCH",e,t)};var ee=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=e=>`<div class="kp-empty">
    ...
    </div>`,I=e=>{let t={1:["running","Running"],2:["stopped","Stopped"],3:["restarting","Restarting"],4:["error","Error"]},[s,a]=t[e]||["stopped","Unknown"];return`<span class="kp-status kp-status-${s}">${a}</span>`},Ie=e=>({3:"8.2",4:"8.3",5:"8.4",6:"8.5"})[e]||"?",N=e=>({1:"WordPress",2:"PHP",3:"Static",4:"Node.js",5:".NET",6:"Reverse Proxy"})[e]||"?",C=()=>window.KP.user.role===window.KP.roles.admin,q=e=>{switch(e.SiteType){case 1:case 2:return`PHP ${Ie(e.PHPVersion)}`;case 4:return`Node ${{2:"22",4:"24",5:"25",6:"26"}[e.RuntimeVersion]||"?"}`;case 5:return`.NET ${{1:"8.0",2:"9.0",3:"10.0"}[e.RuntimeVersion]||"?"}`;case 6:return"Reverse Proxy";default:return""}},B=e=>({id:e.id??e.ID,uname:e.uname??e.UName,uhash:e.uhash??e.UHash,fname:e.fname??e.FName,lname:e.lname??e.LName,email:e.email??e.Email,phone:e.phone??e.Phone,role:e.role??e.Role,totp_enabled:e.totp_enabled??!1,created:e.created??e.Created});function x(e,t){return new Promise(s=>{document.getElementById("kp-confirm-title").textContent=e,document.getElementById("kp-confirm-message").textContent=t;let a=UIkit.modal("#kp-confirm-modal");document.getElementById("kp-confirm-ok").addEventListener("click",()=>{a.hide(),s(!0)},{once:!0}),a.show(),document.getElementById("kp-confirm-modal").addEventListener("hidden",()=>s(!1),{once:!0})})}function w(e,t){let s=`
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",s),UIkit.modal("#kp-progress-modal").show()}function h(){let e=document.getElementById("kp-progress-modal");e&&(UIkit.modal(e).hide(),setTimeout(()=>e.remove(),300))}function U(e){return new Promise(t=>{let s="kp-clone-modal",a=`
            <div id="${s}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",a);let n=UIkit.modal(`#${s}`),o=document.getElementById("kp-clone-name"),i=document.getElementById("kp-clone-ok"),r=document.getElementById("kp-clone-cancel"),c=u=>{n.hide(),setTimeout(()=>document.getElementById(s)?.remove(),300),t(u)};i.addEventListener("click",()=>c(o.value.trim()||null),{once:!0}),r.addEventListener("click",()=>c(null),{once:!0}),document.getElementById(s).addEventListener("hidden",()=>c(null),{once:!0}),n.show(),setTimeout(()=>o.focus(),150),o.addEventListener("keydown",u=>{u.key==="Enter"&&i.click()})})}function Q(e,t,s){return new Promise(a=>{let n="kp-sync-modal",o=e==="pull",i=o?"Pull From Parent":"Push To Parent",r=o?"cloud-download":"cloud-upload",c=o?s:t,u=o?t:s,p=`
            <div id="${n}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",p);let m=UIkit.modal(`#${n}`),b=document.getElementById("kp-sync-ok"),g=document.getElementById("kp-sync-cancel"),k=f=>{m.hide(),setTimeout(()=>document.getElementById(n)?.remove(),300),a(f)};b.addEventListener("click",()=>k(!0),{once:!0}),g.addEventListener("click",()=>k(!1),{once:!0}),document.getElementById(n).addEventListener("hidden",()=>k(!1),{once:!0}),m.show()})}var v={routes:{},register(e,t){this.routes[e]=t},async go(e,t={}){let s=Object.keys(t).length?e+"/"+Object.values(t).join("/"):e;window.location.hash=s,document.querySelectorAll(".kp-nav-link").forEach(o=>{o.classList.toggle("kp-active",o.dataset.view===e)});let a=this.routes[e];if(!a)return;let n=document.getElementById("kp-view");n.innerHTML=ee();try{await a(n,t)}catch(o){n.innerHTML=L(o.message)}}};function G(){let t=(window.location.hash.replace("#","")||"dashboard").split("/"),s=t[0],a={};return s==="site-detail"&&t[1]&&(a.id=t[1]),{view:s,params:a}}var l={show(e,t="info",s=7e3){let a={success:"check",error:"warning",info:"info"},n=document.createElement("div");n.className=`kp-toast kp-toast-${t}`,n.innerHTML=`<span uk-icon="${a[t]||"info"}"></span><span>${e}</span>`,document.getElementById("kp-toasts").appendChild(n),UIkit.icon(n.querySelector("[uk-icon]")),setTimeout(()=>n.remove(),s)},success:e=>l.show(e,"success"),error:e=>l.show(e,"error"),info:e=>l.show(e,"info")};async function F(e){let t=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let s=UIkit.modal("#kp-edit-site-modal"),a=document.getElementById("es-site-type"),n=document.getElementById("es-php-version-wrap"),o=document.getElementById("es-node-version-wrap"),i=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),c=document.getElementById("es-wordpress-wrap");s.show();let u=p=>{n.classList.toggle("uk-hidden",p!==1&&p!==2||p===6),o.classList.toggle("uk-hidden",p!==4),i.classList.toggle("uk-hidden",p!==5),r.classList.toggle("uk-hidden",p!==4&&p!==5),c.classList.toggle("uk-hidden",p!==1)};u(e.SiteType),a.addEventListener("change",()=>u(parseInt(a.value))),document.getElementById("edit-site-form").addEventListener("submit",async p=>{p.preventDefault();let m=p.target.querySelector('[type="submit"]'),b=m.innerHTML;m.disabled=!0,m.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let g=new FormData(p.target),k=parseInt(g.get("site_type")),f=null;k===4&&(f=parseInt(g.get("node_version"))),k===5&&(f=parseInt(g.get("dotnet_version")));let y={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:k,runtime_version:f,start_command:g.get("start_command")?.trim()||""},S=k===1?g.get("install_wordpress")==="on":!1;try{if(await d.put(`/sites/${e.ID}`,y),s.hide(),document.getElementById("kp-edit-site-modal")?.remove(),k!==6){w("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:S}),h(),l.success("Site updated and pod recreated")}catch($){h(),l.error("Site saved but pod recreate failed: "+$.message)}}else l.success("Site updated");v.go("site-detail",{id:String(e.ID)})}catch($){l.error($.message),m.disabled=!1,m.innerHTML=b}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function j(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let t=UIkit.modal("#kp-create-site-modal"),s=document.getElementById("cs-site-type"),a=document.getElementById("cs-php-version-wrap"),n=document.getElementById("cs-node-version-wrap"),o=document.getElementById("cs-dotnet-version-wrap"),i=document.getElementById("cs-start-command-wrap"),r=document.getElementById("cs-wordpress-wrap");t.show();let c=document.getElementById("cs-domains-wrap"),u=document.getElementById("cs-rp-note");s.addEventListener("change",()=>{let p=parseInt(s.value);a.classList.toggle("uk-hidden",p!==1&&p!==2||p===6),n.classList.toggle("uk-hidden",p!==4),o.classList.toggle("uk-hidden",p!==5),i.classList.toggle("uk-hidden",p!==4&&p!==5),r.classList.toggle("uk-hidden",p!==1||p===6),c.classList.toggle("uk-hidden",p===6),u.classList.toggle("uk-hidden",p!==6)}),document.getElementById("create-site-form").addEventListener("submit",async p=>{p.preventDefault();let m=p.target.querySelector('[type="submit"]'),b=m.innerHTML;m.disabled=!0,m.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let g=new FormData(p.target),k=parseInt(g.get("site_type")),f=null;k===4&&(f=parseInt(g.get("node_version"))),k===5&&(f=parseInt(g.get("dotnet_version")));let y={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:k,runtime_version:f,start_command:g.get("start_command")?.trim()||"",domains:g.get("domains").split(`
`).map($=>$.trim()).filter(Boolean),install_wordpress:k===1?g.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove();let S=k===6?`Setting up '${y.name}' as a reverse proxy...`:`Setting up '${y.name}' \u2014 pulling images and provisioning containers...`;w("Creating Site",S);try{await d.post("/sites",y),h(),l.success(`Site '${y.name}' created`),v.go("sites")}catch($){h(),l.error($.message),m.disabled=!1,m.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function te(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        <div class="kp-site-grid">
            ${t.length===0?A("world","No sites yet \u2014 create one to get started"):t.map(s=>Y(s,t)).join("")}
        </div>`,document.getElementById("sites-new-btn").addEventListener("click",()=>j())}function Y(e,t=[]){let s=e.Domains?.[0]??null,a=e.SiteType===6,n=e.ParentID>0?t.find(o=>o.ID===e.ParentID)??null:null;return`
        <div class="kp-site-card" data-site-id="${e.ID}" data-status="${e.SiteStatus}" data-type="${e.SiteType}">
            ...
            <div class="kp-site-actions">
                ...
                `}
                <button class="uk-button kp-btn-ghost kp-btn-sm" data-action="clone" data-id="${e.ID}" data-name="${e.Name}" uk-tooltip="Clone"><span uk-icon="icon: copy;"></span></button>
        ...
        </div>`}async function se(e){let t=await d.get("/sites")??[],s=t.filter(o=>o.SiteStatus===1).length,a=t.filter(o=>o.SiteStatus===2).length,n=t.filter(o=>o.SiteStatus===4).length;e.innerHTML=`
        <div class="kp-view-header">
        ...
        <div class="kp-site-grid">
            ${t.length===0?A("world","No sites yet"):t.slice(0,6).map(o=>Y(o,t)).join("")}
        </div>`,document.getElementById("dash-new-site")?.addEventListener("click",()=>j())}function W(e=null){let t=e?`/sites/${e}/security/ip`:"/security/ip",s=e?`/sites/${e}/security/ua`:"/security/ua",a=e?`/sites/${e}/waf`:"/settings/waf";return`
        ...
        ${W(null)}`,O(e),T(e)}function Ce(e){switch(e){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ne(e){let t=document.getElementById("admin-domain-ssl");if(!(!t||!e))try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`);t.outerHTML=Ce(s.status)}catch{}}async function oe(e){if(!C()){e.innerHTML=L("Access denied");return}let[t,s,a,n]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf"),d.get("/settings/trusted-proxies")]);e.innerHTML=`
    <div class="kp-view-header">
    ...
    `,t.admin_domain&&ne(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await d.put("/settings",u),l.success("Settings saved"),ne(u.admin_domain)}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api/settings/import",{method:"POST",body:r}),u=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(u?.error||`HTTP ${c.status}`);l.success("Settings imported")}catch(c){l.error(c.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),u={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",u),l.success("Backup settings saved")}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),u={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},p=c.get("s3_secret_key").trim();p&&(u.s3_secret_key=p);try{await d.put("/settings/backup",u),l.success("S3 settings saved")}catch(m){l.error(m.message)}finally{i.disabled=!1,i.innerHTML=r}})}function ie(e){return`
        ...
        </div>`}function Be(e){if(!e||e.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No snapshots yet.</p>';let t=n=>n===2?'<span class="kp-mono" style="color:var(--kp-cyan)">S3</span>':'<span class="kp-mono" style="color:var(--kp-blue)">Local</span>',s=n=>n<1024?`${n} B`:n<1048576?`${(n/1024).toFixed(1)} KB`:n<1073741824?`${(n/1048576).toFixed(1)} MB`:`${(n/1073741824).toFixed(2)} GB`;return`
        ...
        </table>`}async function D(e,t){try{let[s,a]=await Promise.all([d.get(`/sites/${t}/backup-repo`),d.get(`/sites/${t}/backups`)]),n=e.querySelector("#backup-local-enabled"),o=e.querySelector("#backup-s3-enabled");n&&(n.checked=!!s.LocalEnabled),o&&(o.checked=!!s.S3Enabled);let i=e.querySelector("#backup-error-banner");if(i)if(s.last_error){let c=s.last_error_at?` (${new Date(s.last_error_at).toLocaleString()})`:"";i.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${c}:</strong> ${s.last_error}</p>
                    </div>`}else i.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=Be(a))}catch(s){let a=e.querySelector("#backup-list-wrap");a&&(a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${s.message}</p>`)}}function le(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,s),l.success("Backup destinations saved")}catch(a){l.error(a.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(o){l.error(o.message);return}w("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let a=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>s||Date.now()>a)&&(clearInterval(n),h(),await D(e,t),Date.now()<=a?l.success("Backup complete"):l.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let a=s.target.closest(".backup-restore-btn");if(a){let i=a.dataset.id;if(!await x("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${i}/restore`)}catch(m){l.error(m.message);return}w("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let c=Date.now(),u=Date.now()+900*1e3,p=setInterval(async()=>{try{let m=await d.get(`/sites/${t}/backups/restore-status`);(!m?.active||Date.now()>u)&&(clearInterval(p),h(),m?.active?l.error("Restore timed out"):l.success("Restore complete"),await D(e,t))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let i=n.dataset.id;if(!await x("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;w("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${i}`),h(),l.success("Snapshot deleted"),await D(e,t)}catch(c){h(),l.error(c.message)}}let o=s.target.closest(".backup-download-btn");if(o){let i=o.dataset.id;w("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${i}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{h()},5e3)},300);return}})}var V={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function H(e,t,s){let a=s?Object.entries(s):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
            ...
            <div class="kp-config-grid cfg-rows" data-type="${t}">
                ${a.map(([n,o])=>M(n,o)).join("")}
            </div>
        </div>`}function re(e,t){let s=t?.enabled==="true",a=t?Object.entries(t).filter(([n])=>n!=="enabled"):[];return`
        <div>
    ...
    </div>`}function ce(e,t){e.addEventListener("click",s=>{if(s.target.closest(".cfg-add-row")){let a=s.target.closest(".cfg-add-row");e.querySelector(`.cfg-rows[data-type="${a.dataset.type}"]`).insertAdjacentHTML("beforeend",M())}}),e.addEventListener("click",s=>{s.target.closest(".cfg-del-row")&&s.target.closest(".kp-config-row").remove()}),e.addEventListener("click",async s=>{let a=s.target.closest(".cfg-save");if(!a)return;let{type:n,site:o}=a.dataset,i=e.querySelectorAll(`.cfg-rows[data-type="${n}"] .kp-config-row`),r={};if(i.forEach(c=>{let u=c.querySelector(".cfg-key").value.trim(),p=c.querySelector(".cfg-val").value.trim();u&&(r[u]=p)}),n==="5"){let c=e.querySelector(".varnish-enabled-toggle");r.enabled=c?.checked?"true":"false"}try{await d.put(`/sites/${o}/configs/${n}`,r),l.success(`${V[n]} config saved`)}catch(c){l.error(c.message)}}),e.addEventListener("click",async s=>{let a=s.target.closest(".cfg-reset");if(!a)return;let{type:n,site:o}=a.dataset;if(await x("Reset Config",`Reset ${V[n]} config to defaults?`))try{let r=await d.post(`/sites/${o}/configs/${n}/reset`),c=e.querySelector(`.cfg-rows[data-type="${n}"]`);c.innerHTML=Object.entries(r).map(([u,p])=>M(u,p)).join(""),l.success(`${V[n]} reset to defaults`)}catch(r){l.error(r.message)}}),e.addEventListener("change",async s=>{let a=s.target.closest(".cfg-import-input");if(!a)return;let{type:n,site:o}=a.dataset,i=a.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch(`/api/sites/${o}/configs/${n}/import`,{method:"POST",body:r}),u=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(u?.error||`HTTP ${c.status}`);let p=e.querySelector(`.cfg-rows[data-type="${n}"]`);p.innerHTML=Object.entries(u).map(([m,b])=>M(m,b)).join(""),l.success(`${V[n]} config imported`)}catch(c){l.error(c.message)}finally{a.value=""}})}function pe(e){return`
        ...
        </div>`}function ue(e){if(!e||e.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No cron jobs configured.</p>';let t=a=>a?new Date(a).toLocaleString():"\u2014";return`
        <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
            <thead>
                <tr>
                    <th>Label</th>
        ...
        <tr>
            ...
            <td>
                ${a.LastError?`<span class="kp-badge kp-badge-error" uk-tooltip="${qe(a.LastError)}">Error</span>`:a.LastRun?'<span class="kp-badge kp-badge-success">OK</span>':'<span class="kp-muted uk-text-small">\u2014</span>'}
            </td>
            <td>
        ...
        </table>`}async function z(e,t){let s=e.querySelector("#cron-list-wrap");if(s)try{let a=await d.get(`/sites/${t}/crons`);s.innerHTML=ue(a)}catch(a){s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${a.message}</p>`}}function me(e,t){let s=[],a=e.querySelector("#cron-modal"),n=e.querySelector("#cron-modal-title"),o=e.querySelector("#cron-modal-id"),i=e.querySelector("#cron-modal-label"),r=e.querySelector("#cron-modal-command"),c=e.querySelector("#cron-modal-schedule"),u=e.querySelector("#cron-schedule-preview"),p=e.querySelector("#cron-modal-enabled");c?.addEventListener("input",()=>{u.textContent=de(c.value.trim())}),e.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",o.value="",i.value="",r.value="",c.value="",u.textContent="",p.checked=!0,UIkit.modal(a).show()}),e.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let m=r.value.trim(),b=c.value.trim();if(!m||!b){l.error("Command and schedule are required");return}let g={label:i.value.trim(),command:m,schedule:b,enabled:p.checked},k=o.value;try{k?(await d.put(`/sites/${t}/crons/${k}`,g),l.success("Cron job updated")):(await d.post(`/sites/${t}/crons`,g),l.success("Cron job created")),UIkit.modal(a).hide(),await z(e,t),s=await d.get(`/sites/${t}/crons`)}catch(f){l.error(f.message)}}),e.querySelector("#cron-list-wrap")?.addEventListener("click",async m=>{let b=m.target.closest(".cron-edit-btn");if(b){let f=b.dataset.id,y=s.find(S=>String(S.ID)===f);if(!y)return;n.textContent="Edit Cron Job",o.value=y.ID,i.value=y.Label||"",r.value=y.Command,c.value=y.Schedule,u.textContent=de(y.Schedule),p.checked=y.Enabled,UIkit.modal(a).show();return}let g=m.target.closest(".cron-delete-btn");if(g){let f=g.dataset.id;if(!await x("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await d.delete(`/sites/${t}/crons/${f}`),l.success("Cron job deleted"),await z(e,t),s=await d.get(`/sites/${t}/crons`)}catch(S){l.error(S.message)}return}let k=m.target.closest(".cron-run-btn");if(k){let f=k.dataset.id;try{await d.post(`/sites/${t}/crons/${f}/run`)}catch(E){l.error(E.message);return}w("Running Cron Job","Executing the job inside the container \u2014 please wait.");let y=null;try{y=(await d.get(`/sites/${t}/crons`)).find(P=>String(P.ID)===f)?.LastRun??null}catch{}let S=Date.now()+300*1e3,$=setInterval(async()=>{try{let E=await d.get(`/sites/${t}/crons`),P=E.find(_=>String(_.ID)===f);if(!P||P.LastRun!==y||Date.now()>S){clearInterval($),h(),s=E??[];let _=e.querySelector("#cron-list-wrap");_&&(_.innerHTML=ue(E)),P?.LastError?l.error(`Job failed: ${P.LastError}`):l.success("Cron job complete")}}catch{}},2e3);return}}),e.querySelector("#cron-list-wrap")?.addEventListener("change",async m=>{let b=m.target.closest(".cron-toggle");if(!b)return;let g=b.dataset.id;try{await d.patch(`/sites/${t}/crons/${g}/toggle`,{enabled:b.checked}),l.success(b.checked?"Cron job enabled":"Cron job disabled")}catch(k){l.error(k.message),b.checked=!b.checked}}),d.get(`/sites/${t}/crons`).then(m=>{s=m??[]}).catch(()=>{})}function qe(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function de(e){if(!e)return"";let t=e.trim().split(/\s+/);if(t.length!==5)return"invalid expression";let[s,a,n,o,i]=t;if(e==="* * * * *")return"every minute";if(s!=="*"&&a!=="*"&&n==="*"&&o==="*"&&i==="*")return`daily at ${a.padStart(2,"0")}:${s.padStart(2,"0")}`;if(s!=="*"&&a!=="*"&&n==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(u=>r[parseInt(u)]??u).join(", ")} at ${a.padStart(2,"0")}:${s.padStart(2,"0")}`}return s.startsWith("*/")?`every ${s.slice(2)} minutes`:a.startsWith("*/")?`every ${a.slice(2)} hours`:e}function ke(e,t){return`
        <div>
        ...
        </div>`}function be(e,t){let s=null,a=!1,n=e.querySelector("#log-output"),o=e.querySelector("#log-connect"),i=e.querySelector("#log-disconnect"),r=e.querySelector("#log-clear"),c=e.querySelector("#log-autoscroll"),u=e.querySelector("#log-status");function p(g){g.split(`
`).forEach(k=>{if(!k)return;let f=document.createElement("div");f.className=k.match(/WAF BLOCK/i)?"kp-log-line-err":k.match(/WAF DETECT/i)?"kp-log-line-warn":k.match(/error|crit|emerg/i)?"kp-log-line-err":k.match(/warn/i)?"kp-log-line-warn":k.match(/info|notice/i)?"kp-log-line-info":"",f.textContent=k,n.appendChild(f)}),c.checked&&(n.scrollTop=n.scrollHeight)}function m(){s&&(s.close(),s=null),a=!1,o.disabled=!1,i.disabled=!0,u&&(u.textContent="Disconnected")}o.addEventListener("click",()=>{m();let g=e.querySelector("#log-container").value,k=e.querySelector("#log-tail").value,f=location.protocol==="https:"?"wss":"ws",y=g==="waf"?`${f}://${location.host}/api/sites/${t}/logs/waf?tail=${k}`:`${f}://${location.host}/api/sites/${t}/logs?container=${g}&tail=${k}`;s=new WebSocket(y),s.onopen=()=>{a=!0,o.disabled=!0,i.disabled=!1,u&&(u.textContent=`Connected \u2014 ${g}`)},s.onmessage=S=>p(S.data),s.onerror=()=>{},s.onclose=()=>{a=!1,o.disabled=!1,i.disabled=!0,u&&(u.textContent="Disconnected")}}),i.addEventListener("click",m),r.addEventListener("click",()=>{n.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{s&&s.readyState===WebSocket.OPEN&&(m(),o.click())});let b=v.go.bind(v);v.go=function(g,k={}){return s&&m(),b(g,k)}}function De(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ge(e,t){try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),a=document.getElementById(`ssl-icon-${t}`);a&&(a.outerHTML=De(s.status))}catch{}}function ve(e){e.forEach(t=>ge(t.Domain,t.ID))}function fe(e,t,s,a=0,n=null){let o=e.SiteType!==3&&e.PMAPort>0;return`
        <div class="uk-grid-medium" uk-grid>
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                        <tbody>
                            ...
                            <tr><td class="kp-muted">Status</td><td>${I(e.SiteStatus)}</td></tr>
            ...
            <div class="uk-width-1-2@m">
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <div id="domain-list">
                        ${t.length?t.map(he).join(""):'<p class="kp-muted uk-text-small">No domains configured</p>'}
                    </div>
        ...
        </div>`}function he(e){return`<div class="uk-flex uk-flex-between uk-flex-middle kp-config-row" data-domain-id="${e.ID}">
    ...
    </div>`}function ye(e,t){e.querySelector("#domain-add-btn")?.addEventListener("click",()=>{e.querySelector("#domain-add-form").classList.remove("uk-hidden")}),e.querySelector("#domain-cancel-btn")?.addEventListener("click",()=>{e.querySelector("#domain-add-form").classList.add("uk-hidden")}),e.querySelector("#domain-save-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#domain-add-input").value.trim();if(s)try{let a=await d.post(`/sites/${t}/domains`,{domain:s});e.querySelector("#domain-list").insertAdjacentHTML("beforeend",he(a)),ge(a.Domain,a.ID),e.querySelector("#domain-add-form").classList.add("uk-hidden"),e.querySelector("#domain-add-input").value="",l.success("Domain added")}catch(a){l.error(a.message)}}),e.querySelector("#domain-list")?.addEventListener("click",async s=>{let a=s.target.closest('[data-action="delete-domain"]');if(!(!a||!await x("Remove Domain","Remove this domain from the site?")))try{await d.delete(`/sites/${t}/domains/${a.dataset.did}`),a.closest("[data-domain-id]").remove(),l.success("Domain removed")}catch(o){l.error(o.message)}})}function we(e,t,s=null){e.querySelector("#sftp-regen-btn")?.addEventListener("click",async()=>{let a=e.querySelector("#sftp-regen-btn"),n=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.post(`/sites/${t}/sftp-regen`),l.success("SFTP password regenerated"),v.go("site-detail",{id:String(t)})}catch(o){l.error(o.message),a.disabled=!1,a.innerHTML=n}}),e.querySelector("#sftp-copy-btn")?.addEventListener("click",()=>{let a=e.querySelector("#sftp-pass-display")?.textContent;if(a)if(navigator.clipboard)navigator.clipboard.writeText(a).then(()=>l.success("Password copied to clipboard")).catch(()=>l.error("Failed to copy password"));else{let n=document.createElement("textarea");n.value=a,n.style.cssText="position:fixed;opacity:0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n),l.success("Password copied to clipboard")}}),e.querySelector("#pma-open-btn")?.addEventListener("click",async()=>{let a=e.querySelector("#pma-open-btn"),n=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.5"></div> Opening...';try{let o=await d.post(`/sites/${t}/pma-token`);window.open(o.url,"_blank")}catch(o){l.error(o.message)}finally{a.disabled=!1,a.innerHTML=n}}),e.querySelector("#sync-pull-btn")?.addEventListener("click",async()=>{if(await Q("pull",s.Name,e.querySelector('[data-action="manage"][data-id="'+s.ParentID+'"]')?.textContent?.trim()??"parent"))try{l.success("Pull from parent complete")}catch(n){l.error(n.message)}}),e.querySelector("#sync-push-btn")?.addEventListener("click",async()=>{if(await Q("push",s.Name,e.querySelector('[data-action="manage"][data-id="'+s.ParentID+'"]')?.textContent?.trim()??"parent"))try{l.success("Push to parent complete")}catch(n){l.error(n.message)}})}var Me=[{label:"Cache Flush",cmd:"cache flush"},{label:"Plugin List",cmd:"plugin list"},{label:"Theme List",cmd:"theme list"},{label:"User List",cmd:"user list"},{label:"Core Check",cmd:"core check-update"},{label:"Core Update",cmd:"core update"},{label:"Plugin Updates",cmd:"plugin update --all"},{label:"Theme Updates",cmd:"theme update --all"},{label:"Rewrite Flush",cmd:"rewrite flush"},{label:"Transient Delete",cmd:"transient delete --all"},{label:"Search Replace",cmd:"search-replace '' ''"}];function xe(e){return`
        <div>
        ...
        </div>`}function Se(e,t){let s=e.querySelector("#wpcli-output"),a=e.querySelector("#wpcli-input"),n=e.querySelector("#wpcli-run"),o=e.querySelector("#wpcli-clear"),i=e.querySelector("#wpcli-status"),r=[],c=-1;function u(b,g=""){b.split(`
`).forEach(k=>{if(!k)return;let f=document.createElement("div");g?f.className=g:f.className=k.match(/error|fatal|critical/i)?"kp-log-line-err":k.match(/warning|warn/i)?"kp-log-line-warn":k.match(/success|done\]/i)?"kp-log-line-info":"",f.textContent=k,s.appendChild(f)}),s.scrollTop=s.scrollHeight}function p(b){if(b=b.trim(),!b)return;r.unshift(b),c=-1,u(`wp> ${b}`,"kp-log-line-info"),a.disabled=!0,n.disabled=!0,i&&(i.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",k=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);k.onopen=()=>{k.send(JSON.stringify({command:b}))},k.onmessage=f=>{let y=f.data;if(y.trim()==="[done]"){k.close();return}if(y.startsWith("[info]")){u(y,"kp-muted");return}if(y.startsWith("[error]")){u(y,"kp-log-line-err");return}u(y)},k.onerror=()=>{u("[error] WebSocket connection failed","kp-log-line-err")},k.onclose=()=>{a.disabled=!1,n.disabled=!1,i&&(i.textContent="Ready"),a.focus()}}n.addEventListener("click",()=>{p(a.value),a.value=""}),a.addEventListener("keydown",b=>{if(b.key==="Enter"){p(a.value),a.value="",c=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),c<r.length-1&&(c++,a.value=r[c]);return}b.key==="ArrowDown"&&(b.preventDefault(),c>0?(c--,a.value=r[c]):(c=-1,a.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let g=b.dataset.cmd;if(g.startsWith("search-replace")){a.value=g,a.focus();let k=g.indexOf("''")+1;a.setSelectionRange(k,k);return}p(g)})}),o.addEventListener("click",()=>{s.innerHTML=""});let m=v.go.bind(v);v.go=function(b,g={}){return m(b,g)},a.focus()}function He(){return`
        ...
        </div>`}function Re(e){let t=e.querySelector(".kp-tab-bar");if(!t)return;let s=Array.from(t.querySelectorAll(":scope > li > a"));if(!s.length)return;let a=document.createElement("div");a.className="kp-tab-select-wrap",a.innerHTML='<span uk-icon="chevron-down"></span>';let n=document.createElement("select");n.className="kp-tab-select",s.forEach((o,i)=>{let r=document.createElement("option");r.value=i,r.textContent=o.textContent.trim(),n.appendChild(r)}),a.insertBefore(n,a.firstChild),t.parentNode.insertBefore(a,t),n.addEventListener("change",()=>{UIkit.tab(t).show(parseInt(n.value,10))}),UIkit.util.on(t,"shown",o=>{let i=o.target,c=Array.from(t.querySelectorAll(":scope > li")).indexOf(i);c!==-1&&(n.value=c)})}async function $e(e){let t=document.getElementById("waf-tab-panel");if(!t)return;t.innerHTML=He();let s=document.getElementById("waf-export-btn");s&&(s.href=`/api/sites/${e}/waf/export`);try{let a=await d.get(`/sites/${e}/waf`),n=document.getElementById("waf-override"),o=document.getElementById("waf-site-exclusions");n&&(n.value=String(a.Override??0)),o&&(o.value=a.Exclusions??"");let i=document.getElementById("waf-plugins-list");if(i){let[r,c]=await Promise.all([d.get("/settings/waf/plugins"),d.get(`/sites/${e}/waf/plugins`)]),u=new Set(c??[]);!r||r.length===0?i.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':(i.innerHTML=`
                    <div class="waf-plugin-pills">
                        ${r.map(p=>`
                        <span class="waf-plugin-pill ${u.has(p)?"active":""}"
                            data-plugin="${p}">${p}</span>
                        `).join("")}
                    </div>`,i.querySelectorAll(".waf-plugin-pill").forEach(p=>{p.addEventListener("click",()=>p.classList.toggle("active"))}))}}catch(a){l.error("Failed to load WAF settings: "+a.message)}}function _e(e,t){e.addEventListener("submit",async s=>{if(s.target.id!=="waf-override-form")return;s.preventDefault();let a=s.target.querySelector('[type="submit"]'),n=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=new FormData(s.target),i={override:parseInt(o.get("override"),10),exclusions:o.get("exclusions").trim()};try{await d.put(`/sites/${t}/waf`,i);let r=[...document.querySelectorAll(".waf-plugin-pill.active")].map(c=>c.dataset.plugin);await d.put(`/sites/${t}/waf/plugins`,r),l.success("WAF override saved \u2014 engine recompiling in background")}catch(r){l.error(r.message)}finally{a.disabled=!1,a.innerHTML=n}}),e.querySelector("#waf-import")?.addEventListener("change",async s=>{let a=s.target.files[0];if(!a)return;let n=new FormData;n.append("file",a);try{let o=await fetch(`/api/sites/${t}/waf/import`,{method:"POST",body:n}),i=o.status===204?null:await o.json().catch(()=>null);if(!o.ok)throw new Error(i?.error||`HTTP ${o.status}`);await $e(t),l.success("WAF settings imported")}catch(o){l.error(o.message)}finally{s.target.value=""}})}function Ae(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}async function Ne(e){let t=document.getElementById("rp-routes-list");if(t)try{let s=await d.get(`/sites/${e}/rp-routes`);t.innerHTML=s.length?s.map(a=>X(a.Domain,a.Upstream)).join(""):X()}catch(s){l.error("Failed to load routes: "+s.message)}}function Ue(e,t){e.addEventListener("click",async s=>{if(s.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",X());return}if(s.target.closest(".rp-remove-row")){s.target.closest(".rp-route-row").remove();return}if(!s.target.closest("#rp-save-btn"))return;let a=s.target.closest("#rp-save-btn"),n=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=[...document.querySelectorAll(".rp-route-row")].map(i=>({Domain:i.querySelector('[data-field="domain"]').value.trim(),Upstream:i.querySelector('[data-field="upstream"]').value.trim()})).filter(i=>i.Domain&&i.Upstream);try{await d.put(`/sites/${t}/rp-routes`,o),l.success("Routes saved")}catch(i){l.error(i.message)}finally{a.disabled=!1,a.innerHTML=n}})}async function Ee(e,{id:t}){let[{site:s,domains:a,sftp:n},o,i]=await Promise.all([d.get(`/sites/${t}`),d.get("/sites"),d.get(`/sites/${t}/configs`)]),r=Array.isArray(o)?o:[],c=s.SiteType===1||s.SiteType===2,u=s.SiteType===6,p=[1,2,4,5].includes(s.SiteType);if(e.innerHTML=`
        <div class="kp-view-header">
            <div class="uk-flex uk-flex-middle" style="gap:12px">
                ...
                <div class="kp-site-nav-wrap">
                    <select id="sd-site-nav" class="uk-select kp-select">
                        ${r.map(m=>`<option value="${m.ID}" ${m.ID===s.ID?"selected":""}>${m.Name}</option>`).join("")}
                    </select>
        ...
        <ul class="uk-switcher">
            <li>${fe(s,a??[],n,s.ParentID??0,r.find(m=>m.ID===s.ParentID)?.Name??null)}</li>
            <li>${H(t,1,i[1])}</li>
            ${c?`<li>${H(t,2,i[2])}</li>`:""}
            <li>${H(t,3,i[3])}</li>
            <li>${H(t,4,i[4])}</li>
            <li>${re(t,i[5])}</li>
            <li>${ke(t,s.SiteType)}</li>
            <li>${W(t)}</li>
            <li id="waf-tab-panel"></li>
            ${s.SiteType===1?`<li>${xe(t)}</li>`:""}
            <li>${ie(t)}</li>
            ${p?`<li>${pe(t)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>v.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>F(s)),document.getElementById("sd-site-nav")?.addEventListener("change",m=>{v.go("site-detail",{id:m.target.value})}),u){Ue(e,t),Ne(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{w("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),h(),l.success("Pod recreated"),v.go("site-detail",{id:t})}catch(m){h(),l.error(m.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let m=await U(s.Name);if(m){w("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${t}/clone`,{name:m}),h(),l.success(`Site cloned as '${m}'`),v.go("sites")}catch(b){h(),l.error(b.message)}}}),e.querySelectorAll("[data-action]").forEach(m=>{m.addEventListener("click",async()=>{let b=m.dataset.action;if(b==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(k){l.error(k.message)}return}w(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${b}`),h(),l.success(`Site ${b} successful`),v.go("site-detail",{id:t})}catch(k){h(),l.error(k.message)}})}),ce(e,t),ye(e,t),be(e,t),O(e),s.SiteType===1&&Se(e,t),T(e),_e(e,t),$e(t),we(e,t,s),le(e,t),D(e,t),p&&(me(e,t),z(e,t)),Re(e),ve(a??[])}async function J(e){let t=document.getElementById("totp-qr-img"),s=document.getElementById("totp-qr-wrap");if(!t||!s)return;if(s.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((o,i)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,c)=>{r?i(r):o(c)})});t.src=n,t.style.display="";return}catch{}let a=document.createElement("p");a.className="totp-uri-text kp-muted uk-text-small",a.style.wordBreak="break-all",a.textContent=e,s.appendChild(a)}function K(e){document.getElementById("kp-backup-codes-modal")?.remove();let s=`
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=n,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{a.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),v.go("users")})}function Le(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let s=UIkit.modal("#kp-create-user-modal");s.show(),document.getElementById("create-user-form").addEventListener("submit",async a=>{a.preventDefault();let n=a.target.querySelector('[type="submit"]'),o=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let i=new FormData(a.target),r={fname:i.get("fname").trim(),lname:i.get("lname").trim(),uname:i.get("uname").trim(),email:i.get("email").trim(),phone:i.get("phone").trim(),password:i.get("password"),role:parseInt(i.get("role"))};try{let c=B(await d.post("/users",r));document.getElementById("users-table-body").insertAdjacentHTML("beforeend",Z(c)),l.success(`User '${c.uname}' created`),document.getElementById("create-user-form").style.display="none",document.getElementById("cu-totp-section").style.display="",Fe(c.id,s)}catch(c){l.error(c.message),n.disabled=!1,n.innerHTML=o}}),document.getElementById("kp-create-user-modal").addEventListener("hidden",()=>document.getElementById("kp-create-user-modal")?.remove())}function Fe(e,t){let s=()=>{t.hide(),document.getElementById("kp-create-user-modal")?.remove(),v.go("users")};document.getElementById("cu-totp-skip-btn").addEventListener("click",s),document.getElementById("cu-totp-setup-btn").addEventListener("click",async()=>{let a=document.getElementById("cu-totp-setup-btn");a.disabled=!0,a.textContent="Setting up\u2026";try{let n=await d.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=n.secret,document.getElementById("totp-setup-area").style.display="",document.getElementById("cu-totp-skip-btn").style.display="none",await J(n.uri)}catch(n){l.error(n.message),a.disabled=!1,a.textContent="Enable TOTP"}}),document.getElementById("cu-totp-confirm-btn").addEventListener("click",async()=>{let a=document.getElementById("totp-confirm-code").value.trim();if(a.length!==6){l.error("Enter a 6-digit code");return}let n=document.getElementById("cu-totp-confirm-btn");n.disabled=!0;try{let o=await d.post(`/users/${e}/totp/confirm`,{code:a});t.hide(),document.getElementById("kp-create-user-modal")?.remove(),l.success("TOTP enabled"),o.backup_codes?.length?K(o.backup_codes):v.go("users")}catch(o){l.error(o.message),n.disabled=!1}})}async function Te(e,t){document.getElementById("kp-edit-user-modal")?.remove();let s;try{s=B(await d.get(`/users/${t}`))}catch(u){l.error(u.message);return}let a=window.KP?.user?.role===99,n=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let o=UIkit.modal("#kp-edit-user-modal");o.show(),document.getElementById("edit-user-form").addEventListener("submit",async u=>{u.preventDefault();let p=u.target.querySelector('[type="submit"]'),m=p.innerHTML;p.disabled=!0,p.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b=new FormData(u.target),g={fname:b.get("fname").trim(),lname:b.get("lname").trim(),email:b.get("email").trim(),phone:b.get("phone").trim()};if(a){g.role=parseInt(b.get("role"));let f=b.get("uname");f&&(g.uname=f.trim())}let k=b.get("password");k&&(g.password=k);try{await d.put(`/users/${t}`,g),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("User updated"),v.go("users")}catch(f){l.error(f.message),p.disabled=!1,p.innerHTML=m}});let i=document.getElementById("totp-setup-btn");i&&i.addEventListener("click",async()=>{i.disabled=!0,i.textContent="Setting up\u2026";try{let u=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=u.secret,document.getElementById("totp-setup-area").style.display="",await J(u.uri)}catch(u){l.error(u.message),i.disabled=!1,i.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let u=document.getElementById("totp-confirm-code").value.trim();if(u.length!==6){l.error("Enter a 6-digit code");return}r.disabled=!0;try{let p=await d.post(`/users/${t}/totp/confirm`,{code:u});o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("TOTP enabled"),p.backup_codes?.length?K(p.backup_codes):v.go("users")}catch(p){l.error(p.message),r.disabled=!1}});let c=document.getElementById("totp-disable-btn");c&&c.addEventListener("click",async()=>{c.disabled=!0;try{await d.delete(`/users/${t}/totp`),l.success("TOTP disabled"),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),v.go("users")}catch(u){l.error(u.message),c.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function Pe(e){if(!C()){e.innerHTML=L("Access denied");return}let t=await d.get("/users");e.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>Le(e)),je(e)}function Z(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
    ...
    </tr>`}function je(e){e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="delete-user"]');if(!(!s||!await x("Delete User","Delete this user? This cannot be undone.")))try{await d.delete(`/users/${s.dataset.uid}`),s.closest("tr").remove(),l.success("User deleted")}catch(n){l.error(n.message)}}),e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="edit-user"]');s&&Te(e,s.dataset.uid)})}v.register("dashboard",e=>se(e));v.register("sites",e=>te(e));v.register("site-detail",(e,t)=>Ee(e,t));v.register("users",e=>Pe(e));v.register("settings",e=>oe(e));v.register("security",e=>ae(e));document.addEventListener("click",e=>{let t=e.target.closest("[data-view]");if(!t)return;e.preventDefault(),v.go(t.dataset.view);let s=document.getElementById("kp-offcanvas");s&&UIkit.offcanvas(s).hide()});document.addEventListener("click",async e=>{let t=e.target.closest("[data-action]");if(!t)return;e.stopPropagation();let{action:s,id:a}=t.dataset;switch(s){case"manage":v.go("site-detail",{id:a});break;case"start":await R(a,"start","Starting Site","Starting all containers - please wait...");break;case"stop":await R(a,"stop","Stopping Site","Gracefully stopping all containers - please wait...");break;case"restart":await R(a,"restart","Restarting Site","Restarting all containers - please wait...");break;case"flush":await R(a,"flush","Flushing Caches","Clearing container caches - please wait...");break;case"edit":{let n=await d.get(`/sites/${a}`);F(n.site);break}case"clone":{let n=await U(t.dataset.name??a);if(!n)break;w("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${a}/clone`,{name:n}),h(),l.success(`Site cloned as '${n}'`),v.go("sites")}catch(o){h(),l.error(o.message)}break}case"delete":await We(a);break;case"update":await R(a,"update","Updating Images","Pulling latest container images - this may take a few minutes...");break;case"recreate":w("Recreating Pod","Recreating containers for this site - this may take a few minutes...");try{await d.post(`/sites/${a}/recreate`),h(),l.success("Pod recreated"),v.go("site-detail",{id:a})}catch(n){h(),l.error(n.message)}break}});async function R(e,t,s,a){w(s,a);try{await d.post(`/sites/${e}/${t}`),h(),l.success(s+" complete"),["start","stop","restart"].includes(t)&&v.go("sites")}catch(n){h(),l.error(n.message)}}async function We(e){if(!await x("Delete Site","This will stop and permanently remove the pod and all its data. Are you sure?"))return;w("Deleting Site","Stopping containers and removing the pod - please wait...");try{await d.delete(`/sites/${e}`)}catch{}let s=!1,a=0;for(;!s&&a<10;){try{await new Promise(o=>setTimeout(o,2e3)),s=!(await d.get("/sites")).find(o=>o.ID===parseInt(e))}catch{}a++}h(),s?(l.success("Site deleted"),v.go("sites")):l.error("Delete failed - site still exists after 20s")}window.addEventListener("hashchange",()=>{let{view:e,params:t}=G();v.go(e,t)});var{view:Oe,params:Ve}=G();v.go(Oe,Ve);})();
