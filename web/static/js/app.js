...
"use strict";(()=>{var p={async _req(t,e,a,s=6e4){let i=new AbortController,n=setTimeout(()=>i.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:i.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let l=await fetch("/api"+e,r);clearTimeout(n);let o=l.status===204?null:await l.json().catch(()=>null);if(l.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!l.ok)throw new Error(o?.error||`HTTP ${l.status}`);return o}catch(l){throw clearTimeout(n),l}},get:t=>p._req("GET",t),post:(t,e,a)=>p._req("POST",t,e,a),put:(t,e,a)=>p._req("PUT",t,e,a),delete:t=>p._req("DELETE",t),patch:(t,e)=>p._req("PATCH",t,e)};var S=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function E(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",u);let b=UIkit.modal(`#${i}`),k=document.getElementById("kp-sync-ok"),m=document.getElementById("kp-sync-cancel"),v=g=>{b.hide(),setTimeout(()=>document.getElementById(i)?.remove(),300),s(g)};k.addEventListener("click",()=>v(!0),{once:!0}),m.addEventListener("click",()=>v(!1),{once:!0}),document.getElementById(i).addEventListener("hidden",()=>v(!1),{once:!0}),b.show()})}var h={routes:{},_ownHashChange:!1,register(t,e){this.routes[t]=e},async go(t,e={}){let a=Object.keys(e).length?t+"/"+Object.values(e).join("/"):t;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)}),document.querySelectorAll(".kp-bn-item[data-view]").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)});let s=this.routes[t];if(!s)return;let i=document.getElementById("kp-view");i.innerHTML=Z();try{await s(i,e)}catch(n){i.innerHTML=_(n.message)}}};function kt(){let e=(window.location.hash.replace("#","")||"dashboard").split("/"),a=e[0],s={};return a==="site-detail"&&e[1]&&(s.id=e[1]),{view:a,params:s}}var c={show(t,e="info",a=7e3){let s={success:"check",error:"warning",info:"info"},i=document.createElement("div");i.className=`kp-toast kp-toast-${e}`,i.innerHTML=`<span uk-icon="${s[e]||"info"}"></span><span>${S(t)}</span>`,document.getElementById("kp-toasts").appendChild(i),UIkit.icon(i.querySelector("[uk-icon]")),setTimeout(()=>i.remove(),a)},success:t=>c.show(t,"success"),error:t=>c.show(t,"error"),info:t=>c.show(t,"info")};async function at(t){let e=`
...
`).map(x=>x.trim()).filter(Boolean),install_wordpress:v===1?m.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let y=v===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",y);try{await p.post("/sites",f,6e5),w(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){w(),c.error(x.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function Ce(t){return`${t.toFixed(1)}%`}var ot=null;function bt(){return ot||(ot=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),ot)}function vt(t,e){return`
        ...
        </table>`}function qe(t,e,a,s){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let i=[...t].sort((u,b)=>{let k,m;switch(a){case"time":k=u.time,m=b.time;break;case"method":k=u.method,m=b.method;break;case"ip":k=u.client_ip,m=b.client_ip;break;default:k=u.status,m=b.status;break}return k<m?s?1:-1:k>m?s?-1:1:0}),n=50,r=Math.ceil(i.length/n),o=i.slice(e*n,(e+1)*n).map(u=>{let b=u.ua,k=u.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
                <td class="kp-stats-table-cell-mono" style="white-space:nowrap">${u.time.slice(11,19)}</td>
                <td class="kp-stats-table-cell-mono">${u.method}</td>
                <td style="word-break:break-all;font-size:0.8rem">${u.path}</td>
                <td><span class="kp-badge ${k}">${u.status}</span>${u.reason?` <span class="kp-badge kp-badge-danger" style="font-size:0.65rem" uk-tooltip="Blocked by security rule">${u.reason}</span>`:""}</td>
                <td class="kp-stats-table-cell-mono">${u.client_ip}</td>
        ...
        </div>`}function Me(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),i=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let n=null,r=!0,l=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],o=()=>{let v=l().length;e.textContent=`${v} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(y=>{let x=document.getElementById(y);x&&(x.disabled=v===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=v===0);let f=document.querySelectorAll(".kp-site-row-check");a.indeterminate=v>0&&v<f.length,a.checked=f.length>0&&v===f.length},d=()=>{let m=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(v=>{let g=v.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",f=v.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";v.style.display=!m||g.includes(m)||f.includes(m)?"":"none"})},u=m=>{n===m?r=!r:(n=m,r=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===m?r?" \u2191":" \u2193":" \u2195"});let v=[...i.querySelectorAll("tr")];v.sort((g,f)=>{let y="",x="";return m==="name"?(y=g.querySelector(".kp-site-row-name")?.textContent??"",x=f.querySelector(".kp-site-row-name")?.textContent??""):m==="status"?(y=g.dataset.status??"",x=f.dataset.status??""):m==="type"?(y=g.dataset.type??"",x=f.dataset.type??""):m==="domain"&&(y=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",x=f.querySelector("td:nth-child(6)")?.textContent.trim()??""),r?y.localeCompare(x):x.localeCompare(y)}),v.forEach(g=>i.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(m=>{m.checked=a.checked}),o()}),i?.addEventListener("change",m=>{m.target.classList.contains("kp-site-row-check")&&o()}),s?.addEventListener("input",d),document.querySelectorAll(".kp-sortable").forEach(m=>{m.addEventListener("click",()=>u(m.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(m=>{let v=m.replace("bulk-","");document.getElementById(m)?.addEventListener("click",()=>{let g=l().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:v,ids:g}}))})});let b=document.getElementById("kp-bulk-mobile-pill"),k=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",m=>{m.stopPropagation(),k.hidden=!k.hidden}),document.addEventListener("click",m=>{k&&!b?.contains(m.target)&&(k.hidden=!0)},{capture:!0}),["start","stop","restart","flush","recreate"].forEach(m=>{document.getElementById(`bulk-mobile-${m}`)?.addEventListener("click",v=>{v.preventDefault(),k.hidden=!0;let g=l().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:m,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(m=>{m.textContent=" \u2195"}),o()}var N=null;async function Dt(t){let[e,a,s]=await Promise.all([p.get("/sites").catch(()=>[]),p.get("/stats/traffic").catch(()=>null),p.get("/stats/pod").catch(()=>null)]),i=e.filter(l=>l.SiteStatus===1).length,n=e.filter(l=>l.SiteStatus===2).length,r=e.filter(l=>l.SiteStatus===4).length;if(t.innerHTML=`
...
${o}`:o),a.hide(),c.success(`${o} added to blacklist \u2014 save to apply`)})}catch(r){n.innerHTML="",c.error(r.message)}};document.getElementById("asn-lookup-go").addEventListener("click",s),document.getElementById("asn-lookup-q").addEventListener("keydown",i=>{i.key==="Enter"&&s()})}async function Rt(t){if(!q()){t.innerHTML=_("Access denied");return}t.innerHTML=`
        ...
        ${z(null)}`,lt(t),B(t)}function De(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Ht(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await p.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Ft(t){if(!q()){t.innerHTML=_("Access denied");return}let[e,a,s,i,n,r]=await Promise.all([p.get("/settings"),p.get("/settings/backup"),p.get("/settings/waf"),p.get("/settings/trusted-proxies"),p.get("/settings/notifications"),p.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Ht(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(l.target).get("admin_domain").trim()};try{await p.put("/settings",b),c.success("Settings saved"),Ht(b.admin_domain)}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("settings-import").addEventListener("change",async l=>{let o=l.target.files[0];if(!o)return;let d=new FormData;d.append("file",o);try{let u=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:d}),b=u.status===204?null:await u.json().catch(()=>null);if(!u.ok)throw new Error(b?.error||`HTTP ${u.status}`);c.success("Settings imported")}catch(u){c.error(u.message)}finally{l.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={backup_schedule:u.get("backup_schedule").trim(),backup_retain_days:u.get("backup_retain_days").trim()};try{await p.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("s3-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={s3_endpoint:u.get("s3_endpoint").trim(),s3_bucket:u.get("s3_bucket").trim(),s3_region:u.get("s3_region").trim(),s3_access_key:u.get("s3_access_key").trim()},k=u.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await p.put("/settings/backup",b),c.success("S3 settings saved")}catch(m){c.error(m.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("smtp-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={smtp_host:u.get("smtp_host").trim(),smtp_port:u.get("smtp_port").trim(),smtp_username:u.get("smtp_username").trim(),smtp_from:u.get("smtp_from").trim(),smtp_tls:u.get("smtp_tls")?"true":"false"},k=u.get("smtp_password").trim();k&&(b.smtp_password=k);try{await p.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(m){c.error(m.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("sns-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={aws_access_key:u.get("aws_access_key").trim(),aws_region:u.get("aws_region").trim(),aws_sns_sender_id:u.get("aws_sns_sender_id").trim()},k=u.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await p.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(m){c.error(m.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("resource-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(l.target),b={resource_ram_reserve_gb:u.get("resource_ram_reserve_gb").trim(),resource_poll_interval:u.get("resource_poll_interval").trim(),resource_throttle_pct:u.get("resource_throttle_pct").trim(),resource_webhook_url:u.get("resource_webhook_url").trim()};try{await p.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=d}})}function Nt(t){return`
        ...
        <tr>
            ...
            <td>
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let m=b.target.closest(".cron-edit-btn");if(m){let f=m.dataset.id,y=a.find(x=>String(x.ID)===f);if(!y)return;i.textContent="Edit Cron Job",n.value=y.ID,r.value=y.Label||"",l.value=y.Command,o.value=y.Schedule,d.textContent=zt(y.Schedule),u.checked=y.Enabled,UIkit.modal(s).show();return}let v=b.target.closest(".cron-delete-btn");if(v){let f=v.dataset.id;if(!await L("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await p.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await rt(t,e),a=await p.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let g=b.target.closest(".cron-run-btn");if(g){let f=g.dataset.id;try{await p.post(`/sites/${e}/crons/${f}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let y=null;try{y=(await p.get(`/sites/${e}/crons`)).find(H=>String(H.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,R=setInterval(async()=>{try{let I=await p.get(`/sites/${e}/crons`),H=I.find(Y=>String(Y.ID)===f);if(!H||H.LastRun!==y||Date.now()>x){clearInterval(R),w(),a=I??[];let Y=t.querySelector("#cron-list-wrap");Y&&(Y.innerHTML=Kt(I)),H?.LastError?c.error(`Job failed: ${H.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let m=k.dataset.id;try{await p.patch(`/sites/${e}/crons/${m}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(v){c.error(v.message),k.checked=!k.checked}}),p.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function zt(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,i,n,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&i==="*"&&n==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&i==="*"&&n==="*"&&r!=="*"){let l=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(d=>l[parseInt(d)]??d).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function Zt(t){return`
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Z(),a&&(a.innerHTML=Fe());try{let s=await p.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=We(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Qt(t,e){P=e||"",C(t)}function G(t,e,a=""){return new Promise(s=>{let i="fm-prompt-modal";document.getElementById(i)?.remove();let n=document.createElement("div");n.id=i,n.setAttribute("uk-modal",""),n.innerHTML=`
        ...
        </div>`}function ee(t,e){let a=null,s=!1,i=t.querySelector("#log-output"),n=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),l=t.querySelector("#log-clear"),o=t.querySelector("#log-autoscroll"),d=t.querySelector("#log-status");function u(m){for(m.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");g.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=v,i.appendChild(g)});i.childElementCount>Ke;)i.removeChild(i.firstChild);o.checked&&(i.scrollTop=i.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,n.disabled=!1,r.disabled=!0,d&&(d.textContent="Disconnected")}n.addEventListener("click",()=>{b();let m=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",f=m==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:m==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:m==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:`${g}://${location.host}/api/sites/${e}/logs?container=${m}&tail=${v}`;a=new WebSocket(f),a.onopen=()=>{s=!0,n.disabled=!0,r.disabled=!1,d&&(d.textContent=`Connected \u2014 ${m}`)},a.onmessage=y=>u(y.data),a.onerror=()=>{},a.onclose=()=>{s=!1,n.disabled=!1,r.disabled=!0,d&&(d.textContent="Disconnected")}}),r.addEventListener("click",b),l.addEventListener("click",()=>{i.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),n.click())});let k=h.go.bind(h);h.go=function(m,v={}){return a&&b(),k(m,v)}}function Je(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ae(t,e){try{let a=await p.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Je(a.status))}catch{}}function se(t){t.forEach(e=>ae(e.Domain,e.ID))}function ne(t,e,a,s=0,i=null){let n=t.SiteType!==3&&t.PMAPort>0;return`
...
`).forEach(v=>{if(!v)return;let g=document.createElement("div");m?g.className=m:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function u(k){if(k=k.trim(),!k)return;l.unshift(k),o=-1,d(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,i.disabled=!0,r&&(r.textContent="Running...");let m=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${m}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let f=g.data;if(f.trim()==="[done]"){v.close();return}if(f.startsWith("[info]")){d(f,"kp-muted");return}if(f.startsWith("[error]")){d(f,"kp-log-line-err");return}d(f)},v.onerror=()=>{d("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,i.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}i.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){u(s.value),s.value="",o=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),o<l.length-1&&(o++,s.value=l[o]);return}k.key==="ArrowDown"&&(k.preventDefault(),o>0?(o--,s.value=l[o]):(o=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let m=k.dataset.cmd;if(m.startsWith("search-replace")){s.value=m,s.focus();let v=m.indexOf("''")+1;s.setSelectionRange(v,v);return}u(m)})}),n.addEventListener("click",()=>{a.innerHTML=""});let b=h.go.bind(h);h.go=function(k,m={}){return b(k,m)},s.focus()}var Q=null,M=null;function be(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),i=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(l,o=!1){UIkit.switcher(a).show(l),e.querySelectorAll(":scope > li[data-pill]").forEach(d=>d.classList.remove("kp-pill-active")),o?(s?.classList.add("kp-pill-active"),i?.querySelectorAll("a[data-switcher]").forEach(d=>{d.classList.toggle("kp-dd-active",parseInt(d.dataset.switcher,10)===l)})):(s?.classList.remove("kp-pill-active"),i?.querySelectorAll("a[data-switcher]").forEach(d=>d.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(l=>{l.addEventListener("click",o=>{o.preventDefault();let d=parseInt(l.closest("li").dataset.pill,10);n(d,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",l=>{l.stopPropagation(),i.hidden=!i.hidden,s.classList.toggle("kp-pill-active",!i.hidden)}),i?.querySelectorAll("a[data-switcher]").forEach(l=>{l.addEventListener("click",o=>{o.preventDefault(),i.hidden=!0,n(parseInt(l.dataset.switcher,10),!0)})}),document.addEventListener("click",l=>{i&&!s.contains(l.target)&&(i.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(1)}function Qe(){return`
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${ne(a,s??[],i,a.ParentID??0,l.find(m=>m.ID===a.ParentID)?.Name??null)}</li>
            <li>${vt(e,a.SiteType)}</li>
            <li>${X(e,1,r[1])}</li>
            ${o?`<li>${X(e,2,r[2])}</li>`:""}
            <li>${X(e,3,r[3])}</li>
            ...
            <li>${re()}</li>
            <li>${Zt(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>at(a)),document.getElementById("sd-site-nav")?.addEventListener("change",m=>{h.go("site-detail",{id:m.target.value})}),lt(t),B(t),ee(t,e),Q&&Q.abort(),Q=new AbortController,pe(t,e,Q.signal),$t(e),d){Ze(t,e),Ye(e),ht(t,e,a.SiteType),ft(e,a.SiteType),wt(t,e),V(e),be(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{$("Recreating Pod","Recreating containers for this site...");try{await p.post(`/sites/${e}/recreate`),w(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(m){w(),c.error(m.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let m=await et(a.Name);if(m){$("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await p.post(`/sites/${e}/clone`,{name:m},6e5),w(),c.success(`Site cloned as '${m}'`),h.go("sites")}catch(v){w(),c.error(v.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(m=>{m.addEventListener("click",async()=>{let v=m.dataset.action;if(v==="flush"){try{await p.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(f){c.error(f.message)}return}$(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[v]??v} Pod`,"Please wait...");try{await p.post(`/sites/${e}/${v}`),w(),c.success(`Site ${v} successful`),h.go("site-detail",{id:e})}catch(f){w(),c.error(f.message)}})}),Ot(t,e),oe(t,e),a.SiteType===1&&ke(t,e),le(t,e,a),Wt(t,e),W(t,e),u&&(Jt(t,e),rt(t,e)),ue(t,e),de(e);let b=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');b&&k&&(k.dataset.switcher=String(b.children.length-1)),te(t,e),C(e),aa(t,e),ht(t,e,a.SiteType),ft(e,a.SiteType),be(t),wt(t,e),V(e),se(s??[])}async function dt(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(i=>i.remove()),typeof QRCode<"u")try{let i=await new Promise((n,r)=>{QRCode.toDataURL(t,{width:220,margin:2},(l,o)=>{l?r(l):n(o)})});e.src=i,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function ut(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
