"use strict";(()=>{var k={async _req(t,e,a,s=6e4){let n=new AbortController,o=setTimeout(()=>n.abort(),s),i={method:t,headers:{"Content-Type":"application/json"},signal:n.signal};t!=="GET"&&t!=="HEAD"&&(i.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(i.body=JSON.stringify(a));try{let r=await fetch("/api"+e,i);clearTimeout(o);let l=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);return l}catch(r){throw clearTimeout(o),r}},get:t=>k._req("GET",t),post:(t,e)=>k._req("POST",t,e),put:(t,e)=>k._req("PUT",t,e),delete:t=>k._req("DELETE",t),patch:(t,e)=>k._req("PATCH",t,e)};var ct=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let n=UIkit.modal(`#${a}`),o=document.getElementById("kp-clone-name"),i=document.getElementById("kp-clone-ok"),r=document.getElementById("kp-clone-cancel"),l=p=>{n.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(p)};i.addEventListener("click",()=>l(o.value.trim()||null),{once:!0}),r.addEventListener("click",()=>l(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>l(null),{once:!0}),n.show(),setTimeout(()=>o.focus(),150),o.addEventListener("keydown",p=>{p.key==="Enter"&&i.click()})})}function et(t,e,a){return new Promise(s=>{let n="kp-sync-modal",o=t==="pull",i=o?"Pull From Parent":"Push To Parent",r=o?"cloud-download":"cloud-upload",l=o?a:e,p=o?e:a,d=`
            <div id="${n}" uk-modal>
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",e);let a=UIkit.modal("#kp-edit-site-modal"),s=document.getElementById("es-site-type"),n=document.getElementById("es-php-version-wrap"),o=document.getElementById("es-node-version-wrap"),i=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),l=document.getElementById("es-wordpress-wrap");a.show();let p=d=>{n.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),o.classList.toggle("uk-hidden",d!==4),i.classList.toggle("uk-hidden",d!==5),r.classList.toggle("uk-hidden",d!==4&&d!==5),l.classList.toggle("uk-hidden",d!==1)};p(t.SiteType),s.addEventListener("change",()=>p(parseInt(s.value))),document.getElementById("edit-site-form").addEventListener("submit",async d=>{d.preventDefault();let g=d.target.querySelector('[type="submit"]'),m=g.innerHTML;g.disabled=!0,g.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(d.target),b=parseInt(u.get("site_type")),v=null;b===4&&(v=parseInt(u.get("node_version"))),b===5&&(v=parseInt(u.get("dotnet_version")));let f={name:u.get("name").trim(),php_version:parseInt(u.get("php_version"))||3,site_type:b,runtime_version:v,start_command:u.get("start_command")?.trim()||""},w=b===1?u.get("install_wordpress")==="on":!1;try{if(await k.put(`/sites/${t.ID}`,f),a.hide(),document.getElementById("kp-edit-site-modal")?.remove(),b!==6){S("Applying Changes","Saving changes and recreating pod...");try{await k.post(`/sites/${t.ID}/recreate`,{install_wordpress:w}),y(),c.success("Site updated and pod recreated")}catch(x){y(),c.error("Site saved but pod recreate failed: "+x.message)}}else c.success("Site updated");h.go("site-detail",{id:String(t.ID)})}catch(x){c.error(x.message),g.disabled=!1,g.innerHTML=m}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}var Ot=2e3;function Vt(){return`
        ...
        </div>`}function Jt(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),n=t.querySelector("#admin-log-connect"),o=t.querySelector("#admin-log-disconnect"),i=t.querySelector("#admin-log-clear"),r=t.querySelector("#admin-log-autoscroll"),l=t.querySelector("#admin-log-status");function p(m){for(m.split(`
`).forEach(u=>{if(!u)return;let b=document.createElement("div");b.className=u.match(/WAF BLOCK/i)?"kp-log-line-err":u.match(/WAF DETECT/i)?"kp-log-line-warn":u.match(/error|crit|emerg/i)?"kp-log-line-err":u.match(/warn/i)?"kp-log-line-warn":u.match(/info|notice/i)?"kp-log-line-info":"",b.textContent=u,s.appendChild(b)});s.childElementCount>Ot;)s.removeChild(s.firstChild);r.checked&&(s.scrollTop=s.scrollHeight)}function d(){e&&(e.close(),e=null),a=!1,n.disabled=!1,o.disabled=!0,l&&(l.textContent="Disconnected")}n.addEventListener("click",()=>{d();let m=t.querySelector("#admin-log-source").value,u=t.querySelector("#admin-log-tail").value,b=location.protocol==="https:"?"wss":"ws",v=m==="waf"?`${b}://${location.host}/api/logs/waf?tail=${u}`:`${b}://${location.host}/api/logs/proxy?tail=${u}`;e=new WebSocket(v),e.onopen=()=>{a=!0,n.disabled=!0,o.disabled=!1,l&&(l.textContent=`Connected \u2014 ${m==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=f=>p(f.data),e.onerror=()=>{},e.onclose=()=>{a=!1,n.disabled=!1,o.disabled=!0,l&&(l.textContent="Disconnected")}}),o.addEventListener("click",d),i.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(d(),n.click())});let g=h.go.bind(h);h.go=function(m,u={}){return e&&d(),g(m,u)}}function dt(t){t.innerHTML=Vt(),Jt(t)}function V(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),n=document.getElementById("cs-node-version-wrap"),o=document.getElementById("cs-dotnet-version-wrap"),i=document.getElementById("cs-start-command-wrap"),r=document.getElementById("cs-wordpress-wrap");e.show();let l=document.getElementById("cs-domains-wrap"),p=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let d=parseInt(a.value);s.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),n.classList.toggle("uk-hidden",d!==4),o.classList.toggle("uk-hidden",d!==5),i.classList.toggle("uk-hidden",d!==4&&d!==5),r.classList.toggle("uk-hidden",d!==1||d===6),l.classList.toggle("uk-hidden",d===6),p.classList.toggle("uk-hidden",d!==6)}),document.getElementById("create-site-form").addEventListener("submit",async d=>{d.preventDefault();let g=d.target.querySelector('[type="submit"]'),m=g.innerHTML;g.disabled=!0,g.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let u=new FormData(d.target),b=parseInt(u.get("site_type")),v=null;b===4&&(v=parseInt(u.get("node_version"))),b===5&&(v=parseInt(u.get("dotnet_version")));let f={name:u.get("name").trim(),php_version:parseInt(u.get("php_version"))||3,site_type:b,runtime_version:v,start_command:u.get("start_command")?.trim()||"",domains:u.get("domains").split(`
`).map(x=>x.trim()).filter(Boolean),install_wordpress:b===1?u.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=b===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;S("Creating Site",w);try{await k.post("/sites",f),y(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){y(),c.error(x.message),g.disabled=!1,g.innerHTML=m}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var _=null;function T(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}function Kt(t){return`${t.toFixed(1)}%`}var J=null;function st(){return J||(J=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),J)}function nt(t,e){return`
        ...
        <div class="kp-card uk-padding-small uk-margin-bottom">
            ...
            <div id="stats-disk-wrap">
                ...
                <tr>
                    ...
                    <td class="kp-stats-table-cell-count">${o.count.toLocaleString()}</td>
                </tr>`).join(""))}async function pt(t){let e=document.getElementById("stats-disk-wrap");if(e){e.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>';try{let a=await k.get(`/sites/${t}/stats/disk`);e.innerHTML=`
        ...
        ${p}`}async function Yt(t,e,a){let s=document.getElementById("stats-drilldown-modal"),n=document.getElementById("stats-drilldown-title"),o=document.getElementById("stats-drilldown-body");if(!s||!o)return;n.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,o.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(s).show();let i=[],r=0,l="time",p=!0;function d(){o.innerHTML=Xt(i,r,l,p),o.querySelectorAll(".kp-dd-sort").forEach(g=>{g.addEventListener("click",()=>{let m=g.dataset.ddCol;l===m?p=!p:(l=m,p=!0),r=0,d()})}),o.querySelectorAll("[data-dd-page]").forEach(g=>{g.addEventListener("click",()=>{r=parseInt(g.dataset.ddPage,10),d()})})}try{i=await k.get(`/sites/${t}/stats/drilldown?hour=${encodeURIComponent(e)}&status=${a}`)}catch(g){o.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${g.message}</p>`;return}d()}function ut(t,e,a){let s=a===6,n=null;function o(){if(s)return;let l=t.querySelector("#stats-pod-indicator"),p=t.querySelector("#stats-pod-table-wrap");if(!p)return;let d=location.protocol==="https:"?"wss":"ws";n=new WebSocket(`${d}://${location.host}/api/sites/${e}/stats/pod`),n.onopen=()=>{l&&(l.className="kp-status kp-status-running",l.textContent="Live")},n.onmessage=g=>{try{let m=JSON.parse(g.data);p.innerHTML=Qt(m.containers??[])}catch{}},n.onerror=()=>{l&&(l.className="kp-status kp-status-error",l.textContent="Error")},n.onclose=()=>{l&&l.textContent==="Live"&&(l.className="kp-status kp-status-stopped",l.textContent="Disconnected")}}function i(){n&&n.readyState===WebSocket.OPEN&&n.close(),n=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{pt(e)}),o();let r=new MutationObserver(()=>{document.getElementById("stats-panel")||(i(),r.disconnect())});r.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function kt(t,e){let a=e===6;await Gt(t),a||await pt(t)}async function mt(t){let e=await k.get("/sites")??[];t.innerHTML=`
        ...
        </div>`}function te(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),n=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let o=null,i=!0,r=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],l=()=>{let b=r().length;e.textContent=`${b} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush"].forEach(w=>{let x=document.getElementById(w);x&&(x.disabled=b===0)});let v=document.getElementById("kp-bulk-mobile-btn");v&&(v.disabled=b===0);let f=document.querySelectorAll(".kp-site-row-check");a.indeterminate=b>0&&b<f.length,a.checked=f.length>0&&b===f.length},p=()=>{let u=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(b=>{let v=b.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",f=b.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";b.style.display=!u||v.includes(u)||f.includes(u)?"":"none"})},d=u=>{o===u?i=!i:(o=u,i=!0),document.querySelectorAll(".kp-sort-icon").forEach(v=>{v.textContent=v.dataset.col===u?i?" \u2191":" \u2193":" \u2195"});let b=[...n.querySelectorAll("tr")];b.sort((v,f)=>{let w="",x="";return u==="name"?(w=v.querySelector(".kp-site-row-name")?.textContent??"",x=f.querySelector(".kp-site-row-name")?.textContent??""):u==="status"?(w=v.dataset.status??"",x=f.dataset.status??""):u==="type"?(w=v.dataset.type??"",x=f.dataset.type??""):u==="domain"&&(w=v.querySelector("td:nth-child(6)")?.textContent.trim()??"",x=f.querySelector("td:nth-child(6)")?.textContent.trim()??""),i?w.localeCompare(x):x.localeCompare(w)}),b.forEach(v=>n.appendChild(v))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(u=>{u.checked=a.checked}),l()}),n?.addEventListener("change",u=>{u.target.classList.contains("kp-site-row-check")&&l()}),s?.addEventListener("input",p),document.querySelectorAll(".kp-sortable").forEach(u=>{u.addEventListener("click",()=>d(u.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush"].forEach(u=>{let b=u.replace("bulk-","");document.getElementById(u)?.addEventListener("click",()=>{let v=r().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:b,ids:v}}))})});let g=document.getElementById("kp-bulk-mobile-pill"),m=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",u=>{u.stopPropagation(),m.hidden=!m.hidden}),document.addEventListener("click",u=>{m&&!g?.contains(u.target)&&(m.hidden=!0)},{capture:!0}),["start","stop","restart","flush"].forEach(u=>{document.getElementById(`bulk-mobile-${u}`)?.addEventListener("click",b=>{b.preventDefault(),m.hidden=!0;let v=r().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:u,ids:v}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(u=>{u.textContent=" \u2195"}),l()}var K=null;async function gt(t){let[e,a,s]=await Promise.all([k.get("/sites").catch(()=>[]),k.get("/stats/traffic").catch(()=>null),k.get("/stats/pod").catch(()=>null)]),n=e.filter(r=>r.SiteStatus===1).length,o=e.filter(r=>r.SiteStatus===2).length,i=e.filter(r=>r.SiteStatus===4).length;if(t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${H(null)}`,G(t),C(t)}function ee(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ht(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await k.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=ee(a.status)}catch{}}async function ft(t){if(!q()){t.innerHTML=L("Access denied");return}let[e,a,s,n]=await Promise.all([k.get("/settings"),k.get("/settings/backup"),k.get("/settings/waf"),k.get("/settings/trusted-proxies")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&ht(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await k.put("/settings",p),c.success("Settings saved"),ht(p.admin_domain)}catch(d){c.error(d.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let l=await fetch("/api/settings/import",{method:"POST",body:r}),p=l.status===204?null:await l.json().catch(()=>null);if(!l.ok)throw new Error(p?.error||`HTTP ${l.status}`);c.success("Settings imported")}catch(l){c.error(l.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(o.target),p={backup_schedule:l.get("backup_schedule").trim(),backup_retain_days:l.get("backup_retain_days").trim()};try{await k.put("/settings/backup",p),c.success("Backup settings saved")}catch(d){c.error(d.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(o.target),p={s3_endpoint:l.get("s3_endpoint").trim(),s3_bucket:l.get("s3_bucket").trim(),s3_region:l.get("s3_region").trim(),s3_access_key:l.get("s3_access_key").trim()},d=l.get("s3_secret_key").trim();d&&(p.s3_secret_key=d);try{await k.put("/settings/backup",p),c.success("S3 settings saved")}catch(g){c.error(g.message)}finally{i.disabled=!1,i.innerHTML=r}})}function wt(t){return`
        ...
        </table>`}function yt(t,e){let a=Date.now()+18e5,s=setInterval(async()=>{try{let n=await k.get(`/sites/${e}/backups/restore-status`);(!n?.active||Date.now()>a)&&(clearInterval(s),y(),n?.active?c.error("Import timed out \u2014 check server logs"):c.success("Import complete"),await M(t,e))}catch{}},3e3)}async function M(t,e){try{let[a,s]=await Promise.all([k.get(`/sites/${e}/backup-repo`),k.get(`/sites/${e}/backups`)]),n=t.querySelector("#backup-local-enabled"),o=t.querySelector("#backup-s3-enabled");n&&(n.checked=!!a.LocalEnabled),o&&(o.checked=!!a.S3Enabled);let i=t.querySelector("#backup-error-banner");if(i)if(a.last_error){let l=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";i.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${l}:</strong> ${a.last_error}</p>
                    </div>`}else i.innerHTML="";let r=t.querySelector("#backup-list-wrap");r&&(r.innerHTML=ae(s))}catch(a){let s=t.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function xt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await k.put(`/sites/${e}/backup-repo`,s),c.success("Backup destinations saved")}catch(n){c.error(n.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await k.get(`/sites/${e}/backups`))?.length??0}catch{}try{await k.post(`/sites/${e}/backups`,{label:"manual"})}catch(i){c.error(i.message);return}S("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let n=Date.now()+1800*1e3,o=setInterval(async()=>{try{(((await k.get(`/sites/${e}/backups`))?.length??0)>s||Date.now()>n)&&(clearInterval(o),y(),await M(t,e),Date.now()<=n?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let n=s.target.closest(".backup-restore-btn");if(n){let r=n.dataset.id;if(!await $("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await k.post(`/sites/${e}/backups/${r}/restore`)}catch(m){c.error(m.message);return}S("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let p=Date.now(),d=Date.now()+900*1e3,g=setInterval(async()=>{try{let m=await k.get(`/sites/${e}/backups/restore-status`);(!m?.active||Date.now()>d)&&(clearInterval(g),y(),m?.active?c.error("Restore timed out"):c.success("Restore complete"),await M(t,e))}catch{}},3e3);return}let o=s.target.closest(".backup-delete-btn");if(o){let r=o.dataset.id;if(!await $("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;S("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await k.delete(`/sites/${e}/backups/${r}`),y(),c.success("Snapshot deleted"),await M(t,e)}catch(p){y(),c.error(p.message)}}let i=s.target.closest(".backup-download-btn");if(i){let r=i.dataset.id;S("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let l=document.createElement("a");l.href=`/api/sites/${e}/backups/${r}/download`,l.style.display="none",document.body.appendChild(l),l.click(),document.body.removeChild(l),setTimeout(()=>{y()},5e3)},300);return}});let a=t.querySelector("#import-backup-modal");a&&(UIkit.util.on(a,"beforeshow",async()=>{let s=a.querySelector("#import-target-site");try{let o=await k.get("/sites");s.innerHTML=o.map(i=>`<option value="${i.ID}"${i.ID===e?" selected":""}>${i.Name}</option>`).join("")}catch{s.innerHTML='<option value="">Failed to load sites</option>'}let n=a.querySelector("#import-sftp-list");try{let o=await k.get(`/sites/${e}/backups/import/files`);!o||o.length===0?n.innerHTML='<p class="kp-muted uk-text-small">No files found.</p>':n.innerHTML=o.map(i=>`
        ...
        <tr>
            ...
            <td>
        ...
        </table>`}async function Q(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await k.get(`/sites/${e}/crons`);a.innerHTML=Tt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${s.message}</p>`}}function Ct(t,e){let a=[],s=t.querySelector("#cron-modal"),n=t.querySelector("#cron-modal-title"),o=t.querySelector("#cron-modal-id"),i=t.querySelector("#cron-modal-label"),r=t.querySelector("#cron-modal-command"),l=t.querySelector("#cron-modal-schedule"),p=t.querySelector("#cron-schedule-preview"),d=t.querySelector("#cron-modal-enabled");l?.addEventListener("input",()=>{p.textContent=Et(l.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",o.value="",i.value="",r.value="",l.value="",p.textContent="",d.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let g=r.value.trim(),m=l.value.trim();if(!g||!m){c.error("Command and schedule are required");return}let u={label:i.value.trim(),command:g,schedule:m,enabled:d.checked},b=o.value;try{b?(await k.put(`/sites/${e}/crons/${b}`,u),c.success("Cron job updated")):(await k.post(`/sites/${e}/crons`,u),c.success("Cron job created")),UIkit.modal(s).hide(),await Q(t,e),a=await k.get(`/sites/${e}/crons`)}catch(v){c.error(v.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async g=>{let m=g.target.closest(".cron-detail-btn");if(m){let f=m.dataset.id,w=a.find(tt=>String(tt.ID)===f);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let u=g.target.closest(".cron-edit-btn");if(u){let f=u.dataset.id,w=a.find(x=>String(x.ID)===f);if(!w)return;n.textContent="Edit Cron Job",o.value=w.ID,i.value=w.Label||"",r.value=w.Command,l.value=w.Schedule,p.textContent=Et(w.Schedule),d.checked=w.Enabled,UIkit.modal(s).show();return}let b=g.target.closest(".cron-delete-btn");if(b){let f=b.dataset.id;if(!await $("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await k.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await Q(t,e),a=await k.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let v=g.target.closest(".cron-run-btn");if(v){let f=v.dataset.id;try{await k.post(`/sites/${e}/crons/${f}/run`)}catch(E){c.error(E.message);return}S("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await k.get(`/sites/${e}/crons`)).find(B=>String(B.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,tt=setInterval(async()=>{try{let E=await k.get(`/sites/${e}/crons`),B=E.find(j=>String(j.ID)===f);if(!B||B.LastRun!==w||Date.now()>x){clearInterval(tt),y(),a=E??[];let j=t.querySelector("#cron-list-wrap");j&&(j.innerHTML=Tt(E)),B?.LastError?c.error(`Job failed: ${B.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async g=>{let m=g.target.closest(".cron-toggle");if(!m)return;let u=m.dataset.id;try{await k.patch(`/sites/${e}/crons/${u}/toggle`,{enabled:m.checked}),c.success(m.checked?"Cron job enabled":"Cron job disabled")}catch(b){c.error(b.message),m.checked=!m.checked}}),k.get(`/sites/${e}/crons`).then(g=>{a=g??[]}).catch(()=>{})}function ot(t){return String(t).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Et(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,n,o,i]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(p=>r[parseInt(p)]??p).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var se=2e3;function it(t,e){return`
        <div>
            <div class="kp-log-controls">
        ...
        </div>`}function Pt(t,e){let a=null,s=!1,n=t.querySelector("#log-output"),o=t.querySelector("#log-connect"),i=t.querySelector("#log-disconnect"),r=t.querySelector("#log-clear"),l=t.querySelector("#log-autoscroll"),p=t.querySelector("#log-status");function d(u){for(u.split(`
`).forEach(b=>{if(!b)return;let v=document.createElement("div");v.className=b.match(/WAF BLOCK/i)?"kp-log-line-err":b.match(/WAF DETECT/i)?"kp-log-line-warn":b.match(/error|crit|emerg/i)?"kp-log-line-err":b.match(/warn/i)?"kp-log-line-warn":b.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=b,n.appendChild(v)});n.childElementCount>se;)n.removeChild(n.firstChild);l.checked&&(n.scrollTop=n.scrollHeight)}function g(){a&&(a.close(),a=null),s=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}o.addEventListener("click",()=>{g();let u=t.querySelector("#log-container").value,b=t.querySelector("#log-tail").value,v=location.protocol==="https:"?"wss":"ws",f=u==="waf"?`${v}://${location.host}/api/sites/${e}/logs/waf?tail=${b}`:u==="proxy"?`${v}://${location.host}/api/sites/${e}/logs/proxy?tail=${b}`:`${v}://${location.host}/api/sites/${e}/logs?container=${u}&tail=${b}`;a=new WebSocket(f),a.onopen=()=>{s=!0,o.disabled=!0,i.disabled=!1,p&&(p.textContent=`Connected \u2014 ${u}`)},a.onmessage=w=>d(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}}),i.addEventListener("click",g),r.addEventListener("click",()=>{n.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(g(),o.click())});let m=h.go.bind(h);h.go=function(u,b={}){return a&&g(),m(u,b)}}function ne(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function It(t,e){try{let a=await k.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=ne(a.status))}catch{}}function Bt(t){t.forEach(e=>It(e.Domain,e.ID))}function qt(t,e,a,s=0,n=null){let o=t.SiteType!==3&&t.PMAPort>0;return`
        ...
        </div>`}function Ht(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),n=t.querySelector("#wpcli-run"),o=t.querySelector("#wpcli-clear"),i=t.querySelector("#wpcli-status"),r=[],l=-1;function p(m,u=""){m.split(`
`).forEach(b=>{if(!b)return;let v=document.createElement("div");u?v.className=u:v.className=b.match(/error|fatal|critical/i)?"kp-log-line-err":b.match(/warning|warn/i)?"kp-log-line-warn":b.match(/success|done\]/i)?"kp-log-line-info":"",v.textContent=b,a.appendChild(v)}),a.scrollTop=a.scrollHeight}function d(m){if(m=m.trim(),!m)return;r.unshift(m),l=-1,p(`wp> ${m}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,i&&(i.textContent="Running...");let u=location.protocol==="https:"?"wss":"ws",b=new WebSocket(`${u}://${location.host}/api/sites/${e}/wpcli`);b.onopen=()=>{b.send(JSON.stringify({command:m}))},b.onmessage=v=>{let f=v.data;if(f.trim()==="[done]"){b.close();return}if(f.startsWith("[info]")){p(f,"kp-muted");return}if(f.startsWith("[error]")){p(f,"kp-log-line-err");return}p(f)},b.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},b.onclose=()=>{s.disabled=!1,n.disabled=!1,i&&(i.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{d(s.value),s.value=""}),s.addEventListener("keydown",m=>{if(m.key==="Enter"){d(s.value),s.value="",l=-1;return}if(m.key==="ArrowUp"){m.preventDefault(),l<r.length-1&&(l++,s.value=r[l]);return}m.key==="ArrowDown"&&(m.preventDefault(),l>0?(l--,s.value=r[l]):(l=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(m=>{m.addEventListener("click",()=>{let u=m.dataset.cmd;if(u.startsWith("search-replace")){s.value=u,s.focus();let b=u.indexOf("''")+1;s.setSelectionRange(b,b);return}d(u)})}),o.addEventListener("click",()=>{a.innerHTML=""});let g=h.go.bind(h);h.go=function(m,u={}){return g(m,u)},s.focus()}var U=null;function ie(){return`
        ...
        <ul class="kp-tab-pills" id="kp-site-pills">
            <li id="kp-manage-pill">
                ...
                <div class="kp-pill-dropdown" id="kp-manage-dropdown" hidden>
                    ...
                    ${d?`<a href="#" data-switcher="${a.SiteType===1?l?12:11:l?11:10}"><span uk-icon="icon: clock; ratio: 0.85"></span> Crons</a>`:""}
                </div>
            </li>
            <li data-pill="1"><a href="#">Stats</a></li>
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${qt(a,s??[],n,a.ParentID??0,r.find(g=>g.ID===a.ParentID)?.Name??null)}</li>
            <li>${nt(e,a.SiteType)}</li>
            <li>${N(e,1,i[1])}</li>
            ${l?`<li>${N(e,2,i[2])}</li>`:""}
            <li>${N(e,3,i[3])}</li>
            ...
            <li>${wt(e)}</li>
            ${d?`<li>${Lt(e)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>O(a)),document.getElementById("sd-site-nav")?.addEventListener("change",g=>{h.go("site-detail",{id:g.target.value})}),G(t),C(t),Pt(t,e),le(t,e),Ft(e),p){de(t,e),ce(e),At(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{S("Recreating Pod","Recreating containers for this site...");try{await k.post(`/sites/${e}/recreate`),y(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(g){y(),c.error(g.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let g=await z(a.Name);if(g){S("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await k.post(`/sites/${e}/clone`,{name:g}),y(),c.success(`Site cloned as '${g}'`),h.go("sites")}catch(m){y(),c.error(m.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(g=>{g.addEventListener("click",async()=>{let m=g.dataset.action;if(m==="flush"){try{await k.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(b){c.error(b.message)}return}S(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[m]??m} Pod`,"Please wait...");try{await k.post(`/sites/${e}/${m}`),y(),c.success(`Site ${m} successful`),h.go("site-detail",{id:e})}catch(b){y(),c.error(b.message)}})}),$t(t,e),_t(t,e),a.SiteType===1&&Ht(t,e),Mt(t,e,a),xt(t,e),M(t,e),d&&(Ct(t,e),Q(t,e)),ut(t,e,a.SiteType),kt(e,a.SiteType),At(t),Bt(s??[])}async function X(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((o,i)=>{QRCode.toDataURL(t,{width:220,margin:2},(r,l)=>{r?i(r):o(l)})});e.src=n,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function Y(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=n,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),h.go("users")})}function Ut(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let o=UIkit.modal("#kp-edit-user-modal");o.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let d=p.target.querySelector('[type="submit"]'),g=d.innerHTML;d.disabled=!0,d.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let m=new FormData(p.target),u={fname:m.get("fname").trim(),lname:m.get("lname").trim(),email:m.get("email").trim(),phone:m.get("phone").trim()};if(s){u.role=parseInt(m.get("role"));let v=m.get("uname");v&&(u.uname=v.trim())}let b=m.get("password");b&&(u.password=b);try{await k.put(`/users/${e}`,u),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("User updated"),h.go("users")}catch(v){c.error(v.message),d.disabled=!1,d.innerHTML=g}});let i=document.getElementById("totp-setup-btn");i&&i.addEventListener("click",async()=>{i.disabled=!0,i.textContent="Setting up\u2026";try{let p=await k.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await X(p.uri)}catch(p){c.error(p.message),i.disabled=!1,i.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){c.error("Enter a 6-digit code");return}r.disabled=!0;try{let d=await k.post(`/users/${e}/totp/confirm`,{code:p});o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("TOTP enabled"),d.backup_codes?.length?Y(d.backup_codes):h.go("users")}catch(d){c.error(d.message),r.disabled=!1}});let l=document.getElementById("totp-disable-btn");l&&l.addEventListener("click",async()=>{l.disabled=!0;try{await k.delete(`/users/${e}/totp`),c.success("TOTP disabled"),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),h.go("users")}catch(p){c.error(p.message),l.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function Wt(t){if(!q()){t.innerHTML=L("Access denied");return}let e=await k.get("/users");t.innerHTML=`
        <div class="kp-view-header">
