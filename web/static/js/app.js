...
"use strict";(()=>{var p={async _req(t,e,a,s=6e4){let i=new AbortController,n=setTimeout(()=>i.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:i.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let l=await fetch("/api"+e,r);clearTimeout(n);let o=l.status===204?null:await l.json().catch(()=>null);if(l.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!l.ok)throw new Error(o?.error||`HTTP ${l.status}`);return o}catch(l){throw clearTimeout(n),l}},get:t=>p._req("GET",t),post:(t,e,a)=>p._req("POST",t,e,a),put:(t,e,a)=>p._req("PUT",t,e,a),delete:t=>p._req("DELETE",t),patch:(t,e)=>p._req("PATCH",t,e)};var S=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function E(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a),UIkit.modal("#kp-progress-modal").show()}function w(){let t=document.getElementById("kp-progress-modal");t&&(UIkit.modal(t).hide(),setTimeout(()=>t.remove(),300))}function tt(t){return new Promise(e=>{let a="kp-clone-modal",s=`
            <div id="${a}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let i=UIkit.modal(`#${a}`),n=document.getElementById("kp-clone-name"),r=document.getElementById("kp-clone-ok"),l=document.getElementById("kp-clone-cancel"),o=u=>{i.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(u)};r.addEventListener("click",()=>o(n.value.trim()||null),{once:!0}),l.addEventListener("click",()=>o(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>o(null),{once:!0}),i.show(),setTimeout(()=>n.focus(),150),n.addEventListener("keydown",u=>{u.key==="Enter"&&r.click()})})}function mt(t,e,a){return new Promise(s=>{let i="kp-sync-modal",n=t==="pull",r=n?"Pull From Parent":"Push To Parent",l=n?"cloud-download":"cloud-upload",o=n?a:e,u=n?e:a,d=`
            <div id="${i}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",d);let v=UIkit.modal(`#${i}`),k=document.getElementById("kp-sync-ok"),m=document.getElementById("kp-sync-cancel"),b=g=>{v.hide(),setTimeout(()=>document.getElementById(i)?.remove(),300),s(g)};k.addEventListener("click",()=>b(!0),{once:!0}),m.addEventListener("click",()=>b(!1),{once:!0}),document.getElementById(i).addEventListener("hidden",()=>b(!1),{once:!0}),v.show()})}var h={routes:{},_ownHashChange:!1,register(t,e){this.routes[t]=e},async go(t,e={}){let a=Object.keys(e).length?t+"/"+Object.values(e).join("/"):t;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)}),document.querySelectorAll(".kp-bn-item[data-view]").forEach(n=>{n.classList.toggle("kp-active",n.dataset.view===t)});let s=this.routes[t];if(!s)return;let i=document.getElementById("kp-view");i.innerHTML=Z();try{await s(i,e)}catch(n){i.innerHTML=_(n.message)}}};function kt(){let e=(window.location.hash.replace("#","")||"dashboard").split("/"),a=e[0],s={};return a==="site-detail"&&e[1]&&(s.id=e[1]),{view:a,params:s}}var c={show(t,e="info",a=7e3){let s={success:"check",error:"warning",info:"info"},i=document.createElement("div");i.className=`kp-toast kp-toast-${e}`,i.innerHTML=`<span uk-icon="${s[e]||"info"}"></span><span>${S(t)}</span>`,document.getElementById("kp-toasts").appendChild(i),UIkit.icon(i.querySelector("[uk-icon]")),setTimeout(()=>i.remove(),a)},success:t=>c.show(t,"success"),error:t=>c.show(t,"error"),info:t=>c.show(t,"info")};async function et(t){let e=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",e);let a=UIkit.modal("#kp-edit-site-modal"),s=document.getElementById("es-site-type"),i=document.getElementById("es-php-version-wrap"),n=document.getElementById("es-node-version-wrap"),r=document.getElementById("es-dotnet-version-wrap"),l=document.getElementById("es-start-command-wrap"),o=document.getElementById("es-wordpress-wrap");a.show();let u=d=>{i.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),n.classList.toggle("uk-hidden",d!==4),r.classList.toggle("uk-hidden",d!==5),l.classList.toggle("uk-hidden",d!==4&&d!==5),o.classList.toggle("uk-hidden",d!==1)};u(t.SiteType),s.addEventListener("change",()=>u(parseInt(s.value))),document.getElementById("edit-site-form").addEventListener("submit",async d=>{d.preventDefault();let v=d.target.querySelector('[type="submit"]'),k=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let m=new FormData(d.target),b=parseInt(m.get("site_type")),g=null;b===4&&(g=parseInt(m.get("node_version"))),b===5&&(g=parseInt(m.get("dotnet_version")));let f={name:m.get("name").trim(),php_version:parseInt(m.get("php_version"))||3,site_type:b,runtime_version:g,start_command:m.get("start_command")?.trim()||""},y=b===1?m.get("install_wordpress")==="on":!1;try{if(await p.put(`/sites/${t.ID}`,f),a.hide(),document.getElementById("kp-edit-site-modal")?.remove(),b!==6){$("Applying Changes","Saving changes and recreating pod...");try{await p.post(`/sites/${t.ID}/recreate`,{install_wordpress:y}),w(),c.success("Site updated and pod recreated")}catch(x){w(),c.error("Site saved but pod recreate failed: "+x.message)}}else c.success("Site updated");h.go("site-detail",{id:String(t.ID)})}catch(x){c.error(x.message),v.disabled=!1,v.innerHTML=k}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}var we=2e3;function xe(){return`
        ...
        </div>`}function Se(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),i=t.querySelector("#admin-log-connect"),n=t.querySelector("#admin-log-disconnect"),r=t.querySelector("#admin-log-clear"),l=t.querySelector("#admin-log-autoscroll"),o=t.querySelector("#admin-log-status");function u(k){for(k.split(`
`).forEach(m=>{if(!m)return;let b=document.createElement("div");b.className=m.match(/WAF BLOCK/i)?"kp-log-line-err":m.match(/WAF DETECT/i)?"kp-log-line-warn":m.match(/error|crit|emerg/i)?"kp-log-line-err":m.match(/warn/i)?"kp-log-line-warn":m.match(/info|notice/i)?"kp-log-line-info":"",b.textContent=m,s.appendChild(b)});s.childElementCount>we;)s.removeChild(s.firstChild);l.checked&&(s.scrollTop=s.scrollHeight)}function d(){e&&(e.close(),e=null),a=!1,i.disabled=!1,n.disabled=!0,o&&(o.textContent="Disconnected")}i.addEventListener("click",()=>{d();let k=t.querySelector("#admin-log-source").value,m=t.querySelector("#admin-log-tail").value,b=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${b}://${location.host}/api/logs/waf?tail=${m}`:`${b}://${location.host}/api/logs/proxy?tail=${m}`;e=new WebSocket(g),e.onopen=()=>{a=!0,i.disabled=!0,n.disabled=!1,o&&(o.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=f=>u(f.data),e.onerror=()=>{},e.onclose=()=>{a=!1,i.disabled=!1,n.disabled=!0,o&&(o.textContent="Disconnected")}}),n.addEventListener("click",d),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(d(),i.click())});let v=h.go.bind(h);h.go=function(k,m={}){return e&&d(),v(k,m)}}function Tt(t){t.innerHTML=xe(),Se(t)}var st=50;function $e(t,e,a){let s=Math.max(1,Math.ceil(t.total/st)),i=(t.entries??[]).map(_t).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        ...
        </div>`}function _t(t){let e=new Date(t.ts).toLocaleString(),a=t.username?`<span style="font-family:monospace">${T(t.username)}</span>`:'<span style="color:var(--kp-text-dim)">\u2014</span>',s=Le(t.status),n=t.prior_state||t.new_state?`<button class="uk-button kp-btn-ghost kp-btn-sm al-diff-btn"
        ...
        ${s}
    </div>`}function Le(t){return`<span class="kp-badge ${t>=500?"kp-badge-error":t>=400?"kp-badge-warn":t>=300?"kp-badge-info":"kp-badge-ok"}">${t}</span>`}var T=t=>S(t??"");async function Pt(t,e){let a=new URLSearchParams({page:e,page_size:st});return t.username&&a.set("username",t.username),t.action&&a.set("action",t.action),t.target_type&&a.set("target_type",t.target_type),t.date_from&&a.set("date_from",t.date_from),t.date_to&&a.set("date_to",t.date_to),t.auth!==""&&a.set("auth",t.auth),p.get(`/audit?${a}`)}function Ee(t){return{username:t.querySelector("#al-filter-user").value.trim(),action:t.querySelector("#al-filter-action").value.trim(),target_type:t.querySelector("#al-filter-target").value.trim(),date_from:t.querySelector("#al-filter-date-from").value,date_to:t.querySelector("#al-filter-date-to").value,auth:t.querySelector("#al-filter-auth").value}}async function Te(t,e,a){async function s(r,l){let o=await Pt(r,l);t.querySelector("#al-table-body").innerHTML=(o.entries??[]).map(_t).join("")||'<tr><td colspan="8" class="uk-text-center kp-text-dim">No records found</td></tr>';let u=Math.max(1,Math.ceil(o.total/st)),d=t.querySelector("#al-pager");d&&(d.innerHTML=u>1?Ct(l,u):""),t.querySelector("#al-record-count").textContent=`${o.total} record${o.total!==1?"s":""}`,i(t,r,l,u),e=r,a=l}function i(r,l,o,u){r.querySelector("#al-prev")?.addEventListener("click",()=>s(l,o-1)),r.querySelector("#al-next")?.addEventListener("click",()=>s(l,o+1))}t.querySelector("#al-filter-apply")?.addEventListener("click",()=>{s(Ee(t),1)}),t.querySelector("#al-filter-clear")?.addEventListener("click",()=>{["al-filter-user","al-filter-action","al-filter-target","al-filter-date-from","al-filter-date-to"].forEach(r=>{let l=t.querySelector(`#${r}`);l&&(l.value="")}),t.querySelector("#al-filter-auth").value="",s({username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},1)});let n=Math.max(1,Math.ceil(parseInt(t.querySelector("#al-record-count")?.textContent??"0")/st));i(t,e,a,n),t.querySelector("#audit-log-panel")?.addEventListener("click",r=>{let l=r.target.closest(".al-diff-btn");if(!l)return;r.preventDefault(),r.stopPropagation();let o=l.dataset.prior??"",u=l.dataset.new??"",d="";o&&u?d=`=== BEFORE ===
`+at(o)+`

=== AFTER ===
`+at(u):u?d=at(u):d=at(o),document.body.insertAdjacentHTML("beforeend",`
            <div id="al-diff-modal-inst" uk-modal>
                <div class="uk-modal-dialog kp-modal uk-modal-body uk-width-large">
                    <button class="uk-modal-close-default" type="button" uk-close></button>
                    <h3 class="kp-view-title uk-margin-bottom">Request Detail</h3>
                    <pre class="kp-cron-output">${T(d)}</pre>
                </div>
            </div>`);let v=document.getElementById("al-diff-modal-inst");UIkit.modal(v).show(),v.addEventListener("hidden",()=>v.remove(),{once:!0})})}function at(t){try{return JSON.stringify(JSON.parse(t),null,2)}catch{return t}}async function It(t){if(!B()){t.innerHTML=_("Access denied");return}let e={username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},a=await Pt(e,1);t.innerHTML=$e(a,e,1),Te(t,e,1)}function nt(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),i=document.getElementById("cs-node-version-wrap"),n=document.getElementById("cs-dotnet-version-wrap"),r=document.getElementById("cs-start-command-wrap"),l=document.getElementById("cs-wordpress-wrap");e.show();let o=document.getElementById("cs-domains-wrap"),u=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let d=parseInt(a.value);s.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),i.classList.toggle("uk-hidden",d!==4),n.classList.toggle("uk-hidden",d!==5),r.classList.toggle("uk-hidden",d!==4&&d!==5),l.classList.toggle("uk-hidden",d!==1||d===6),o.classList.toggle("uk-hidden",d===6),u.classList.toggle("uk-hidden",d!==6)}),document.getElementById("create-site-form").addEventListener("submit",async d=>{d.preventDefault();let v=d.target.querySelector('[type="submit"]'),k=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let m=new FormData(d.target),b=parseInt(m.get("site_type")),g=null;b===4&&(g=parseInt(m.get("node_version"))),b===5&&(g=parseInt(m.get("dotnet_version")));let f={name:m.get("name").trim(),php_version:parseInt(m.get("php_version"))||3,site_type:b,runtime_version:g,start_command:m.get("start_command")?.trim()||"",domains:m.get("domains").split(`
`).map(x=>x.trim()).filter(Boolean),install_wordpress:b===1?m.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let y=b===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",y);try{await p.post("/sites",f,6e5),w(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){w(),c.error(x.message),v.disabled=!1,v.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var N=null;function _e(t){return`${t.toFixed(1)}%`}var it=null;function bt(){return it||(it=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),it)}function vt(t,e){return`
        ...
        </table>`}function Ie(t,e,a,s){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let i=[...t].sort((d,v)=>{let k,m;switch(a){case"time":k=d.time,m=v.time;break;case"method":k=d.method,m=v.method;break;case"ip":k=d.client_ip,m=v.client_ip;break;default:k=d.status,m=v.status;break}return k<m?s?1:-1:k>m?s?-1:1:0}),n=50,r=Math.ceil(i.length/n),o=i.slice(e*n,(e+1)*n).map(d=>{let v=d.ua,k=d.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
                ...
                <td><span class="kp-badge ${k}">${d.status}</span>${d.reason?` <span class="kp-badge kp-badge-danger" style="font-size:0.65rem" uk-tooltip="Blocked by security rule">${d.reason}</span>`:""}</td>
                <td class="kp-stats-table-cell-mono">${d.client_ip}</td>
                <td class="kp-dd-ua-cell">${v}</td>
            </tr>`}).join(""),u=r>1?`
        ...
        ${u}`}async function Be(t,e,a){let s=document.getElementById("stats-drilldown-modal"),i=document.getElementById("stats-drilldown-title"),n=document.getElementById("stats-drilldown-body");if(!s||!n)return;i.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,n.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(s).show();let r=[],l=0,o="time",u=!0;function d(){n.innerHTML=Ie(r,l,o,u),n.querySelectorAll("th[data-dd-col]").forEach(v=>{v.addEventListener("click",()=>{let k=v.dataset.ddCol;o===k?u=!u:(o=k,u=!0),l=0,d()})}),n.querySelectorAll("[data-dd-page]").forEach(v=>{v.addEventListener("click",()=>{l=parseInt(v.dataset.ddPage,10),d()})})}try{r=await p.get(`/sites/${t}/stats/drilldown?hour=${encodeURIComponent(e)}&status=${a}`)}catch(v){n.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${S(v.message)}</p>`;return}d()}function gt(t,e,a){let s=a===6,i=null;function n(){if(s)return;let o=t.querySelector("#stats-pod-indicator"),u=t.querySelector("#stats-pod-table-wrap");if(!u)return;let d=location.protocol==="https:"?"wss":"ws";i=new WebSocket(`${d}://${location.host}/api/sites/${e}/stats/pod`),i.onopen=()=>{o&&(o.className="kp-status kp-status-running",o.textContent="Live")},i.onmessage=v=>{try{let k=JSON.parse(v.data);u.innerHTML=Pe(k.containers??[]),u.querySelectorAll(".kp-stats-pod-role-btn").forEach(m=>{m.addEventListener("click",async()=>{let b=m.style.color;m.style.color="var(--kp-warning)";let g=m.dataset.container.split("-").pop();try{await p.post(`/sites/${e}/containers/${g}/restart`),c.success(`${g} restarted`)}catch(f){m.style.color=b,c.error(f.message)}})})}catch{}},i.onerror=()=>{o&&(o.className="kp-status kp-status-error",o.textContent="Error")},i.onclose=()=>{o&&o.textContent==="Live"&&(o.className="kp-status kp-status-stopped",o.textContent="Disconnected")}}function r(){i&&i.readyState===WebSocket.OPEN&&i.close(),i=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{Bt(e)}),n();let l=new MutationObserver(()=>{document.getElementById("stats-panel")||(r(),l.disconnect())});l.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ht(t,e){let a=e===6;await Ce(t),a||await Bt(t)}async function qt(t){let e=await p.get("/sites")??[];t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${e.length===0?F("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
                <div class="uk-overflow-auto">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>nt()),Me()}function qe(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,i=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
        ...
        </tr>`}function Mt(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,i=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
        ...
        </div>`}function Me(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),i=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let n=null,r=!0,l=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],o=()=>{let b=l().length;e.textContent=`${b} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(y=>{let x=document.getElementById(y);x&&(x.disabled=b===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=b===0);let f=document.querySelectorAll(".kp-site-row-check");a.indeterminate=b>0&&b<f.length,a.checked=f.length>0&&b===f.length},u=()=>{let m=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(b=>{let g=b.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",f=b.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";b.style.display=!m||g.includes(m)||f.includes(m)?"":"none"})},d=m=>{n===m?r=!r:(n=m,r=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===m?r?" \u2191":" \u2193":" \u2195"});let b=[...i.querySelectorAll("tr")];b.sort((g,f)=>{let y="",x="";return m==="name"?(y=g.querySelector(".kp-site-row-name")?.textContent??"",x=f.querySelector(".kp-site-row-name")?.textContent??""):m==="status"?(y=g.dataset.status??"",x=f.dataset.status??""):m==="type"?(y=g.dataset.type??"",x=f.dataset.type??""):m==="domain"&&(y=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",x=f.querySelector("td:nth-child(6)")?.textContent.trim()??""),r?y.localeCompare(x):x.localeCompare(y)}),b.forEach(g=>i.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(m=>{m.checked=a.checked}),o()}),i?.addEventListener("change",m=>{m.target.classList.contains("kp-site-row-check")&&o()}),s?.addEventListener("input",u),document.querySelectorAll(".kp-sortable").forEach(m=>{m.addEventListener("click",()=>d(m.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(m=>{let b=m.replace("bulk-","");document.getElementById(m)?.addEventListener("click",()=>{let g=l().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:b,ids:g}}))})});let v=document.getElementById("kp-bulk-mobile-pill"),k=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",m=>{m.stopPropagation(),k.hidden=!k.hidden}),document.addEventListener("click",m=>{k&&!v?.contains(m.target)&&(k.hidden=!0)},{capture:!0}),["start","stop","restart","flush","recreate"].forEach(m=>{document.getElementById(`bulk-mobile-${m}`)?.addEventListener("click",b=>{b.preventDefault(),k.hidden=!0;let g=l().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:m,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(m=>{m.textContent=" \u2195"}),o()}var ot=null;async function At(t){let[e,a,s]=await Promise.all([p.get("/sites").catch(()=>[]),p.get("/stats/traffic").catch(()=>null),p.get("/stats/pod").catch(()=>null)]),i=e.filter(l=>l.SiteStatus===1).length,n=e.filter(l=>l.SiteStatus===2).length,r=e.filter(l=>l.SiteStatus===4).length;if(t.innerHTML=`
        <div class="kp-view-header">
        ...
        <div class="kp-site-grid">
            ${e.length===0?F("world","No sites yet"):e.slice(-3).reverse().map(l=>Mt(l,e)).join("")}
        </div>`,a?.hits_per_hour?.length){await bt();let l=document.getElementById("dash-traffic-chart");l&&window.Chart&&(ot&&(ot.destroy(),ot=null),ot=new window.Chart(l,{type:"bar",data:{labels:a.hits_per_hour.map(o=>new Date(o.hour).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),datasets:[{label:"2xx",data:a.hits_per_hour.map(o=>o["2xx"]),backgroundColor:"rgba(39,174,96,0.75)",borderColor:"rgba(39,174,96,1)",borderWidth:1,borderRadius:3},{label:"3xx",data:a.hits_per_hour.map(o=>o["3xx"]),backgroundColor:"rgba(43,142,255,0.75)",borderColor:"rgba(43,142,255,1)",borderWidth:1,borderRadius:3},{label:"4xx",data:a.hits_per_hour.map(o=>o["4xx"]),backgroundColor:"rgba(255,171,0,0.75)",borderColor:"rgba(255,171,0,1)",borderWidth:1,borderRadius:3},{label:"5xx",data:a.hits_per_hour.map(o=>o["5xx"]),backgroundColor:"rgba(235,59,90,0.75)",borderColor:"rgba(235,59,90,1)",borderWidth:1,borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!0,labels:{color:"#6b8cae",font:{size:11}},onHover:o=>{o.native.target.style.cursor="pointer"},onLeave:o=>{o.native.target.style.cursor="default"}},tooltip:{mode:"index",backgroundColor:"#0c1530",borderColor:"#1a2a4a",borderWidth:1,titleColor:"#dde8f5",bodyColor:"#6b8cae"}},scales:{x:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10},maxRotation:45},grid:{color:"rgba(26,42,74,0.6)"}},y:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10}},grid:{color:"rgba(26,42,74,0.6)"},beginAtZero:!0}}}}))}document.getElementById("dash-new-site")?.addEventListener("click",()=>nt())}function z(t=null){let e=t?`/sites/${t}/security/ip`:"/security/ip",a=t?`/sites/${t}/security/ua`:"/security/ua",s=t?`/sites/${t}/waf`:"/settings/waf",i=t?`/sites/${t}/security/country`:"/security/country",n=t?`/sites/${t}/security/asn`:"/security/asn";return`
        ...
        </div>`);let a=UIkit.modal("#kp-asn-lookup-modal");a.show();let s=async()=>{let i=document.getElementById("asn-lookup-q").value.trim();if(!i)return;let n=document.getElementById("asn-lookup-result");n.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{let r=await p.get(`/security/asn/lookup?q=${encodeURIComponent(i)}`);if(!r?.asn){n.innerHTML='<p class="kp-muted uk-text-small">No ASN found for <span class="kp-mono"></span>.</p>',n.querySelector(".kp-mono").textContent=r?.ip||i;return}n.innerHTML=`
...
${o}`:o),a.hide(),c.success(`${o} added to blacklist \u2014 save to apply`)})}catch(r){n.innerHTML="",c.error(r.message)}};document.getElementById("asn-lookup-go").addEventListener("click",s),document.getElementById("asn-lookup-q").addEventListener("keydown",i=>{i.key==="Enter"&&s()})}async function Dt(t){if(!B()){t.innerHTML=_("Access denied");return}t.innerHTML=`
        ...
        ${z(null)}`,lt(t),q(t)}function De(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Rt(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await p.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Ht(t){if(!B()){t.innerHTML=_("Access denied");return}let[e,a,s,i,n,r]=await Promise.all([p.get("/settings"),p.get("/settings/backup"),p.get("/settings/waf"),p.get("/settings/trusted-proxies"),p.get("/settings/notifications"),p.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Rt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let v={admin_domain:new FormData(l.target).get("admin_domain").trim()};try{await p.put("/settings",v),c.success("Settings saved"),Rt(v.admin_domain)}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("settings-import").addEventListener("change",async l=>{let o=l.target.files[0];if(!o)return;let u=new FormData;u.append("file",o);try{let d=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:u}),v=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(v?.error||`HTTP ${d.status}`);c.success("Settings imported")}catch(d){c.error(d.message)}finally{l.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={backup_schedule:d.get("backup_schedule").trim(),backup_retain_days:d.get("backup_retain_days").trim()};try{await p.put("/settings/backup",v),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("s3-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={s3_endpoint:d.get("s3_endpoint").trim(),s3_bucket:d.get("s3_bucket").trim(),s3_region:d.get("s3_region").trim(),s3_access_key:d.get("s3_access_key").trim()},k=d.get("s3_secret_key").trim();k&&(v.s3_secret_key=k);try{await p.put("/settings/backup",v),c.success("S3 settings saved")}catch(m){c.error(m.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("smtp-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={smtp_host:d.get("smtp_host").trim(),smtp_port:d.get("smtp_port").trim(),smtp_username:d.get("smtp_username").trim(),smtp_from:d.get("smtp_from").trim(),smtp_tls:d.get("smtp_tls")?"true":"false"},k=d.get("smtp_password").trim();k&&(v.smtp_password=k);try{await p.put("/settings/notifications",v),c.success("Email notification settings saved")}catch(m){c.error(m.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("sns-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={aws_access_key:d.get("aws_access_key").trim(),aws_region:d.get("aws_region").trim(),aws_sns_sender_id:d.get("aws_sns_sender_id").trim()},k=d.get("aws_secret_key").trim();k&&(v.aws_secret_key=k);try{await p.put("/settings/notifications",v),c.success("SMS notification settings saved")}catch(m){c.error(m.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("resource-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={resource_ram_reserve_gb:d.get("resource_ram_reserve_gb").trim(),resource_poll_interval:d.get("resource_poll_interval").trim(),resource_throttle_pct:d.get("resource_throttle_pct").trim(),resource_webhook_url:d.get("resource_webhook_url").trim()};try{await p.put("/settings/resources",v),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=u}})}function Ut(t){return`
        ...
        </div>`}function Ft(t,e){let a=Date.now()+18e5,s=setInterval(async()=>{try{let i=await p.get(`/sites/${e}/backups/restore-status`);(!i?.active||Date.now()>a)&&(clearInterval(s),w(),i?.active?c.error("Import timed out \u2014 check server logs"):c.success("Import complete"),await W(t,e))}catch{}},3e3)}async function W(t,e){try{let[a,s]=await Promise.all([p.get(`/sites/${e}/backup-repo`),p.get(`/sites/${e}/backups`)]),i=t.querySelector("#backup-local-enabled"),n=t.querySelector("#backup-s3-enabled");i&&(i.checked=!!a.LocalEnabled),n&&(n.checked=!!a.S3Enabled);let r=t.querySelector("#backup-error-banner");if(r)if(a.last_error){let o=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";r.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${o}:</strong> ${a.last_error}</p>
                    </div>`}else r.innerHTML="";let l=t.querySelector("#backup-list-wrap");l&&(l.innerHTML=Re(s))}catch(a){let s=t.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function Nt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await p.put(`/sites/${e}/backup-repo`,s),c.success("Backup destinations saved")}catch(i){c.error(i.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await p.get(`/sites/${e}/backups`))?.length??0}catch{}try{await p.post(`/sites/${e}/backups`,{label:"manual"})}catch(r){c.error(r.message);return}$("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let i=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await p.get(`/sites/${e}/backups`))?.length??0)>s||Date.now()>i)&&(clearInterval(n),w(),await W(t,e),Date.now()<=i?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let i=s.target.closest(".backup-restore-btn");if(i){let l=i.dataset.id;if(!await L("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await p.post(`/sites/${e}/backups/${l}/restore`)}catch(k){c.error(k.message);return}$("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let u=Date.now(),d=Date.now()+900*1e3,v=setInterval(async()=>{try{let k=await p.get(`/sites/${e}/backups/restore-status`);(!k?.active||Date.now()>d)&&(clearInterval(v),w(),k?.active?c.error("Restore timed out"):c.success("Restore complete"),await W(t,e))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let l=n.dataset.id;if(!await L("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;$("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await p.delete(`/sites/${e}/backups/${l}`),w(),c.success("Snapshot deleted"),await W(t,e)}catch(u){w(),c.error(u.message)}}let r=s.target.closest(".backup-download-btn");if(r){let l=r.dataset.id;$("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let o=document.createElement("a");o.href=`/api/sites/${e}/backups/${l}/download`,o.style.display="none",document.body.appendChild(o),o.click(),document.body.removeChild(o),setTimeout(()=>{w()},5e3)},300);return}});let a=t.querySelector("#import-backup-modal");a&&(UIkit.util.on(a,"beforeshow",async()=>{let s=a.querySelector("#import-target-site");try{let n=await p.get("/sites");s.innerHTML=n.map(r=>`<option value="${r.ID}"${r.ID===e?" selected":""}>${r.Name}</option>`).join("")}catch{s.innerHTML='<option value="">Failed to load sites</option>'}let i=a.querySelector("#import-sftp-list");try{let n=await p.get(`/sites/${e}/backups/import/files`);!n||n.length===0?i.innerHTML='<p class="kp-muted uk-text-small">No files found.</p>':i.innerHTML=n.map(r=>`
        ...
        </div>`}function Wt(t,e){let a=e?.enabled==="true",s=e?Object.entries(e).filter(([i])=>i!=="enabled"):[];return`
        <div>
    ...
    </div>`}function jt(t,e){t.addEventListener("click",a=>{if(a.target.closest(".cfg-add-row")){let s=a.target.closest(".cfg-add-row");t.querySelector(`.cfg-rows[data-type="${s.dataset.type}"]`).insertAdjacentHTML("beforeend",J())}}),t.addEventListener("click",a=>{a.target.closest(".cfg-del-row")&&a.target.closest(".kp-config-row").remove()}),t.addEventListener("click",async a=>{let s=a.target.closest(".cfg-save");if(!s)return;let{type:i,site:n}=s.dataset,r=t.querySelectorAll(`.cfg-rows[data-type="${i}"] .kp-config-row`),l={};if(r.forEach(o=>{let u=o.querySelector(".cfg-key").value.trim(),d=o.querySelector(".cfg-val").value.trim();u&&(l[u]=d)}),i==="5"){let o=t.querySelector(".varnish-enabled-toggle");l.enabled=o?.checked?"true":"false"}try{await p.put(`/sites/${n}/configs/${i}`,l),c.success(`${K[i]} config saved`)}catch(o){c.error(o.message)}}),t.addEventListener("click",async a=>{let s=a.target.closest(".cfg-reset");if(!s)return;let{type:i,site:n}=s.dataset;if(await L("Reset Config",`Reset ${K[i]} config to defaults?`))try{let l=await p.post(`/sites/${n}/configs/${i}/reset`),o=t.querySelector(`.cfg-rows[data-type="${i}"]`);o.innerHTML=Object.entries(l).map(([u,d])=>J(u,d)).join(""),c.success(`${K[i]} reset to defaults`)}catch(l){c.error(l.message)}}),t.addEventListener("change",async a=>{let s=a.target.closest(".cfg-import-input");if(!s)return;let{type:i,site:n}=s.dataset,r=s.files[0];if(!r)return;let l=new FormData;l.append("file",r);try{let o=await fetch(`/api/sites/${n}/configs/${i}/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:l}),u=o.status===204?null:await o.json().catch(()=>null);if(!o.ok)throw new Error(u?.error||`HTTP ${o.status}`);let d=t.querySelector(`.cfg-rows[data-type="${i}"]`);d.innerHTML=Object.entries(u).map(([v,k])=>J(v,k)).join(""),c.success(`${K[i]} config imported`)}catch(o){c.error(o.message)}finally{s.value=""}})}function zt(t){return`
        ...
        <tr>
            ...
            <td>
        ...
        </div>`}async function rt(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await p.get(`/sites/${e}/crons`);a.innerHTML=Vt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${S(s.message)}</p>`}}function Kt(t,e){let a=[],s=t.querySelector("#cron-modal"),i=t.querySelector("#cron-modal-title"),n=t.querySelector("#cron-modal-id"),r=t.querySelector("#cron-modal-label"),l=t.querySelector("#cron-modal-command"),o=t.querySelector("#cron-modal-schedule"),u=t.querySelector("#cron-schedule-preview"),d=t.querySelector("#cron-modal-enabled");o?.addEventListener("input",()=>{u.textContent=Ot(o.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{i.textContent="Add Cron Job",n.value="",r.value="",l.value="",o.value="",u.textContent="",d.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let v=l.value.trim(),k=o.value.trim();if(!v||!k){c.error("Command and schedule are required");return}let m={label:r.value.trim(),command:v,schedule:k,enabled:d.checked},b=n.value;try{b?(await p.put(`/sites/${e}/crons/${b}`,m),c.success("Cron job updated")):(await p.post(`/sites/${e}/crons`,m),c.success("Cron job created")),UIkit.modal(s).hide(),await rt(t,e),a=await p.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async v=>{let k=v.target.closest(".cron-detail-btn");if(k){let f=k.dataset.id,y=a.find(R=>String(R.ID)===f);if(!y)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let m=v.target.closest(".cron-edit-btn");if(m){let f=m.dataset.id,y=a.find(x=>String(x.ID)===f);if(!y)return;i.textContent="Edit Cron Job",n.value=y.ID,r.value=y.Label||"",l.value=y.Command,o.value=y.Schedule,u.textContent=Ot(y.Schedule),d.checked=y.Enabled,UIkit.modal(s).show();return}let b=v.target.closest(".cron-delete-btn");if(b){let f=b.dataset.id;if(!await L("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await p.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await rt(t,e),a=await p.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let g=v.target.closest(".cron-run-btn");if(g){let f=g.dataset.id;try{await p.post(`/sites/${e}/crons/${f}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let y=null;try{y=(await p.get(`/sites/${e}/crons`)).find(H=>String(H.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,R=setInterval(async()=>{try{let I=await p.get(`/sites/${e}/crons`),H=I.find(Y=>String(Y.ID)===f);if(!H||H.LastRun!==y||Date.now()>x){clearInterval(R),w(),a=I??[];let Y=t.querySelector("#cron-list-wrap");Y&&(Y.innerHTML=Vt(I)),H?.LastError?c.error(`Job failed: ${H.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async v=>{let k=v.target.closest(".cron-toggle");if(!k)return;let m=k.dataset.id;try{await p.patch(`/sites/${e}/crons/${m}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(b){c.error(b.message),k.checked=!k.checked}}),p.get(`/sites/${e}/crons`).then(v=>{a=v??[]}).catch(()=>{})}function Ot(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,i,n,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&i==="*"&&n==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&i==="*"&&n==="*"&&r!=="*"){let l=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(u=>l[parseInt(u)]??u).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function Yt(t){return`
        <div class="kp-card uk-padding uk-margin-top" id="fm-root" data-site="${t}">
            ...
            <div id="fm-list"></div>
        </div>`}function Fe(){let t=P?P.split("/"):[],e="",a=['<a href="#" data-path="">html</a>'];for(let s of t)e=e?e+"/"+s:s,a.push(`<span class="kp-fm-sep">/</span><a href="#" data-path="${S(e)}">${S(s)}</a>`);return a.join("")}function Ue(t){let e=new Date(t);return isNaN(e)?"":e.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Jt(t){return t==="d"?"folder":t==="l"?"link":"file-text"}function j(t,e){return t?t+"/"+e:e}var Xt=new Set(["php","js","jsx","ts","tsx","css","scss","sass","less","html","htm","xml","json","txt","md","markdown","yml","yaml","ini","conf","cnf","toml","env","sh","bash","sql","log","csv","tsv","svg","htaccess","gitignore","lock","map"]);function Ne(t,e){if(e)return!1;let a=t.lastIndexOf(".");return t.startsWith(".")&&a===0?Xt.has(t.slice(1).toLowerCase()):a>=0&&Xt.has(t.slice(a+1).toLowerCase())}function We(t){if(!t||!t.length)return F("folder","This folder is empty");let e=document.getElementById("fm-root").dataset.site;return`
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            <thead>
                <tr>
            ...
            <tbody>${t.map(s=>{let i=j(P,s.name),n=s.is_dir,r=Ne(s.name,n),l=n?`<a href="#" class="fm-nav" data-path="${S(i)}"><span uk-icon="icon: ${Jt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</a>`:`<span><span uk-icon="icon: ${Jt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</span>`;return`
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Z(),a&&(a.innerHTML=Fe());try{let s=await p.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=We(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Gt(t,e){P=e||"",C(t)}function G(t,e,a=""){return new Promise(s=>{let i="fm-prompt-modal";document.getElementById(i)?.remove();let n=document.createElement("div");n.id=i,n.setAttribute("uk-modal",""),n.innerHTML=`
            ...
            </div>`,document.body.appendChild(n),window.UIkit&&UIkit.icon(n);let r=UIkit.modal(n),l=n.querySelector("#fm-prompt-input"),o=!1,u=d=>{o||(o=!0,s(d),r.hide())};n.querySelector("#fm-prompt-ok").addEventListener("click",()=>u(l.value.trim()||null)),l.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),u(l.value.trim()||null))}),UIkit.util.on(n,"hidden",()=>{o||(o=!0,s(null)),n.remove()}),r.show(),setTimeout(()=>l.focus(),50)})}async function je(t,e){let a=j(P,e.name),s=await fetch(`/api/sites/${t}/files/upload?path=${encodeURIComponent(a)}`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:e}),i=s.status===204?null:await s.json().catch(()=>null);if(!s.ok)throw new Error(i?.error||`HTTP ${s.status}`)}function Zt(t,e){P="",t.addEventListener("click",a=>{let s=a.target.closest("#fm-breadcrumb a");if(s){a.preventDefault(),Gt(e,s.dataset.path);return}let i=a.target.closest(".fm-nav");if(i){a.preventDefault(),Gt(e,i.dataset.path);return}let n=a.target.closest(".fm-edit");if(n){a.preventDefault(),Ve(e,n.dataset.path,n.dataset.path.split("/").pop());return}}),t.querySelector("#fm-new-file")?.addEventListener("click",async()=>{let a=await G("New File","File name");if(a)try{await p.post(`/sites/${e}/files/file`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-new-dir")?.addEventListener("click",async()=>{let a=await G("New Folder","Folder name");if(a)try{await p.post(`/sites/${e}/files/dir`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-upload")?.addEventListener("change",async a=>{let s=[...a.target.files];if(s.length)try{for(let i of s)await je(e,i);c.success(s.length===1?"File uploaded":`${s.length} files uploaded`),C(e)}catch(i){c.error(i.message)}finally{a.target.value=""}}),t.querySelector("#fm-refresh")?.addEventListener("click",()=>C(e)),t.addEventListener("click",async a=>{let s=a.target.closest("tr[data-path]");if(!s)return;let i=s.dataset.path,n=s.dataset.name;if(a.target.closest(".fm-chmod")){let r=await G("Permissions",`Octal mode for "${n}"`,s.dataset.mode);if(!r)return;try{await p.patch(`/sites/${e}/files/chmod`,{path:i,mode:r}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-rename")){let r=await G("Rename / Move","New path (relative to current folder)",n);if(!r||r===n)return;try{await p.post(`/sites/${e}/files/move`,{src:i,dst:j(P,r)}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-copy")){let r=await G("Copy","Destination name",n+"-copy");if(!r)return;try{await p.post(`/sites/${e}/files/copy`,{src:i,dst:j(P,r)}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-delete")){if(!await L("Delete",`Delete "${n}"? This cannot be undone.`))return;try{await p.delete(`/sites/${e}/files?path=${encodeURIComponent(i)}`),C(e)}catch(l){c.error(l.message)}return}})}var ct=null;function Qt(t){if(document.querySelector(`link[href="${t}"]`))return;let e=document.createElement("link");e.rel="stylesheet",e.href=t,document.head.appendChild(e)}function wt(t){return new Promise((e,a)=>{let s=document.querySelector(`script[src="${t}"]`);if(s){if(s.dataset.loaded)return e();s.addEventListener("load",()=>e()),s.addEventListener("error",()=>a(new Error("failed to load "+t)));return}let i=document.createElement("script");i.src=t,i.addEventListener("load",()=>{i.dataset.loaded="1",e()}),i.addEventListener("error",()=>a(new Error("failed to load "+t))),document.head.appendChild(i)})}function Oe(){if(ct)return ct;let t="https://cdn.jsdelivr.net/npm/codemirror@5";return Qt(`${t}/lib/codemirror.css`),Qt(`${t}/theme/material-darker.css`),ct=wt(`${t}/lib/codemirror.js`).then(()=>Promise.all([wt(`${t}/mode/meta.js`),wt(`${t}/addon/mode/loadmode.js`)])).then(()=>{window.CodeMirror.modeURL=`${t}/mode/%N/%N.js`}),ct}function ze(t){let e=window.CodeMirror.findModeByFileName(t);return e?e.mode:null}async function Ve(t,e,a){let s;try{s=await p.get(`/sites/${t}/files/content?path=${encodeURIComponent(e)}`)}catch(k){let m=/too large/i.test(k.message)?"File is too large to edit \u2014 download it instead.":/binary/i.test(k.message)?"Binary file \u2014 download it instead of editing.":k.message;c.error(m);return}try{await Oe()}catch(k){c.error("Editor failed to load: "+k.message);return}let i="fm-editor-modal";document.getElementById(i)?.remove();let n=document.createElement("div");n.id=i,n.setAttribute("uk-modal",""),n.innerHTML=`
        ...
        </div>`,document.body.appendChild(n),window.UIkit&&UIkit.icon(n);let r=UIkit.modal(n,{bgClose:!1,escClose:!0}),l=n.querySelector("#fm-ed-dirty"),o=null,u=!0,d=k=>{u=!k,l.hidden=!k};UIkit.util.on(n,"shown",()=>{if(o)return;o=window.CodeMirror.fromTextArea(n.querySelector("#fm-ed-area"),{value:s.content,lineNumbers:!0,theme:"material-darker",indentUnit:4,lineWrapping:!1,extraKeys:{"Ctrl-S":v,"Cmd-S":v,"Ctrl-F":"findPersistent","Ctrl-/":"toggleComment"}}),o.setValue(s.content),o.on("change",()=>d(!0));let k=ze(a);k&&(o.setOption("mode",k),window.CodeMirror.autoLoadMode(o,k)),setTimeout(()=>o.refresh(),30)}),UIkit.util.on(n,"hidden",()=>n.remove());async function v(){if(!o)return;let k=n.querySelector("#fm-ed-save"),m=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div>';try{await p.put(`/sites/${t}/files/content`,{path:e,content:o.getValue()}),d(!1),c.success("Saved"),C(t)}catch(b){c.error(b.message)}finally{k.disabled=!1,k.innerHTML=m}}n.querySelector("#fm-ed-save").addEventListener("click",v),UIkit.util.on(n,"beforehide",k=>{!u&&!window.confirm("Discard unsaved changes?")&&k.preventDefault()}),r.show()}var Ke=2e3;function xt(t,e){return`
        <div>
            <div class="kp-log-controls">
        ...
        </div>`}function te(t,e){let a=null,s=!1,i=t.querySelector("#log-output"),n=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),l=t.querySelector("#log-clear"),o=t.querySelector("#log-autoscroll"),u=t.querySelector("#log-status");function d(m){for(m.split(`
`).forEach(b=>{if(!b)return;let g=document.createElement("div");g.className=b.match(/WAF BLOCK/i)?"kp-log-line-err":b.match(/WAF DETECT/i)?"kp-log-line-warn":b.match(/error|crit|emerg/i)?"kp-log-line-err":b.match(/warn/i)?"kp-log-line-warn":b.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=b,i.appendChild(g)});i.childElementCount>Ke;)i.removeChild(i.firstChild);o.checked&&(i.scrollTop=i.scrollHeight)}function v(){a&&(a.close(),a=null),s=!1,n.disabled=!1,r.disabled=!0,u&&(u.textContent="Disconnected")}n.addEventListener("click",()=>{v();let m=t.querySelector("#log-container").value,b=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",f=m==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${b}`:m==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${b}`:m==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${b}`:`${g}://${location.host}/api/sites/${e}/logs?container=${m}&tail=${b}`;a=new WebSocket(f),a.onopen=()=>{s=!0,n.disabled=!0,r.disabled=!1,u&&(u.textContent=`Connected \u2014 ${m}`)},a.onmessage=y=>d(y.data),a.onerror=()=>{},a.onclose=()=>{s=!1,n.disabled=!1,r.disabled=!0,u&&(u.textContent="Disconnected")}}),r.addEventListener("click",v),l.addEventListener("click",()=>{i.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(v(),n.click())});let k=h.go.bind(h);h.go=function(m,b={}){return a&&v(),k(m,b)}}function Je(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ee(t,e){try{let a=await p.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Je(a.status))}catch{}}function ae(t){t.forEach(e=>ee(e.Domain,e.ID))}function se(t,e,a,s=0,i=null){let n=t.SiteType!==3&&t.PMAPort>0;return`
        ...
        </div>`}async function ce(t){let e=document.getElementById("redirects-list");if(!e)return;e.innerHTML="";let a=await p.get(`/sites/${t}/redirects`);e.innerHTML=a.map(s=>re(s.Source,s.Target,s.Code)).join("")}function de(t,e){let a=new AbortController,s={signal:a.signal};t.addEventListener("click",i=>{i.target.closest("#redirect-add-btn")&&document.getElementById("redirects-list").insertAdjacentHTML("beforeend",re()),i.target.closest(".redirect-remove-btn")&&i.target.closest(".redirect-row").remove()},s),t.addEventListener("click",async i=>{if(!i.target.closest("#redirect-save-btn"))return;let n=[...document.querySelectorAll(".redirect-row")].map(r=>({Source:r.querySelector(".redirect-source").value.trim(),Target:r.querySelector(".redirect-target").value.trim(),Code:parseInt(r.querySelector(".redirect-code").value,10)}));try{await p.put(`/sites/${e}/redirects`,n),c.success("Redirects saved")}catch(r){c.error(r.message||"Failed to save redirects")}},s),t.__redirectsAbort?.abort(),t.__redirectsAbort=a}function Xe(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}function me(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),i=t.querySelector("#wpcli-run"),n=t.querySelector("#wpcli-clear"),r=t.querySelector("#wpcli-status"),l=[],o=-1;function u(k,m=""){k.split(`
`).forEach(b=>{if(!b)return;let g=document.createElement("div");m?g.className=m:g.className=b.match(/error|fatal|critical/i)?"kp-log-line-err":b.match(/warning|warn/i)?"kp-log-line-warn":b.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=b,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function d(k){if(k=k.trim(),!k)return;l.unshift(k),o=-1,u(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,i.disabled=!0,r&&(r.textContent="Running...");let m=location.protocol==="https:"?"wss":"ws",b=new WebSocket(`${m}://${location.host}/api/sites/${e}/wpcli`);b.onopen=()=>{b.send(JSON.stringify({command:k}))},b.onmessage=g=>{let f=g.data;if(f.trim()==="[done]"){b.close();return}if(f.startsWith("[info]")){u(f,"kp-muted");return}if(f.startsWith("[error]")){u(f,"kp-log-line-err");return}u(f)},b.onerror=()=>{u("[error] WebSocket connection failed","kp-log-line-err")},b.onclose=()=>{s.disabled=!1,i.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}i.addEventListener("click",()=>{d(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){d(s.value),s.value="",o=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),o<l.length-1&&(o++,s.value=l[o]);return}k.key==="ArrowDown"&&(k.preventDefault(),o>0?(o--,s.value=l[o]):(o=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let m=k.dataset.cmd;if(m.startsWith("search-replace")){s.value=m,s.focus();let b=m.indexOf("''")+1;s.setSelectionRange(b,b);return}d(m)})}),n.addEventListener("click",()=>{a.innerHTML=""});let v=h.go.bind(h);h.go=function(k,m={}){return v(k,m)},s.focus()}var Q=null,M=null;function ke(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),i=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(l,o=!1){UIkit.switcher(a).show(l),e.querySelectorAll(":scope > li[data-pill]").forEach(u=>u.classList.remove("kp-pill-active")),o?(s?.classList.add("kp-pill-active"),i?.querySelectorAll("a[data-switcher]").forEach(u=>{u.classList.toggle("kp-dd-active",parseInt(u.dataset.switcher,10)===l)})):(s?.classList.remove("kp-pill-active"),i?.querySelectorAll("a[data-switcher]").forEach(u=>u.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(l=>{l.addEventListener("click",o=>{o.preventDefault();let u=parseInt(l.closest("li").dataset.pill,10);n(u,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",l=>{l.stopPropagation(),i.hidden=!i.hidden,s.classList.toggle("kp-pill-active",!i.hidden)}),i?.querySelectorAll("a[data-switcher]").forEach(l=>{l.addEventListener("click",o=>{o.preventDefault(),i.hidden=!0,n(parseInt(l.dataset.switcher,10),!0)})}),document.addEventListener("click",l=>{i&&!s.contains(l.target)&&(i.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(0)}function Qe(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}async function Ye(t){let e=document.getElementById("rp-routes-list");if(e)try{let a=await p.get(`/sites/${t}/rp-routes`);e.innerHTML=a.length?a.map(s=>$t(s.Domain,s.Upstream,s.PassHost)).join(""):$t()}catch(a){c.error("Failed to load routes: "+a.message)}}function Ze(t,e){t.addEventListener("click",async a=>{if(a.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",$t());return}if(a.target.closest(".rp-remove-row")){a.target.closest(".rp-route-row").remove();return}if(!a.target.closest("#rp-save-btn"))return;let s=a.target.closest("#rp-save-btn"),i=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let n=[...document.querySelectorAll(".rp-route-row")].map(r=>({Domain:r.querySelector('[data-field="domain"]').value.trim(),Upstream:r.querySelector('[data-field="upstream"]').value.trim(),PassHost:r.querySelector('[data-field="pass_host"]').checked})).filter(r=>r.Domain&&r.Upstream);try{await p.put(`/sites/${e}/rp-routes`,n),c.success("Routes saved")}catch(r){c.error(r.message)}finally{s.disabled=!1,s.innerHTML=i}},{signal:Q.signal})}function ta(t){return t.endsWith("-nginx")?"world":t.endsWith("-php")?"code":t.endsWith("-db")?"database":t.endsWith("-redis")?"server":t.endsWith("-varnish")?"grid":t.endsWith("-pma")?"table":t.endsWith("-app")?"laptop":"bolt"}function be(t){let e=t.split("-").pop();return{nginx:"Nginx",php:"PHP-FPM",db:"MariaDB",redis:"Redis",varnish:"Varnish",pma:"phpMyAdmin",app:"App"}[e]??e}function Lt(t){switch(t){case"healthy":return"var(--kp-success)";case"unhealthy":return"var(--kp-danger)";case"starting":return"var(--kp-warning)";default:return"var(--kp-text-dim)"}}function ea(t){return!t||!t.length?"":t.filter(e=>!e.name.endsWith("-infra")).map(e=>`
            <span class="kp-health-badge"
                data-container="${e.name}"
                title="Restart the Container"
                style="cursor:pointer;color:${Lt(e.status)}">
                <span uk-icon="icon: ${ta(e.name)}; ratio: 1.1"></span>
                <span class="kp-health-badge-label">${be(e.name)}</span>
            </span>
        `).join("")}function aa(t,e){M&&(M.close(),M=null);let a=document.getElementById("sd-health-badges");if(!a)return;let s=location.protocol==="https:"?"wss":"ws";M=new WebSocket(`${s}://${location.host}/api/sites/${e}/health/stream`),M.onmessage=i=>{try{let n=JSON.parse(i.data);a.innerHTML=ea(n),a.querySelectorAll(".kp-health-badge").forEach(r=>{r.addEventListener("click",async()=>{r.style.color=Lt("starting");let l=r.dataset.container,o=l.split("-").pop();try{await p.post(`/sites/${e}/containers/${o}/restart`),c.success(`${be(l)} restarted`)}catch(u){r.style.color=Lt("none"),c.error(u.message)}})})}catch{}},M.onerror=()=>{},M.onclose=()=>{M=null}}async function ve(t,{id:e}){let[{site:a,domains:s,sftp:i},n,r]=await Promise.all([p.get(`/sites/${e}`),p.get("/sites"),p.get(`/sites/${e}/configs`)]),l=Array.isArray(n)?n:[],o=a.SiteType===1||a.SiteType===2,u=a.SiteType===6,d=[1,2,4,5].includes(a.SiteType);if(t.innerHTML=`
        <div class="kp-view-header">
            <div class="uk-flex uk-flex-middle" style="gap:12px">
                ...
                <div class="kp-site-nav-wrap">
                    <select id="sd-site-nav" class="uk-select kp-select">
                        ${l.map(m=>`<option value="${m.ID}" ${m.ID===a.ID?"selected":""}>${m.Name}</option>`).join("")}
                    </select>
        ...
        <ul class="kp-tab-pills" id="kp-site-pills">
            ...
            <li id="kp-manage-pill">
                ...
                <div class="kp-pill-dropdown" id="kp-manage-dropdown" hidden>
                    ...
                    ${d?`<a href="#" data-switcher="${a.SiteType===1?o?13:12:o?12:11}"><span uk-icon="icon: clock; ratio: 0.85"></span> Crons</a>`:""}
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${se(a,s??[],i,a.ParentID??0,l.find(m=>m.ID===a.ParentID)?.Name??null)}</li>
            <li>${vt(e,a.SiteType)}</li>
            <li>${X(e,1,r[1])}</li>
            ${o?`<li>${X(e,2,r[2])}</li>`:""}
            <li>${X(e,3,r[3])}</li>
            ...
            <li>${le()}</li>
            <li>${Yt(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>et(a)),document.getElementById("sd-site-nav")?.addEventListener("change",m=>{h.go("site-detail",{id:m.target.value})}),lt(t),q(t),te(t,e),Q&&Q.abort(),Q=new AbortController,ue(t,e,Q.signal),St(e),u){Ze(t,e),Ye(e),gt(t,e,a.SiteType),ht(e,a.SiteType),yt(t,e),V(e),ke(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{$("Recreating Pod","Recreating containers for this site...");try{await p.post(`/sites/${e}/recreate`),w(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(m){w(),c.error(m.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let m=await tt(a.Name);if(m){$("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await p.post(`/sites/${e}/clone`,{name:m},6e5),w(),c.success(`Site cloned as '${m}'`),h.go("sites")}catch(b){w(),c.error(b.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(m=>{m.addEventListener("click",async()=>{let b=m.dataset.action;if(b==="flush"){try{await p.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(f){c.error(f.message)}return}$(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await p.post(`/sites/${e}/${b}`),w(),c.success(`Site ${b} successful`),h.go("site-detail",{id:e})}catch(f){w(),c.error(f.message)}})}),jt(t,e),ie(t,e),a.SiteType===1&&me(t,e),oe(t,e,a),Nt(t,e),W(t,e),d&&(Kt(t,e),rt(t,e)),de(t,e),ce(e);let v=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');v&&k&&(k.dataset.switcher=String(v.children.length-1)),Zt(t,e),C(e),aa(t,e),gt(t,e,a.SiteType),ht(e,a.SiteType),ke(t),yt(t,e),V(e),ae(s??[])}async function dt(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(i=>i.remove()),typeof QRCode<"u")try{let i=await new Promise((n,r)=>{QRCode.toDataURL(t,{width:220,margin:2},(l,o)=>{l?r(l):n(o)})});e.src=i,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function ut(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),n=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(i).then(()=>{n.textContent="Copied!"});else{let r=document.createElement("textarea");r.value=i,r.style.cssText="position:fixed;opacity:0",document.body.appendChild(r),r.select();try{document.execCommand("copy"),n.textContent="Copied!"}catch{}r.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),h.go("users")})}function ge(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",i);let n=UIkit.modal("#kp-edit-user-modal");n.show(),document.getElementById("edit-user-form").addEventListener("submit",async u=>{u.preventDefault();let d=u.target.querySelector('[type="submit"]'),v=d.innerHTML;d.disabled=!0,d.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(u.target),m={fname:k.get("fname").trim(),lname:k.get("lname").trim(),email:k.get("email").trim(),phone:k.get("phone").trim(),notify_email:k.get("notify_email")==="on",notify_sms:k.get("notify_sms")==="on"};if(s){m.role=parseInt(k.get("role"));let g=k.get("uname");g&&(m.uname=g.trim())}let b=k.get("password");b&&(m.password=b);try{await p.put(`/users/${e}`,m),n.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("User updated"),h.go("users")}catch(g){c.error(g.message),d.disabled=!1,d.innerHTML=v}});let r=document.getElementById("totp-setup-btn");r&&r.addEventListener("click",async()=>{r.disabled=!0,r.textContent="Setting up\u2026";try{let u=await p.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=u.secret,document.getElementById("totp-setup-area").style.display="",await dt(u.uri)}catch(u){c.error(u.message),r.disabled=!1,r.textContent="Enable TOTP"}});let l=document.getElementById("totp-confirm-btn");l&&l.addEventListener("click",async()=>{let u=document.getElementById("totp-confirm-code").value.trim();if(u.length!==6){c.error("Enter a 6-digit code");return}l.disabled=!0;try{let d=await p.post(`/users/${e}/totp/confirm`,{code:u});n.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("TOTP enabled"),d.backup_codes?.length?ut(d.backup_codes):h.go("users")}catch(d){c.error(d.message),l.disabled=!1}});let o=document.getElementById("totp-disable-btn");o&&o.addEventListener("click",async()=>{o.disabled=!0;try{await p.delete(`/users/${e}/totp`),c.success("TOTP disabled"),n.hide(),document.getElementById("kp-edit-user-modal")?.remove(),h.go("users")}catch(u){c.error(u.message),o.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function fe(t){if(!B()){t.innerHTML=_("Access denied");return}let e=await p.get("/users");t.innerHTML=`
        <div class="kp-view-header">
