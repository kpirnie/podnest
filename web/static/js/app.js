"use strict";(()=>{var d={async _req(e,t,s,a=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),a),o={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};s!==void 0&&(o.body=JSON.stringify(s));try{let r=await fetch("/api"+t,o);clearTimeout(i);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(i),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e)};var Q=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',S=e=>`<div class="kp-empty">
        <div class="kp-empty-icon" uk-icon="icon: warning; ratio: 2.5"></div>
        <div class="kp-empty-text">${e}</div>
    </div>`,H=(e,t)=>`<div class="kp-empty">
        <div class="kp-empty-icon" uk-icon="icon: ${e}; ratio: 2.5"></div>
        <div class="kp-empty-text">${t}</div>
    </div>`,T=e=>{let t={1:["running","Running"],2:["stopped","Stopped"],3:["restarting","Restarting"],4:["error","Error"]},[s,a]=t[e]||["stopped","Unknown"];return`<span class="kp-status kp-status-${s}">${a}</span>`},he=e=>({3:"8.2",4:"8.3",5:"8.4",6:"8.5"})[e]||"?",R=e=>({1:"WordPress",2:"PHP",3:"Static",4:"Node.js",5:".NET",6:"Reverse Proxy"})[e]||"?",L=()=>window.KP.user.role===window.KP.roles.admin,I=e=>{switch(e.SiteType){case 1:case 2:return`PHP ${he(e.PHPVersion)}`;case 4:return`Node ${{2:"22",4:"24",5:"25",6:"26"}[e.RuntimeVersion]||"?"}`;case 5:return`.NET ${{1:"8.0",2:"9.0",3:"10.0"}[e.RuntimeVersion]||"?"}`;case 6:return"Reverse Proxy";default:return""}},P=e=>({id:e.id??e.ID,uname:e.uname??e.UName,uhash:e.uhash??e.UHash,fname:e.fname??e.FName,lname:e.lname??e.LName,email:e.email??e.Email,phone:e.phone??e.Phone,role:e.role??e.Role,totp_enabled:e.totp_enabled??!1,created:e.created??e.Created});function x(e,t){return new Promise(s=>{document.getElementById("kp-confirm-title").textContent=e,document.getElementById("kp-confirm-message").textContent=t;let a=UIkit.modal("#kp-confirm-modal");document.getElementById("kp-confirm-ok").addEventListener("click",()=>{a.hide(),s(!0)},{once:!0}),a.show(),document.getElementById("kp-confirm-modal").addEventListener("hidden",()=>s(!1),{once:!0})})}function y(e,t){let s=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let s=UIkit.modal("#kp-edit-site-modal"),a=document.getElementById("es-site-type"),n=document.getElementById("es-php-version-wrap"),i=document.getElementById("es-node-version-wrap"),o=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),c=document.getElementById("es-wordpress-wrap");s.show();let u=p=>{n.classList.toggle("uk-hidden",p!==1&&p!==2||p===6),i.classList.toggle("uk-hidden",p!==4),o.classList.toggle("uk-hidden",p!==5),r.classList.toggle("uk-hidden",p!==4&&p!==5),c.classList.toggle("uk-hidden",p!==1)};u(e.SiteType),a.addEventListener("change",()=>u(parseInt(a.value))),document.getElementById("edit-site-form").addEventListener("submit",async p=>{p.preventDefault();let v=p.target.querySelector('[type="submit"]'),b=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(p.target),m=parseInt(k.get("site_type")),f=null;m===4&&(f=parseInt(k.get("node_version"))),m===5&&(f=parseInt(k.get("dotnet_version")));let w={name:k.get("name").trim(),php_version:parseInt(k.get("php_version"))||3,site_type:m,runtime_version:f,start_command:k.get("start_command")?.trim()||""},B=m===1?k.get("install_wordpress")==="on":!1;try{if(await d.put(`/sites/${e.ID}`,w),s.hide(),document.getElementById("kp-edit-site-modal")?.remove(),m!==6){y("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:B}),h(),l.success("Site updated and pod recreated")}catch(E){h(),l.error("Site saved but pod recreate failed: "+E.message)}}else l.success("Site updated");g.go("site-detail",{id:String(e.ID)})}catch(E){l.error(E.message),v.disabled=!1,v.innerHTML=b}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function A(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let t=UIkit.modal("#kp-create-site-modal"),s=document.getElementById("cs-site-type"),a=document.getElementById("cs-php-version-wrap"),n=document.getElementById("cs-node-version-wrap"),i=document.getElementById("cs-dotnet-version-wrap"),o=document.getElementById("cs-start-command-wrap"),r=document.getElementById("cs-wordpress-wrap");t.show();let c=document.getElementById("cs-domains-wrap"),u=document.getElementById("cs-rp-note");s.addEventListener("change",()=>{let p=parseInt(s.value);a.classList.toggle("uk-hidden",p!==1&&p!==2||p===6),n.classList.toggle("uk-hidden",p!==4),i.classList.toggle("uk-hidden",p!==5),o.classList.toggle("uk-hidden",p!==4&&p!==5),r.classList.toggle("uk-hidden",p!==1||p===6),c.classList.toggle("uk-hidden",p===6),u.classList.toggle("uk-hidden",p!==6)}),document.getElementById("create-site-form").addEventListener("submit",async p=>{p.preventDefault();let v=p.target.querySelector('[type="submit"]'),b=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let k=new FormData(p.target),m=parseInt(k.get("site_type")),f=null;m===4&&(f=parseInt(k.get("node_version"))),m===5&&(f=parseInt(k.get("dotnet_version")));let w={name:k.get("name").trim(),php_version:parseInt(k.get("php_version"))||3,site_type:m,runtime_version:f,start_command:k.get("start_command")?.trim()||"",domains:k.get("domains").split(`
`).map(E=>E.trim()).filter(Boolean),install_wordpress:m===1?k.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove();let B=m===6?`Setting up '${w.name}' as a reverse proxy...`:`Setting up '${w.name}' \u2014 pulling images and provisioning containers...`;y("Creating Site",B);try{await d.post("/sites",w),h(),l.success(`Site '${w.name}' created`),g.go("sites")}catch(E){h(),l.error(E.message),v.disabled=!1,v.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function G(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        <div class="kp-site-grid">
            ${t.length===0?H("world","No sites yet \u2014 create one to get started"):t.map(V).join("")}
        </div>`,document.getElementById("sites-new-btn").addEventListener("click",()=>A())}function V(e){let t=e.Domains?.[0]??null,s=e.SiteType===6;return`
            ...
            <div class="kp-site-card-header">
                <div>
                    ...
                    <div class="kp-site-meta">
                        ...
                        <span class="kp-site-meta-item"><span uk-icon="icon: code; ratio: 0.75"></span> ${R(e.SiteType)}${I(e)?" / "+I(e):""}</span>
        ...
        </div>`}async function Y(e){let t=await d.get("/sites")??[],s=t.filter(i=>i.SiteStatus===1).length,a=t.filter(i=>i.SiteStatus===2).length,n=t.filter(i=>i.SiteStatus===4).length;e.innerHTML=`
        <div class="kp-view-header">
        ...
        <div class="kp-site-grid">
            ${t.length===0?H("world","No sites yet"):t.slice(0,6).map(V).join("")}
        </div>`,document.getElementById("dash-new-site")?.addEventListener("click",()=>A())}function N(e=null){let t=e?`/sites/${e}/security/ip`:"/security/ip",s=e?`/sites/${e}/security/ua`:"/security/ua",a=e?`/sites/${e}/waf`:"/settings/waf";return`
        ...
        ${N(null)}`,U(e),$(e)}function ye(e){switch(e){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function X(e){let t=document.getElementById("admin-domain-ssl");if(!(!t||!e))try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`);t.outerHTML=ye(s.status)}catch{}}async function Z(e){if(!L()){e.innerHTML=S("Access denied");return}let[t,s,a,n]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf"),d.get("/settings/trusted-proxies")]);e.innerHTML=`
    <div class="kp-view-header">
    ...
    `,t.admin_domain&&X(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),r=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u={admin_domain:new FormData(i.target).get("admin_domain").trim()};try{await d.put("/settings",u),l.success("Settings saved"),X(u.admin_domain)}catch(p){l.error(p.message)}finally{o.disabled=!1,o.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async i=>{let o=i.target.files[0];if(!o)return;let r=new FormData;r.append("file",o);try{let c=await fetch("/api/settings/import",{method:"POST",body:r}),u=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(u?.error||`HTTP ${c.status}`);l.success("Settings imported")}catch(c){l.error(c.message)}finally{i.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),r=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(i.target),u={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",u),l.success("Backup settings saved")}catch(p){l.error(p.message)}finally{o.disabled=!1,o.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),r=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(i.target),u={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},p=c.get("s3_secret_key").trim();p&&(u.s3_secret_key=p);try{await d.put("/settings/backup",u),l.success("S3 settings saved")}catch(v){l.error(v.message)}finally{o.disabled=!1,o.innerHTML=r}})}function ee(e){return`
        ...
        </div>`}function we(e){if(!e||e.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No snapshots yet.</p>';let t=n=>n===2?'<span class="kp-mono" style="color:var(--kp-cyan)">S3</span>':'<span class="kp-mono" style="color:var(--kp-blue)">Local</span>',s=n=>n<1024?`${n} B`:n<1048576?`${(n/1024).toFixed(1)} KB`:n<1073741824?`${(n/1048576).toFixed(1)} MB`:`${(n/1073741824).toFixed(2)} GB`;return`
        ...
        </table>`}async function C(e,t){try{let[s,a]=await Promise.all([d.get(`/sites/${t}/backup-repo`),d.get(`/sites/${t}/backups`)]),n=e.querySelector("#backup-local-enabled"),i=e.querySelector("#backup-s3-enabled");n&&(n.checked=!!s.LocalEnabled),i&&(i.checked=!!s.S3Enabled);let o=e.querySelector("#backup-error-banner");if(o)if(s.last_error){let c=s.last_error_at?` (${new Date(s.last_error_at).toLocaleString()})`:"";o.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${c}:</strong> ${s.last_error}</p>
                    </div>`}else o.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=we(a))}catch(s){let a=e.querySelector("#backup-list-wrap");a&&(a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${s.message}</p>`)}}function te(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,s),l.success("Backup destinations saved")}catch(a){l.error(a.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(i){l.error(i.message);return}y("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let a=Date.now()+600*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>s||Date.now()>a)&&(clearInterval(n),h(),await C(e,t),Date.now()<=a&&l.success("Backup complete"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let a=s.target.closest(".backup-restore-btn");if(a){let o=a.dataset.id;if(!await x("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${o}/restore`)}catch(v){l.error(v.message);return}y("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let c=Date.now(),u=Date.now()+900*1e3,p=setInterval(async()=>{try{let v=await d.get(`/sites/${t}/backups/restore-status`);(!v?.active||Date.now()>u)&&(clearInterval(p),h(),v?.active?l.error("Restore timed out"):l.success("Restore complete"),await C(e,t))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let o=n.dataset.id;if(!await x("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;y("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${o}`),h(),l.success("Snapshot deleted"),await C(e,t)}catch(c){h(),l.error(c.message)}}let i=s.target.closest(".backup-download-btn");if(i){let o=i.dataset.id;y("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${o}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{h()},5e3)},300);return}})}var F={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function q(e,t,s){let a=s?Object.entries(s):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
        ...
        </div>`}function se(e,t){let s=t?.enabled==="true",a=t?Object.entries(t).filter(([n])=>n!=="enabled"):[];return`
        <div>
        ...
        </div>`}function ie(e,t){let s=null,a=!1,n=e.querySelector("#log-output"),i=e.querySelector("#log-connect"),o=e.querySelector("#log-disconnect"),r=e.querySelector("#log-clear"),c=e.querySelector("#log-autoscroll"),u=e.querySelector("#log-status");function p(k){k.split(`
`).forEach(m=>{if(!m)return;let f=document.createElement("div");f.className=m.match(/WAF BLOCK/i)?"kp-log-line-err":m.match(/WAF DETECT/i)?"kp-log-line-warn":m.match(/error|crit|emerg/i)?"kp-log-line-err":m.match(/warn/i)?"kp-log-line-warn":m.match(/info|notice/i)?"kp-log-line-info":"",f.textContent=m,n.appendChild(f)}),c.checked&&(n.scrollTop=n.scrollHeight)}function v(){s&&(s.close(),s=null),a=!1,i.disabled=!1,o.disabled=!0,u&&(u.textContent="Disconnected")}i.addEventListener("click",()=>{v();let k=e.querySelector("#log-container").value,m=e.querySelector("#log-tail").value,f=location.protocol==="https:"?"wss":"ws",w=k==="waf"?`${f}://${location.host}/api/sites/${t}/logs/waf?tail=${m}`:`${f}://${location.host}/api/sites/${t}/logs?container=${k}&tail=${m}`;s=new WebSocket(w),s.onopen=()=>{a=!0,i.disabled=!0,o.disabled=!1,u&&(u.textContent=`Connected \u2014 ${k}`)},s.onmessage=B=>p(B.data),s.onerror=()=>{},s.onclose=()=>{a=!1,i.disabled=!1,o.disabled=!0,u&&(u.textContent="Disconnected")}}),o.addEventListener("click",v),r.addEventListener("click",()=>{n.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{s&&s.readyState===WebSocket.OPEN&&(v(),i.click())});let b=g.go.bind(g);g.go=function(k,m={}){return s&&v(),b(k,m)}}function xe(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function oe(e,t){try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),a=document.getElementById(`ssl-icon-${t}`);a&&(a.outerHTML=xe(s.status))}catch{}}function le(e){e.forEach(t=>oe(t.Domain,t.ID))}function re(e,t,s){let a=e.SiteType!==3&&e.PMAPort>0;return`
        <div class="uk-grid-medium" uk-grid>
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                        <tbody>
                            ...
                            <tr><td class="kp-muted">Type</td><td>${R(e.SiteType)}</td></tr>
                            <tr><td class="kp-muted">Version</td><td>${I(e)}</td></tr>
                            <tr><td class="kp-muted">Status</td><td>${T(e.SiteStatus)}</td></tr>
            ...
            <div class="uk-width-1-2@m">
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <div id="domain-list">
                        ${t.length?t.map(ce).join(""):'<p class="kp-muted uk-text-small">No domains configured</p>'}
                    </div>
        ...
        </div>`}function ce(e){return`<div class="uk-flex uk-flex-between uk-flex-middle kp-config-row" data-domain-id="${e.ID}">
    ...
    </div>`}function de(e,t){e.querySelector("#domain-add-btn")?.addEventListener("click",()=>{e.querySelector("#domain-add-form").classList.remove("uk-hidden")}),e.querySelector("#domain-cancel-btn")?.addEventListener("click",()=>{e.querySelector("#domain-add-form").classList.add("uk-hidden")}),e.querySelector("#domain-save-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#domain-add-input").value.trim();if(s)try{let a=await d.post(`/sites/${t}/domains`,{domain:s});e.querySelector("#domain-list").insertAdjacentHTML("beforeend",ce(a)),oe(a.Domain,a.ID),e.querySelector("#domain-add-form").classList.add("uk-hidden"),e.querySelector("#domain-add-input").value="",l.success("Domain added")}catch(a){l.error(a.message)}}),e.querySelector("#domain-list")?.addEventListener("click",async s=>{let a=s.target.closest('[data-action="delete-domain"]');if(!(!a||!await x("Remove Domain","Remove this domain from the site?")))try{await d.delete(`/sites/${t}/domains/${a.dataset.did}`),a.closest("[data-domain-id]").remove(),l.success("Domain removed")}catch(i){l.error(i.message)}})}function pe(e,t){e.querySelector("#sftp-regen-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#sftp-regen-btn"),a=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.post(`/sites/${t}/sftp-regen`),l.success("SFTP password regenerated"),g.go("site-detail",{id:String(t)})}catch(n){l.error(n.message),s.disabled=!1,s.innerHTML=a}}),e.querySelector("#sftp-copy-btn")?.addEventListener("click",()=>{let s=e.querySelector("#sftp-pass-display")?.textContent;if(s)if(navigator.clipboard)navigator.clipboard.writeText(s).then(()=>l.success("Password copied to clipboard")).catch(()=>l.error("Failed to copy password"));else{let a=document.createElement("textarea");a.value=s,a.style.cssText="position:fixed;opacity:0",document.body.appendChild(a),a.select(),document.execCommand("copy"),document.body.removeChild(a),l.success("Password copied to clipboard")}}),e.querySelector("#pma-open-btn")?.addEventListener("click",async()=>{let s=e.querySelector("#pma-open-btn"),a=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div> Opening...';try{let n=await d.post(`/sites/${t}/pma-token`);window.open(n.url,"_blank")}catch(n){l.error(n.message)}finally{s.disabled=!1,s.innerHTML=a}})}var Se=[{label:"Cache Flush",cmd:"cache flush"},{label:"Plugin List",cmd:"plugin list"},{label:"Theme List",cmd:"theme list"},{label:"User List",cmd:"user list"},{label:"Core Check",cmd:"core check-update"},{label:"Core Update",cmd:"core update"},{label:"Plugin Updates",cmd:"plugin update --all"},{label:"Theme Updates",cmd:"theme update --all"},{label:"Rewrite Flush",cmd:"rewrite flush"},{label:"Transient Delete",cmd:"transient delete --all"},{label:"Search Replace",cmd:"search-replace '' ''"}];function ue(e){return`
        <div>
        ...
        </div>`}function me(e,t){let s=e.querySelector("#wpcli-output"),a=e.querySelector("#wpcli-input"),n=e.querySelector("#wpcli-run"),i=e.querySelector("#wpcli-clear"),o=e.querySelector("#wpcli-status"),r=[],c=-1;function u(b,k=""){b.split(`
`).forEach(m=>{if(!m)return;let f=document.createElement("div");k?f.className=k:f.className=m.match(/error|fatal|critical/i)?"kp-log-line-err":m.match(/warning|warn/i)?"kp-log-line-warn":m.match(/success|done\]/i)?"kp-log-line-info":"",f.textContent=m,s.appendChild(f)}),s.scrollTop=s.scrollHeight}function p(b){if(b=b.trim(),!b)return;r.unshift(b),c=-1,u(`wp> ${b}`,"kp-log-line-info"),a.disabled=!0,n.disabled=!0,o&&(o.textContent="Running...");let k=location.protocol==="https:"?"wss":"ws",m=new WebSocket(`${k}://${location.host}/api/sites/${t}/wpcli`);m.onopen=()=>{m.send(JSON.stringify({command:b}))},m.onmessage=f=>{let w=f.data;if(w.trim()==="[done]"){m.close();return}if(w.startsWith("[info]")){u(w,"kp-muted");return}if(w.startsWith("[error]")){u(w,"kp-log-line-err");return}u(w)},m.onerror=()=>{u("[error] WebSocket connection failed","kp-log-line-err")},m.onclose=()=>{a.disabled=!1,n.disabled=!1,o&&(o.textContent="Ready"),a.focus()}}n.addEventListener("click",()=>{p(a.value),a.value=""}),a.addEventListener("keydown",b=>{if(b.key==="Enter"){p(a.value),a.value="",c=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),c<r.length-1&&(c++,a.value=r[c]);return}b.key==="ArrowDown"&&(b.preventDefault(),c>0?(c--,a.value=r[c]):(c=-1,a.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let k=b.dataset.cmd;if(k.startsWith("search-replace")){a.value=k,a.focus();let m=k.indexOf("''")+1;a.setSelectionRange(m,m);return}p(k)})}),i.addEventListener("click",()=>{s.innerHTML=""});let v=g.go.bind(g);g.go=function(b,k={}){return v(b,k)},a.focus()}function $e(){return`
        ...
        </div>`}async function Le(e){let t=document.getElementById("rp-routes-list");if(t)try{let s=await d.get(`/sites/${e}/rp-routes`);t.innerHTML=s.length?s.map(a=>z(a.Domain,a.Upstream)).join(""):z()}catch(s){l.error("Failed to load routes: "+s.message)}}function Pe(e,t){e.addEventListener("click",async s=>{if(s.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",z());return}if(s.target.closest(".rp-remove-row")){s.target.closest(".rp-route-row").remove();return}if(!s.target.closest("#rp-save-btn"))return;let a=s.target.closest("#rp-save-btn"),n=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let i=[...document.querySelectorAll(".rp-route-row")].map(o=>({Domain:o.querySelector('[data-field="domain"]').value.trim(),Upstream:o.querySelector('[data-field="upstream"]').value.trim()})).filter(o=>o.Domain&&o.Upstream);try{await d.put(`/sites/${t}/rp-routes`,i),l.success("Routes saved")}catch(o){l.error(o.message)}finally{a.disabled=!1,a.innerHTML=n}})}async function be(e,{id:t}){let[{site:s,domains:a,sftp:n},i,o]=await Promise.all([d.get(`/sites/${t}`),d.get("/sites"),d.get(`/sites/${t}/configs`)]),r=s.SiteType===1||s.SiteType===2,c=s.SiteType===6;if(e.innerHTML=`
        <div class="kp-view-header">
            <div class="uk-flex uk-flex-middle" style="gap:12px">
                ...
                <div class="kp-site-nav-wrap">
                    <select id="sd-site-nav" class="uk-select kp-select">
                        ${i.map(u=>`<option value="${u.ID}" ${u.ID===s.ID?"selected":""}>${u.Name}</option>`).join("")}
                    </select>
        ...
        <ul class="uk-switcher">
            ...
            <li>${q(t,4,o[4])}</li>
            <li>${se(t,o[5])}</li>
            <li>${ne(t,s.SiteType)}</li>
            <li>${N(t)}</li>
            <li id="waf-tab-panel"></li>
            ${s.SiteType===1?`<li>${ue(t)}</li>`:""}
            <li>${ee(t)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>g.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>_(s)),document.getElementById("sd-site-nav")?.addEventListener("change",u=>{g.go("site-detail",{id:u.target.value})}),c){Pe(e,t),Le(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{y("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),h(),l.success("Pod recreated"),g.go("site-detail",{id:t})}catch(u){h(),l.error(u.message)}}),e.querySelectorAll("[data-action]").forEach(u=>{u.addEventListener("click",async()=>{let p=u.dataset.action;if(p==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(b){l.error(b.message)}return}y(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[p]??p} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${p}`),h(),l.success(`Site ${p} successful`),g.go("site-detail",{id:t})}catch(b){h(),l.error(b.message)}})}),ae(e,t),de(e,t),ie(e,t),U(e),s.SiteType===1&&me(e,t),$(e),Ee(e,t),ke(t),pe(e,t),te(e,t),C(e,t),le(a??[])}async function W(e){let t=document.getElementById("totp-qr-img"),s=document.getElementById("totp-qr-wrap");if(!t||!s)return;if(s.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((i,o)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,c)=>{r?o(r):i(c)})});t.src=n,t.style.display="";return}catch{}let a=document.createElement("p");a.className="totp-uri-text kp-muted uk-text-small",a.style.wordBreak="break-all",a.textContent=e,s.appendChild(a)}function O(e){document.getElementById("kp-backup-codes-modal")?.remove();let s=`
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{i.textContent="Copied!"});else{let o=document.createElement("textarea");o.value=n,o.style.cssText="position:fixed;opacity:0",document.body.appendChild(o),o.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}o.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{a.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),g.go("users")})}function ge(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let s=UIkit.modal("#kp-create-user-modal");s.show(),document.getElementById("create-user-form").addEventListener("submit",async a=>{a.preventDefault();let n=a.target.querySelector('[type="submit"]'),i=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let o=new FormData(a.target),r={fname:o.get("fname").trim(),lname:o.get("lname").trim(),uname:o.get("uname").trim(),email:o.get("email").trim(),phone:o.get("phone").trim(),password:o.get("password"),role:parseInt(o.get("role"))};try{let c=P(await d.post("/users",r));document.getElementById("users-table-body").insertAdjacentHTML("beforeend",K(c)),l.success(`User '${c.uname}' created`),document.getElementById("create-user-form").style.display="none",document.getElementById("cu-totp-section").style.display="",Be(c.id,s)}catch(c){l.error(c.message),n.disabled=!1,n.innerHTML=i}}),document.getElementById("kp-create-user-modal").addEventListener("hidden",()=>document.getElementById("kp-create-user-modal")?.remove())}function Be(e,t){let s=()=>{t.hide(),document.getElementById("kp-create-user-modal")?.remove(),g.go("users")};document.getElementById("cu-totp-skip-btn").addEventListener("click",s),document.getElementById("cu-totp-setup-btn").addEventListener("click",async()=>{let a=document.getElementById("cu-totp-setup-btn");a.disabled=!0,a.textContent="Setting up\u2026";try{let n=await d.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=n.secret,document.getElementById("totp-setup-area").style.display="",document.getElementById("cu-totp-skip-btn").style.display="none",await W(n.uri)}catch(n){l.error(n.message),a.disabled=!1,a.textContent="Enable TOTP"}}),document.getElementById("cu-totp-confirm-btn").addEventListener("click",async()=>{let a=document.getElementById("totp-confirm-code").value.trim();if(a.length!==6){l.error("Enter a 6-digit code");return}let n=document.getElementById("cu-totp-confirm-btn");n.disabled=!0;try{let i=await d.post(`/users/${e}/totp/confirm`,{code:a});t.hide(),document.getElementById("kp-create-user-modal")?.remove(),l.success("TOTP enabled"),i.backup_codes?.length?O(i.backup_codes):g.go("users")}catch(i){l.error(i.message),n.disabled=!1}})}async function ve(e,t){document.getElementById("kp-edit-user-modal")?.remove();let s;try{s=P(await d.get(`/users/${t}`))}catch(u){l.error(u.message);return}let a=window.KP?.user?.role===99,n=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let i=UIkit.modal("#kp-edit-user-modal");i.show(),document.getElementById("edit-user-form").addEventListener("submit",async u=>{u.preventDefault();let p=u.target.querySelector('[type="submit"]'),v=p.innerHTML;p.disabled=!0,p.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b=new FormData(u.target),k={fname:b.get("fname").trim(),lname:b.get("lname").trim(),email:b.get("email").trim(),phone:b.get("phone").trim()};if(a){k.role=parseInt(b.get("role"));let f=b.get("uname");f&&(k.uname=f.trim())}let m=b.get("password");m&&(k.password=m);try{await d.put(`/users/${t}`,k),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("User updated"),g.go("users")}catch(f){l.error(f.message),p.disabled=!1,p.innerHTML=v}});let o=document.getElementById("totp-setup-btn");o&&o.addEventListener("click",async()=>{o.disabled=!0,o.textContent="Setting up\u2026";try{let u=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=u.secret,document.getElementById("totp-setup-area").style.display="",await W(u.uri)}catch(u){l.error(u.message),o.disabled=!1,o.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let u=document.getElementById("totp-confirm-code").value.trim();if(u.length!==6){l.error("Enter a 6-digit code");return}r.disabled=!0;try{let p=await d.post(`/users/${t}/totp/confirm`,{code:u});i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("TOTP enabled"),p.backup_codes?.length?O(p.backup_codes):g.go("users")}catch(p){l.error(p.message),r.disabled=!1}});let c=document.getElementById("totp-disable-btn");c&&c.addEventListener("click",async()=>{c.disabled=!0;try{await d.delete(`/users/${t}/totp`),l.success("TOTP disabled"),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),g.go("users")}catch(u){l.error(u.message),c.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function fe(e){if(!L()){e.innerHTML=S("Access denied");return}let t=await d.get("/users");e.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>ge(e)),Ie(e)}function K(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
    ...
    </tr>`}function Ie(e){e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="delete-user"]');if(!(!s||!await x("Delete User","Delete this user? This cannot be undone.")))try{await d.delete(`/users/${s.dataset.uid}`),s.closest("tr").remove(),l.success("User deleted")}catch(n){l.error(n.message)}}),e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="edit-user"]');s&&ve(e,s.dataset.uid)})}g.register("dashboard",e=>Y(e));g.register("sites",e=>G(e));g.register("site-detail",(e,t)=>be(e,t));g.register("users",e=>fe(e));g.register("settings",e=>Z(e));g.register("security",e=>J(e));document.addEventListener("click",e=>{let t=e.target.closest("[data-view]");if(!t)return;e.preventDefault(),g.go(t.dataset.view);let s=document.getElementById("kp-offcanvas");s&&UIkit.offcanvas(s).hide()});document.addEventListener("click",async e=>{let t=e.target.closest("[data-action]");if(!t)return;e.stopPropagation();let{action:s,id:a}=t.dataset;switch(s){case"manage":g.go("site-detail",{id:a});break;case"start":await D(a,"start","Starting Site","Starting all containers - please wait...");break;case"stop":await D(a,"stop","Stopping Site","Gracefully stopping all containers - please wait...");break;case"restart":await D(a,"restart","Restarting Site","Restarting all containers - please wait...");break;case"flush":await D(a,"flush","Flushing Caches","Clearing container caches - please wait...");break;case"edit":{let n=await d.get(`/sites/${a}`);_(n.site);break}case"delete":await Ce(a);break;case"update":await D(a,"update","Updating Images","Pulling latest container images - this may take a few minutes...");break;case"recreate":y("Recreating Pod","Recreating containers for this site - this may take a few minutes...");try{await d.post(`/sites/${a}/recreate`),h(),l.success("Pod recreated"),g.go("site-detail",{id:a})}catch(n){h(),l.error(n.message)}break}});async function D(e,t,s,a){y(s,a);try{await d.post(`/sites/${e}/${t}`),h(),l.success(s+" complete"),["start","stop","restart"].includes(t)&&g.go("sites")}catch(n){h(),l.error(n.message)}}async function Ce(e){if(!await x("Delete Site","This will stop and permanently remove the pod and all its data. Are you sure?"))return;y("Deleting Site","Stopping containers and removing the pod - please wait...");try{await d.delete(`/sites/${e}`)}catch{}let s=!1,a=0;for(;!s&&a<10;){try{await new Promise(i=>setTimeout(i,2e3)),s=!(await d.get("/sites")).find(i=>i.ID===parseInt(e))}catch{}a++}h(),s?(l.success("Site deleted"),g.go("sites")):l.error("Delete failed - site still exists after 20s")}window.addEventListener("hashchange",()=>{let{view:e,params:t}=j();g.go(e,t)});var{view:Me,params:qe}=j();g.go(Me,qe);})();
