...
`).map($=>$.trim()).filter(Boolean),install_wordpress:k===1?g.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove();let S=k===6?`Setting up '${y.name}' as a reverse proxy...`:`Setting up '${y.name}' \u2014 pulling images and provisioning containers...`;w("Creating Site",S);try{await d.post("/sites",y),h(),l.success(`Site '${y.name}' created`),v.go("sites")}catch($){h(),l.error($.message),m.disabled=!1,m.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function ee(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        </div>`}async function te(e){let t=await d.get("/sites")??[],s=t.filter(o=>o.SiteStatus===1).length,a=t.filter(o=>o.SiteStatus===2).length,n=t.filter(o=>o.SiteStatus===4).length;e.innerHTML=`
        <div class="kp-view-header">
        ...
        ${W(null)}`,O(e),T(e)}function Ie(e){switch(e){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ae(e){let t=document.getElementById("admin-domain-ssl");if(!(!t||!e))try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`);t.outerHTML=Ie(s.status)}catch{}}async function ne(e){if(!C()){e.innerHTML=L("Access denied");return}let[t,s,a,n]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf"),d.get("/settings/trusted-proxies")]);e.innerHTML=`
    <div class="kp-view-header">
    ...
    `,t.admin_domain&&ae(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await d.put("/settings",u),l.success("Settings saved"),ae(u.admin_domain)}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api/settings/import",{method:"POST",body:r}),u=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(u?.error||`HTTP ${c.status}`);l.success("Settings imported")}catch(c){l.error(c.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),u={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",u),l.success("Backup settings saved")}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),u={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},p=c.get("s3_secret_key").trim();p&&(u.s3_secret_key=p);try{await d.put("/settings/backup",u),l.success("S3 settings saved")}catch(m){l.error(m.message)}finally{i.disabled=!1,i.innerHTML=r}})}function oe(e){return`
        ...
        </div>`}function le(e,t){let s=t?.enabled==="true",a=t?Object.entries(t).filter(([n])=>n!=="enabled"):[];return`
        <div>
        ...
        </div>`}function pe(e){if(!e||e.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No cron jobs configured.</p>';let t=a=>a?new Date(a).toLocaleString():"\u2014";return`
        <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
            <thead>
                <tr>
                    <th>Label</th>
        ...
        </table>`}async function z(e,t){let s=e.querySelector("#cron-list-wrap");if(s)try{let a=await d.get(`/sites/${t}/crons`);s.innerHTML=pe(a)}catch(a){s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${a.message}</p>`}}function ue(e,t){let s=[],a=e.querySelector("#cron-modal"),n=e.querySelector("#cron-modal-title"),o=e.querySelector("#cron-modal-id"),i=e.querySelector("#cron-modal-label"),r=e.querySelector("#cron-modal-command"),c=e.querySelector("#cron-modal-schedule"),u=e.querySelector("#cron-schedule-preview"),p=e.querySelector("#cron-modal-enabled");c?.addEventListener("input",()=>{u.textContent=ce(c.value.trim())}),e.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",o.value="",i.value="",r.value="",c.value="",u.textContent="",p.checked=!0,UIkit.modal(a).show()}),e.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let m=r.value.trim(),b=c.value.trim();if(!m||!b){l.error("Command and schedule are required");return}let g={label:i.value.trim(),command:m,schedule:b,enabled:p.checked},k=o.value;try{k?(await d.put(`/sites/${t}/crons/${k}`,g),l.success("Cron job updated")):(await d.post(`/sites/${t}/crons`,g),l.success("Cron job created")),UIkit.modal(a).hide(),await z(e,t),s=await d.get(`/sites/${t}/crons`)}catch(f){l.error(f.message)}}),e.querySelector("#cron-list-wrap")?.addEventListener("click",async m=>{let b=m.target.closest(".cron-edit-btn");if(b){let f=b.dataset.id,y=s.find(S=>String(S.ID)===f);if(!y)return;n.textContent="Edit Cron Job",o.value=y.ID,i.value=y.Label||"",r.value=y.Command,c.value=y.Schedule,u.textContent=ce(y.Schedule),p.checked=y.Enabled,UIkit.modal(a).show();return}let g=m.target.closest(".cron-delete-btn");if(g){let f=g.dataset.id;if(!await x("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await d.delete(`/sites/${t}/crons/${f}`),l.success("Cron job deleted"),await z(e,t),s=await d.get(`/sites/${t}/crons`)}catch(S){l.error(S.message)}return}let k=m.target.closest(".cron-run-btn");if(k){let f=k.dataset.id;try{await d.post(`/sites/${t}/crons/${f}/run`)}catch(E){l.error(E.message);return}w("Running Cron Job","Executing the job inside the container \u2014 please wait.");let y=null;try{y=(await d.get(`/sites/${t}/crons`)).find(P=>String(P.ID)===f)?.LastRun??null}catch{}let S=Date.now()+300*1e3,$=setInterval(async()=>{try{let E=await d.get(`/sites/${t}/crons`),P=E.find(_=>String(_.ID)===f);if(!P||P.LastRun!==y||Date.now()>S){clearInterval($),h(),s=E??[];let _=e.querySelector("#cron-list-wrap");_&&(_.innerHTML=pe(E)),P?.LastError?l.error(`Job failed: ${P.LastError}`):l.success("Cron job complete")}}catch{}},2e3);return}}),e.querySelector("#cron-list-wrap")?.addEventListener("change",async m=>{let b=m.target.closest(".cron-toggle");if(!b)return;let g=b.dataset.id;try{await d.patch(`/sites/${t}/crons/${g}/toggle`,{enabled:b.checked}),l.success(b.checked?"Cron job enabled":"Cron job disabled")}catch(k){l.error(k.message),b.checked=!b.checked}}),d.get(`/sites/${t}/crons`).then(m=>{s=m??[]}).catch(()=>{})}function Be(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ce(e){if(!e)return"";let t=e.trim().split(/\s+/);if(t.length!==5)return"invalid expression";let[s,a,n,o,i]=t;if(e==="* * * * *")return"every minute";if(s!=="*"&&a!=="*"&&n==="*"&&o==="*"&&i==="*")return`daily at ${a.padStart(2,"0")}:${s.padStart(2,"0")}`;if(s!=="*"&&a!=="*"&&n==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(u=>r[parseInt(u)]??u).join(", ")} at ${a.padStart(2,"0")}:${s.padStart(2,"0")}`}return s.startsWith("*/")?`every ${s.slice(2)} minutes`:a.startsWith("*/")?`every ${a.slice(2)} hours`:e}function me(e,t){return`
        <div>
        ...
        </div>`}function ke(e,t){let s=null,a=!1,n=e.querySelector("#log-output"),o=e.querySelector("#log-connect"),i=e.querySelector("#log-disconnect"),r=e.querySelector("#log-clear"),c=e.querySelector("#log-autoscroll"),u=e.querySelector("#log-status");function p(g){g.split(`
`).forEach(k=>{if(!k)return;let f=document.createElement("div");f.className=k.match(/WAF BLOCK/i)?"kp-log-line-err":k.match(/WAF DETECT/i)?"kp-log-line-warn":k.match(/error|crit|emerg/i)?"kp-log-line-err":k.match(/warn/i)?"kp-log-line-warn":k.match(/info|notice/i)?"kp-log-line-info":"",f.textContent=k,n.appendChild(f)}),c.checked&&(n.scrollTop=n.scrollHeight)}function m(){s&&(s.close(),s=null),a=!1,o.disabled=!1,i.disabled=!0,u&&(u.textContent="Disconnected")}o.addEventListener("click",()=>{m();let g=e.querySelector("#log-container").value,k=e.querySelector("#log-tail").value,f=location.protocol==="https:"?"wss":"ws",y=g==="waf"?`${f}://${location.host}/api/sites/${t}/logs/waf?tail=${k}`:`${f}://${location.host}/api/sites/${t}/logs?container=${g}&tail=${k}`;s=new WebSocket(y),s.onopen=()=>{a=!0,o.disabled=!0,i.disabled=!1,u&&(u.textContent=`Connected \u2014 ${g}`)},s.onmessage=S=>p(S.data),s.onerror=()=>{},s.onclose=()=>{a=!1,o.disabled=!1,i.disabled=!0,u&&(u.textContent="Disconnected")}}),i.addEventListener("click",m),r.addEventListener("click",()=>{n.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{s&&s.readyState===WebSocket.OPEN&&(m(),o.click())});let b=v.go.bind(v);v.go=function(g,k={}){return s&&m(),b(g,k)}}function qe(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function be(e,t){try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),a=document.getElementById(`ssl-icon-${t}`);a&&(a.outerHTML=qe(s.status))}catch{}}function ge(e){e.forEach(t=>be(t.Domain,t.ID))}function ve(e,t,s,a=0,n=null){let o=e.SiteType!==3&&e.PMAPort>0;return`
        <div class="uk-grid-medium" uk-grid>
            ...
            <div class="uk-width-1-2@m">
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <div id="domain-list">
                        ${t.length?t.map(fe).join(""):'<p class="kp-muted uk-text-small">No domains configured</p>'}
                    </div>
        ...
        </div>`}function fe(e){return`<div class="uk-flex uk-flex-between uk-flex-middle kp-config-row" data-domain-id="${e.ID}">
...
`).forEach(k=>{if(!k)return;let f=document.createElement("div");g?f.className=g:f.className=k.match(/error|fatal|critical/i)?"kp-log-line-err":k.match(/warning|warn/i)?"kp-log-line-warn":k.match(/success|done\]/i)?"kp-log-line-info":"",f.textContent=k,s.appendChild(f)}),s.scrollTop=s.scrollHeight}function p(b){if(b=b.trim(),!b)return;r.unshift(b),c=-1,u(`wp> ${b}`,"kp-log-line-info"),a.disabled=!0,n.disabled=!0,i&&(i.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",k=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);k.onopen=()=>{k.send(JSON.stringify({command:b}))},k.onmessage=f=>{let y=f.data;if(y.trim()==="[done]"){k.close();return}if(y.startsWith("[info]")){u(y,"kp-muted");return}if(y.startsWith("[error]")){u(y,"kp-log-line-err");return}u(y)},k.onerror=()=>{u("[error] WebSocket connection failed","kp-log-line-err")},k.onclose=()=>{a.disabled=!1,n.disabled=!1,i&&(i.textContent="Ready"),a.focus()}}n.addEventListener("click",()=>{p(a.value),a.value=""}),a.addEventListener("keydown",b=>{if(b.key==="Enter"){p(a.value),a.value="",c=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),c<r.length-1&&(c++,a.value=r[c]);return}b.key==="ArrowDown"&&(b.preventDefault(),c>0?(c--,a.value=r[c]):(c=-1,a.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let g=b.dataset.cmd;if(g.startsWith("search-replace")){a.value=g,a.focus();let k=g.indexOf("''")+1;a.setSelectionRange(k,k);return}p(g)})}),o.addEventListener("click",()=>{s.innerHTML=""});let m=v.go.bind(v);v.go=function(b,g={}){return m(b,g)},a.focus()}function Me(){return`
        ...
        </div>`}function Ne(e,t){e.addEventListener("click",async s=>{if(s.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",Ae());return}if(s.target.closest(".rp-remove-row")){s.target.closest(".rp-route-row").remove();return}if(!s.target.closest("#rp-save-btn"))return;let a=s.target.closest("#rp-save-btn"),n=a.innerHTML;a.disabled=!0,a.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=[...document.querySelectorAll(".rp-route-row")].map(i=>({Domain:i.querySelector('[data-field="domain"]').value.trim(),Upstream:i.querySelector('[data-field="upstream"]').value.trim()})).filter(i=>i.Domain&&i.Upstream);try{await d.put(`/sites/${t}/rp-routes`,o),l.success("Routes saved")}catch(i){l.error(i.message)}finally{a.disabled=!1,a.innerHTML=n}})}async function $e(e,{id:t}){let[{site:s,domains:a,sftp:n},o,i]=await Promise.all([d.get(`/sites/${t}`),d.get("/sites"),d.get(`/sites/${t}/configs`)]),r=Array.isArray(o)?o:[],c=s.SiteType===1||s.SiteType===2,u=s.SiteType===6,p=[1,2,4,5].includes(s.SiteType);if(e.innerHTML=`
        <div class="kp-view-header">
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=n,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{a.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),v.go("users")})}function Ee(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>Ee(e)),Fe(e)}function X(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
