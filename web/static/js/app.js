...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:n.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let o=await fetch("/api"+e,r);clearTimeout(i);let l=o.status===204?null:await o.json().catch(()=>null);if(o.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!o.ok)throw new Error(l?.error||`HTTP ${o.status}`);return l}catch(o){throw clearTimeout(i),o}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var S=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function L(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Y=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        </div>`}function xe(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),n=t.querySelector("#admin-log-connect"),i=t.querySelector("#admin-log-disconnect"),r=t.querySelector("#admin-log-clear"),o=t.querySelector("#admin-log-autoscroll"),l=t.querySelector("#admin-log-status");function u(k){for(k.split(`
`).forEach(p=>{if(!p)return;let v=document.createElement("div");v.className=p.match(/WAF BLOCK/i)?"kp-log-line-err":p.match(/WAF DETECT/i)?"kp-log-line-warn":p.match(/error|crit|emerg/i)?"kp-log-line-err":p.match(/warn/i)?"kp-log-line-warn":p.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=p,s.appendChild(v)});s.childElementCount>ye;)s.removeChild(s.firstChild);o.checked&&(s.scrollTop=s.scrollHeight)}function d(){e&&(e.close(),e=null),a=!1,n.disabled=!1,i.disabled=!0,l&&(l.textContent="Disconnected")}n.addEventListener("click",()=>{d();let k=t.querySelector("#admin-log-source").value,p=t.querySelector("#admin-log-tail").value,v=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${v}://${location.host}/api/logs/waf?tail=${p}`:`${v}://${location.host}/api/logs/proxy?tail=${p}`;e=new WebSocket(g),e.onopen=()=>{a=!0,n.disabled=!0,i.disabled=!1,l&&(l.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=f=>u(f.data),e.onerror=()=>{},e.onclose=()=>{a=!1,n.disabled=!1,i.disabled=!0,l&&(l.textContent="Disconnected")}}),i.addEventListener("click",d),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(d(),n.click())});let b=h.go.bind(h);h.go=function(k,p={}){return e&&d(),b(k,p)}}function Lt(t){t.innerHTML=we(),xe(t)}var at=50;function Se(t,e,a){let s=Math.max(1,Math.ceil(t.total/at)),n=(t.entries??[]).map(Tt).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        <div id="audit-log-panel">
            ...
            ${s>1?`<div id="al-pager">${_t(a,s)}</div>`:'<div id="al-pager"></div>'}
        </div>`}function Tt(t){let e=new Date(t.ts).toLocaleString(),a=t.username?`<span style="font-family:monospace">${T(t.username)}</span>`:'<span style="color:var(--kp-text-dim)">\u2014</span>',s=$e(t.status),i=t.prior_state||t.new_state?`<button class="uk-button kp-btn-ghost kp-btn-sm al-diff-btn"
        ...
        <td>${r}</td>
        <td>${i}</td>
    </tr>`}function _t(t,e){let a=t>1?'<button class="uk-button kp-btn-ghost kp-btn-sm" id="al-prev">\u2039 Prev</button>':"",s=t<e?'<button class="uk-button kp-btn-ghost kp-btn-sm" id="al-next">Next \u203A</button>':"";return`<div class="uk-flex uk-flex-middle uk-flex-center uk-margin-small-top" style="gap:12px">
        ${a}
        <span style="font-size:0.85rem;color:var(--kp-text-dim)">Page ${t} of ${e}</span>
        ${s}
    </div>`}function $e(t){return`<span class="kp-badge ${t>=500?"kp-badge-error":t>=400?"kp-badge-warn":t>=300?"kp-badge-info":"kp-badge-ok"}">${t}</span>`}var T=t=>S(t??"");async function Ct(t,e){let a=new URLSearchParams({page:e,page_size:at});return t.username&&a.set("username",t.username),t.action&&a.set("action",t.action),t.target_type&&a.set("target_type",t.target_type),t.date_from&&a.set("date_from",t.date_from),t.date_to&&a.set("date_to",t.date_to),t.auth!==""&&a.set("auth",t.auth),m.get(`/audit?${a}`)}function Ee(t){return{username:t.querySelector("#al-filter-user").value.trim(),action:t.querySelector("#al-filter-action").value.trim(),target_type:t.querySelector("#al-filter-target").value.trim(),date_from:t.querySelector("#al-filter-date-from").value,date_to:t.querySelector("#al-filter-date-to").value,auth:t.querySelector("#al-filter-auth").value}}async function Le(t,e,a){async function s(r,o){let l=await Ct(r,o);t.querySelector("#al-table-body").innerHTML=(l.entries??[]).map(Tt).join("")||'<tr><td colspan="8" class="uk-text-center kp-text-dim">No records found</td></tr>';let u=Math.max(1,Math.ceil(l.total/at)),d=t.querySelector("#al-pager");d&&(d.innerHTML=u>1?_t(o,u):""),t.querySelector("#al-record-count").textContent=`${l.total} record${l.total!==1?"s":""}`,n(t,r,o,u),e=r,a=o}function n(r,o,l,u){r.querySelector("#al-prev")?.addEventListener("click",()=>s(o,l-1)),r.querySelector("#al-next")?.addEventListener("click",()=>s(o,l+1))}t.querySelector("#al-filter-apply")?.addEventListener("click",()=>{s(Ee(t),1)}),t.querySelector("#al-filter-clear")?.addEventListener("click",()=>{["al-filter-user","al-filter-action","al-filter-target","al-filter-date-from","al-filter-date-to"].forEach(r=>{let o=t.querySelector(`#${r}`);o&&(o.value="")}),t.querySelector("#al-filter-auth").value="",s({username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},1)});let i=Math.max(1,Math.ceil(parseInt(t.querySelector("#al-record-count")?.textContent??"0")/at));n(t,e,a,i),t.querySelector("#audit-log-panel")?.addEventListener("click",r=>{let o=r.target.closest(".al-diff-btn");if(!o)return;r.preventDefault(),r.stopPropagation();let l=o.dataset.prior??"",u=o.dataset.new??"",d="";l&&u?d=`=== BEFORE ===
`+et(l)+`
...
`+et(u):u?d=et(u):d=et(l),document.body.insertAdjacentHTML("beforeend",`
            ...
            </div>`);let b=document.getElementById("al-diff-modal-inst");UIkit.modal(b).show(),b.addEventListener("hidden",()=>b.remove(),{once:!0})})}function et(t){try{return JSON.stringify(JSON.parse(t),null,2)}catch{return t}}async function Pt(t){if(!B()){t.innerHTML=_("Access denied");return}let e={username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},a=await Ct(e,1);t.innerHTML=Se(a,e,1),Le(t,e,1)}function st(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),n=document.getElementById("cs-node-version-wrap"),i=document.getElementById("cs-dotnet-version-wrap"),r=document.getElementById("cs-start-command-wrap"),o=document.getElementById("cs-wordpress-wrap");e.show();let l=document.getElementById("cs-domains-wrap"),u=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let d=parseInt(a.value);s.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),n.classList.toggle("uk-hidden",d!==4),i.classList.toggle("uk-hidden",d!==5),r.classList.toggle("uk-hidden",d!==4&&d!==5),o.classList.toggle("uk-hidden",d!==1||d===6),l.classList.toggle("uk-hidden",d===6),u.classList.toggle("uk-hidden",d!==6)}),document.getElementById("create-site-form").addEventListener("submit",async d=>{d.preventDefault();let b=d.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(d.target),v=parseInt(p.get("site_type")),g=null;v===4&&(g=parseInt(p.get("node_version"))),v===5&&(g=parseInt(p.get("dotnet_version")));let f={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:v,runtime_version:g,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(x=>x.trim()).filter(Boolean),install_wordpress:v===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",w);try{await m.post("/sites",f,6e5),y(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){y(),c.error(x.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function Te(t){return`${t.toFixed(1)}%`}var nt=null;function bt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function vt(t,e){return`
        ...
        <div class="kp-card uk-padding-small uk-margin-bottom">
            ...
            <div id="stats-disk-wrap">
                ...
                <tr>
                    ...
                    <td class="kp-stats-table-cell-count">${i.count.toLocaleString()}</td>
                </tr>`).join(""))}async function It(t){let e=document.getElementById("stats-disk-wrap");if(e){e.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>';try{let a=await m.get(`/sites/${t}/stats/disk`);e.innerHTML=`
            ...
            </div>`}catch(a){e.innerHTML=`<p class="kp-muted uk-text-small">Failed to load disk usage: ${a.message}</p>`}}}function Ce(t){return!t||t.length===0?'<p class="kp-muted uk-text-small uk-margin-remove">No container data.</p>':`
        <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
            <thead><tr>
        ...
        ${u}`}async function Ie(t,e,a){let s=document.getElementById("stats-drilldown-modal"),n=document.getElementById("stats-drilldown-title"),i=document.getElementById("stats-drilldown-body");if(!s||!i)return;n.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,i.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(s).show();let r=[],o=0,l="time",u=!0;function d(){i.innerHTML=Pe(r,o,l,u),i.querySelectorAll("th[data-dd-col]").forEach(b=>{b.addEventListener("click",()=>{let k=b.dataset.ddCol;l===k?u=!u:(l=k,u=!0),o=0,d()})}),i.querySelectorAll("[data-dd-page]").forEach(b=>{b.addEventListener("click",()=>{o=parseInt(b.dataset.ddPage,10),d()})})}try{r=await m.get(`/sites/${t}/stats/drilldown?hour=${encodeURIComponent(e)}&status=${a}`)}catch(b){i.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${S(b.message)}</p>`;return}d()}function gt(t,e,a){let s=a===6,n=null;function i(){if(s)return;let l=t.querySelector("#stats-pod-indicator"),u=t.querySelector("#stats-pod-table-wrap");if(!u)return;let d=location.protocol==="https:"?"wss":"ws";n=new WebSocket(`${d}://${location.host}/api/sites/${e}/stats/pod`),n.onopen=()=>{l&&(l.className="kp-status kp-status-running",l.textContent="Live")},n.onmessage=b=>{try{let k=JSON.parse(b.data);u.innerHTML=Ce(k.containers??[]),u.querySelectorAll(".kp-stats-pod-role-btn").forEach(p=>{p.addEventListener("click",async()=>{let v=p.style.color;p.style.color="var(--kp-warning)";let g=p.dataset.container.split("-").pop();try{await m.post(`/sites/${e}/containers/${g}/restart`),c.success(`${g} restarted`)}catch(f){p.style.color=v,c.error(f.message)}})})}catch{}},n.onerror=()=>{l&&(l.className="kp-status kp-status-error",l.textContent="Error")},n.onclose=()=>{l&&l.textContent==="Live"&&(l.className="kp-status kp-status-stopped",l.textContent="Disconnected")}}function r(){n&&n.readyState===WebSocket.OPEN&&n.close(),n=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{It(e)}),i();let o=new MutationObserver(()=>{document.getElementById("stats-panel")||(r(),o.disconnect())});o.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ht(t,e){let a=e===6;await _e(t),a||await It(t)}async function Bt(t){let e=await m.get("/sites")??[];t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${e.length===0?H("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>st()),qe()}function Be(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,n=t.ParentID>0?e.find(i=>i.ID===t.ParentID)??null:null;return`
        ...
        </tr>`}function qt(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,n=t.ParentID>0?e.find(i=>i.ID===t.ParentID)??null:null;return`
        ...
        <div class="kp-site-grid">
            ${e.length===0?H("world","No sites yet"):e.slice(-3).reverse().map(o=>qt(o,e)).join("")}
        ...
        ${O(null)}`,ot(t),q(t)}function Me(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Rt(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=Me(a.status)}catch{}}async function At(t){if(!B()){t.innerHTML=_("Access denied");return}let[e,a,s,n,i,r]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Rt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),u=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await m.put("/settings",b),c.success("Settings saved"),Rt(b.admin_domain)}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=u}}),document.getElementById("settings-import").addEventListener("change",async o=>{let l=o.target.files[0];if(!l)return;let u=new FormData;u.append("file",l);try{let d=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:u}),b=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(b?.error||`HTTP ${d.status}`);c.success("Settings imported")}catch(d){c.error(d.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),u=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={backup_schedule:d.get("backup_schedule").trim(),backup_retain_days:d.get("backup_retain_days").trim()};try{await m.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=u}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),u=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={s3_endpoint:d.get("s3_endpoint").trim(),s3_bucket:d.get("s3_bucket").trim(),s3_region:d.get("s3_region").trim(),s3_access_key:d.get("s3_access_key").trim()},k=d.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await m.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=u}}),document.getElementById("smtp-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),u=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={smtp_host:d.get("smtp_host").trim(),smtp_port:d.get("smtp_port").trim(),smtp_username:d.get("smtp_username").trim(),smtp_from:d.get("smtp_from").trim(),smtp_tls:d.get("smtp_tls")?"true":"false"},k=d.get("smtp_password").trim();k&&(b.smtp_password=k);try{await m.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=u}}),document.getElementById("sns-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),u=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={aws_access_key:d.get("aws_access_key").trim(),aws_region:d.get("aws_region").trim(),aws_sns_sender_id:d.get("aws_sns_sender_id").trim()},k=d.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await m.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=u}}),document.getElementById("resource-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),u=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={resource_ram_reserve_gb:d.get("resource_ram_reserve_gb").trim(),resource_poll_interval:d.get("resource_poll_interval").trim(),resource_throttle_pct:d.get("resource_throttle_pct").trim(),resource_webhook_url:d.get("resource_webhook_url").trim()};try{await m.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=u}})}function Ft(t){return`
        ...
        </div>`}function Nt(t,e){let a=e?.enabled==="true",s=e?Object.entries(e).filter(([n])=>n!=="enabled"):[];return`
        <div>
        ...
        </div>`}function zt(t){if(!t||t.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No cron jobs configured.</p>';let e=s=>s?new Date(s).toLocaleString():"\u2014";return`
        <div class="uk-overflow-auto">
        ...
        <tr>
            ...
            <td>
        ...
        </div>`}async function lt(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await m.get(`/sites/${e}/crons`);a.innerHTML=zt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${S(s.message)}</p>`}}function Vt(t,e){let a=[],s=t.querySelector("#cron-modal"),n=t.querySelector("#cron-modal-title"),i=t.querySelector("#cron-modal-id"),r=t.querySelector("#cron-modal-label"),o=t.querySelector("#cron-modal-command"),l=t.querySelector("#cron-modal-schedule"),u=t.querySelector("#cron-schedule-preview"),d=t.querySelector("#cron-modal-enabled");l?.addEventListener("input",()=>{u.textContent=Wt(l.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",i.value="",r.value="",o.value="",l.value="",u.textContent="",d.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let b=o.value.trim(),k=l.value.trim();if(!b||!k){c.error("Command and schedule are required");return}let p={label:r.value.trim(),command:b,schedule:k,enabled:d.checked},v=i.value;try{v?(await m.put(`/sites/${e}/crons/${v}`,p),c.success("Cron job updated")):(await m.post(`/sites/${e}/crons`,p),c.success("Cron job created")),UIkit.modal(s).hide(),await lt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async b=>{let k=b.target.closest(".cron-detail-btn");if(k){let f=k.dataset.id,w=a.find(pt=>String(pt.ID)===f);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let p=b.target.closest(".cron-edit-btn");if(p){let f=p.dataset.id,w=a.find(x=>String(x.ID)===f);if(!w)return;n.textContent="Edit Cron Job",i.value=w.ID,r.value=w.Label||"",o.value=w.Command,l.value=w.Schedule,u.textContent=Wt(w.Schedule),d.checked=w.Enabled,UIkit.modal(s).show();return}let v=b.target.closest(".cron-delete-btn");if(v){let f=v.dataset.id;if(!await E("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await m.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await lt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let g=b.target.closest(".cron-run-btn");if(g){let f=g.dataset.id;try{await m.post(`/sites/${e}/crons/${f}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await m.get(`/sites/${e}/crons`)).find(A=>String(A.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,pt=setInterval(async()=>{try{let I=await m.get(`/sites/${e}/crons`),A=I.find(Q=>String(Q.ID)===f);if(!A||A.LastRun!==w||Date.now()>x){clearInterval(pt),y(),a=I??[];let Q=t.querySelector("#cron-list-wrap");Q&&(Q.innerHTML=zt(I)),A?.LastError?c.error(`Job failed: ${A.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let p=k.dataset.id;try{await m.patch(`/sites/${e}/crons/${p}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(v){c.error(v.message),k.checked=!k.checked}}),m.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function Wt(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,n,i,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&i==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&i==="*"&&r!=="*"){let o=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(u=>o[parseInt(u)]??u).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function Qt(t){return`
        <div class="kp-card uk-padding uk-margin-top" id="fm-root" data-site="${t}">
            ...
            <div id="fm-list"></div>
        </div>`}function Ae(){let t=P?P.split("/"):[],e="",a=['<a href="#" data-path="">html</a>'];for(let s of t)e=e?e+"/"+s:s,a.push(`<span class="kp-fm-sep">/</span><a href="#" data-path="${S(e)}">${S(s)}</a>`);return a.join("")}function He(t){let e=new Date(t);return isNaN(e)?"":e.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Kt(t){return t==="d"?"folder":t==="l"?"link":"file-text"}function j(t,e){return t?t+"/"+e:e}var Jt=new Set(["php","js","jsx","ts","tsx","css","scss","sass","less","html","htm","xml","json","txt","md","markdown","yml","yaml","ini","conf","cnf","toml","env","sh","bash","sql","log","csv","tsv","svg","htaccess","gitignore","lock","map"]);function Fe(t,e){if(e)return!1;let a=t.lastIndexOf(".");return t.startsWith(".")&&a===0?Jt.has(t.slice(1).toLowerCase()):a>=0&&Jt.has(t.slice(a+1).toLowerCase())}function Ue(t){if(!t||!t.length)return H("folder","This folder is empty");let e=document.getElementById("fm-root").dataset.site;return`
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            <thead>
                <tr>
            ...
            <tbody>${t.map(s=>{let n=j(P,s.name),i=s.is_dir,r=Fe(s.name,i),o=i?`<a href="#" class="fm-nav" data-path="${S(n)}"><span uk-icon="icon: ${Kt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</a>`:`<span><span uk-icon="icon: ${Kt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</span>`;return`
            <tr data-path="${S(n)}" data-name="${S(s.name)}" data-dir="${i?1:0}" data-mode="${S(s.mode)}">
                ...
                <td class="uk-text-nowrap uk-text-small kp-muted">${He(s.mod_time)}</td>
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Y(),a&&(a.innerHTML=Ae());try{let s=await m.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=Ue(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Xt(t,e){P=e||"",C(t)}function X(t,e,a=""){return new Promise(s=>{let n="fm-prompt-modal";document.getElementById(n)?.remove();let i=document.createElement("div");i.id=n,i.setAttribute("uk-modal",""),i.innerHTML=`
            ...
            </div>`,document.body.appendChild(i),window.UIkit&&UIkit.icon(i);let r=UIkit.modal(i),o=i.querySelector("#fm-prompt-input"),l=!1,u=d=>{l||(l=!0,s(d),r.hide())};i.querySelector("#fm-prompt-ok").addEventListener("click",()=>u(o.value.trim()||null)),o.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),u(o.value.trim()||null))}),UIkit.util.on(i,"hidden",()=>{l||(l=!0,s(null)),i.remove()}),r.show(),setTimeout(()=>o.focus(),50)})}async function Ne(t,e){let a=j(P,e.name),s=await fetch(`/api/sites/${t}/files/upload?path=${encodeURIComponent(a)}`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:e}),n=s.status===204?null:await s.json().catch(()=>null);if(!s.ok)throw new Error(n?.error||`HTTP ${s.status}`)}function Yt(t,e){P="",t.addEventListener("click",a=>{let s=a.target.closest("#fm-breadcrumb a");if(s){a.preventDefault(),Xt(e,s.dataset.path);return}let n=a.target.closest(".fm-nav");if(n){a.preventDefault(),Xt(e,n.dataset.path);return}let i=a.target.closest(".fm-edit");if(i){a.preventDefault(),Oe(e,i.dataset.path,i.dataset.path.split("/").pop());return}}),t.querySelector("#fm-new-file")?.addEventListener("click",async()=>{let a=await X("New File","File name");if(a)try{await m.post(`/sites/${e}/files/file`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-new-dir")?.addEventListener("click",async()=>{let a=await X("New Folder","Folder name");if(a)try{await m.post(`/sites/${e}/files/dir`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-upload")?.addEventListener("change",async a=>{let s=[...a.target.files];if(s.length)try{for(let n of s)await Ne(e,n);c.success(s.length===1?"File uploaded":`${s.length} files uploaded`),C(e)}catch(n){c.error(n.message)}finally{a.target.value=""}}),t.querySelector("#fm-refresh")?.addEventListener("click",()=>C(e)),t.addEventListener("click",async a=>{let s=a.target.closest("tr[data-path]");if(!s)return;let n=s.dataset.path,i=s.dataset.name;if(a.target.closest(".fm-chmod")){let r=await X("Permissions",`Octal mode for "${i}"`,s.dataset.mode);if(!r)return;try{await m.patch(`/sites/${e}/files/chmod`,{path:n,mode:r}),C(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-rename")){let r=await X("Rename / Move","New path (relative to current folder)",i);if(!r||r===i)return;try{await m.post(`/sites/${e}/files/move`,{src:n,dst:j(P,r)}),C(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-copy")){let r=await X("Copy","Destination name",i+"-copy");if(!r)return;try{await m.post(`/sites/${e}/files/copy`,{src:n,dst:j(P,r)}),C(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-delete")){if(!await E("Delete",`Delete "${i}"? This cannot be undone.`))return;try{await m.delete(`/sites/${e}/files?path=${encodeURIComponent(n)}`),C(e)}catch(o){c.error(o.message)}return}})}var rt=null;function Gt(t){if(document.querySelector(`link[href="${t}"]`))return;let e=document.createElement("link");e.rel="stylesheet",e.href=t,document.head.appendChild(e)}function wt(t){return new Promise((e,a)=>{let s=document.querySelector(`script[src="${t}"]`);if(s){if(s.dataset.loaded)return e();s.addEventListener("load",()=>e()),s.addEventListener("error",()=>a(new Error("failed to load "+t)));return}let n=document.createElement("script");n.src=t,n.addEventListener("load",()=>{n.dataset.loaded="1",e()}),n.addEventListener("error",()=>a(new Error("failed to load "+t))),document.head.appendChild(n)})}function je(){if(rt)return rt;let t="https://cdn.jsdelivr.net/npm/codemirror@5";return Gt(`${t}/lib/codemirror.css`),Gt(`${t}/theme/material-darker.css`),rt=wt(`${t}/lib/codemirror.js`).then(()=>Promise.all([wt(`${t}/mode/meta.js`),wt(`${t}/addon/mode/loadmode.js`)])).then(()=>{window.CodeMirror.modeURL=`${t}/mode/%N/%N.js`}),rt}function We(t){let e=window.CodeMirror.findModeByFileName(t);return e?e.mode:null}async function Oe(t,e,a){let s;try{s=await m.get(`/sites/${t}/files/content?path=${encodeURIComponent(e)}`)}catch(k){let p=/too large/i.test(k.message)?"File is too large to edit \u2014 download it instead.":/binary/i.test(k.message)?"Binary file \u2014 download it instead of editing.":k.message;c.error(p);return}try{await je()}catch(k){c.error("Editor failed to load: "+k.message);return}let n="fm-editor-modal";document.getElementById(n)?.remove();let i=document.createElement("div");i.id=n,i.setAttribute("uk-modal",""),i.innerHTML=`
        ...
        </div>`,document.body.appendChild(i),window.UIkit&&UIkit.icon(i);let r=UIkit.modal(i,{bgClose:!1,escClose:!0}),o=i.querySelector("#fm-ed-dirty"),l=null,u=!0,d=k=>{u=!k,o.hidden=!k};UIkit.util.on(i,"shown",()=>{if(l)return;l=window.CodeMirror.fromTextArea(i.querySelector("#fm-ed-area"),{value:s.content,lineNumbers:!0,theme:"material-darker",indentUnit:4,lineWrapping:!1,extraKeys:{"Ctrl-S":b,"Cmd-S":b,"Ctrl-F":"findPersistent","Ctrl-/":"toggleComment"}}),l.setValue(s.content),l.on("change",()=>d(!0));let k=We(a);k&&(l.setOption("mode",k),window.CodeMirror.autoLoadMode(l,k)),setTimeout(()=>l.refresh(),30)}),UIkit.util.on(i,"hidden",()=>i.remove());async function b(){if(!l)return;let k=i.querySelector("#fm-ed-save"),p=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div>';try{await m.put(`/sites/${t}/files/content`,{path:e,content:l.getValue()}),d(!1),c.success("Saved"),C(t)}catch(v){c.error(v.message)}finally{k.disabled=!1,k.innerHTML=p}}i.querySelector("#fm-ed-save").addEventListener("click",b),UIkit.util.on(i,"beforehide",k=>{!u&&!window.confirm("Discard unsaved changes?")&&k.preventDefault()}),r.show()}var ze=2e3;function xt(t,e){return`
        <div>
            <div class="kp-log-controls">
        ...
        </div>`}function Zt(t,e){let a=null,s=!1,n=t.querySelector("#log-output"),i=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),o=t.querySelector("#log-clear"),l=t.querySelector("#log-autoscroll"),u=t.querySelector("#log-status");function d(p){for(p.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");g.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=v,n.appendChild(g)});n.childElementCount>ze;)n.removeChild(n.firstChild);l.checked&&(n.scrollTop=n.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,i.disabled=!1,r.disabled=!0,u&&(u.textContent="Disconnected")}i.addEventListener("click",()=>{b();let p=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",f=p==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:p==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:p==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:`${g}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${v}`;a=new WebSocket(f),a.onopen=()=>{s=!0,i.disabled=!0,r.disabled=!1,u&&(u.textContent=`Connected \u2014 ${p}`)},a.onmessage=w=>d(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,i.disabled=!1,r.disabled=!0,u&&(u.textContent="Disconnected")}}),r.addEventListener("click",b),o.addEventListener("click",()=>{n.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),i.click())});let k=h.go.bind(h);h.go=function(p,v={}){return a&&b(),k(p,v)}}function Ve(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function te(t,e){try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Ve(a.status))}catch{}}function ee(t){t.forEach(e=>te(e.Domain,e.ID))}function ae(t,e,a,s=0,n=null){let i=t.SiteType!==3&&t.PMAPort>0;return`
        <div class="uk-grid-medium" uk-grid>
            ...
            <div class="uk-width-1-2@m">
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <div id="domain-list">
                        ${e.length?e.map(se).join(""):'<p class="kp-muted uk-text-small">No domains configured</p>'}
                    </div>
        ...
        </div>`}function se(t){return`<div class="uk-flex uk-flex-between uk-flex-middle kp-config-row" data-domain-id="${t.ID}">
        ...
        </div>`}function le(t="",e="",a=301){return`
...
`).forEach(v=>{if(!v)return;let g=document.createElement("div");p?g.className=p:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function d(k){if(k=k.trim(),!k)return;o.unshift(k),l=-1,u(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,r&&(r.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let f=g.data;if(f.trim()==="[done]"){v.close();return}if(f.startsWith("[info]")){u(f,"kp-muted");return}if(f.startsWith("[error]")){u(f,"kp-log-line-err");return}u(f)},v.onerror=()=>{u("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,n.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{d(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){d(s.value),s.value="",l=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),l<o.length-1&&(l++,s.value=o[l]);return}k.key==="ArrowDown"&&(k.preventDefault(),l>0?(l--,s.value=o[l]):(l=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let v=p.indexOf("''")+1;s.setSelectionRange(v,v);return}d(p)})}),i.addEventListener("click",()=>{a.innerHTML=""});let b=h.go.bind(h);h.go=function(k,p={}){return b(k,p)},s.focus()}var G=null,M=null;function Je(){return`
        <div class="kp-card uk-padding uk-margin-top">
            ...
            <form id="waf-override-form" class="uk-form-stacked">
                ...
                <div class="uk-flex uk-flex-right uk-margin-top" style="gap:8px">
                    ...
                    <div class="waf-plugin-pills">
                        ...
                        `).join("")}
                    </div>`,r.querySelectorAll(".waf-plugin-pill").forEach(d=>{d.addEventListener("click",()=>d.classList.toggle("active"))}))}}catch(s){c.error("Failed to load WAF settings: "+s.message)}}function Xe(t,e){G&&G.abort(),G=new AbortController,t.addEventListener("submit",async a=>{if(a.target.id!=="waf-override-form")return;a.preventDefault();let s=a.target.querySelector('[type="submit"]'),n=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let i=new FormData(a.target),r={override:parseInt(i.get("override"),10),exclusions:i.get("exclusions").trim()};try{await m.put(`/sites/${e}/waf`,r);let o=document.querySelector(".waf-plugin-select"),l=o?[...o.selectedOptions].map(u=>u.value):[...document.querySelectorAll(".waf-plugin-pill.active")].map(u=>u.dataset.plugin);await m.put(`/sites/${e}/waf/plugins`,l),c.success("WAF override saved \u2014 engine recompiling in background")}catch(o){c.error(o.message)}finally{s.disabled=!1,s.innerHTML=n}},{signal:G.signal}),t.querySelector("#waf-import")?.addEventListener("change",async a=>{let s=a.target.files[0];if(!s)return;let n=new FormData;n.append("file",s);try{let i=await fetch(`/api/sites/${e}/waf/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:n}),r=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(r?.error||`HTTP ${i.status}`);await me(e),c.success("WAF settings imported")}catch(i){c.error(i.message)}finally{a.target.value=""}})}function Ge(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}async function Qe(t){let e=document.getElementById("rp-routes-list");if(e)try{let a=await m.get(`/sites/${t}/rp-routes`);e.innerHTML=a.length?a.map(s=>St(s.Domain,s.Upstream,s.PassHost)).join(""):St()}catch(a){c.error("Failed to load routes: "+a.message)}}function Ye(t,e){t.addEventListener("click",async a=>{if(a.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",St());return}if(a.target.closest(".rp-remove-row")){a.target.closest(".rp-route-row").remove();return}if(!a.target.closest("#rp-save-btn"))return;let s=a.target.closest("#rp-save-btn"),n=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let i=[...document.querySelectorAll(".rp-route-row")].map(r=>({Domain:r.querySelector('[data-field="domain"]').value.trim(),Upstream:r.querySelector('[data-field="upstream"]').value.trim(),PassHost:r.querySelector('[data-field="pass_host"]').checked})).filter(r=>r.Domain&&r.Upstream);try{await m.put(`/sites/${e}/rp-routes`,i),c.success("Routes saved")}catch(r){c.error(r.message)}finally{s.disabled=!1,s.innerHTML=n}},{signal:G.signal})}function Ze(t){return t.endsWith("-nginx")?"world":t.endsWith("-php")?"code":t.endsWith("-db")?"database":t.endsWith("-redis")?"server":t.endsWith("-varnish")?"grid":t.endsWith("-pma")?"table":t.endsWith("-app")?"laptop":"bolt"}function ke(t){let e=t.split("-").pop();return{nginx:"Nginx",php:"PHP-FPM",db:"MariaDB",redis:"Redis",varnish:"Varnish",pma:"phpMyAdmin",app:"App"}[e]??e}function $t(t){switch(t){case"healthy":return"var(--kp-success)";case"unhealthy":return"var(--kp-danger)";case"starting":return"var(--kp-warning)";default:return"var(--kp-text-dim)"}}function ta(t){return!t||!t.length?"":t.filter(e=>!e.name.endsWith("-infra")).map(e=>`
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${ae(a,s??[],n,a.ParentID??0,o.find(p=>p.ID===a.ParentID)?.Name??null)}</li>
            <li>${vt(e,a.SiteType)}</li>
            <li>${J(e,1,r[1])}</li>
            ${l?`<li>${J(e,2,r[2])}</li>`:""}
            <li>${J(e,3,r[3])}</li>
            ...
            <li id="waf-tab-panel"></li>
            <li>${ft()}</li>
            ${a.SiteType===1?`<li>${de(e)}</li>`:""}
            <li>${Ft(e)}</li>
            ${d?`<li>${Ot(e)}</li>`:""}
            <li>${oe()}</li>
            <li>${Qt(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>tt(a)),document.getElementById("sd-site-nav")?.addEventListener("change",p=>{h.go("site-detail",{id:p.target.value})}),ot(t),q(t),Zt(t,e),Xe(t,e),me(e),u){Ye(t,e),Qe(e),gt(t,e,a.SiteType),ht(e,a.SiteType),yt(t,e),z(e),pe(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{$("Recreating Pod","Recreating containers for this site...");try{await m.post(`/sites/${e}/recreate`),y(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(p){y(),c.error(p.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let p=await Z(a.Name);if(p){$("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await m.post(`/sites/${e}/clone`,{name:p},6e5),y(),c.success(`Site cloned as '${p}'`),h.go("sites")}catch(v){y(),c.error(v.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(p=>{p.addEventListener("click",async()=>{let v=p.dataset.action;if(v==="flush"){try{await m.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(f){c.error(f.message)}return}$(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[v]??v} Pod`,"Please wait...");try{await m.post(`/sites/${e}/${v}`),y(),c.success(`Site ${v} successful`),h.go("site-detail",{id:e})}catch(f){y(),c.error(f.message)}})}),jt(t,e),ne(t,e),a.SiteType===1&&ue(t,e),ie(t,e,a),Ut(t,e),N(t,e),d&&(Vt(t,e),lt(t,e)),ce(t,e),re(e);let b=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');b&&k&&(k.dataset.switcher=String(b.children.length-1)),Yt(t,e),C(e),ea(t,e),gt(t,e,a.SiteType),ht(e,a.SiteType),pe(t),yt(t,e),z(e),ee(s??[])}async function ct(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((i,r)=>{QRCode.toDataURL(t,{width:220,margin:2},(o,l)=>{o?r(o):i(l)})});e.src=n,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function dt(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
