"use strict";(()=>{var d={async _req(e,t,a,s=6e4){let n=new AbortController,o=setTimeout(()=>n.abort(),s),i={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};a!==void 0&&(i.body=JSON.stringify(a));try{let r=await fetch("/api"+t,i);clearTimeout(o);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(o),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e),patch:(e,t)=>d._req("PATCH",e,t)};var se=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=e=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let a=UIkit.modal("#kp-edit-site-modal"),s=document.getElementById("es-site-type"),n=document.getElementById("es-php-version-wrap"),o=document.getElementById("es-node-version-wrap"),i=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),c=document.getElementById("es-wordpress-wrap");a.show();let p=u=>{n.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),o.classList.toggle("uk-hidden",u!==4),i.classList.toggle("uk-hidden",u!==5),r.classList.toggle("uk-hidden",u!==4&&u!==5),c.classList.toggle("uk-hidden",u!==1)};p(e.SiteType),s.addEventListener("change",()=>p(parseInt(s.value))),document.getElementById("edit-site-form").addEventListener("submit",async u=>{u.preventDefault();let k=u.target.querySelector('[type="submit"]'),b=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let g=new FormData(u.target),m=parseInt(g.get("site_type")),h=null;m===4&&(h=parseInt(g.get("node_version"))),m===5&&(h=parseInt(g.get("dotnet_version")));let y={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:m,runtime_version:h,start_command:g.get("start_command")?.trim()||""},w=m===1?g.get("install_wordpress")==="on":!1;try{if(await d.put(`/sites/${e.ID}`,y),a.hide(),document.getElementById("kp-edit-site-modal")?.remove(),m!==6){x("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:w}),f(),l.success("Site updated and pod recreated")}catch(S){f(),l.error("Site saved but pod recreate failed: "+S.message)}}else l.success("Site updated");v.go("site-detail",{id:String(e.ID)})}catch(S){l.error(S.message),k.disabled=!1,k.innerHTML=b}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function W(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let t=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),n=document.getElementById("cs-node-version-wrap"),o=document.getElementById("cs-dotnet-version-wrap"),i=document.getElementById("cs-start-command-wrap"),r=document.getElementById("cs-wordpress-wrap");t.show();let c=document.getElementById("cs-domains-wrap"),p=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let u=parseInt(a.value);s.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),n.classList.toggle("uk-hidden",u!==4),o.classList.toggle("uk-hidden",u!==5),i.classList.toggle("uk-hidden",u!==4&&u!==5),r.classList.toggle("uk-hidden",u!==1||u===6),c.classList.toggle("uk-hidden",u===6),p.classList.toggle("uk-hidden",u!==6)}),document.getElementById("create-site-form").addEventListener("submit",async u=>{u.preventDefault();let k=u.target.querySelector('[type="submit"]'),b=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let g=new FormData(u.target),m=parseInt(g.get("site_type")),h=null;m===4&&(h=parseInt(g.get("node_version"))),m===5&&(h=parseInt(g.get("dotnet_version")));let y={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:m,runtime_version:h,start_command:g.get("start_command")?.trim()||"",domains:g.get("domains").split(`
`).map(S=>S.trim()).filter(Boolean),install_wordpress:m===1?g.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=m===6?`Setting up '${y.name}' as a reverse proxy...`:`Setting up '${y.name}' \u2014 pulling images and provisioning containers...`;x("Creating Site",w);try{await d.post("/sites",y),f(),l.success(`Site '${y.name}' created`),v.go("sites")}catch(S){f(),l.error(S.message),k.disabled=!1,k.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function ne(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        </div>`}function Me(){let e=document.getElementById("sites-bulk-bar"),t=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all");if(!e||!a)return;let s=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],n=()=>{let i=s().length;e.classList.toggle("uk-hidden",i===0),t.textContent=`${i} site${i!==1?"s":""} selected`;let r=document.querySelectorAll(".kp-site-row-check");a.indeterminate=i>0&&i<r.length,a.checked=i>0&&i===r.length};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(o=>{o.checked=a.checked}),n()}),document.querySelector(".kp-table-wrap tbody")?.addEventListener("change",o=>{o.target.classList.contains("kp-site-row-check")&&n()}),["start","stop","restart","flush"].forEach(o=>{document.getElementById(`bulk-${o}`)?.addEventListener("click",()=>{let i=s().map(r=>r.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:o,ids:i}}))})})}async function ie(e){let t=await d.get("/sites")??[],a=t.filter(o=>o.SiteStatus===1).length,s=t.filter(o=>o.SiteStatus===2).length,n=t.filter(o=>o.SiteStatus===4).length;e.innerHTML=`
        <div class="kp-view-header">
    ...
    `,t.admin_domain&&re(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await d.put("/settings",p),l.success("Settings saved"),re(p.admin_domain)}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api/settings/import",{method:"POST",body:r}),p=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(p?.error||`HTTP ${c.status}`);l.success("Settings imported")}catch(c){l.error(c.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),p={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",p),l.success("Backup settings saved")}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),p={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},u=c.get("s3_secret_key").trim();u&&(p.s3_secret_key=u);try{await d.put("/settings/backup",p),l.success("S3 settings saved")}catch(k){l.error(k.message)}finally{i.disabled=!1,i.innerHTML=r}})}function de(e){return`
        ...
        <tr>
            ...
            <td>
                <div class="uk-flex" style="gap:6px">
                    ...
                    </div>`}else i.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=He(s))}catch(a){let s=e.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function ue(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let a={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,a),l.success("Backup destinations saved")}catch(s){l.error(s.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let a=0;try{a=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(o){l.error(o.message);return}x("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let s=Date.now()+1800*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>a||Date.now()>s)&&(clearInterval(n),f(),await R(e,t),Date.now()<=s?l.success("Backup complete"):l.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async a=>{let s=a.target.closest(".backup-restore-btn");if(s){let i=s.dataset.id;if(!await $("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${i}/restore`)}catch(k){l.error(k.message);return}x("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let c=Date.now(),p=Date.now()+900*1e3,u=setInterval(async()=>{try{let k=await d.get(`/sites/${t}/backups/restore-status`);(!k?.active||Date.now()>p)&&(clearInterval(u),f(),k?.active?l.error("Restore timed out"):l.success("Restore complete"),await R(e,t))}catch{}},3e3);return}let n=a.target.closest(".backup-delete-btn");if(n){let i=n.dataset.id;if(!await $("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;x("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${i}`),f(),l.success("Snapshot deleted"),await R(e,t)}catch(c){f(),l.error(c.message)}}let o=a.target.closest(".backup-download-btn");if(o){let i=o.dataset.id;x("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${i}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{f()},5e3)},300);return}})}var V={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function A(e,t,a){let s=a?Object.entries(a):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
        ...
        <tr>
            ...
            <td>
                ...
                </div>`);let S=document.getElementById("cron-detail-modal");UIkit.modal(S).show(),S.addEventListener("hidden",()=>S.remove(),{once:!0});return}let g=k.target.closest(".cron-edit-btn");if(g){let y=g.dataset.id,w=a.find(S=>String(S.ID)===y);if(!w)return;n.textContent="Edit Cron Job",o.value=w.ID,i.value=w.Label||"",r.value=w.Command,c.value=w.Schedule,p.textContent=me(w.Schedule),u.checked=w.Enabled,UIkit.modal(s).show();return}let m=k.target.closest(".cron-delete-btn");if(m){let y=m.dataset.id;if(!await $("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await d.delete(`/sites/${t}/crons/${y}`),l.success("Cron job deleted"),await z(e,t),a=await d.get(`/sites/${t}/crons`)}catch(S){l.error(S.message)}return}let h=k.target.closest(".cron-run-btn");if(h){let y=h.dataset.id;try{await d.post(`/sites/${t}/crons/${y}/run`)}catch(E){l.error(E.message);return}x("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await d.get(`/sites/${t}/crons`)).find(C=>String(C.ID)===y)?.LastRun??null}catch{}let S=Date.now()+300*1e3,G=setInterval(async()=>{try{let E=await d.get(`/sites/${t}/crons`),C=E.find(N=>String(N.ID)===y);if(!C||C.LastRun!==w||Date.now()>S){clearInterval(G),f(),a=E??[];let N=e.querySelector("#cron-list-wrap");N&&(N.innerHTML=ge(E)),C?.LastError?l.error(`Job failed: ${C.LastError}`):l.success("Cron job complete")}}catch{}},2e3);return}}),e.querySelector("#cron-list-wrap")?.addEventListener("change",async k=>{let b=k.target.closest(".cron-toggle");if(!b)return;let g=b.dataset.id;try{await d.patch(`/sites/${t}/crons/${g}/toggle`,{enabled:b.checked}),l.success(b.checked?"Cron job enabled":"Cron job disabled")}catch(m){l.error(m.message),b.checked=!b.checked}}),d.get(`/sites/${t}/crons`).then(k=>{a=k??[]}).catch(()=>{})}function Z(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function me(e){if(!e)return"";let t=e.trim().split(/\s+/);if(t.length!==5)return"invalid expression";let[a,s,n,o,i]=t;if(e==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(p=>r[parseInt(p)]??p).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:e}function ee(e,t){return`
        <div>
...
`).forEach(m=>{if(!m)return;let h=document.createElement("div");g?h.className=g:h.className=m.match(/error|fatal|critical/i)?"kp-log-line-err":m.match(/warning|warn/i)?"kp-log-line-warn":m.match(/success|done\]/i)?"kp-log-line-info":"",h.textContent=m,a.appendChild(h)}),a.scrollTop=a.scrollHeight}function u(b){if(b=b.trim(),!b)return;r.unshift(b),c=-1,p(`wp> ${b}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,i&&(i.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",m=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);m.onopen=()=>{m.send(JSON.stringify({command:b}))},m.onmessage=h=>{let y=h.data;if(y.trim()==="[done]"){m.close();return}if(y.startsWith("[info]")){p(y,"kp-muted");return}if(y.startsWith("[error]")){p(y,"kp-log-line-err");return}p(y)},m.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},m.onclose=()=>{s.disabled=!1,n.disabled=!1,i&&(i.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",b=>{if(b.key==="Enter"){u(s.value),s.value="",c=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),c<r.length-1&&(c++,s.value=r[c]);return}b.key==="ArrowDown"&&(b.preventDefault(),c>0?(c--,s.value=r[c]):(c=-1,s.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let g=b.dataset.cmd;if(g.startsWith("search-replace")){s.value=g,s.focus();let m=g.indexOf("''")+1;s.setSelectionRange(m,m);return}u(g)})}),o.addEventListener("click",()=>{a.innerHTML=""});let k=v.go.bind(v);v.go=function(b,g={}){return k(b,g)},s.focus()}var _=null;function Ne(){return`
            ...
            <div class="uk-flex uk-flex-middle" style="gap:12px">
                ...
                <div class="kp-site-nav-wrap">
                    <select id="sd-site-nav" class="uk-select kp-select">
                        ${r.map(k=>`<option value="${k.ID}" ${k.ID===a.ID?"selected":""}>${k.Name}</option>`).join("")}
                    </select>
        ...
        <ul class="uk-switcher">
            <li>${we(a,s??[],n,a.ParentID??0,r.find(k=>k.ID===a.ParentID)?.Name??null)}</li>
            <li>${A(t,1,i[1])}</li>
            ${c?`<li>${A(t,2,i[2])}</li>`:""}
            <li>${A(t,3,i[3])}</li>
            <li>${A(t,4,i[4])}</li>
            ...
            <li>${de(t)}</li>
            ${u?`<li>${be(t)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>v.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>j(a)),document.getElementById("sd-site-nav")?.addEventListener("change",k=>{v.go("site-detail",{id:k.target.value})}),O(e),T(e),he(e,t),Ue(e,t),Te(t),p){Oe(e,t),We(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{x("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),f(),l.success("Pod recreated"),v.go("site-detail",{id:t})}catch(k){f(),l.error(k.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let k=await U(a.Name);if(k){x("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${t}/clone`,{name:k}),f(),l.success(`Site cloned as '${k}'`),v.go("sites")}catch(b){f(),l.error(b.message)}}}),e.querySelectorAll("[data-action]").forEach(k=>{k.addEventListener("click",async()=>{let b=k.dataset.action;if(b==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(m){l.error(m.message)}return}x(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${b}`),f(),l.success(`Site ${b} successful`),v.go("site-detail",{id:t})}catch(m){f(),l.error(m.message)}})}),ke(e,t),Se(e,t),a.SiteType===1&&Le(e,t),$e(e,t,a),ue(e,t),R(e,t),u&&(ve(e,t),z(e,t)),Fe(e),ye(s??[])}async function J(e){let t=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!t||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((o,i)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,c)=>{r?i(r):o(c)})});t.src=n,t.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=e,a.appendChild(s)}function K(e){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=n,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),v.go("users")})}function Ie(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>Ie(e)),ze(e)}function ae(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
