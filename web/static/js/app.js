"use strict";(()=>{var d={async _req(t,e,a,n=6e4){let s=new AbortController,o=setTimeout(()=>s.abort(),n),i={method:t,headers:{"Content-Type":"application/json"},signal:s.signal};t!=="GET"&&t!=="HEAD"&&(i.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(i.body=JSON.stringify(a));try{let r=await fetch("/api"+e,i);clearTimeout(o);let l=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);return l}catch(r){throw clearTimeout(o),r}},get:t=>d._req("GET",t),post:(t,e)=>d._req("POST",t,e),put:(t,e)=>d._req("PUT",t,e),delete:t=>d._req("DELETE",t),patch:(t,e)=>d._req("PATCH",t,e)};var ct=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a),UIkit.modal("#kp-progress-modal").show()}function y(){let t=document.getElementById("kp-progress-modal");t&&(UIkit.modal(t).hide(),setTimeout(()=>t.remove(),300))}function W(t){return new Promise(e=>{let a="kp-clone-modal",n=`
            <div id="${a}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",n);let s=UIkit.modal(`#${a}`),o=document.getElementById("kp-clone-name"),i=document.getElementById("kp-clone-ok"),r=document.getElementById("kp-clone-cancel"),l=p=>{s.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(p)};i.addEventListener("click",()=>l(o.value.trim()||null),{once:!0}),r.addEventListener("click",()=>l(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>l(null),{once:!0}),s.show(),setTimeout(()=>o.focus(),150),o.addEventListener("keydown",p=>{p.key==="Enter"&&i.click()})})}function et(t,e,a){return new Promise(n=>{let s="kp-sync-modal",o=t==="pull",i=o?"Pull From Parent":"Push To Parent",r=o?"cloud-download":"cloud-upload",l=o?a:e,p=o?e:a,k=`
            <div id="${s}" uk-modal>
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),n=document.getElementById("cs-php-version-wrap"),s=document.getElementById("cs-node-version-wrap"),o=document.getElementById("cs-dotnet-version-wrap"),i=document.getElementById("cs-start-command-wrap"),r=document.getElementById("cs-wordpress-wrap");e.show();let l=document.getElementById("cs-domains-wrap"),p=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let k=parseInt(a.value);n.classList.toggle("uk-hidden",k!==1&&k!==2||k===6),s.classList.toggle("uk-hidden",k!==4),o.classList.toggle("uk-hidden",k!==5),i.classList.toggle("uk-hidden",k!==4&&k!==5),r.classList.toggle("uk-hidden",k!==1||k===6),l.classList.toggle("uk-hidden",k===6),p.classList.toggle("uk-hidden",k!==6)}),document.getElementById("create-site-form").addEventListener("submit",async k=>{k.preventDefault();let v=k.target.querySelector('[type="submit"]'),b=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let u=new FormData(k.target),m=parseInt(u.get("site_type")),g=null;m===4&&(g=parseInt(u.get("node_version"))),m===5&&(g=parseInt(u.get("dotnet_version")));let f={name:u.get("name").trim(),php_version:parseInt(u.get("php_version"))||3,site_type:m,runtime_version:g,start_command:u.get("start_command")?.trim()||"",domains:u.get("domains").split(`
`).map(x=>x.trim()).filter(Boolean),install_wordpress:m===1?u.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=m===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;S("Creating Site",w);try{await d.post("/sites",f),y(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){y(),c.error(x.message),v.disabled=!1,v.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var V=null;function T(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}function Ot(t){return`${t.toFixed(1)}%`}var J=null;function st(){return J||(J=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),J)}function nt(t,e){return`
        ...
        </table>`}function pt(t,e,a){let n=a===6,s=null;function o(){if(n)return;let l=t.querySelector("#stats-pod-indicator"),p=t.querySelector("#stats-pod-table-wrap");if(!p)return;let k=location.protocol==="https:"?"wss":"ws";s=new WebSocket(`${k}://${location.host}/api/sites/${e}/stats/pod`),s.onopen=()=>{l&&(l.className="kp-status kp-status-running",l.textContent="Live")},s.onmessage=v=>{try{let b=JSON.parse(v.data);p.innerHTML=Vt(b.containers??[])}catch{}},s.onerror=()=>{l&&(l.className="kp-status kp-status-error",l.textContent="Error")},s.onclose=()=>{l&&l.textContent==="Live"&&(l.className="kp-status kp-status-stopped",l.textContent="Disconnected")}}function i(){s&&s.readyState===WebSocket.OPEN&&s.close(),s=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{dt(e)}),o();let r=new MutationObserver(()=>{document.getElementById("stats-panel")||(i(),r.disconnect())});r.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ut(t,e){let a=e===6;await zt(t),a||await dt(t)}async function kt(t){let e=await d.get("/sites")??[];t.innerHTML=`
        ...
        ${e.length===0?j("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>z()),Kt()}function Jt(t,e=[]){let a=t.Domains?.[0]??null,n=t.SiteType===6,s=t.ParentID>0?e.find(o=>o.ID===t.ParentID)??null:null;return`
        ...
        </div>`}function Kt(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),n=document.getElementById("sites-search"),s=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let o=null,i=!0,r=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],l=()=>{let m=r().length;e.textContent=`${m} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush"].forEach(w=>{let x=document.getElementById(w);x&&(x.disabled=m===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=m===0);let f=document.querySelectorAll(".kp-site-row-check");a.indeterminate=m>0&&m<f.length,a.checked=f.length>0&&m===f.length},p=()=>{let u=n.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(m=>{let g=m.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",f=m.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";m.style.display=!u||g.includes(u)||f.includes(u)?"":"none"})},k=u=>{o===u?i=!i:(o=u,i=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===u?i?" \u2191":" \u2193":" \u2195"});let m=[...s.querySelectorAll("tr")];m.sort((g,f)=>{let w="",x="";return u==="name"?(w=g.querySelector(".kp-site-row-name")?.textContent??"",x=f.querySelector(".kp-site-row-name")?.textContent??""):u==="status"?(w=g.dataset.status??"",x=f.dataset.status??""):u==="type"?(w=g.dataset.type??"",x=f.dataset.type??""):u==="domain"&&(w=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",x=f.querySelector("td:nth-child(6)")?.textContent.trim()??""),i?w.localeCompare(x):x.localeCompare(w)}),m.forEach(g=>s.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(u=>{u.checked=a.checked}),l()}),s?.addEventListener("change",u=>{u.target.classList.contains("kp-site-row-check")&&l()}),n?.addEventListener("input",p),document.querySelectorAll(".kp-sortable").forEach(u=>{u.addEventListener("click",()=>k(u.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush"].forEach(u=>{let m=u.replace("bulk-","");document.getElementById(u)?.addEventListener("click",()=>{let g=r().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:m,ids:g}}))})});let v=document.getElementById("kp-bulk-mobile-pill"),b=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",u=>{u.stopPropagation(),b.hidden=!b.hidden}),document.addEventListener("click",u=>{b&&!v?.contains(u.target)&&(b.hidden=!0)},{capture:!0}),["start","stop","restart","flush"].forEach(u=>{document.getElementById(`bulk-mobile-${u}`)?.addEventListener("click",m=>{m.preventDefault(),b.hidden=!0;let g=r().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:u,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(u=>{u.textContent=" \u2195"}),l()}var K=null;async function bt(t){let[e,a,n]=await Promise.all([d.get("/sites").catch(()=>[]),d.get("/stats/traffic").catch(()=>null),d.get("/stats/pod").catch(()=>null)]),s=e.filter(r=>r.SiteStatus===1).length,o=e.filter(r=>r.SiteStatus===2).length,i=e.filter(r=>r.SiteStatus===4).length;if(t.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`}async function C(t){let e=t.querySelector("#security-panel");if(!e)return;let a=e.dataset.ipBase,n=e.dataset.uaBase,s=e.dataset.wafBase;try{let o=[d.get(a),d.get(n)];e.dataset.siteId||o.push(d.get(s),d.get("/settings/trusted-proxies"));let[i,r,l,p]=await Promise.all(o);if(!t.querySelector("#sec-ip-whitelist"))return;if(t.querySelector("#sec-ip-whitelist").value=i.whitelist??"",t.querySelector("#sec-ip-blacklist").value=i.blacklist??"",t.querySelector("#sec-ua-whitelist").value=r.whitelist??"",t.querySelector("#sec-ua-blacklist").value=r.blacklist??"",l){let k=t.querySelector("#sec-waf-enabled"),v=t.querySelector("#sec-waf-audit"),b=t.querySelector("#sec-waf-mode"),u=t.querySelector("#sec-waf-paranoia"),m=t.querySelector("#sec-waf-exclusions");k&&(k.checked=!!l.Enabled),v&&(v.checked=!!l.AuditLog),b&&(b.value=String(l.Mode??0)),u&&(u.value=String(l.ParanoiaLevel??1)),m&&(m.value=l.Exclusions??"")}if(p){let k=t.querySelector("#sec-tp-cidrs");k&&(k.value=p.trusted_proxies_custom??"")}}catch(o){c.error("Failed to load security rules: "+o.message)}}function G(t){let e=t.querySelector("#security-panel");if(!e)return;let a=e.dataset.ipBase,n=e.dataset.uaBase;t.querySelector("#sec-ip-save")?.addEventListener("click",async()=>{let s=t.querySelector("#sec-ip-save"),o=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put(a,{whitelist:t.querySelector("#sec-ip-whitelist").value,blacklist:t.querySelector("#sec-ip-blacklist").value}),c.success("IP rules saved")}catch(i){c.error(i.message)}finally{s.disabled=!1,s.innerHTML=o}}),t.querySelector("#sec-ua-save")?.addEventListener("click",async()=>{let s=t.querySelector("#sec-ua-save"),o=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put(n,{whitelist:t.querySelector("#sec-ua-whitelist").value,blacklist:t.querySelector("#sec-ua-blacklist").value}),c.success("UA rules saved")}catch(i){c.error(i.message)}finally{s.disabled=!1,s.innerHTML=o}}),t.querySelector("#sec-tp-save")?.addEventListener("click",async()=>{let s=t.querySelector("#sec-tp-save"),o=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put("/settings/trusted-proxies",{trusted_proxies_custom:t.querySelector("#sec-tp-cidrs").value.trim()}),c.success("Trusted proxy ranges saved")}catch(i){c.error(i.message)}finally{s.disabled=!1,s.innerHTML=o}}),t.querySelector("#sec-tp-import")?.addEventListener("change",async s=>{let o=s.target.files[0];if(!o)return;let i=new FormData;i.append("file",o);try{let r=await fetch("/api/settings/trusted-proxies/import",{method:"POST",body:i}),l=r.status===204?null:await r.json().catch(()=>null);if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);await C(t),c.success("Trusted proxies imported")}catch(r){c.error(r.message)}finally{s.target.value=""}}),t.querySelector("#sec-waf-save")?.addEventListener("click",async()=>{let s=t.querySelector("#sec-waf-save"),o=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.5"></div>';try{await d.put(e.dataset.wafBase,{enabled:t.querySelector("#sec-waf-enabled").checked,mode:parseInt(t.querySelector("#sec-waf-mode").value,10),paranoia_level:parseInt(t.querySelector("#sec-waf-paranoia").value,10),audit_log:t.querySelector("#sec-waf-audit").checked,exclusions:t.querySelector("#sec-waf-exclusions").value.trim()}),c.success("WAF settings saved \u2014 engine recompiling in background")}catch(i){c.error(i.message)}finally{s.disabled=!1,s.innerHTML=o}}),t.querySelector("#sec-ip-import")?.addEventListener("change",async s=>{let o=s.target.files[0];if(!o)return;let i=new FormData;i.append("file",o);try{let r=await fetch("/api"+a+"/import",{method:"POST",body:i}),l=r.status===204?null:await r.json().catch(()=>null);if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);await C(t),c.success("IP rules imported")}catch(r){c.error(r.message)}finally{s.target.value=""}}),t.querySelector("#sec-ua-import")?.addEventListener("change",async s=>{let o=s.target.files[0];if(!o)return;let i=new FormData;i.append("file",o);try{let r=await fetch("/api"+n+"/import",{method:"POST",body:i}),l=r.status===204?null:await r.json().catch(()=>null);if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);await C(t),c.success("UA rules imported")}catch(r){c.error(r.message)}finally{s.target.value=""}}),t.querySelector("#sec-waf-import")?.addEventListener("change",async s=>{let o=s.target.files[0];if(!o)return;let i=new FormData;i.append("file",o);try{let r=await fetch("/api/settings/waf/import",{method:"POST",body:i}),l=r.status===204?null:await r.json().catch(()=>null);if(!r.ok)throw new Error(l?.error||`HTTP ${r.status}`);await C(t),c.success("WAF settings imported")}catch(r){c.error(r.message)}finally{s.target.value=""}})}async function vt(t){if(!D()){t.innerHTML=L("Access denied");return}t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${R(null)}`,G(t),C(t)}function Gt(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function gt(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=Gt(a.status)}catch{}}async function ht(t){if(!D()){t.innerHTML=L("Access denied");return}let[e,a,n,s]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf"),d.get("/settings/trusted-proxies")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&gt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await d.put("/settings",p),c.success("Settings saved"),gt(p.admin_domain)}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let l=await fetch("/api/settings/import",{method:"POST",body:r}),p=l.status===204?null:await l.json().catch(()=>null);if(!l.ok)throw new Error(p?.error||`HTTP ${l.status}`);c.success("Settings imported")}catch(l){c.error(l.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(o.target),p={backup_schedule:l.get("backup_schedule").trim(),backup_retain_days:l.get("backup_retain_days").trim()};try{await d.put("/settings/backup",p),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let l=new FormData(o.target),p={s3_endpoint:l.get("s3_endpoint").trim(),s3_bucket:l.get("s3_bucket").trim(),s3_region:l.get("s3_region").trim(),s3_access_key:l.get("s3_access_key").trim()},k=l.get("s3_secret_key").trim();k&&(p.s3_secret_key=k);try{await d.put("/settings/backup",p),c.success("S3 settings saved")}catch(v){c.error(v.message)}finally{i.disabled=!1,i.innerHTML=r}})}function yt(t){return`
        ...
        </div>`}function Qt(t){if(!t||t.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No snapshots yet.</p>';let e=s=>s===2?'<span class="kp-mono" style="color:var(--kp-cyan)">S3</span>':'<span class="kp-mono" style="color:var(--kp-blue)">Local</span>',a=s=>s<1024?`${s} B`:s<1048576?`${(s/1024).toFixed(1)} KB`:s<1073741824?`${(s/1048576).toFixed(1)} MB`:`${(s/1073741824).toFixed(2)} GB`;return`
        <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
            <thead>
                <tr>
                    <th>Snapshot ID</th>
        ...
        </table>`}function ft(t,e){let a=Date.now()+18e5,n=setInterval(async()=>{try{let s=await d.get(`/sites/${e}/backups/restore-status`);(!s?.active||Date.now()>a)&&(clearInterval(n),y(),s?.active?c.error("Import timed out \u2014 check server logs"):c.success("Import complete"),await _(t,e))}catch{}},3e3)}async function _(t,e){try{let[a,n]=await Promise.all([d.get(`/sites/${e}/backup-repo`),d.get(`/sites/${e}/backups`)]),s=t.querySelector("#backup-local-enabled"),o=t.querySelector("#backup-s3-enabled");s&&(s.checked=!!a.LocalEnabled),o&&(o.checked=!!a.S3Enabled);let i=t.querySelector("#backup-error-banner");if(i)if(a.last_error){let l=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";i.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${l}:</strong> ${a.last_error}</p>
                    </div>`}else i.innerHTML="";let r=t.querySelector("#backup-list-wrap");r&&(r.innerHTML=Qt(n))}catch(a){let n=t.querySelector("#backup-list-wrap");n&&(n.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function wt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let n={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${e}/backup-repo`,n),c.success("Backup destinations saved")}catch(s){c.error(s.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let n=0;try{n=(await d.get(`/sites/${e}/backups`))?.length??0}catch{}try{await d.post(`/sites/${e}/backups`,{label:"manual"})}catch(i){c.error(i.message);return}S("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let s=Date.now()+1800*1e3,o=setInterval(async()=>{try{(((await d.get(`/sites/${e}/backups`))?.length??0)>n||Date.now()>s)&&(clearInterval(o),y(),await _(t,e),Date.now()<=s?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async n=>{let s=n.target.closest(".backup-restore-btn");if(s){let r=s.dataset.id;if(!await $("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${e}/backups/${r}/restore`)}catch(b){c.error(b.message);return}S("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let p=Date.now(),k=Date.now()+900*1e3,v=setInterval(async()=>{try{let b=await d.get(`/sites/${e}/backups/restore-status`);(!b?.active||Date.now()>k)&&(clearInterval(v),y(),b?.active?c.error("Restore timed out"):c.success("Restore complete"),await _(t,e))}catch{}},3e3);return}let o=n.target.closest(".backup-delete-btn");if(o){let r=o.dataset.id;if(!await $("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;S("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${e}/backups/${r}`),y(),c.success("Snapshot deleted"),await _(t,e)}catch(p){y(),c.error(p.message)}}let i=n.target.closest(".backup-download-btn");if(i){let r=i.dataset.id;S("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let l=document.createElement("a");l.href=`/api/sites/${e}/backups/${r}/download`,l.style.display="none",document.body.appendChild(l),l.click(),document.body.removeChild(l),setTimeout(()=>{y()},5e3)},300);return}});let a=t.querySelector("#import-backup-modal");a&&(UIkit.util.on(a,"beforeshow",async()=>{let n=a.querySelector("#import-target-site");try{let o=await d.get("/sites");n.innerHTML=o.map(i=>`<option value="${i.ID}"${i.ID===e?" selected":""}>${i.Name}</option>`).join("")}catch{n.innerHTML='<option value="">Failed to load sites</option>'}let s=a.querySelector("#import-sftp-list");try{let o=await d.get(`/sites/${e}/backups/import/files`);!o||o.length===0?s.innerHTML='<p class="kp-muted uk-text-small">No files found.</p>':s.innerHTML=o.map(i=>`
    ...
    </div>`}function St(t,e){t.addEventListener("click",a=>{if(a.target.closest(".cfg-add-row")){let n=a.target.closest(".cfg-add-row");t.querySelector(`.cfg-rows[data-type="${n.dataset.type}"]`).insertAdjacentHTML("beforeend",A())}}),t.addEventListener("click",a=>{a.target.closest(".cfg-del-row")&&a.target.closest(".kp-config-row").remove()}),t.addEventListener("click",async a=>{let n=a.target.closest(".cfg-save");if(!n)return;let{type:s,site:o}=n.dataset,i=t.querySelectorAll(`.cfg-rows[data-type="${s}"] .kp-config-row`),r={};if(i.forEach(l=>{let p=l.querySelector(".cfg-key").value.trim(),k=l.querySelector(".cfg-val").value.trim();p&&(r[p]=k)}),s==="5"){let l=t.querySelector(".varnish-enabled-toggle");r.enabled=l?.checked?"true":"false"}try{await d.put(`/sites/${o}/configs/${s}`,r),c.success(`${H[s]} config saved`)}catch(l){c.error(l.message)}}),t.addEventListener("click",async a=>{let n=a.target.closest(".cfg-reset");if(!n)return;let{type:s,site:o}=n.dataset;if(await $("Reset Config",`Reset ${H[s]} config to defaults?`))try{let r=await d.post(`/sites/${o}/configs/${s}/reset`),l=t.querySelector(`.cfg-rows[data-type="${s}"]`);l.innerHTML=Object.entries(r).map(([p,k])=>A(p,k)).join(""),c.success(`${H[s]} reset to defaults`)}catch(r){c.error(r.message)}}),t.addEventListener("change",async a=>{let n=a.target.closest(".cfg-import-input");if(!n)return;let{type:s,site:o}=n.dataset,i=n.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let l=await fetch(`/api/sites/${o}/configs/${s}/import`,{method:"POST",body:r}),p=l.status===204?null:await l.json().catch(()=>null);if(!l.ok)throw new Error(p?.error||`HTTP ${l.status}`);let k=t.querySelector(`.cfg-rows[data-type="${s}"]`);k.innerHTML=Object.entries(p).map(([v,b])=>A(v,b)).join(""),c.success(`${H[s]} config imported`)}catch(l){c.error(l.message)}finally{n.value=""}})}function Et(t){return`
        ...
        <tr>
            ...
            <td>
        ...
        </table>`}async function Q(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let n=await d.get(`/sites/${e}/crons`);a.innerHTML=Lt(n)}catch(n){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${n.message}</p>`}}function Tt(t,e){let a=[],n=t.querySelector("#cron-modal"),s=t.querySelector("#cron-modal-title"),o=t.querySelector("#cron-modal-id"),i=t.querySelector("#cron-modal-label"),r=t.querySelector("#cron-modal-command"),l=t.querySelector("#cron-modal-schedule"),p=t.querySelector("#cron-schedule-preview"),k=t.querySelector("#cron-modal-enabled");l?.addEventListener("input",()=>{p.textContent=$t(l.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{s.textContent="Add Cron Job",o.value="",i.value="",r.value="",l.value="",p.textContent="",k.checked=!0,UIkit.modal(n).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let v=r.value.trim(),b=l.value.trim();if(!v||!b){c.error("Command and schedule are required");return}let u={label:i.value.trim(),command:v,schedule:b,enabled:k.checked},m=o.value;try{m?(await d.put(`/sites/${e}/crons/${m}`,u),c.success("Cron job updated")):(await d.post(`/sites/${e}/crons`,u),c.success("Cron job created")),UIkit.modal(n).hide(),await Q(t,e),a=await d.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async v=>{let b=v.target.closest(".cron-detail-btn");if(b){let f=b.dataset.id,w=a.find(tt=>String(tt.ID)===f);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let u=v.target.closest(".cron-edit-btn");if(u){let f=u.dataset.id,w=a.find(x=>String(x.ID)===f);if(!w)return;s.textContent="Edit Cron Job",o.value=w.ID,i.value=w.Label||"",r.value=w.Command,l.value=w.Schedule,p.textContent=$t(w.Schedule),k.checked=w.Enabled,UIkit.modal(n).show();return}let m=v.target.closest(".cron-delete-btn");if(m){let f=m.dataset.id;if(!await $("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await d.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await Q(t,e),a=await d.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let g=v.target.closest(".cron-run-btn");if(g){let f=g.dataset.id;try{await d.post(`/sites/${e}/crons/${f}/run`)}catch(E){c.error(E.message);return}S("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await d.get(`/sites/${e}/crons`)).find(B=>String(B.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,tt=setInterval(async()=>{try{let E=await d.get(`/sites/${e}/crons`),B=E.find(U=>String(U.ID)===f);if(!B||B.LastRun!==w||Date.now()>x){clearInterval(tt),y(),a=E??[];let U=t.querySelector("#cron-list-wrap");U&&(U.innerHTML=Lt(E)),B?.LastError?c.error(`Job failed: ${B.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async v=>{let b=v.target.closest(".cron-toggle");if(!b)return;let u=b.dataset.id;try{await d.patch(`/sites/${e}/crons/${u}/toggle`,{enabled:b.checked}),c.success(b.checked?"Cron job enabled":"Cron job disabled")}catch(m){c.error(m.message),b.checked=!b.checked}}),d.get(`/sites/${e}/crons`).then(v=>{a=v??[]}).catch(()=>{})}function it(t){return String(t).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function $t(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,n,s,o,i]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&n!=="*"&&s==="*"&&o==="*"&&i==="*")return`daily at ${n.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&n!=="*"&&s==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(p=>r[parseInt(p)]??p).join(", ")} at ${n.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:n.startsWith("*/")?`every ${n.slice(2)} hours`:t}var Yt=2e3;function ot(t,e){return`
        <div>
        ...
        </div>`}function Ct(t,e){let a=null,n=!1,s=t.querySelector("#log-output"),o=t.querySelector("#log-connect"),i=t.querySelector("#log-disconnect"),r=t.querySelector("#log-clear"),l=t.querySelector("#log-autoscroll"),p=t.querySelector("#log-status");function k(u){for(u.split(`
`).forEach(m=>{if(!m)return;let g=document.createElement("div");g.className=m.match(/WAF BLOCK/i)?"kp-log-line-err":m.match(/WAF DETECT/i)?"kp-log-line-warn":m.match(/error|crit|emerg/i)?"kp-log-line-err":m.match(/warn/i)?"kp-log-line-warn":m.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=m,s.appendChild(g)});s.childElementCount>Yt;)s.removeChild(s.firstChild);l.checked&&(s.scrollTop=s.scrollHeight)}function v(){a&&(a.close(),a=null),n=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}o.addEventListener("click",()=>{v();let u=t.querySelector("#log-container").value,m=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",f=u==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${m}`:u==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${m}`:`${g}://${location.host}/api/sites/${e}/logs?container=${u}&tail=${m}`;a=new WebSocket(f),a.onopen=()=>{n=!0,o.disabled=!0,i.disabled=!1,p&&(p.textContent=`Connected \u2014 ${u}`)},a.onmessage=w=>k(w.data),a.onerror=()=>{},a.onclose=()=>{n=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}}),i.addEventListener("click",v),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(v(),o.click())});let b=h.go.bind(h);h.go=function(u,m={}){return a&&v(),b(u,m)}}function Xt(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Pt(t,e){try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(t)}`),n=document.getElementById(`ssl-icon-${e}`);n&&(n.outerHTML=Xt(a.status))}catch{}}function It(t){t.forEach(e=>Pt(e.Domain,e.ID))}function Bt(t,e,a,n=0,s=null){let o=t.SiteType!==3&&t.PMAPort>0;return`
        ...
        </div>`}function Rt(t,e){let a=t.querySelector("#wpcli-output"),n=t.querySelector("#wpcli-input"),s=t.querySelector("#wpcli-run"),o=t.querySelector("#wpcli-clear"),i=t.querySelector("#wpcli-status"),r=[],l=-1;function p(b,u=""){b.split(`
`).forEach(m=>{if(!m)return;let g=document.createElement("div");u?g.className=u:g.className=m.match(/error|fatal|critical/i)?"kp-log-line-err":m.match(/warning|warn/i)?"kp-log-line-warn":m.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=m,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function k(b){if(b=b.trim(),!b)return;r.unshift(b),l=-1,p(`wp> ${b}`,"kp-log-line-info"),n.disabled=!0,s.disabled=!0,i&&(i.textContent="Running...");let u=location.protocol==="https:"?"wss":"ws",m=new WebSocket(`${u}://${location.host}/api/sites/${e}/wpcli`);m.onopen=()=>{m.send(JSON.stringify({command:b}))},m.onmessage=g=>{let f=g.data;if(f.trim()==="[done]"){m.close();return}if(f.startsWith("[info]")){p(f,"kp-muted");return}if(f.startsWith("[error]")){p(f,"kp-log-line-err");return}p(f)},m.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},m.onclose=()=>{n.disabled=!1,s.disabled=!1,i&&(i.textContent="Ready"),n.focus()}}s.addEventListener("click",()=>{k(n.value),n.value=""}),n.addEventListener("keydown",b=>{if(b.key==="Enter"){k(n.value),n.value="",l=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),l<r.length-1&&(l++,n.value=r[l]);return}b.key==="ArrowDown"&&(b.preventDefault(),l>0?(l--,n.value=r[l]):(l=-1,n.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let u=b.dataset.cmd;if(u.startsWith("search-replace")){n.value=u,n.focus();let m=u.indexOf("''")+1;n.setSelectionRange(m,m);return}k(u)})}),o.addEventListener("click",()=>{a.innerHTML=""});let v=h.go.bind(h);h.go=function(b,u={}){return v(b,u)},n.focus()}var N=null;function te(){return`
        ...
        </div>`}function Ht(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),n=t.querySelector("#kp-manage-pill"),s=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function o(r,l=!1){UIkit.switcher(a).show(r),e.querySelectorAll(":scope > li[data-pill]").forEach(p=>p.classList.remove("kp-pill-active")),l?(n?.classList.add("kp-pill-active"),s?.querySelectorAll("a[data-switcher]").forEach(p=>{p.classList.toggle("kp-dd-active",parseInt(p.dataset.switcher,10)===r)})):(n?.classList.remove("kp-pill-active"),s?.querySelectorAll("a[data-switcher]").forEach(p=>p.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(r=>{r.addEventListener("click",l=>{l.preventDefault();let p=parseInt(r.closest("li").dataset.pill,10);o(p,!1)})}),n?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",r=>{r.stopPropagation(),s.hidden=!s.hidden,n.classList.toggle("kp-pill-active",!s.hidden)}),s?.querySelectorAll("a[data-switcher]").forEach(r=>{r.addEventListener("click",l=>{l.preventDefault(),s.hidden=!0,o(parseInt(r.dataset.switcher,10),!0)})}),document.addEventListener("click",r=>{s&&!n.contains(r.target)&&(s.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(0)}async function At(t){let e=document.getElementById("waf-tab-panel");if(!e)return;e.innerHTML=te();let a=document.getElementById("waf-export-btn");a&&(a.href=`/api/sites/${t}/waf/export`);try{let n=await d.get(`/sites/${t}/waf`),s=document.getElementById("waf-override"),o=document.getElementById("waf-site-exclusions");s&&(s.value=String(n.Override??0)),o&&(o.value=n.Exclusions??"");let i=document.getElementById("waf-plugins-list");if(i){let[r,l]=await Promise.all([d.get("/settings/waf/plugins"),d.get(`/sites/${t}/waf/plugins`)]),p=new Set(l??[]);!r||r.length===0?i.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':(i.innerHTML=`
                    <div class="waf-plugin-pills">
                        ${r.map(k=>`
                        <span class="waf-plugin-pill ${p.has(k)?"active":""}"
                            data-plugin="${k}">${k}</span>
                        `).join("")}
                    </div>`,i.querySelectorAll(".waf-plugin-pill").forEach(k=>{k.addEventListener("click",()=>k.classList.toggle("active"))}))}}catch(n){c.error("Failed to load WAF settings: "+n.message)}}function ee(t,e){N&&N.abort(),N=new AbortController,t.addEventListener("submit",async a=>{if(a.target.id!=="waf-override-form")return;a.preventDefault();let n=a.target.querySelector('[type="submit"]'),s=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=new FormData(a.target),i={override:parseInt(o.get("override"),10),exclusions:o.get("exclusions").trim()};try{await d.put(`/sites/${e}/waf`,i);let r=[...document.querySelectorAll(".waf-plugin-pill.active")].map(l=>l.dataset.plugin);await d.put(`/sites/${e}/waf/plugins`,r),c.success("WAF override saved \u2014 engine recompiling in background")}catch(r){c.error(r.message)}finally{n.disabled=!1,n.innerHTML=s}},{signal:N.signal}),t.querySelector("#waf-import")?.addEventListener("change",async a=>{let n=a.target.files[0];if(!n)return;let s=new FormData;s.append("file",n);try{let o=await fetch(`/api/sites/${e}/waf/import`,{method:"POST",body:s}),i=o.status===204?null:await o.json().catch(()=>null);if(!o.ok)throw new Error(i?.error||`HTTP ${o.status}`);await At(e),c.success("WAF settings imported")}catch(o){c.error(o.message)}finally{a.target.value=""}})}function ae(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}async function se(t){let e=document.getElementById("rp-routes-list");if(e)try{let a=await d.get(`/sites/${t}/rp-routes`);e.innerHTML=a.length?a.map(n=>lt(n.Domain,n.Upstream)).join(""):lt()}catch(a){c.error("Failed to load routes: "+a.message)}}function ne(t,e){t.addEventListener("click",async a=>{if(a.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",lt());return}if(a.target.closest(".rp-remove-row")){a.target.closest(".rp-route-row").remove();return}if(!a.target.closest("#rp-save-btn"))return;let n=a.target.closest("#rp-save-btn"),s=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o=[...document.querySelectorAll(".rp-route-row")].map(i=>({Domain:i.querySelector('[data-field="domain"]').value.trim(),Upstream:i.querySelector('[data-field="upstream"]').value.trim()})).filter(i=>i.Domain&&i.Upstream);try{await d.put(`/sites/${e}/rp-routes`,o),c.success("Routes saved")}catch(i){c.error(i.message)}finally{n.disabled=!1,n.innerHTML=s}},{signal:N.signal})}async function Ft(t,{id:e}){let[{site:a,domains:n,sftp:s},o,i]=await Promise.all([d.get(`/sites/${e}`),d.get("/sites"),d.get(`/sites/${e}/configs`)]),r=Array.isArray(o)?o:[],l=a.SiteType===1||a.SiteType===2,p=a.SiteType===6,k=[1,2,4,5].includes(a.SiteType);if(t.innerHTML=`
        <div class="kp-view-header">
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${Bt(a,n??[],s,a.ParentID??0,r.find(v=>v.ID===a.ParentID)?.Name??null)}</li>
            <li>${nt(e,a.SiteType)}</li>
            <li>${F(e,1,i[1])}</li>
            ${l?`<li>${F(e,2,i[2])}</li>`:""}
            <li>${F(e,3,i[3])}</li>
            ...
            <li>${yt(e)}</li>
            ${k?`<li>${Et(e)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>O(a)),document.getElementById("sd-site-nav")?.addEventListener("change",v=>{h.go("site-detail",{id:v.target.value})}),G(t),C(t),Ct(t,e),ee(t,e),At(e),p){ne(t,e),se(e),Ht(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{S("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${e}/recreate`),y(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(v){y(),c.error(v.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let v=await W(a.Name);if(v){S("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${e}/clone`,{name:v}),y(),c.success(`Site cloned as '${v}'`),h.go("sites")}catch(b){y(),c.error(b.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(v=>{v.addEventListener("click",async()=>{let b=v.dataset.action;if(b==="flush"){try{await d.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(m){c.error(m.message)}return}S(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await d.post(`/sites/${e}/${b}`),y(),c.success(`Site ${b} successful`),h.go("site-detail",{id:e})}catch(m){y(),c.error(m.message)}})}),St(t,e),qt(t,e),a.SiteType===1&&Rt(t,e),_t(t,e,a),wt(t,e),_(t,e),k&&(Tt(t,e),Q(t,e)),pt(t,e,a.SiteType),ut(e,a.SiteType),Ht(t),It(n??[])}async function Y(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(s=>s.remove()),typeof QRCode<"u")try{let s=await new Promise((o,i)=>{QRCode.toDataURL(t,{width:220,margin:2},(r,l)=>{r?i(r):o(l)})});e.src=s,e.style.display="";return}catch{}let n=document.createElement("p");n.className="totp-uri-text kp-muted uk-text-small",n.style.wordBreak="break-all",n.textContent=t,a.appendChild(n)}function X(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(s).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=s,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{n.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),h.go("users")})}function Nt(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let a=UIkit.modal("#kp-create-user-modal");a.show(),document.getElementById("create-user-form").addEventListener("submit",async n=>{n.preventDefault();let s=n.target.querySelector('[type="submit"]'),o=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let i=new FormData(n.target),r={fname:i.get("fname").trim(),lname:i.get("lname").trim(),uname:i.get("uname").trim(),email:i.get("email").trim(),phone:i.get("phone").trim(),password:i.get("password"),role:parseInt(i.get("role"))};try{let l=q(await d.post("/users",r));document.getElementById("users-table-body").insertAdjacentHTML("beforeend",rt(l)),c.success(`User '${l.uname}' created`),document.getElementById("create-user-form").style.display="none",document.getElementById("cu-totp-section").style.display="",ie(l.id,a)}catch(l){c.error(l.message),s.disabled=!1,s.innerHTML=o}}),document.getElementById("kp-create-user-modal").addEventListener("hidden",()=>document.getElementById("kp-create-user-modal")?.remove())}function ie(t,e){let a=()=>{e.hide(),document.getElementById("kp-create-user-modal")?.remove(),h.go("users")};document.getElementById("cu-totp-skip-btn").addEventListener("click",a),document.getElementById("cu-totp-setup-btn").addEventListener("click",async()=>{let n=document.getElementById("cu-totp-setup-btn");n.disabled=!0,n.textContent="Setting up\u2026";try{let s=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=s.secret,document.getElementById("totp-setup-area").style.display="",document.getElementById("cu-totp-skip-btn").style.display="none",await Y(s.uri)}catch(s){c.error(s.message),n.disabled=!1,n.textContent="Enable TOTP"}}),document.getElementById("cu-totp-confirm-btn").addEventListener("click",async()=>{let n=document.getElementById("totp-confirm-code").value.trim();if(n.length!==6){c.error("Enter a 6-digit code");return}let s=document.getElementById("cu-totp-confirm-btn");s.disabled=!0;try{let o=await d.post(`/users/${t}/totp/confirm`,{code:n});e.hide(),document.getElementById("kp-create-user-modal")?.remove(),c.success("TOTP enabled"),o.backup_codes?.length?X(o.backup_codes):h.go("users")}catch(o){c.error(o.message),s.disabled=!1}})}async function Ut(t,e){document.getElementById("kp-edit-user-modal")?.remove();let a;try{a=q(await d.get(`/users/${e}`))}catch(p){c.error(p.message);return}let n=window.KP?.user?.role===99,s=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",s);let o=UIkit.modal("#kp-edit-user-modal");o.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let k=p.target.querySelector('[type="submit"]'),v=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b=new FormData(p.target),u={fname:b.get("fname").trim(),lname:b.get("lname").trim(),email:b.get("email").trim(),phone:b.get("phone").trim()};if(n){u.role=parseInt(b.get("role"));let g=b.get("uname");g&&(u.uname=g.trim())}let m=b.get("password");m&&(u.password=m);try{await d.put(`/users/${e}`,u),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("User updated"),h.go("users")}catch(g){c.error(g.message),k.disabled=!1,k.innerHTML=v}});let i=document.getElementById("totp-setup-btn");i&&i.addEventListener("click",async()=>{i.disabled=!0,i.textContent="Setting up\u2026";try{let p=await d.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await Y(p.uri)}catch(p){c.error(p.message),i.disabled=!1,i.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){c.error("Enter a 6-digit code");return}r.disabled=!0;try{let k=await d.post(`/users/${e}/totp/confirm`,{code:p});o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("TOTP enabled"),k.backup_codes?.length?X(k.backup_codes):h.go("users")}catch(k){c.error(k.message),r.disabled=!1}});let l=document.getElementById("totp-disable-btn");l&&l.addEventListener("click",async()=>{l.disabled=!0;try{await d.delete(`/users/${e}/totp`),c.success("TOTP disabled"),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),h.go("users")}catch(p){c.error(p.message),l.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function jt(t){if(!D()){t.innerHTML=L("Access denied");return}let e=await d.get("/users");t.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>Nt(t)),oe(t)}function rt(t){let e=t.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${t.id}">
