...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let i=new AbortController,n=setTimeout(()=>i.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:i.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let l=await fetch("/api"+e,r);clearTimeout(n);let o=l.status===204?null:await l.json().catch(()=>null);if(l.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!l.ok)throw new Error(o?.error||`HTTP ${l.status}`);return o}catch(l){throw clearTimeout(n),l}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var S=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function L(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Y=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a),UIkit.modal("#kp-progress-modal").show()}function w(){let t=document.getElementById("kp-progress-modal");t&&(UIkit.modal(t).hide(),setTimeout(()=>t.remove(),300))}function Z(t){return new Promise(e=>{let a="kp-clone-modal",s=`
            <div id="${a}" uk-modal>
...
`).forEach(p=>{if(!p)return;let b=document.createElement("div");b.className=p.match(/WAF BLOCK/i)?"kp-log-line-err":p.match(/WAF DETECT/i)?"kp-log-line-warn":p.match(/error|crit|emerg/i)?"kp-log-line-err":p.match(/warn/i)?"kp-log-line-warn":p.match(/info|notice/i)?"kp-log-line-info":"",b.textContent=p,s.appendChild(b)});s.childElementCount>we;)s.removeChild(s.firstChild);l.checked&&(s.scrollTop=s.scrollHeight)}function d(){e&&(e.close(),e=null),a=!1,i.disabled=!1,n.disabled=!0,o&&(o.textContent="Disconnected")}i.addEventListener("click",()=>{d();let k=t.querySelector("#admin-log-source").value,p=t.querySelector("#admin-log-tail").value,b=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${b}://${location.host}/api/logs/waf?tail=${p}`:`${b}://${location.host}/api/logs/proxy?tail=${p}`;e=new WebSocket(g),e.onopen=()=>{a=!0,i.disabled=!0,n.disabled=!1,o&&(o.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=f=>u(f.data),e.onerror=()=>{},e.onclose=()=>{a=!1,i.disabled=!1,n.disabled=!0,o&&(o.textContent="Disconnected")}}),n.addEventListener("click",d),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(d(),i.click())});let v=h.go.bind(h);h.go=function(k,p={}){return e&&d(),v(k,p)}}function Tt(t){t.innerHTML=xe(),Se(t)}var at=50;function $e(t,e,a){let s=Math.max(1,Math.ceil(t.total/at)),i=(t.entries??[]).map(_t).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
...
`+et(u):u?d=et(u):d=et(o),document.body.insertAdjacentHTML("beforeend",`
            ...
            </div>`);let v=document.getElementById("al-diff-modal-inst");UIkit.modal(v).show(),v.addEventListener("hidden",()=>v.remove(),{once:!0})})}function et(t){try{return JSON.stringify(JSON.parse(t),null,2)}catch{return t}}async function It(t){if(!B()){t.innerHTML=_("Access denied");return}let e={username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},a=await Pt(e,1);t.innerHTML=$e(a,e,1),Te(t,e,1)}function st(){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),i=document.getElementById("cs-node-version-wrap"),n=document.getElementById("cs-dotnet-version-wrap"),r=document.getElementById("cs-start-command-wrap"),l=document.getElementById("cs-wordpress-wrap");e.show();let o=document.getElementById("cs-domains-wrap"),u=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let d=parseInt(a.value);s.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),i.classList.toggle("uk-hidden",d!==4),n.classList.toggle("uk-hidden",d!==5),r.classList.toggle("uk-hidden",d!==4&&d!==5),l.classList.toggle("uk-hidden",d!==1||d===6),o.classList.toggle("uk-hidden",d===6),u.classList.toggle("uk-hidden",d!==6)}),document.getElementById("create-site-form").addEventListener("submit",async d=>{d.preventDefault();let v=d.target.querySelector('[type="submit"]'),k=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(d.target),b=parseInt(p.get("site_type")),g=null;b===4&&(g=parseInt(p.get("node_version"))),b===5&&(g=parseInt(p.get("dotnet_version")));let f={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:b,runtime_version:g,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(x=>x.trim()).filter(Boolean),install_wordpress:b===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let y=b===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",y);try{await m.post("/sites",f,6e5),w(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){w(),c.error(x.message),v.disabled=!1,v.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function _e(t){return`${t.toFixed(1)}%`}var nt=null;function bt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function vt(t,e){return`
        ...
        </table>`}function Ie(t,e,a,s){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let i=[...t].sort((d,v)=>{let k,p;switch(a){case"time":k=d.time,p=v.time;break;case"method":k=d.method,p=v.method;break;case"ip":k=d.client_ip,p=v.client_ip;break;default:k=d.status,p=v.status;break}return k<p?s?1:-1:k>p?s?-1:1:0}),n=50,r=Math.ceil(i.length/n),o=i.slice(e*n,(e+1)*n).map(d=>{let v=d.ua,k=d.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
        ...
        ${e.length===0?H("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>st()),Me()}function qe(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,i=t.ParentID>0?e.find(n=>n.ID===t.ParentID)??null:null;return`
        ...
        </div>`}function Me(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),i=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let n=null,r=!0,l=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],o=()=>{let b=l().length;e.textContent=`${b} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(y=>{let x=document.getElementById(y);x&&(x.disabled=b===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=b===0);let f=document.querySelectorAll(".kp-site-row-check");a.indeterminate=b>0&&b<f.length,a.checked=f.length>0&&b===f.length},u=()=>{let p=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(b=>{let g=b.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",f=b.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";b.style.display=!p||g.includes(p)||f.includes(p)?"":"none"})},d=p=>{n===p?r=!r:(n=p,r=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===p?r?" \u2191":" \u2193":" \u2195"});let b=[...i.querySelectorAll("tr")];b.sort((g,f)=>{let y="",x="";return p==="name"?(y=g.querySelector(".kp-site-row-name")?.textContent??"",x=f.querySelector(".kp-site-row-name")?.textContent??""):p==="status"?(y=g.dataset.status??"",x=f.dataset.status??""):p==="type"?(y=g.dataset.type??"",x=f.dataset.type??""):p==="domain"&&(y=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",x=f.querySelector("td:nth-child(6)")?.textContent.trim()??""),r?y.localeCompare(x):x.localeCompare(y)}),b.forEach(g=>i.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(p=>{p.checked=a.checked}),o()}),i?.addEventListener("change",p=>{p.target.classList.contains("kp-site-row-check")&&o()}),s?.addEventListener("input",u),document.querySelectorAll(".kp-sortable").forEach(p=>{p.addEventListener("click",()=>d(p.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(p=>{let b=p.replace("bulk-","");document.getElementById(p)?.addEventListener("click",()=>{let g=l().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:b,ids:g}}))})});let v=document.getElementById("kp-bulk-mobile-pill"),k=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",p=>{p.stopPropagation(),k.hidden=!k.hidden}),document.addEventListener("click",p=>{k&&!v?.contains(p.target)&&(k.hidden=!0)},{capture:!0}),["start","stop","restart","flush","recreate"].forEach(p=>{document.getElementById(`bulk-mobile-${p}`)?.addEventListener("click",b=>{b.preventDefault(),k.hidden=!0;let g=l().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:p,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(p=>{p.textContent=" \u2195"}),o()}var it=null;async function Dt(t){let[e,a,s]=await Promise.all([m.get("/sites").catch(()=>[]),m.get("/stats/traffic").catch(()=>null),m.get("/stats/pod").catch(()=>null)]),i=e.filter(l=>l.SiteStatus===1).length,n=e.filter(l=>l.SiteStatus===2).length,r=e.filter(l=>l.SiteStatus===4).length;if(t.innerHTML=`
        <div class="kp-view-header">
            ...
            <div>
                ...
                <div class="uk-grid-small uk-child-width-1-2" uk-grid>
                    ...
                    <div><div class="kp-stat-card">
                        <div class="uk-flex uk-flex-between">
                            <div>
                                <div class="kp-stat-value">${L(s?.mem_used??0)}</div>
        ...
        <div class="kp-site-grid">
            ${e.length===0?H("world","No sites yet"):e.slice(-3).reverse().map(l=>Mt(l,e)).join("")}
        </div>`,a?.hits_per_hour?.length){await bt();let l=document.getElementById("dash-traffic-chart");l&&window.Chart&&(it&&(it.destroy(),it=null),it=new window.Chart(l,{type:"bar",data:{labels:a.hits_per_hour.map(o=>new Date(o.hour).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})),datasets:[{label:"2xx",data:a.hits_per_hour.map(o=>o["2xx"]),backgroundColor:"rgba(39,174,96,0.75)",borderColor:"rgba(39,174,96,1)",borderWidth:1,borderRadius:3},{label:"3xx",data:a.hits_per_hour.map(o=>o["3xx"]),backgroundColor:"rgba(43,142,255,0.75)",borderColor:"rgba(43,142,255,1)",borderWidth:1,borderRadius:3},{label:"4xx",data:a.hits_per_hour.map(o=>o["4xx"]),backgroundColor:"rgba(255,171,0,0.75)",borderColor:"rgba(255,171,0,1)",borderWidth:1,borderRadius:3},{label:"5xx",data:a.hits_per_hour.map(o=>o["5xx"]),backgroundColor:"rgba(235,59,90,0.75)",borderColor:"rgba(235,59,90,1)",borderWidth:1,borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!0,labels:{color:"#6b8cae",font:{size:11}},onHover:o=>{o.native.target.style.cursor="pointer"},onLeave:o=>{o.native.target.style.cursor="default"}},tooltip:{mode:"index",backgroundColor:"#0c1530",borderColor:"#1a2a4a",borderWidth:1,titleColor:"#dde8f5",bodyColor:"#6b8cae"}},scales:{x:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10},maxRotation:45},grid:{color:"rgba(26,42,74,0.6)"}},y:{stacked:!0,ticks:{color:"#6b8cae",font:{size:10}},grid:{color:"rgba(26,42,74,0.6)"},beginAtZero:!0}}}}))}document.getElementById("dash-new-site")?.addEventListener("click",()=>st())}function O(t=null){let e=t?`/sites/${t}/security/ip`:"/security/ip",a=t?`/sites/${t}/security/ua`:"/security/ua",s=t?`/sites/${t}/waf`:"/settings/waf",i=t?`/sites/${t}/security/country`:"/security/country";return`
        <div id="security-panel" data-ip-base="${e}" data-ua-base="${a}" data-geo-base="${i}" data-waf-base="${s}" ${t?`data-site-id="${t}"`:""}>
        ...
        ${O(null)}`,ot(t),q(t)}function De(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function At(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Ht(t){if(!B()){t.innerHTML=_("Access denied");return}let[e,a,s,i,n,r]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&At(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let v={admin_domain:new FormData(l.target).get("admin_domain").trim()};try{await m.put("/settings",v),c.success("Settings saved"),At(v.admin_domain)}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("settings-import").addEventListener("change",async l=>{let o=l.target.files[0];if(!o)return;let u=new FormData;u.append("file",o);try{let d=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:u}),v=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(v?.error||`HTTP ${d.status}`);c.success("Settings imported")}catch(d){c.error(d.message)}finally{l.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={backup_schedule:d.get("backup_schedule").trim(),backup_retain_days:d.get("backup_retain_days").trim()};try{await m.put("/settings/backup",v),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("s3-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={s3_endpoint:d.get("s3_endpoint").trim(),s3_bucket:d.get("s3_bucket").trim(),s3_region:d.get("s3_region").trim(),s3_access_key:d.get("s3_access_key").trim()},k=d.get("s3_secret_key").trim();k&&(v.s3_secret_key=k);try{await m.put("/settings/backup",v),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("smtp-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={smtp_host:d.get("smtp_host").trim(),smtp_port:d.get("smtp_port").trim(),smtp_username:d.get("smtp_username").trim(),smtp_from:d.get("smtp_from").trim(),smtp_tls:d.get("smtp_tls")?"true":"false"},k=d.get("smtp_password").trim();k&&(v.smtp_password=k);try{await m.put("/settings/notifications",v),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("sns-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={aws_access_key:d.get("aws_access_key").trim(),aws_region:d.get("aws_region").trim(),aws_sns_sender_id:d.get("aws_sns_sender_id").trim()},k=d.get("aws_secret_key").trim();k&&(v.aws_secret_key=k);try{await m.put("/settings/notifications",v),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{o.disabled=!1,o.innerHTML=u}}),document.getElementById("resource-form").addEventListener("submit",async l=>{l.preventDefault();let o=l.target.querySelector('[type="submit"]'),u=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(l.target),v={resource_ram_reserve_gb:d.get("resource_ram_reserve_gb").trim(),resource_poll_interval:d.get("resource_poll_interval").trim(),resource_throttle_pct:d.get("resource_throttle_pct").trim(),resource_webhook_url:d.get("resource_webhook_url").trim()};try{await m.put("/settings/resources",v),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=u}})}function Ut(t){return`
        ...
        </div>`}async function z(t){let e=document.getElementById("basicauth-panel");if(e)try{let[a,s]=await Promise.all([m.get(`/sites/${t}/basicauth`),m.get(`/sites/${t}/basicauth/users`)]),i=e.querySelector("#ba-enabled"),n=e.querySelector("#ba-realm");i&&(i.checked=!!a.Enabled),n&&(n.value=a.Realm??"Restricted"),Ae(e,s??[])}catch(a){c.error("Failed to load basic auth settings: "+a.message)}}function Ae(t,e){let a=t.querySelector("#ba-users-list");if(a){if(!e.length){a.innerHTML='<p class="kp-muted uk-text-small">No credentials configured.</p>';return}a.innerHTML=e.map(s=>`
        ...
        <tr>
            ...
            <td>
        ...
        </div>`}async function lt(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await m.get(`/sites/${e}/crons`);a.innerHTML=Vt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${S(s.message)}</p>`}}function Kt(t,e){let a=[],s=t.querySelector("#cron-modal"),i=t.querySelector("#cron-modal-title"),n=t.querySelector("#cron-modal-id"),r=t.querySelector("#cron-modal-label"),l=t.querySelector("#cron-modal-command"),o=t.querySelector("#cron-modal-schedule"),u=t.querySelector("#cron-schedule-preview"),d=t.querySelector("#cron-modal-enabled");o?.addEventListener("input",()=>{u.textContent=Ot(o.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{i.textContent="Add Cron Job",n.value="",r.value="",l.value="",o.value="",u.textContent="",d.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let v=l.value.trim(),k=o.value.trim();if(!v||!k){c.error("Command and schedule are required");return}let p={label:r.value.trim(),command:v,schedule:k,enabled:d.checked},b=n.value;try{b?(await m.put(`/sites/${e}/crons/${b}`,p),c.success("Cron job updated")):(await m.post(`/sites/${e}/crons`,p),c.success("Cron job created")),UIkit.modal(s).hide(),await lt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async v=>{let k=v.target.closest(".cron-detail-btn");if(k){let f=k.dataset.id,y=a.find(pt=>String(pt.ID)===f);if(!y)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let p=v.target.closest(".cron-edit-btn");if(p){let f=p.dataset.id,y=a.find(x=>String(x.ID)===f);if(!y)return;i.textContent="Edit Cron Job",n.value=y.ID,r.value=y.Label||"",l.value=y.Command,o.value=y.Schedule,u.textContent=Ot(y.Schedule),d.checked=y.Enabled,UIkit.modal(s).show();return}let b=v.target.closest(".cron-delete-btn");if(b){let f=b.dataset.id;if(!await E("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await m.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await lt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let g=v.target.closest(".cron-run-btn");if(g){let f=g.dataset.id;try{await m.post(`/sites/${e}/crons/${f}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let y=null;try{y=(await m.get(`/sites/${e}/crons`)).find(A=>String(A.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,pt=setInterval(async()=>{try{let I=await m.get(`/sites/${e}/crons`),A=I.find(Q=>String(Q.ID)===f);if(!A||A.LastRun!==y||Date.now()>x){clearInterval(pt),w(),a=I??[];let Q=t.querySelector("#cron-list-wrap");Q&&(Q.innerHTML=Vt(I)),A?.LastError?c.error(`Job failed: ${A.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async v=>{let k=v.target.closest(".cron-toggle");if(!k)return;let p=k.dataset.id;try{await m.patch(`/sites/${e}/crons/${p}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(b){c.error(b.message),k.checked=!k.checked}}),m.get(`/sites/${e}/crons`).then(v=>{a=v??[]}).catch(()=>{})}function Ot(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,i,n,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&i==="*"&&n==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&i==="*"&&n==="*"&&r!=="*"){let l=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(u=>l[parseInt(u)]??u).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function Yt(t){return`
        <div class="kp-card uk-padding uk-margin-top" id="fm-root" data-site="${t}">
            ...
            <div id="fm-list"></div>
        </div>`}function He(){let t=P?P.split("/"):[],e="",a=['<a href="#" data-path="">html</a>'];for(let s of t)e=e?e+"/"+s:s,a.push(`<span class="kp-fm-sep">/</span><a href="#" data-path="${S(e)}">${S(s)}</a>`);return a.join("")}function Fe(t){let e=new Date(t);return isNaN(e)?"":e.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Jt(t){return t==="d"?"folder":t==="l"?"link":"file-text"}function W(t,e){return t?t+"/"+e:e}var Xt=new Set(["php","js","jsx","ts","tsx","css","scss","sass","less","html","htm","xml","json","txt","md","markdown","yml","yaml","ini","conf","cnf","toml","env","sh","bash","sql","log","csv","tsv","svg","htaccess","gitignore","lock","map"]);function Ue(t,e){if(e)return!1;let a=t.lastIndexOf(".");return t.startsWith(".")&&a===0?Xt.has(t.slice(1).toLowerCase()):a>=0&&Xt.has(t.slice(a+1).toLowerCase())}function Ne(t){if(!t||!t.length)return H("folder","This folder is empty");let e=document.getElementById("fm-root").dataset.site;return`
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            <thead>
                <tr>
            ...
            <tbody>${t.map(s=>{let i=W(P,s.name),n=s.is_dir,r=Ue(s.name,n),l=n?`<a href="#" class="fm-nav" data-path="${S(i)}"><span uk-icon="icon: ${Jt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</a>`:`<span><span uk-icon="icon: ${Jt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</span>`;return`
            <tr data-path="${S(i)}" data-name="${S(s.name)}" data-dir="${n?1:0}" data-mode="${S(s.mode)}">
                ...
                <td class="uk-text-nowrap uk-text-small kp-muted">${Fe(s.mod_time)}</td>
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Y(),a&&(a.innerHTML=He());try{let s=await m.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=Ne(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Gt(t,e){P=e||"",C(t)}function X(t,e,a=""){return new Promise(s=>{let i="fm-prompt-modal";document.getElementById(i)?.remove();let n=document.createElement("div");n.id=i,n.setAttribute("uk-modal",""),n.innerHTML=`
            ...
            </div>`,document.body.appendChild(n),window.UIkit&&UIkit.icon(n);let r=UIkit.modal(n),l=n.querySelector("#fm-prompt-input"),o=!1,u=d=>{o||(o=!0,s(d),r.hide())};n.querySelector("#fm-prompt-ok").addEventListener("click",()=>u(l.value.trim()||null)),l.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),u(l.value.trim()||null))}),UIkit.util.on(n,"hidden",()=>{o||(o=!0,s(null)),n.remove()}),r.show(),setTimeout(()=>l.focus(),50)})}async function We(t,e){let a=W(P,e.name),s=await fetch(`/api/sites/${t}/files/upload?path=${encodeURIComponent(a)}`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:e}),i=s.status===204?null:await s.json().catch(()=>null);if(!s.ok)throw new Error(i?.error||`HTTP ${s.status}`)}function Zt(t,e){P="",t.addEventListener("click",a=>{let s=a.target.closest("#fm-breadcrumb a");if(s){a.preventDefault(),Gt(e,s.dataset.path);return}let i=a.target.closest(".fm-nav");if(i){a.preventDefault(),Gt(e,i.dataset.path);return}let n=a.target.closest(".fm-edit");if(n){a.preventDefault(),ze(e,n.dataset.path,n.dataset.path.split("/").pop());return}}),t.querySelector("#fm-new-file")?.addEventListener("click",async()=>{let a=await X("New File","File name");if(a)try{await m.post(`/sites/${e}/files/file`,{path:W(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-new-dir")?.addEventListener("click",async()=>{let a=await X("New Folder","Folder name");if(a)try{await m.post(`/sites/${e}/files/dir`,{path:W(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-upload")?.addEventListener("change",async a=>{let s=[...a.target.files];if(s.length)try{for(let i of s)await We(e,i);c.success(s.length===1?"File uploaded":`${s.length} files uploaded`),C(e)}catch(i){c.error(i.message)}finally{a.target.value=""}}),t.querySelector("#fm-refresh")?.addEventListener("click",()=>C(e)),t.addEventListener("click",async a=>{let s=a.target.closest("tr[data-path]");if(!s)return;let i=s.dataset.path,n=s.dataset.name;if(a.target.closest(".fm-chmod")){let r=await X("Permissions",`Octal mode for "${n}"`,s.dataset.mode);if(!r)return;try{await m.patch(`/sites/${e}/files/chmod`,{path:i,mode:r}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-rename")){let r=await X("Rename / Move","New path (relative to current folder)",n);if(!r||r===n)return;try{await m.post(`/sites/${e}/files/move`,{src:i,dst:W(P,r)}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-copy")){let r=await X("Copy","Destination name",n+"-copy");if(!r)return;try{await m.post(`/sites/${e}/files/copy`,{src:i,dst:W(P,r)}),C(e)}catch(l){c.error(l.message)}return}if(a.target.closest(".fm-delete")){if(!await E("Delete",`Delete "${n}"? This cannot be undone.`))return;try{await m.delete(`/sites/${e}/files?path=${encodeURIComponent(i)}`),C(e)}catch(l){c.error(l.message)}return}})}var rt=null;function Qt(t){if(document.querySelector(`link[href="${t}"]`))return;let e=document.createElement("link");e.rel="stylesheet",e.href=t,document.head.appendChild(e)}function wt(t){return new Promise((e,a)=>{let s=document.querySelector(`script[src="${t}"]`);if(s){if(s.dataset.loaded)return e();s.addEventListener("load",()=>e()),s.addEventListener("error",()=>a(new Error("failed to load "+t)));return}let i=document.createElement("script");i.src=t,i.addEventListener("load",()=>{i.dataset.loaded="1",e()}),i.addEventListener("error",()=>a(new Error("failed to load "+t))),document.head.appendChild(i)})}function je(){if(rt)return rt;let t="https://cdn.jsdelivr.net/npm/codemirror@5";return Qt(`${t}/lib/codemirror.css`),Qt(`${t}/theme/material-darker.css`),rt=wt(`${t}/lib/codemirror.js`).then(()=>Promise.all([wt(`${t}/mode/meta.js`),wt(`${t}/addon/mode/loadmode.js`)])).then(()=>{window.CodeMirror.modeURL=`${t}/mode/%N/%N.js`}),rt}function Oe(t){let e=window.CodeMirror.findModeByFileName(t);return e?e.mode:null}async function ze(t,e,a){let s;try{s=await m.get(`/sites/${t}/files/content?path=${encodeURIComponent(e)}`)}catch(k){let p=/too large/i.test(k.message)?"File is too large to edit \u2014 download it instead.":/binary/i.test(k.message)?"Binary file \u2014 download it instead of editing.":k.message;c.error(p);return}try{await je()}catch(k){c.error("Editor failed to load: "+k.message);return}let i="fm-editor-modal";document.getElementById(i)?.remove();let n=document.createElement("div");n.id=i,n.setAttribute("uk-modal",""),n.innerHTML=`
        ...
        </div>`}function te(t,e){let a=null,s=!1,i=t.querySelector("#log-output"),n=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),l=t.querySelector("#log-clear"),o=t.querySelector("#log-autoscroll"),u=t.querySelector("#log-status");function d(p){for(p.split(`
`).forEach(b=>{if(!b)return;let g=document.createElement("div");g.className=b.match(/WAF BLOCK/i)?"kp-log-line-err":b.match(/WAF DETECT/i)?"kp-log-line-warn":b.match(/error|crit|emerg/i)?"kp-log-line-err":b.match(/warn/i)?"kp-log-line-warn":b.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=b,i.appendChild(g)});i.childElementCount>Ve;)i.removeChild(i.firstChild);o.checked&&(i.scrollTop=i.scrollHeight)}function v(){a&&(a.close(),a=null),s=!1,n.disabled=!1,r.disabled=!0,u&&(u.textContent="Disconnected")}n.addEventListener("click",()=>{v();let p=t.querySelector("#log-container").value,b=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",f=p==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${b}`:p==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${b}`:p==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${b}`:`${g}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${b}`;a=new WebSocket(f),a.onopen=()=>{s=!0,n.disabled=!0,r.disabled=!1,u&&(u.textContent=`Connected \u2014 ${p}`)},a.onmessage=y=>d(y.data),a.onerror=()=>{},a.onclose=()=>{s=!1,n.disabled=!1,r.disabled=!0,u&&(u.textContent="Disconnected")}}),r.addEventListener("click",v),l.addEventListener("click",()=>{i.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(v(),n.click())});let k=h.go.bind(h);h.go=function(p,b={}){return a&&v(),k(p,b)}}function Ke(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ee(t,e){try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Ke(a.status))}catch{}}function ae(t){t.forEach(e=>ee(e.Domain,e.ID))}function se(t,e,a,s=0,i=null){let n=t.SiteType!==3&&t.PMAPort>0;return`
        ...
        </div>`}function me(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),i=t.querySelector("#wpcli-run"),n=t.querySelector("#wpcli-clear"),r=t.querySelector("#wpcli-status"),l=[],o=-1;function u(k,p=""){k.split(`
`).forEach(b=>{if(!b)return;let g=document.createElement("div");p?g.className=p:g.className=b.match(/error|fatal|critical/i)?"kp-log-line-err":b.match(/warning|warn/i)?"kp-log-line-warn":b.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=b,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function d(k){if(k=k.trim(),!k)return;l.unshift(k),o=-1,u(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,i.disabled=!0,r&&(r.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",b=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);b.onopen=()=>{b.send(JSON.stringify({command:k}))},b.onmessage=g=>{let f=g.data;if(f.trim()==="[done]"){b.close();return}if(f.startsWith("[info]")){u(f,"kp-muted");return}if(f.startsWith("[error]")){u(f,"kp-log-line-err");return}u(f)},b.onerror=()=>{u("[error] WebSocket connection failed","kp-log-line-err")},b.onclose=()=>{s.disabled=!1,i.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}i.addEventListener("click",()=>{d(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){d(s.value),s.value="",o=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),o<l.length-1&&(o++,s.value=l[o]);return}k.key==="ArrowDown"&&(k.preventDefault(),o>0?(o--,s.value=l[o]):(o=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let b=p.indexOf("''")+1;s.setSelectionRange(b,b);return}d(p)})}),n.addEventListener("click",()=>{a.innerHTML=""});let v=h.go.bind(h);h.go=function(k,p={}){return v(k,p)},s.focus()}var G=null,M=null;function ke(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),i=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(l,o=!1){UIkit.switcher(a).show(l),e.querySelectorAll(":scope > li[data-pill]").forEach(u=>u.classList.remove("kp-pill-active")),o?(s?.classList.add("kp-pill-active"),i?.querySelectorAll("a[data-switcher]").forEach(u=>{u.classList.toggle("kp-dd-active",parseInt(u.dataset.switcher,10)===l)})):(s?.classList.remove("kp-pill-active"),i?.querySelectorAll("a[data-switcher]").forEach(u=>u.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(l=>{l.addEventListener("click",o=>{o.preventDefault();let u=parseInt(l.closest("li").dataset.pill,10);n(u,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",l=>{l.stopPropagation(),i.hidden=!i.hidden,s.classList.toggle("kp-pill-active",!i.hidden)}),i?.querySelectorAll("a[data-switcher]").forEach(l=>{l.addEventListener("click",o=>{o.preventDefault(),i.hidden=!0,n(parseInt(l.dataset.switcher,10),!0)})}),document.addEventListener("click",l=>{i&&!s.contains(l.target)&&(i.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(0)}function Ge(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}async function Qe(t){let e=document.getElementById("rp-routes-list");if(e)try{let a=await m.get(`/sites/${t}/rp-routes`);e.innerHTML=a.length?a.map(s=>$t(s.Domain,s.Upstream,s.PassHost)).join(""):$t()}catch(a){c.error("Failed to load routes: "+a.message)}}function Ye(t,e){t.addEventListener("click",async a=>{if(a.target.closest("#rp-add-row")){document.getElementById("rp-routes-list").insertAdjacentHTML("beforeend",$t());return}if(a.target.closest(".rp-remove-row")){a.target.closest(".rp-route-row").remove();return}if(!a.target.closest("#rp-save-btn"))return;let s=a.target.closest("#rp-save-btn"),i=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let n=[...document.querySelectorAll(".rp-route-row")].map(r=>({Domain:r.querySelector('[data-field="domain"]').value.trim(),Upstream:r.querySelector('[data-field="upstream"]').value.trim(),PassHost:r.querySelector('[data-field="pass_host"]').checked})).filter(r=>r.Domain&&r.Upstream);try{await m.put(`/sites/${e}/rp-routes`,n),c.success("Routes saved")}catch(r){c.error(r.message)}finally{s.disabled=!1,s.innerHTML=i}},{signal:G.signal})}function Ze(t){return t.endsWith("-nginx")?"world":t.endsWith("-php")?"code":t.endsWith("-db")?"database":t.endsWith("-redis")?"server":t.endsWith("-varnish")?"grid":t.endsWith("-pma")?"table":t.endsWith("-app")?"laptop":"bolt"}function be(t){let e=t.split("-").pop();return{nginx:"Nginx",php:"PHP-FPM",db:"MariaDB",redis:"Redis",varnish:"Varnish",pma:"phpMyAdmin",app:"App"}[e]??e}function Et(t){switch(t){case"healthy":return"var(--kp-success)";case"unhealthy":return"var(--kp-danger)";case"starting":return"var(--kp-warning)";default:return"var(--kp-text-dim)"}}function ta(t){return!t||!t.length?"":t.filter(e=>!e.name.endsWith("-infra")).map(e=>`
            <span class="kp-health-badge"
                data-container="${e.name}"
                title="Restart the Container"
                style="cursor:pointer;color:${Et(e.status)}">
                <span uk-icon="icon: ${Ze(e.name)}; ratio: 1.1"></span>
                <span class="kp-health-badge-label">${be(e.name)}</span>
            </span>
        `).join("")}function ea(t,e){M&&(M.close(),M=null);let a=document.getElementById("sd-health-badges");if(!a)return;let s=location.protocol==="https:"?"wss":"ws";M=new WebSocket(`${s}://${location.host}/api/sites/${e}/health/stream`),M.onmessage=i=>{try{let n=JSON.parse(i.data);a.innerHTML=ta(n),a.querySelectorAll(".kp-health-badge").forEach(r=>{r.addEventListener("click",async()=>{r.style.color=Et("starting");let l=r.dataset.container,o=l.split("-").pop();try{await m.post(`/sites/${e}/containers/${o}/restart`),c.success(`${be(l)} restarted`)}catch(u){r.style.color=Et("none"),c.error(u.message)}})})}catch{}},M.onerror=()=>{},M.onclose=()=>{M=null}}async function ve(t,{id:e}){let[{site:a,domains:s,sftp:i},n,r]=await Promise.all([m.get(`/sites/${e}`),m.get("/sites"),m.get(`/sites/${e}/configs`)]),l=Array.isArray(n)?n:[],o=a.SiteType===1||a.SiteType===2,u=a.SiteType===6,d=[1,2,4,5].includes(a.SiteType);if(t.innerHTML=`
        <div class="kp-view-header">
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${se(a,s??[],i,a.ParentID??0,l.find(p=>p.ID===a.ParentID)?.Name??null)}</li>
            <li>${vt(e,a.SiteType)}</li>
            <li>${J(e,1,r[1])}</li>
            ${o?`<li>${J(e,2,r[2])}</li>`:""}
            <li>${J(e,3,r[3])}</li>
            ...
            <li>${le()}</li>
            <li>${Yt(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>tt(a)),document.getElementById("sd-site-nav")?.addEventListener("change",p=>{h.go("site-detail",{id:p.target.value})}),ot(t),q(t),te(t,e),G&&G.abort(),G=new AbortController,ue(t,e,G.signal),St(e),u){Ye(t,e),Qe(e),gt(t,e,a.SiteType),ht(e,a.SiteType),yt(t,e),z(e),ke(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{$("Recreating Pod","Recreating containers for this site...");try{await m.post(`/sites/${e}/recreate`),w(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(p){w(),c.error(p.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let p=await Z(a.Name);if(p){$("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await m.post(`/sites/${e}/clone`,{name:p},6e5),w(),c.success(`Site cloned as '${p}'`),h.go("sites")}catch(b){w(),c.error(b.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(p=>{p.addEventListener("click",async()=>{let b=p.dataset.action;if(b==="flush"){try{await m.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(f){c.error(f.message)}return}$(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await m.post(`/sites/${e}/${b}`),w(),c.success(`Site ${b} successful`),h.go("site-detail",{id:e})}catch(f){w(),c.error(f.message)}})}),jt(t,e),ie(t,e),a.SiteType===1&&me(t,e),oe(t,e,a),Nt(t,e),N(t,e),d&&(Kt(t,e),lt(t,e)),de(t,e),ce(e);let v=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');v&&k&&(k.dataset.switcher=String(v.children.length-1)),Zt(t,e),C(e),ea(t,e),gt(t,e,a.SiteType),ht(e,a.SiteType),ke(t),yt(t,e),z(e),ae(s??[])}async function ct(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(i=>i.remove()),typeof QRCode<"u")try{let i=await new Promise((n,r)=>{QRCode.toDataURL(t,{width:220,margin:2},(l,o)=>{l?r(l):n(o)})});e.src=i,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function dt(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
