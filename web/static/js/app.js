"use strict";(()=>{var d={async _req(e,t,s,a=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),a),l={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};s!==void 0&&(l.body=JSON.stringify(s));try{let r=await fetch("/api"+t,l);clearTimeout(i);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(i),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e)};var K=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',S=e=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let s=UIkit.modal("#kp-edit-site-modal"),a=document.getElementById("es-site-type"),n=document.getElementById("es-php-version-wrap"),i=document.getElementById("es-node-version-wrap"),l=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),c=document.getElementById("es-wordpress-wrap");s.show();let p=m=>{n.classList.toggle("uk-hidden",m!==1&&m!==2),i.classList.toggle("uk-hidden",m!==4),l.classList.toggle("uk-hidden",m!==5),r.classList.toggle("uk-hidden",m!==4&&m!==5),c.classList.toggle("uk-hidden",m!==1)};p(e.SiteType),a.addEventListener("change",()=>p(parseInt(a.value))),document.getElementById("edit-site-form").addEventListener("submit",async m=>{m.preventDefault();let v=m.target.querySelector('[type="submit"]'),k=v.innerHTML;v.disabled=!0,v.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let g=new FormData(m.target),b=parseInt(g.get("site_type")),h=null;b===4&&(h=parseInt(g.get("node_version"))),b===5&&(h=parseInt(g.get("dotnet_version")));let x={name:g.get("name").trim(),php_version:parseInt(g.get("php_version"))||3,site_type:b,runtime_version:h,start_command:g.get("start_command")?.trim()||""},be=b===1?g.get("install_wordpress")==="on":!1;try{await d.put(`/sites/${e.ID}`,x),s.hide(),document.getElementById("kp-edit-site-modal")?.remove(),y("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:be}),f(),o.success("Site updated and pod recreated")}catch(O){f(),o.error("Site saved but pod recreate failed: "+O.message)}u.go("site-detail",{id:String(e.ID)})}catch(O){o.error(O.message),v.disabled=!1,v.innerHTML=k}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function H(){document.body.insertAdjacentHTML("beforeend",`
        ...
        <tr>
            ...
            <td>
                <div class="uk-flex" style="gap:6px">
                    ...
                    </div>`}else l.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=he(a))}catch(s){let a=e.querySelector("#backup-list-wrap");a&&(a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${s.message}</p>`)}}function Z(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,s),o.success("Backup destinations saved")}catch(a){o.error(a.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(i){o.error(i.message);return}y("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let a=Date.now()+10*60*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>s||Date.now()>a)&&(clearInterval(n),f(),await B(e,t),Date.now()<=a&&o.success("Backup complete"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let a=s.target.closest(".backup-restore-btn");if(a){let l=a.dataset.id;if(!await w("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${l}/restore`)}catch(v){o.error(v.message);return}y("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let c=Date.now(),p=Date.now()+15*60*1e3,m=setInterval(async()=>{try{let v=await d.get(`/sites/${t}/backups/restore-status`);(!v?.active||Date.now()>p)&&(clearInterval(m),f(),v?.active?o.error("Restore timed out"):o.success("Restore complete"),await B(e,t))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let l=n.dataset.id;if(!await w("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;y("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${l}`),f(),o.success("Snapshot deleted"),await B(e,t)}catch(c){f(),o.error(c.message)}}let i=s.target.closest(".backup-download-btn");if(i){let l=i.dataset.id;y("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${l}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{f()},5e3)},300);return}})}var N={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis"};function I(e,t,s){let a=s?Object.entries(s):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{i.textContent="Copied!"});else{let l=document.createElement("textarea");l.value=n,l.style.cssText="position:fixed;opacity:0",document.body.appendChild(l),l.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}l.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{a.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),u.go("users")})}function ue(e){document.body.insertAdjacentHTML("beforeend",`
    ...
    </tr>`}function xe(e){e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="delete-user"]');if(!(!s||!await w("Delete User","Delete this user? This cannot be undone.")))try{await d.delete(`/users/${s.dataset.uid}`),s.closest("tr").remove(),o.success("User deleted")}catch(n){o.error(n.message)}}),e.addEventListener("click",async t=>{let s=t.target.closest('[data-action="edit-user"]');s&&me(e,s.dataset.uid)})}u.register("dashboard",e=>G(e));u.register("sites",e=>Q(e));u.register("site-detail",(e,t)=>pe(e,t));u.register("users",e=>ke(e));u.register("settings",e=>W(e));u.register("security",e=>Y(e));document.addEventListener("click",e=>{let t=e.target.closest("[data-view]");if(!t)return;e.preventDefault(),u.go(t.dataset.view);let s=document.getElementById("kp-offcanvas");s&&UIkit.offcanvas(s).hide()});document.addEventListener("click",async e=>{let t=e.target.closest("[data-action]");if(!t)return;e.stopPropagation();let{action:s,id:a}=t.dataset;switch(s){case"manage":u.go("site-detail",{id:a});break;case"start":await C(a,"start","Starting Site","Starting all containers - please wait...");break;case"stop":await C(a,"stop","Stopping Site","Gracefully stopping all containers - please wait...");break;case"restart":await C(a,"restart","Restarting Site","Restarting all containers - please wait...");break;case"flush":await C(a,"flush","Flushing Caches","Clearing container caches - please wait...");break;case"edit":{let n=await d.get(`/sites/${a}`);_(n.site);break}case"delete":await Se(a);break;case"update":await C(a,"update","Updating Images","Pulling latest container images - this may take a few minutes...");break;case"recreate":y("Recreating Pod","Recreating containers for this site - this may take a few minutes...");try{await d.post(`/sites/${a}/recreate`),f(),o.success("Pod recreated"),u.go("site-detail",{id:a})}catch(n){f(),o.error(n.message)}break}});async function C(e,t,s,a){y(s,a);try{await d.post(`/sites/${e}/${t}`),f(),o.success(s+" complete"),["start","stop","restart"].includes(t)&&u.go("sites")}catch(n){f(),o.error(n.message)}}async function Se(e){if(!await w("Delete Site","This will stop and permanently remove the pod and all its data. Are you sure?"))return;y("Deleting Site","Stopping containers and removing the pod - please wait...");try{await d.delete(`/sites/${e}`)}catch{}let s=!1,a=0;for(;!s&&a<10;){try{await new Promise(i=>setTimeout(i,2e3)),s=!(await d.get("/sites")).find(i=>i.ID===parseInt(e))}catch{}a++}f(),s?(o.success("Site deleted"),u.go("sites")):o.error("Delete failed - site still exists after 20s")}window.addEventListener("hashchange",()=>{let{view:e,params:t}=F();u.go(e,t)});var{view:$e,params:Ee}=F();u.go($e,Ee);})();
