...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:n.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let o=await fetch("/api"+e,r);clearTimeout(i);let l=o.status===204?null:await o.json().catch(()=>null);if(o.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!o.ok)throw new Error(l?.error||`HTTP ${o.status}`);return l}catch(o){throw clearTimeout(i),o}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var S=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function L(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Y=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",d);let b=UIkit.modal(`#${n}`),k=document.getElementById("kp-sync-ok"),u=document.getElementById("kp-sync-cancel"),v=g=>{b.hide(),setTimeout(()=>document.getElementById(n)?.remove(),300),s(g)};k.addEventListener("click",()=>v(!0),{once:!0}),u.addEventListener("click",()=>v(!1),{once:!0}),document.getElementById(n).addEventListener("hidden",()=>v(!1),{once:!0}),b.show()})}var h={routes:{},_ownHashChange:!1,register(t,e){this.routes[t]=e},async go(t,e={}){let a=Object.keys(e).length?t+"/"+Object.values(e).join("/"):t;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(i=>{i.classList.toggle("kp-active",i.dataset.view===t)}),document.querySelectorAll(".kp-bn-item[data-view]").forEach(i=>{i.classList.toggle("kp-active",i.dataset.view===t)});let s=this.routes[t];if(!s)return;let n=document.getElementById("kp-view");n.innerHTML=Y();try{await s(n,e)}catch(i){n.innerHTML=_(i.message)}}};function kt(){let e=(window.location.hash.replace("#","")||"dashboard").split("/"),a=e[0],s={};return a==="site-detail"&&e[1]&&(s.id=e[1]),{view:a,params:s}}var c={show(t,e="info",a=7e3){let s={success:"check",error:"warning",info:"info"},n=document.createElement("div");n.className=`kp-toast kp-toast-${e}`,n.innerHTML=`<span uk-icon="${s[e]||"info"}"></span><span>${S(t)}</span>`,document.getElementById("kp-toasts").appendChild(n),UIkit.icon(n.querySelector("[uk-icon]")),setTimeout(()=>n.remove(),a)},success:t=>c.show(t,"success"),error:t=>c.show(t,"error"),info:t=>c.show(t,"info")};async function tt(t){let e=`
        ...
        </div>`}function Se(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),n=t.querySelector("#admin-log-connect"),i=t.querySelector("#admin-log-disconnect"),r=t.querySelector("#admin-log-clear"),o=t.querySelector("#admin-log-autoscroll"),l=t.querySelector("#admin-log-status");function p(k){for(k.split(`
`).forEach(u=>{if(!u)return;let v=document.createElement("div");v.className=u.match(/WAF BLOCK/i)?"kp-log-line-err":u.match(/WAF DETECT/i)?"kp-log-line-warn":u.match(/error|crit|emerg/i)?"kp-log-line-err":u.match(/warn/i)?"kp-log-line-warn":u.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=u,s.appendChild(v)});s.childElementCount>we;)s.removeChild(s.firstChild);o.checked&&(s.scrollTop=s.scrollHeight)}function d(){e&&(e.close(),e=null),a=!1,n.disabled=!1,i.disabled=!0,l&&(l.textContent="Disconnected")}n.addEventListener("click",()=>{d();let k=t.querySelector("#admin-log-source").value,u=t.querySelector("#admin-log-tail").value,v=location.protocol==="https:"?"wss":"ws",g=k==="waf"?`${v}://${location.host}/api/logs/waf?tail=${u}`:`${v}://${location.host}/api/logs/proxy?tail=${u}`;e=new WebSocket(g),e.onopen=()=>{a=!0,n.disabled=!0,i.disabled=!1,l&&(l.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=f=>p(f.data),e.onerror=()=>{},e.onclose=()=>{a=!1,n.disabled=!1,i.disabled=!0,l&&(l.textContent="Disconnected")}}),i.addEventListener("click",d),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(d(),n.click())});let b=h.go.bind(h);h.go=function(k,u={}){return e&&d(),b(k,u)}}function Tt(t){t.innerHTML=xe(),Se(t)}var at=50;function $e(t,e,a){let s=Math.max(1,Math.ceil(t.total/at)),n=(t.entries??[]).map(_t).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        ...
        </div>`}function _t(t){let e=new Date(t.ts).toLocaleString(),a=t.username?`<span style="font-family:monospace">${T(t.username)}</span>`:'<span style="color:var(--kp-text-dim)">\u2014</span>',s=Ee(t.status),i=t.prior_state||t.new_state?`<button class="uk-button kp-btn-ghost kp-btn-sm al-diff-btn"
...
`).map(x=>x.trim()).filter(Boolean),install_wordpress:v===1?u.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",w);try{await m.post("/sites",f,6e5),y(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){y(),c.error(x.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function _e(t){return`${t.toFixed(1)}%`}var nt=null;function bt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function vt(t,e){return`
        ...
        </table>`}function Ie(t,e,a,s){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let n=[...t].sort((d,b)=>{let k,u;switch(a){case"time":k=d.time,u=b.time;break;case"method":k=d.method,u=b.method;break;case"ip":k=d.client_ip,u=b.client_ip;break;default:k=d.status,u=b.status;break}return k<u?s?1:-1:k>u?s?-1:1:0}),i=50,r=Math.ceil(n.length/i),l=n.slice(e*i,(e+1)*i).map(d=>{let b=d.ua,k=d.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
            ...
            <div class="kp-site-card-header">
                <div>
                    ...
                    <div class="kp-site-meta">
                        ...
                        <span class="kp-site-meta-item"><span uk-icon="icon: code; ratio: 0.75"></span> ${W(t.SiteType)}${R(t)?" / "+R(t):""}</span>
        ...
        </div>`}function Me(){let t=document.getElementById("sites-bulk-bar"),e=document.getElementById("sites-bulk-count"),a=document.getElementById("sites-select-all"),s=document.getElementById("sites-search"),n=document.querySelector(".kp-table-wrap tbody");if(!t||!a)return;let i=null,r=!0,o=()=>[...document.querySelectorAll(".kp-site-row-check:checked")],l=()=>{let v=o().length;e.textContent=`${v} selected`,["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(w=>{let x=document.getElementById(w);x&&(x.disabled=v===0)});let g=document.getElementById("kp-bulk-mobile-btn");g&&(g.disabled=v===0);let f=document.querySelectorAll(".kp-site-row-check");a.indeterminate=v>0&&v<f.length,a.checked=f.length>0&&v===f.length},p=()=>{let u=s.value.trim().toLowerCase();document.querySelectorAll(".kp-table-wrap tbody tr").forEach(v=>{let g=v.querySelector(".kp-site-row-name")?.textContent.toLowerCase()??"",f=v.querySelector("td:nth-child(6)")?.textContent.toLowerCase()??"";v.style.display=!u||g.includes(u)||f.includes(u)?"":"none"})},d=u=>{i===u?r=!r:(i=u,r=!0),document.querySelectorAll(".kp-sort-icon").forEach(g=>{g.textContent=g.dataset.col===u?r?" \u2191":" \u2193":" \u2195"});let v=[...n.querySelectorAll("tr")];v.sort((g,f)=>{let w="",x="";return u==="name"?(w=g.querySelector(".kp-site-row-name")?.textContent??"",x=f.querySelector(".kp-site-row-name")?.textContent??""):u==="status"?(w=g.dataset.status??"",x=f.dataset.status??""):u==="type"?(w=g.dataset.type??"",x=f.dataset.type??""):u==="domain"&&(w=g.querySelector("td:nth-child(6)")?.textContent.trim()??"",x=f.querySelector("td:nth-child(6)")?.textContent.trim()??""),r?w.localeCompare(x):x.localeCompare(w)}),v.forEach(g=>n.appendChild(g))};a.addEventListener("change",()=>{document.querySelectorAll(".kp-site-row-check").forEach(u=>{u.checked=a.checked}),l()}),n?.addEventListener("change",u=>{u.target.classList.contains("kp-site-row-check")&&l()}),s?.addEventListener("input",p),document.querySelectorAll(".kp-sortable").forEach(u=>{u.addEventListener("click",()=>d(u.dataset.col))}),["bulk-start","bulk-stop","bulk-restart","bulk-flush","bulk-recreate"].forEach(u=>{let v=u.replace("bulk-","");document.getElementById(u)?.addEventListener("click",()=>{let g=o().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:v,ids:g}}))})});let b=document.getElementById("kp-bulk-mobile-pill"),k=document.getElementById("kp-bulk-mobile-dropdown");document.getElementById("kp-bulk-mobile-btn")?.addEventListener("click",u=>{u.stopPropagation(),k.hidden=!k.hidden}),document.addEventListener("click",u=>{k&&!b?.contains(u.target)&&(k.hidden=!0)},{capture:!0}),["start","stop","restart","flush","recreate"].forEach(u=>{document.getElementById(`bulk-mobile-${u}`)?.addEventListener("click",v=>{v.preventDefault(),k.hidden=!0;let g=o().map(f=>f.dataset.siteId);document.dispatchEvent(new CustomEvent("kp:bulk-action",{detail:{action:u,ids:g}}))})}),document.querySelectorAll(".kp-sort-icon").forEach(u=>{u.textContent=" \u2195"}),l()}var it=null;async function Dt(t){let[e,a,s]=await Promise.all([m.get("/sites").catch(()=>[]),m.get("/stats/traffic").catch(()=>null),m.get("/stats/pod").catch(()=>null)]),n=e.filter(o=>o.SiteStatus===1).length,i=e.filter(o=>o.SiteStatus===2).length,r=e.filter(o=>o.SiteStatus===4).length;if(t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${O(null)}`,ot(t),q(t)}function De(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function At(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=De(a.status)}catch{}}async function Ht(t){if(!B()){t.innerHTML=_("Access denied");return}let[e,a,s,n,i,r]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&At(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),p=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await m.put("/settings",b),c.success("Settings saved"),At(b.admin_domain)}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=p}}),document.getElementById("settings-import").addEventListener("change",async o=>{let l=o.target.files[0];if(!l)return;let p=new FormData;p.append("file",l);try{let d=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:p}),b=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(b?.error||`HTTP ${d.status}`);c.success("Settings imported")}catch(d){c.error(d.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),p=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={backup_schedule:d.get("backup_schedule").trim(),backup_retain_days:d.get("backup_retain_days").trim()};try{await m.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=p}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),p=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={s3_endpoint:d.get("s3_endpoint").trim(),s3_bucket:d.get("s3_bucket").trim(),s3_region:d.get("s3_region").trim(),s3_access_key:d.get("s3_access_key").trim()},k=d.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await m.put("/settings/backup",b),c.success("S3 settings saved")}catch(u){c.error(u.message)}finally{l.disabled=!1,l.innerHTML=p}}),document.getElementById("smtp-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),p=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={smtp_host:d.get("smtp_host").trim(),smtp_port:d.get("smtp_port").trim(),smtp_username:d.get("smtp_username").trim(),smtp_from:d.get("smtp_from").trim(),smtp_tls:d.get("smtp_tls")?"true":"false"},k=d.get("smtp_password").trim();k&&(b.smtp_password=k);try{await m.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(u){c.error(u.message)}finally{l.disabled=!1,l.innerHTML=p}}),document.getElementById("sns-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),p=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={aws_access_key:d.get("aws_access_key").trim(),aws_region:d.get("aws_region").trim(),aws_sns_sender_id:d.get("aws_sns_sender_id").trim()},k=d.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await m.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(u){c.error(u.message)}finally{l.disabled=!1,l.innerHTML=p}}),document.getElementById("resource-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),p=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={resource_ram_reserve_gb:d.get("resource_ram_reserve_gb").trim(),resource_poll_interval:d.get("resource_poll_interval").trim(),resource_throttle_pct:d.get("resource_throttle_pct").trim(),resource_webhook_url:d.get("resource_webhook_url").trim()};try{await m.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=p}})}function Ut(t){return`
        ...
        </div>`}function Ft(t,e){let a=Date.now()+18e5,s=setInterval(async()=>{try{let n=await m.get(`/sites/${e}/backups/restore-status`);(!n?.active||Date.now()>a)&&(clearInterval(s),y(),n?.active?c.error("Import timed out \u2014 check server logs"):c.success("Import complete"),await N(t,e))}catch{}},3e3)}async function N(t,e){try{let[a,s]=await Promise.all([m.get(`/sites/${e}/backup-repo`),m.get(`/sites/${e}/backups`)]),n=t.querySelector("#backup-local-enabled"),i=t.querySelector("#backup-s3-enabled");n&&(n.checked=!!a.LocalEnabled),i&&(i.checked=!!a.S3Enabled);let r=t.querySelector("#backup-error-banner");if(r)if(a.last_error){let l=a.last_error_at?` (${new Date(a.last_error_at).toLocaleString()})`:"";r.innerHTML=`
                    <div uk-alert class="uk-alert-warning">
                        <a class="uk-alert-close" uk-close></a>
                        <p><strong>Last scheduled backup failed${l}:</strong> ${a.last_error}</p>
                    </div>`}else r.innerHTML="";let o=t.querySelector("#backup-list-wrap");o&&(o.innerHTML=Re(s))}catch(a){let s=t.querySelector("#backup-list-wrap");s&&(s.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${a.message}</p>`)}}function Nt(t,e){t.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:t.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:t.querySelector("#backup-s3-enabled")?.checked??!1};try{await m.put(`/sites/${e}/backup-repo`,s),c.success("Backup destinations saved")}catch(n){c.error(n.message)}}),t.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await m.get(`/sites/${e}/backups`))?.length??0}catch{}try{await m.post(`/sites/${e}/backups`,{label:"manual"})}catch(r){c.error(r.message);return}$("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let n=Date.now()+1800*1e3,i=setInterval(async()=>{try{(((await m.get(`/sites/${e}/backups`))?.length??0)>s||Date.now()>n)&&(clearInterval(i),y(),await N(t,e),Date.now()<=n?c.success("Backup complete"):c.error("Backup is taking longer than expected \u2014 check server logs for status"))}catch{}},4e3)}),t.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let n=s.target.closest(".backup-restore-btn");if(n){let o=n.dataset.id;if(!await E("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await m.post(`/sites/${e}/backups/${o}/restore`)}catch(k){c.error(k.message);return}$("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let p=Date.now(),d=Date.now()+900*1e3,b=setInterval(async()=>{try{let k=await m.get(`/sites/${e}/backups/restore-status`);(!k?.active||Date.now()>d)&&(clearInterval(b),y(),k?.active?c.error("Restore timed out"):c.success("Restore complete"),await N(t,e))}catch{}},3e3);return}let i=s.target.closest(".backup-delete-btn");if(i){let o=i.dataset.id;if(!await E("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;$("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await m.delete(`/sites/${e}/backups/${o}`),y(),c.success("Snapshot deleted"),await N(t,e)}catch(p){y(),c.error(p.message)}}let r=s.target.closest(".backup-download-btn");if(r){let o=r.dataset.id;$("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let l=document.createElement("a");l.href=`/api/sites/${e}/backups/${o}/download`,l.style.display="none",document.body.appendChild(l),l.click(),document.body.removeChild(l),setTimeout(()=>{y()},5e3)},300);return}});let a=t.querySelector("#import-backup-modal");a&&(UIkit.util.on(a,"beforeshow",async()=>{let s=a.querySelector("#import-target-site");try{let i=await m.get("/sites");s.innerHTML=i.map(r=>`<option value="${r.ID}"${r.ID===e?" selected":""}>${r.Name}</option>`).join("")}catch{s.innerHTML='<option value="">Failed to load sites</option>'}let n=a.querySelector("#import-sftp-list");try{let i=await m.get(`/sites/${e}/backups/import/files`);!i||i.length===0?n.innerHTML='<p class="kp-muted uk-text-small">No files found.</p>':n.innerHTML=i.map(r=>`
        ...
        </div>`}async function z(t){let e=document.getElementById("basicauth-panel");if(e)try{let[a,s]=await Promise.all([m.get(`/sites/${t}/basicauth`),m.get(`/sites/${t}/basicauth/users`)]),n=e.querySelector("#ba-enabled"),i=e.querySelector("#ba-realm");n&&(n.checked=!!a.Enabled),i&&(i.value=a.Realm??"Restricted"),Ae(e,s??[])}catch(a){c.error("Failed to load basic auth settings: "+a.message)}}function Ae(t,e){let a=t.querySelector("#ba-users-list");if(a){if(!e.length){a.innerHTML='<p class="kp-muted uk-text-small">No credentials configured.</p>';return}a.innerHTML=e.map(s=>`
        ...
        </div>`}async function lt(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await m.get(`/sites/${e}/crons`);a.innerHTML=Vt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${S(s.message)}</p>`}}function Kt(t,e){let a=[],s=t.querySelector("#cron-modal"),n=t.querySelector("#cron-modal-title"),i=t.querySelector("#cron-modal-id"),r=t.querySelector("#cron-modal-label"),o=t.querySelector("#cron-modal-command"),l=t.querySelector("#cron-modal-schedule"),p=t.querySelector("#cron-schedule-preview"),d=t.querySelector("#cron-modal-enabled");l?.addEventListener("input",()=>{p.textContent=Ot(l.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",i.value="",r.value="",o.value="",l.value="",p.textContent="",d.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let b=o.value.trim(),k=l.value.trim();if(!b||!k){c.error("Command and schedule are required");return}let u={label:r.value.trim(),command:b,schedule:k,enabled:d.checked},v=i.value;try{v?(await m.put(`/sites/${e}/crons/${v}`,u),c.success("Cron job updated")):(await m.post(`/sites/${e}/crons`,u),c.success("Cron job created")),UIkit.modal(s).hide(),await lt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async b=>{let k=b.target.closest(".cron-detail-btn");if(k){let f=k.dataset.id,w=a.find(ut=>String(ut.ID)===f);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                <div id="cron-detail-modal" uk-modal>
                    <div class="uk-modal-dialog kp-modal uk-modal-body uk-width-large">
                        <button class="uk-modal-close-default" type="button" uk-close></button>
                        <h3 class="kp-view-title uk-margin-bottom">Run Details \u2014 ${S(w.Label||String(w.ID))}</h3>
                        <div class="uk-margin-small-bottom">
                            <label class="kp-label">Output</label>
                            <pre class="kp-cron-output">${S(w.LastOutput||"(no output)")}</pre>
                        </div>
                        <div class="uk-margin-small-top">
                            <label class="kp-label">Error</label>
                            <pre class="kp-cron-output kp-cron-output-error">${S(w.LastError||"(no error)")}</pre>
                        </div>
                    </div>
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let u=b.target.closest(".cron-edit-btn");if(u){let f=u.dataset.id,w=a.find(x=>String(x.ID)===f);if(!w)return;n.textContent="Edit Cron Job",i.value=w.ID,r.value=w.Label||"",o.value=w.Command,l.value=w.Schedule,p.textContent=Ot(w.Schedule),d.checked=w.Enabled,UIkit.modal(s).show();return}let v=b.target.closest(".cron-delete-btn");if(v){let f=v.dataset.id;if(!await E("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await m.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await lt(t,e),a=await m.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let g=b.target.closest(".cron-run-btn");if(g){let f=g.dataset.id;try{await m.post(`/sites/${e}/crons/${f}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await m.get(`/sites/${e}/crons`)).find(A=>String(A.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,ut=setInterval(async()=>{try{let I=await m.get(`/sites/${e}/crons`),A=I.find(Q=>String(Q.ID)===f);if(!A||A.LastRun!==w||Date.now()>x){clearInterval(ut),y(),a=I??[];let Q=t.querySelector("#cron-list-wrap");Q&&(Q.innerHTML=Vt(I)),A?.LastError?c.error(`Job failed: ${A.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let u=k.dataset.id;try{await m.patch(`/sites/${e}/crons/${u}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(v){c.error(v.message),k.checked=!k.checked}}),m.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function Ot(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,n,i,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&i==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&i==="*"&&r!=="*"){let o=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(p=>o[parseInt(p)]??p).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function Yt(t){return`
        ...
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            ...
            <tbody>${t.map(s=>{let n=j(P,s.name),i=s.is_dir,r=Ue(s.name,i),o=i?`<a href="#" class="fm-nav" data-path="${S(n)}"><span uk-icon="icon: ${Jt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</a>`:`<span><span uk-icon="icon: ${Jt(s.type)}; ratio: 0.9"></span> ${S(s.name)}</span>`;return`
            <tr data-path="${S(n)}" data-name="${S(s.name)}" data-dir="${i?1:0}" data-mode="${S(s.mode)}">
                <td class="kp-fm-name">${o}</td>
                <td class="uk-text-nowrap">${L(s.size,i)}</td>
                <td><code class="kp-mono">${S(s.mode)}</code></td>
                <td class="uk-text-nowrap uk-text-small kp-muted">${Fe(s.mod_time)}</td>
                <td class="uk-text-right uk-text-nowrap">
                    ${r?`<button class="kp-fm-act fm-edit" data-path="${S(n)}" uk-tooltip="Edit"><span uk-icon="icon: pencil; ratio: 0.85"></span></button>`:""}
            ...
            </div>`,document.body.appendChild(i),window.UIkit&&UIkit.icon(i);let r=UIkit.modal(i),o=i.querySelector("#fm-prompt-input"),l=!1,p=d=>{l||(l=!0,s(d),r.hide())};i.querySelector("#fm-prompt-ok").addEventListener("click",()=>p(o.value.trim()||null)),o.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),p(o.value.trim()||null))}),UIkit.util.on(i,"hidden",()=>{l||(l=!0,s(null)),i.remove()}),r.show(),setTimeout(()=>o.focus(),50)})}async function je(t,e){let a=j(P,e.name),s=await fetch(`/api/sites/${t}/files/upload?path=${encodeURIComponent(a)}`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:e}),n=s.status===204?null:await s.json().catch(()=>null);if(!s.ok)throw new Error(n?.error||`HTTP ${s.status}`)}function Zt(t,e){P="",t.addEventListener("click",a=>{let s=a.target.closest("#fm-breadcrumb a");if(s){a.preventDefault(),Gt(e,s.dataset.path);return}let n=a.target.closest(".fm-nav");if(n){a.preventDefault(),Gt(e,n.dataset.path);return}let i=a.target.closest(".fm-edit");if(i){a.preventDefault(),ze(e,i.dataset.path,i.dataset.path.split("/").pop());return}}),t.querySelector("#fm-new-file")?.addEventListener("click",async()=>{let a=await X("New File","File name");if(a)try{await m.post(`/sites/${e}/files/file`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-new-dir")?.addEventListener("click",async()=>{let a=await X("New Folder","Folder name");if(a)try{await m.post(`/sites/${e}/files/dir`,{path:j(P,a)}),C(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-upload")?.addEventListener("change",async a=>{let s=[...a.target.files];if(s.length)try{for(let n of s)await je(e,n);c.success(s.length===1?"File uploaded":`${s.length} files uploaded`),C(e)}catch(n){c.error(n.message)}finally{a.target.value=""}}),t.querySelector("#fm-refresh")?.addEventListener("click",()=>C(e)),t.addEventListener("click",async a=>{let s=a.target.closest("tr[data-path]");if(!s)return;let n=s.dataset.path,i=s.dataset.name;if(a.target.closest(".fm-chmod")){let r=await X("Permissions",`Octal mode for "${i}"`,s.dataset.mode);if(!r)return;try{await m.patch(`/sites/${e}/files/chmod`,{path:n,mode:r}),C(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-rename")){let r=await X("Rename / Move","New path (relative to current folder)",i);if(!r||r===i)return;try{await m.post(`/sites/${e}/files/move`,{src:n,dst:j(P,r)}),C(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-copy")){let r=await X("Copy","Destination name",i+"-copy");if(!r)return;try{await m.post(`/sites/${e}/files/copy`,{src:n,dst:j(P,r)}),C(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-delete")){if(!await E("Delete",`Delete "${i}"? This cannot be undone.`))return;try{await m.delete(`/sites/${e}/files?path=${encodeURIComponent(n)}`),C(e)}catch(o){c.error(o.message)}return}})}var rt=null;function Qt(t){if(document.querySelector(`link[href="${t}"]`))return;let e=document.createElement("link");e.rel="stylesheet",e.href=t,document.head.appendChild(e)}function wt(t){return new Promise((e,a)=>{let s=document.querySelector(`script[src="${t}"]`);if(s){if(s.dataset.loaded)return e();s.addEventListener("load",()=>e()),s.addEventListener("error",()=>a(new Error("failed to load "+t)));return}let n=document.createElement("script");n.src=t,n.addEventListener("load",()=>{n.dataset.loaded="1",e()}),n.addEventListener("error",()=>a(new Error("failed to load "+t))),document.head.appendChild(n)})}function We(){if(rt)return rt;let t="https://cdn.jsdelivr.net/npm/codemirror@5";return Qt(`${t}/lib/codemirror.css`),Qt(`${t}/theme/material-darker.css`),rt=wt(`${t}/lib/codemirror.js`).then(()=>Promise.all([wt(`${t}/mode/meta.js`),wt(`${t}/addon/mode/loadmode.js`)])).then(()=>{window.CodeMirror.modeURL=`${t}/mode/%N/%N.js`}),rt}function Oe(t){let e=window.CodeMirror.findModeByFileName(t);return e?e.mode:null}async function ze(t,e,a){let s;try{s=await m.get(`/sites/${t}/files/content?path=${encodeURIComponent(e)}`)}catch(k){let u=/too large/i.test(k.message)?"File is too large to edit \u2014 download it instead.":/binary/i.test(k.message)?"Binary file \u2014 download it instead of editing.":k.message;c.error(u);return}try{await We()}catch(k){c.error("Editor failed to load: "+k.message);return}let n="fm-editor-modal";document.getElementById(n)?.remove();let i=document.createElement("div");i.id=n,i.setAttribute("uk-modal",""),i.innerHTML=`
        ...
        </div>`,document.body.appendChild(i),window.UIkit&&UIkit.icon(i);let r=UIkit.modal(i,{bgClose:!1,escClose:!0}),o=i.querySelector("#fm-ed-dirty"),l=null,p=!0,d=k=>{p=!k,o.hidden=!k};UIkit.util.on(i,"shown",()=>{if(l)return;l=window.CodeMirror.fromTextArea(i.querySelector("#fm-ed-area"),{value:s.content,lineNumbers:!0,theme:"material-darker",indentUnit:4,lineWrapping:!1,extraKeys:{"Ctrl-S":b,"Cmd-S":b,"Ctrl-F":"findPersistent","Ctrl-/":"toggleComment"}}),l.setValue(s.content),l.on("change",()=>d(!0));let k=Oe(a);k&&(l.setOption("mode",k),window.CodeMirror.autoLoadMode(l,k)),setTimeout(()=>l.refresh(),30)}),UIkit.util.on(i,"hidden",()=>i.remove());async function b(){if(!l)return;let k=i.querySelector("#fm-ed-save"),u=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div>';try{await m.put(`/sites/${t}/files/content`,{path:e,content:l.getValue()}),d(!1),c.success("Saved"),C(t)}catch(v){c.error(v.message)}finally{k.disabled=!1,k.innerHTML=u}}i.querySelector("#fm-ed-save").addEventListener("click",b),UIkit.util.on(i,"beforehide",k=>{!p&&!window.confirm("Discard unsaved changes?")&&k.preventDefault()}),r.show()}var Ve=2e3;function xt(t,e){return`
        <div>
            <div class="kp-log-controls">
        ...
        </div>`}function te(t,e){let a=null,s=!1,n=t.querySelector("#log-output"),i=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),o=t.querySelector("#log-clear"),l=t.querySelector("#log-autoscroll"),p=t.querySelector("#log-status");function d(u){for(u.split(`
`).forEach(v=>{if(!v)return;let g=document.createElement("div");g.className=v.match(/WAF BLOCK/i)?"kp-log-line-err":v.match(/WAF DETECT/i)?"kp-log-line-warn":v.match(/error|crit|emerg/i)?"kp-log-line-err":v.match(/warn/i)?"kp-log-line-warn":v.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=v,n.appendChild(g)});n.childElementCount>Ve;)n.removeChild(n.firstChild);l.checked&&(n.scrollTop=n.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,i.disabled=!1,r.disabled=!0,p&&(p.textContent="Disconnected")}i.addEventListener("click",()=>{b();let u=t.querySelector("#log-container").value,v=t.querySelector("#log-tail").value,g=location.protocol==="https:"?"wss":"ws",f=u==="waf"?`${g}://${location.host}/api/sites/${e}/logs/waf?tail=${v}`:u==="proxy"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:u==="access"?`${g}://${location.host}/api/sites/${e}/logs/proxy?tail=${v}`:`${g}://${location.host}/api/sites/${e}/logs?container=${u}&tail=${v}`;a=new WebSocket(f),a.onopen=()=>{s=!0,i.disabled=!0,r.disabled=!1,p&&(p.textContent=`Connected \u2014 ${u}`)},a.onmessage=w=>d(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,i.disabled=!1,r.disabled=!0,p&&(p.textContent="Disconnected")}}),r.addEventListener("click",b),o.addEventListener("click",()=>{n.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),i.click())});let k=h.go.bind(h);h.go=function(u,v={}){return a&&b(),k(u,v)}}function Ke(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ee(t,e){try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Ke(a.status))}catch{}}function ae(t){t.forEach(e=>ee(e.Domain,e.ID))}function se(t,e,a,s=0,n=null){let i=t.SiteType!==3&&t.PMAPort>0;return`
            ...
            <form id="waf-override-form" class="uk-form-stacked">
                ...
                <div class="uk-flex uk-flex-right uk-margin-top" style="gap:8px">
                    ...
                    <div class="waf-plugin-pills">
                        ...
                        `).join("")}
                    </div>`,r.querySelectorAll(".waf-plugin-pill").forEach(d=>{d.addEventListener("click",()=>d.classList.toggle("active"))}))}}catch(s){c.error("Failed to load WAF settings: "+s.message)}}function pe(t,e,a){t.addEventListener("submit",async s=>{if(s.target.id!=="waf-override-form")return;s.preventDefault();let n=s.target.querySelector('[type="submit"]'),i=n.innerHTML;n.disabled=!0,n.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let r=new FormData(s.target),o={override:parseInt(r.get("override"),10),exclusions:r.get("exclusions").trim()};try{await m.put(`/sites/${e}/waf`,o);let l=document.querySelector(".waf-plugin-select"),p=l?[...l.selectedOptions].map(d=>d.value):[...document.querySelectorAll(".waf-plugin-pill.active")].map(d=>d.dataset.plugin);await m.put(`/sites/${e}/waf/plugins`,p),c.success("WAF override saved \u2014 engine recompiling in background")}catch(l){c.error(l.message)}finally{n.disabled=!1,n.innerHTML=i}},{signal:a}),t.querySelector("#waf-import")?.addEventListener("change",async s=>{let n=s.target.files[0];if(!n)return;let i=new FormData;i.append("file",n);try{let r=await fetch(`/api/sites/${e}/waf/import`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:i}),o=r.status===204?null:await r.json().catch(()=>null);if(!r.ok)throw new Error(o?.error||`HTTP ${r.status}`);await St(e),c.success("WAF settings imported")}catch(r){c.error(r.message)}finally{s.target.value=""}})}var Xe=[{label:"Cache Flush",cmd:"cache flush"},{label:"Plugin List",cmd:"plugin list"},{label:"Theme List",cmd:"theme list"},{label:"User List",cmd:"user list"},{label:"Core Check",cmd:"core check-update"},{label:"Core Update",cmd:"core update"},{label:"Plugin Updates",cmd:"plugin update --all"},{label:"Theme Updates",cmd:"theme update --all"},{label:"Rewrite Flush",cmd:"rewrite flush"},{label:"Transient Delete",cmd:"transient delete --all"},{label:"Search Replace",cmd:"search-replace '' ''"}];function ue(t){return`
        <div class="kp-wpcli">
...
`).forEach(v=>{if(!v)return;let g=document.createElement("div");u?g.className=u:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function d(k){if(k=k.trim(),!k)return;o.unshift(k),l=-1,p(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,r&&(r.textContent="Running...");let u=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${u}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let f=g.data;if(f.trim()==="[done]"){v.close();return}if(f.startsWith("[info]")){p(f,"kp-muted");return}if(f.startsWith("[error]")){p(f,"kp-log-line-err");return}p(f)},v.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,n.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{d(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){d(s.value),s.value="",l=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),l<o.length-1&&(l++,s.value=o[l]);return}k.key==="ArrowDown"&&(k.preventDefault(),l>0?(l--,s.value=o[l]):(l=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let u=k.dataset.cmd;if(u.startsWith("search-replace")){s.value=u,s.focus();let v=u.indexOf("''")+1;s.setSelectionRange(v,v);return}d(u)})}),i.addEventListener("click",()=>{a.innerHTML=""});let b=h.go.bind(h);h.go=function(k,u={}){return b(k,u)},s.focus()}var G=null,M=null;function ke(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),n=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function i(o,l=!1){UIkit.switcher(a).show(o),e.querySelectorAll(":scope > li[data-pill]").forEach(p=>p.classList.remove("kp-pill-active")),l?(s?.classList.add("kp-pill-active"),n?.querySelectorAll("a[data-switcher]").forEach(p=>{p.classList.toggle("kp-dd-active",parseInt(p.dataset.switcher,10)===o)})):(s?.classList.remove("kp-pill-active"),n?.querySelectorAll("a[data-switcher]").forEach(p=>p.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(o=>{o.addEventListener("click",l=>{l.preventDefault();let p=parseInt(o.closest("li").dataset.pill,10);i(p,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",o=>{o.stopPropagation(),n.hidden=!n.hidden,s.classList.toggle("kp-pill-active",!n.hidden)}),n?.querySelectorAll("a[data-switcher]").forEach(o=>{o.addEventListener("click",l=>{l.preventDefault(),n.hidden=!0,i(parseInt(o.dataset.switcher,10),!0)})}),document.addEventListener("click",o=>{n&&!s.contains(o.target)&&(n.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(0)}function Ge(){return`
        ...
        `).join("")}function ea(t,e){M&&(M.close(),M=null);let a=document.getElementById("sd-health-badges");if(!a)return;let s=location.protocol==="https:"?"wss":"ws";M=new WebSocket(`${s}://${location.host}/api/sites/${e}/health/stream`),M.onmessage=n=>{try{let i=JSON.parse(n.data);a.innerHTML=ta(i),a.querySelectorAll(".kp-health-badge").forEach(r=>{r.addEventListener("click",async()=>{r.style.color=Et("starting");let o=r.dataset.container,l=o.split("-").pop();try{await m.post(`/sites/${e}/containers/${l}/restart`),c.success(`${be(o)} restarted`)}catch(p){r.style.color=Et("none"),c.error(p.message)}})})}catch{}},M.onerror=()=>{},M.onclose=()=>{M=null}}async function ve(t,{id:e}){let[{site:a,domains:s,sftp:n},i,r]=await Promise.all([m.get(`/sites/${e}`),m.get("/sites"),m.get(`/sites/${e}/configs`)]),o=Array.isArray(i)?i:[],l=a.SiteType===1||a.SiteType===2,p=a.SiteType===6,d=[1,2,4,5].includes(a.SiteType);if(t.innerHTML=`
        <div class="kp-view-header">
            <div class="uk-flex uk-flex-middle" style="gap:12px">
                ...
                <div class="kp-site-nav-wrap">
                    <select id="sd-site-nav" class="uk-select kp-select">
                        ${o.map(u=>`<option value="${u.ID}" ${u.ID===a.ID?"selected":""}>${u.Name}</option>`).join("")}
                    </select>
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${se(a,s??[],n,a.ParentID??0,o.find(u=>u.ID===a.ParentID)?.Name??null)}</li>
            <li>${vt(e,a.SiteType)}</li>
            <li>${J(e,1,r[1])}</li>
            ${l?`<li>${J(e,2,r[2])}</li>`:""}
            <li>${J(e,3,r[3])}</li>
            ...
            <li id="waf-tab-panel"></li>
            <li>${ft()}</li>
            ${a.SiteType===1?`<li>${ue(e)}</li>`:""}
            <li>${Ut(e)}</li>
            ${d?`<li>${zt(e)}</li>`:""}
            <li>${le()}</li>
            <li>${Yt(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>tt(a)),document.getElementById("sd-site-nav")?.addEventListener("change",u=>{h.go("site-detail",{id:u.target.value})}),ot(t),q(t),te(t,e),G&&G.abort(),G=new AbortController,pe(t,e,G.signal),St(e),p){Ye(t,e),Qe(e),gt(t,e,a.SiteType),ht(e,a.SiteType),yt(t,e),z(e),ke(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{$("Recreating Pod","Recreating containers for this site...");try{await m.post(`/sites/${e}/recreate`),y(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(u){y(),c.error(u.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let u=await Z(a.Name);if(u){$("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await m.post(`/sites/${e}/clone`,{name:u},6e5),y(),c.success(`Site cloned as '${u}'`),h.go("sites")}catch(v){y(),c.error(v.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(u=>{u.addEventListener("click",async()=>{let v=u.dataset.action;if(v==="flush"){try{await m.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(f){c.error(f.message)}return}$(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[v]??v} Pod`,"Please wait...");try{await m.post(`/sites/${e}/${v}`),y(),c.success(`Site ${v} successful`),h.go("site-detail",{id:e})}catch(f){y(),c.error(f.message)}})}),Wt(t,e),ie(t,e),a.SiteType===1&&me(t,e),oe(t,e,a),Nt(t,e),N(t,e),d&&(Kt(t,e),lt(t,e)),de(t,e),ce(e);let b=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');b&&k&&(k.dataset.switcher=String(b.children.length-1)),Zt(t,e),C(e),ea(t,e),gt(t,e,a.SiteType),ht(e,a.SiteType),ke(t),yt(t,e),z(e),ae(s??[])}async function ct(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((i,r)=>{QRCode.toDataURL(t,{width:220,margin:2},(o,l)=>{o?r(o):i(l)})});e.src=n,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function dt(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
