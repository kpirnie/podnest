"use strict";(()=>{var d={async _req(e,t,a,s=6e4){let n=new AbortController,o=setTimeout(()=>n.abort(),s),i={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};a!==void 0&&(i.body=JSON.stringify(a));try{let r=await fetch("/api"+t,i);clearTimeout(o);let c=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(c?.error||`HTTP ${r.status}`);return c}catch(r){throw clearTimeout(o),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e),patch:(e,t)=>d._req("PATCH",e,t)};var ne=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',L=e=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",a),UIkit.modal("#kp-progress-modal").show()}function h(){let e=document.getElementById("kp-progress-modal");e&&(UIkit.modal(e).hide(),setTimeout(()=>e.remove(),300))}function j(e){return new Promise(t=>{let a="kp-clone-modal",s=`
            <div id="${a}" uk-modal>
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",u);let m=UIkit.modal(`#${n}`),b=document.getElementById("kp-sync-ok"),g=document.getElementById("kp-sync-cancel"),k=f=>{m.hide(),setTimeout(()=>document.getElementById(n)?.remove(),300),s(f)};b.addEventListener("click",()=>k(!0),{once:!0}),g.addEventListener("click",()=>k(!1),{once:!0}),document.getElementById(n).addEventListener("hidden",()=>k(!1),{once:!0}),m.show()})}var v={routes:{},_ownHashChange:!1,register(e,t){this.routes[e]=t},async go(e,t={}){let a=Object.keys(t).length?e+"/"+Object.values(t).join("/"):e;this._ownHashChange=!0,window.location.hash=a,setTimeout(()=>{this._ownHashChange=!1},0),document.querySelectorAll(".kp-nav-link").forEach(o=>{o.classList.toggle("kp-active",o.dataset.view===e)});let s=this.routes[e];if(!s)return;let n=document.getElementById("kp-view");n.innerHTML=ne();try{await s(n,t)}catch(o){n.innerHTML=L(o.message)}}};function X(){let t=(window.location.hash.replace("#","")||"dashboard").split("/"),a=t[0],s={};return a==="site-detail"&&t[1]&&(s.id=t[1]),{view:a,params:s}}var l={show(e,t="info",a=7e3){let s={success:"check",error:"warning",info:"info"},n=document.createElement("div");n.className=`kp-toast kp-toast-${t}`,n.innerHTML=`<span uk-icon="${s[t]||"info"}"></span><span>${e}</span>`,document.getElementById("kp-toasts").appendChild(n),UIkit.icon(n.querySelector("[uk-icon]")),setTimeout(()=>n.remove(),a)},success:e=>l.show(e,"success"),error:e=>l.show(e,"error"),info:e=>l.show(e,"info")};async function W(e){let t=`
...
`).map(S=>S.trim()).filter(Boolean),install_wordpress:k===1?g.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=k===6?`Setting up '${y.name}' as a reverse proxy...`:`Setting up '${y.name}' \u2014 pulling images and provisioning containers...`;x("Creating Site",w);try{await d.post("/sites",y),h(),l.success(`Site '${y.name}' created`),v.go("sites")}catch(S){h(),l.error(S.message),m.disabled=!1,m.innerHTML=b}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function oe(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        <div class="kp-site-grid">
            ${t.length===0?U("world","No sites yet \u2014 create one to get started"):t.map(a=>Z(a,t)).join("")}
        </div>`,document.getElementById("sites-new-btn").addEventListener("click",()=>O())}function Z(e,t=[]){let a=e.Domains?.[0]??null,s=e.SiteType===6,n=e.ParentID>0?t.find(o=>o.ID===e.ParentID)??null:null;return`
            ...
            <div class="kp-site-card-header">
                <div>
                    ...
                    <div class="kp-site-meta">
                        ...
                        <span class="kp-site-meta-item"><span uk-icon="icon: code; ratio: 0.75"></span> ${F(e.SiteType)}${D(e)?" / "+D(e):""}</span>
        ...
        </div>`}async function ie(e){let t=await d.get("/sites")??[],a=t.filter(o=>o.SiteStatus===1).length,s=t.filter(o=>o.SiteStatus===2).length,n=t.filter(o=>o.SiteStatus===4).length;e.innerHTML=`
        <div class="kp-view-header">
        ...
        <div class="kp-site-grid">
            ${t.length===0?U("world","No sites yet"):t.slice(0,6).map(o=>Z(o,t)).join("")}
        </div>`,document.getElementById("dash-new-site")?.addEventListener("click",()=>O())}function q(e=null){let t=e?`/sites/${e}/security/ip`:"/security/ip",a=e?`/sites/${e}/security/ua`:"/security/ua",s=e?`/sites/${e}/waf`:"/settings/waf";return`
        ...
        ${q(null)}`,V(e),T(e)}function qe(e){switch(e){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function re(e){let t=document.getElementById("admin-domain-ssl");if(!(!t||!e))try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`);t.outerHTML=qe(a.status)}catch{}}async function ce(e){if(!C()){e.innerHTML=L("Access denied");return}let[t,a,s,n]=await Promise.all([d.get("/settings"),d.get("/settings/backup"),d.get("/settings/waf"),d.get("/settings/trusted-proxies")]);e.innerHTML=`
    <div class="kp-view-header">
    ...
    `,t.admin_domain&&re(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let p={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await d.put("/settings",p),l.success("Settings saved"),re(p.admin_domain)}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("settings-import").addEventListener("change",async o=>{let i=o.target.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch("/api/settings/import",{method:"POST",body:r}),p=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(p?.error||`HTTP ${c.status}`);l.success("Settings imported")}catch(c){l.error(c.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),p={backup_schedule:c.get("backup_schedule").trim(),backup_retain_days:c.get("backup_retain_days").trim()};try{await d.put("/settings/backup",p),l.success("Backup settings saved")}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=r}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let i=o.target.querySelector('[type="submit"]'),r=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let c=new FormData(o.target),p={s3_endpoint:c.get("s3_endpoint").trim(),s3_bucket:c.get("s3_bucket").trim(),s3_region:c.get("s3_region").trim(),s3_access_key:c.get("s3_access_key").trim()},u=c.get("s3_secret_key").trim();u&&(p.s3_secret_key=u);try{await d.put("/settings/backup",p),l.success("S3 settings saved")}catch(m){l.error(m.message)}finally{i.disabled=!1,i.innerHTML=r}})}function de(e){return`
        ...
        </div>`}function Me(e){if(!e||e.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No snapshots yet.</p>';let t=n=>n===2?'<span class="kp-mono" style="color:var(--kp-cyan)">S3</span>':'<span class="kp-mono" style="color:var(--kp-blue)">Local</span>',a=n=>n<1024?`${n} B`:n<1048576?`${(n/1024).toFixed(1)} KB`:n<1073741824?`${(n/1048576).toFixed(1)} MB`:`${(n/1073741824).toFixed(2)} GB`;return`
    ...
    </div>`}function me(e,t){e.addEventListener("click",a=>{if(a.target.closest(".cfg-add-row")){let s=a.target.closest(".cfg-add-row");e.querySelector(`.cfg-rows[data-type="${s.dataset.type}"]`).insertAdjacentHTML("beforeend",H())}}),e.addEventListener("click",a=>{a.target.closest(".cfg-del-row")&&a.target.closest(".kp-config-row").remove()}),e.addEventListener("click",async a=>{let s=a.target.closest(".cfg-save");if(!s)return;let{type:n,site:o}=s.dataset,i=e.querySelectorAll(`.cfg-rows[data-type="${n}"] .kp-config-row`),r={};if(i.forEach(c=>{let p=c.querySelector(".cfg-key").value.trim(),u=c.querySelector(".cfg-val").value.trim();p&&(r[p]=u)}),n==="5"){let c=e.querySelector(".varnish-enabled-toggle");r.enabled=c?.checked?"true":"false"}try{await d.put(`/sites/${o}/configs/${n}`,r),l.success(`${z[n]} config saved`)}catch(c){l.error(c.message)}}),e.addEventListener("click",async a=>{let s=a.target.closest(".cfg-reset");if(!s)return;let{type:n,site:o}=s.dataset;if(await $("Reset Config",`Reset ${z[n]} config to defaults?`))try{let r=await d.post(`/sites/${o}/configs/${n}/reset`),c=e.querySelector(`.cfg-rows[data-type="${n}"]`);c.innerHTML=Object.entries(r).map(([p,u])=>H(p,u)).join(""),l.success(`${z[n]} reset to defaults`)}catch(r){l.error(r.message)}}),e.addEventListener("change",async a=>{let s=a.target.closest(".cfg-import-input");if(!s)return;let{type:n,site:o}=s.dataset,i=s.files[0];if(!i)return;let r=new FormData;r.append("file",i);try{let c=await fetch(`/api/sites/${o}/configs/${n}/import`,{method:"POST",body:r}),p=c.status===204?null:await c.json().catch(()=>null);if(!c.ok)throw new Error(p?.error||`HTTP ${c.status}`);let u=e.querySelector(`.cfg-rows[data-type="${n}"]`);u.innerHTML=Object.entries(p).map(([m,b])=>H(m,b)).join(""),l.success(`${z[n]} config imported`)}catch(c){l.error(c.message)}finally{s.value=""}})}function be(e){return`
        ...
        </table>`}async function J(e,t){let a=e.querySelector("#cron-list-wrap");if(a)try{let s=await d.get(`/sites/${t}/crons`);a.innerHTML=ge(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${s.message}</p>`}}function ve(e,t){let a=[],s=e.querySelector("#cron-modal"),n=e.querySelector("#cron-modal-title"),o=e.querySelector("#cron-modal-id"),i=e.querySelector("#cron-modal-label"),r=e.querySelector("#cron-modal-command"),c=e.querySelector("#cron-modal-schedule"),p=e.querySelector("#cron-schedule-preview"),u=e.querySelector("#cron-modal-enabled");c?.addEventListener("input",()=>{p.textContent=ke(c.value.trim())}),e.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",o.value="",i.value="",r.value="",c.value="",p.textContent="",u.checked=!0,UIkit.modal(s).show()}),e.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let m=r.value.trim(),b=c.value.trim();if(!m||!b){l.error("Command and schedule are required");return}let g={label:i.value.trim(),command:m,schedule:b,enabled:u.checked},k=o.value;try{k?(await d.put(`/sites/${t}/crons/${k}`,g),l.success("Cron job updated")):(await d.post(`/sites/${t}/crons`,g),l.success("Cron job created")),UIkit.modal(s).hide(),await J(e,t),a=await d.get(`/sites/${t}/crons`)}catch(f){l.error(f.message)}}),e.querySelector("#cron-list-wrap")?.addEventListener("click",async m=>{let b=m.target.closest(".cron-detail-btn");if(b){let y=b.dataset.id,w=a.find(G=>String(G.ID)===y);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                <div id="cron-detail-modal" uk-modal>
                    <div class="uk-modal-dialog kp-modal uk-modal-body uk-width-large">
                        <button class="uk-modal-close-default" type="button" uk-close></button>
                        <h3 class="kp-view-title uk-margin-bottom">Run Details \u2014 ${ee(w.Label||String(w.ID))}</h3>
                        <div class="uk-margin-small-bottom">
                            <label class="kp-label">Output</label>
                            <pre class="kp-cron-output">${ee(w.LastOutput||"(no output)")}</pre>
                        </div>
                        <div class="uk-margin-small-top">
                            <label class="kp-label">Error</label>
                            <pre class="kp-cron-output kp-cron-output-error">${ee(w.LastError||"(no error)")}</pre>
                        </div>
                    </div>
                </div>`);let S=document.getElementById("cron-detail-modal");UIkit.modal(S).show(),S.addEventListener("hidden",()=>S.remove(),{once:!0});return}let g=m.target.closest(".cron-edit-btn");if(g){let y=g.dataset.id,w=a.find(S=>String(S.ID)===y);if(!w)return;n.textContent="Edit Cron Job",o.value=w.ID,i.value=w.Label||"",r.value=w.Command,c.value=w.Schedule,p.textContent=ke(w.Schedule),u.checked=w.Enabled,UIkit.modal(s).show();return}let k=m.target.closest(".cron-delete-btn");if(k){let y=k.dataset.id;if(!await $("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await d.delete(`/sites/${t}/crons/${y}`),l.success("Cron job deleted"),await J(e,t),a=await d.get(`/sites/${t}/crons`)}catch(S){l.error(S.message)}return}let f=m.target.closest(".cron-run-btn");if(f){let y=f.dataset.id;try{await d.post(`/sites/${t}/crons/${y}/run`)}catch(E){l.error(E.message);return}x("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await d.get(`/sites/${t}/crons`)).find(P=>String(P.ID)===y)?.LastRun??null}catch{}let S=Date.now()+300*1e3,G=setInterval(async()=>{try{let E=await d.get(`/sites/${t}/crons`),P=E.find(N=>String(N.ID)===y);if(!P||P.LastRun!==w||Date.now()>S){clearInterval(G),h(),a=E??[];let N=e.querySelector("#cron-list-wrap");N&&(N.innerHTML=ge(E)),P?.LastError?l.error(`Job failed: ${P.LastError}`):l.success("Cron job complete")}}catch{}},2e3);return}}),e.querySelector("#cron-list-wrap")?.addEventListener("change",async m=>{let b=m.target.closest(".cron-toggle");if(!b)return;let g=b.dataset.id;try{await d.patch(`/sites/${t}/crons/${g}/toggle`,{enabled:b.checked}),l.success(b.checked?"Cron job enabled":"Cron job disabled")}catch(k){l.error(k.message),b.checked=!b.checked}}),d.get(`/sites/${t}/crons`).then(m=>{a=m??[]}).catch(()=>{})}function ee(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ke(e){if(!e)return"";let t=e.trim().split(/\s+/);if(t.length!==5)return"invalid expression";let[a,s,n,o,i]=t;if(e==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&o==="*"&&i!=="*"){let r=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${i.split(",").map(p=>r[parseInt(p)]??p).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:e}function te(e,t){return`
        <div>
        ...
        </div>`}function fe(e,t){let a=null,s=!1,n=e.querySelector("#log-output"),o=e.querySelector("#log-connect"),i=e.querySelector("#log-disconnect"),r=e.querySelector("#log-clear"),c=e.querySelector("#log-autoscroll"),p=e.querySelector("#log-status");function u(g){g.split(`
`).forEach(k=>{if(!k)return;let f=document.createElement("div");f.className=k.match(/WAF BLOCK/i)?"kp-log-line-err":k.match(/WAF DETECT/i)?"kp-log-line-warn":k.match(/error|crit|emerg/i)?"kp-log-line-err":k.match(/warn/i)?"kp-log-line-warn":k.match(/info|notice/i)?"kp-log-line-info":"",f.textContent=k,n.appendChild(f)}),c.checked&&(n.scrollTop=n.scrollHeight)}function m(){a&&(a.close(),a=null),s=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}o.addEventListener("click",()=>{m();let g=e.querySelector("#log-container").value,k=e.querySelector("#log-tail").value,f=location.protocol==="https:"?"wss":"ws",y=g==="waf"?`${f}://${location.host}/api/sites/${t}/logs/waf?tail=${k}`:`${f}://${location.host}/api/sites/${t}/logs?container=${g}&tail=${k}`;a=new WebSocket(y),a.onopen=()=>{s=!0,o.disabled=!0,i.disabled=!1,p&&(p.textContent=`Connected \u2014 ${g}`)},a.onmessage=w=>u(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,o.disabled=!1,i.disabled=!0,p&&(p.textContent="Disconnected")}}),i.addEventListener("click",m),r.addEventListener("click",()=>{n.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(m(),o.click())});let b=v.go.bind(v);v.go=function(g,k={}){return a&&m(),b(g,k)}}function He(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function he(e,t){try{let a=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),s=document.getElementById(`ssl-icon-${t}`);s&&(s.outerHTML=He(a.status))}catch{}}function ye(e){e.forEach(t=>he(t.Domain,t.ID))}function we(e,t,a,s=0,n=null){let o=e.SiteType!==3&&e.PMAPort>0;return`
                ...
                <div class="kp-card uk-padding-small">
                    ...
                    <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
                        <tbody>
                            ...
                            <tr><td class="kp-muted">Type</td><td>${F(e.SiteType)}</td></tr>
...
`).forEach(k=>{if(!k)return;let f=document.createElement("div");g?f.className=g:f.className=k.match(/error|fatal|critical/i)?"kp-log-line-err":k.match(/warning|warn/i)?"kp-log-line-warn":k.match(/success|done\]/i)?"kp-log-line-info":"",f.textContent=k,a.appendChild(f)}),a.scrollTop=a.scrollHeight}function u(b){if(b=b.trim(),!b)return;r.unshift(b),c=-1,p(`wp> ${b}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,i&&(i.textContent="Running...");let g=location.protocol==="https:"?"wss":"ws",k=new WebSocket(`${g}://${location.host}/api/sites/${t}/wpcli`);k.onopen=()=>{k.send(JSON.stringify({command:b}))},k.onmessage=f=>{let y=f.data;if(y.trim()==="[done]"){k.close();return}if(y.startsWith("[info]")){p(y,"kp-muted");return}if(y.startsWith("[error]")){p(y,"kp-log-line-err");return}p(y)},k.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},k.onclose=()=>{s.disabled=!1,n.disabled=!1,i&&(i.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",b=>{if(b.key==="Enter"){u(s.value),s.value="",c=-1;return}if(b.key==="ArrowUp"){b.preventDefault(),c<r.length-1&&(c++,s.value=r[c]);return}b.key==="ArrowDown"&&(b.preventDefault(),c>0?(c--,s.value=r[c]):(c=-1,s.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(b=>{b.addEventListener("click",()=>{let g=b.dataset.cmd;if(g.startsWith("search-replace")){s.value=g,s.focus();let k=g.indexOf("''")+1;s.setSelectionRange(k,k);return}u(g)})}),o.addEventListener("click",()=>{a.innerHTML=""});let m=v.go.bind(v);v.go=function(b,g={}){return m(b,g)},s.focus()}var _=null;function _e(){return`
        ...
        <ul class="uk-switcher">
            <li>${we(a,s??[],n,a.ParentID??0,r.find(m=>m.ID===a.ParentID)?.Name??null)}</li>
            <li>${R(t,1,i[1])}</li>
            ${c?`<li>${R(t,2,i[2])}</li>`:""}
            <li>${R(t,3,i[3])}</li>
            <li>${R(t,4,i[4])}</li>
            ...
            <li>${de(t)}</li>
            ${u?`<li>${be(t)}</li>`:""}
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>v.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>W(a)),document.getElementById("sd-site-nav")?.addEventListener("change",m=>{v.go("site-detail",{id:m.target.value})}),V(e),T(e),fe(e,t),Ne(e,t),Te(t),p){je(e,t),Fe(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{x("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),h(),l.success("Pod recreated"),v.go("site-detail",{id:t})}catch(m){h(),l.error(m.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let m=await j(a.Name);if(m){x("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${t}/clone`,{name:m}),h(),l.success(`Site cloned as '${m}'`),v.go("sites")}catch(b){h(),l.error(b.message)}}}),e.querySelectorAll("[data-action]").forEach(m=>{m.addEventListener("click",async()=>{let b=m.dataset.action;if(b==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(k){l.error(k.message)}return}x(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[b]??b} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${b}`),h(),l.success(`Site ${b} successful`),v.go("site-detail",{id:t})}catch(k){h(),l.error(k.message)}})}),me(e,t),Se(e,t),a.SiteType===1&&Le(e,t),$e(e,t,a),ue(e,t),M(e,t),u&&(ve(e,t),J(e,t)),Ae(e),ye(s??[])}async function K(e){let t=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!t||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((o,i)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,c)=>{r?i(r):o(c)})});t.src=n,t.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=e,a.appendChild(s)}function Q(e){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),o=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{o.textContent="Copied!"});else{let i=document.createElement("textarea");i.value=n,i.style.cssText="position:fixed;opacity:0",document.body.appendChild(i),i.select();try{document.execCommand("copy"),o.textContent="Copied!"}catch{}i.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),v.go("users")})}function Ie(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let o=UIkit.modal("#kp-edit-user-modal");o.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let u=p.target.querySelector('[type="submit"]'),m=u.innerHTML;u.disabled=!0,u.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b=new FormData(p.target),g={fname:b.get("fname").trim(),lname:b.get("lname").trim(),email:b.get("email").trim(),phone:b.get("phone").trim()};if(s){g.role=parseInt(b.get("role"));let f=b.get("uname");f&&(g.uname=f.trim())}let k=b.get("password");k&&(g.password=k);try{await d.put(`/users/${t}`,g),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("User updated"),v.go("users")}catch(f){l.error(f.message),u.disabled=!1,u.innerHTML=m}});let i=document.getElementById("totp-setup-btn");i&&i.addEventListener("click",async()=>{i.disabled=!0,i.textContent="Setting up\u2026";try{let p=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await K(p.uri)}catch(p){l.error(p.message),i.disabled=!1,i.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){l.error("Enter a 6-digit code");return}r.disabled=!0;try{let u=await d.post(`/users/${t}/totp/confirm`,{code:p});o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("TOTP enabled"),u.backup_codes?.length?Q(u.backup_codes):v.go("users")}catch(u){l.error(u.message),r.disabled=!1}});let c=document.getElementById("totp-disable-btn");c&&c.addEventListener("click",async()=>{c.disabled=!0;try{await d.delete(`/users/${t}/totp`),l.success("TOTP disabled"),o.hide(),document.getElementById("kp-edit-user-modal")?.remove(),v.go("users")}catch(p){l.error(p.message),c.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function Be(e){if(!C()){e.innerHTML=L("Access denied");return}let t=await d.get("/users");e.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>Ie(e)),Oe(e)}function se(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
    ...
    </tr>`}function Oe(e){e.addEventListener("click",async t=>{let a=t.target.closest('[data-action="delete-user"]');if(!(!a||!await $("Delete User","Delete this user? This cannot be undone.")))try{await d.delete(`/users/${a.dataset.uid}`),a.closest("tr").remove(),l.success("User deleted")}catch(n){l.error(n.message)}}),e.addEventListener("click",async t=>{let a=t.target.closest('[data-action="edit-user"]');a&&Ce(e,a.dataset.uid)})}v.register("dashboard",e=>ie(e));v.register("sites",e=>oe(e));v.register("site-detail",(e,t)=>Pe(e,t));v.register("users",e=>Be(e));v.register("settings",e=>ce(e));v.register("security",e=>le(e));document.addEventListener("click",e=>{let t=e.target.closest("[data-view]");if(!t)return;e.preventDefault(),v.go(t.dataset.view);let a=document.getElementById("kp-offcanvas");a&&UIkit.offcanvas(a).hide()});document.addEventListener("click",async e=>{let t=e.target.closest("[data-action]");if(!t)return;e.stopPropagation();let{action:a,id:s}=t.dataset;switch(a){case"manage":v.go("site-detail",{id:s});break;case"start":await A(s,"start","Starting Site","Starting all containers - please wait...");break;case"stop":await A(s,"stop","Stopping Site","Gracefully stopping all containers - please wait...");break;case"restart":await A(s,"restart","Restarting Site","Restarting all containers - please wait...");break;case"flush":await A(s,"flush","Flushing Caches","Clearing container caches - please wait...");break;case"edit":{let n=await d.get(`/sites/${s}`);W(n.site);break}case"clone":{let n=await j(t.dataset.name??s);if(!n)break;x("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await d.post(`/sites/${s}/clone`,{name:n}),h(),l.success(`Site cloned as '${n}'`),v.go("sites")}catch(o){h(),l.error(o.message)}break}case"delete":await Ve(s);break;case"update":await A(s,"update","Updating Images","Pulling latest container images - this may take a few minutes...");break;case"recreate":x("Recreating Pod","Recreating containers for this site - this may take a few minutes...");try{await d.post(`/sites/${s}/recreate`),h(),l.success("Pod recreated"),v.go("site-detail",{id:s})}catch(n){h(),l.error(n.message)}break}});async function A(e,t,a,s){x(a,s);try{await d.post(`/sites/${e}/${t}`),h(),l.success(a+" complete"),["start","stop","restart"].includes(t)&&v.go("sites")}catch(n){h(),l.error(n.message)}}async function Ve(e){if(!await $("Delete Site","This will stop and permanently remove the pod and all its data. Are you sure?"))return;x("Deleting Site","Stopping containers and removing the pod - please wait...");try{await d.delete(`/sites/${e}`)}catch{}let a=!1,s=0;for(;!a&&s<10;){try{await new Promise(o=>setTimeout(o,2e3)),a=!(await d.get("/sites")).find(o=>o.ID===parseInt(e))}catch{}s++}h(),a?(l.success("Site deleted"),v.go("sites")):l.error("Delete failed - site still exists after 20s")}window.addEventListener("hashchange",()=>{if(v._ownHashChange)return;let{view:e,params:t}=X();v.go(e,t)});var{view:ze,params:Je}=X();v.go(ze,Je);})();
