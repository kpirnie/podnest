...
"use strict";(()=>{var u={async _req(t,e,a,s=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),s),r={method:t,headers:{"Content-Type":"application/json"},signal:n.signal};t!=="GET"&&t!=="HEAD"&&(r.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(r.body=JSON.stringify(a));try{let o=await fetch("/api"+e,r);clearTimeout(i);let l=o.status===204?null:await o.json().catch(()=>null);if(o.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!o.ok)throw new Error(l?.error||`HTTP ${o.status}`);return l}catch(o){throw clearTimeout(i),o}},get:t=>u._req("GET",t),post:(t,e,a)=>u._req("POST",t,e,a),put:(t,e,a)=>u._req("PUT",t,e,a),delete:t=>u._req("DELETE",t),patch:(t,e)=>u._req("PATCH",t,e)};var $=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),Y=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',T=t=>`<div class="kp-empty">
        ...
        <div id="kp-progress-modal" uk-modal="bg-close: false; esc-close: false; keyboard: false">
            ...
            </div>`;document.body.insertAdjacentHTML("beforeend",s);let n=UIkit.modal(`#${a}`),i=document.getElementById("kp-clone-name"),r=document.getElementById("kp-clone-ok"),o=document.getElementById("kp-clone-cancel"),l=m=>{n.hide(),setTimeout(()=>document.getElementById(a)?.remove(),300),e(m)};r.addEventListener("click",()=>l(i.value.trim()||null),{once:!0}),o.addEventListener("click",()=>l(null),{once:!0}),document.getElementById(a).addEventListener("hidden",()=>l(null),{once:!0}),n.show(),setTimeout(()=>i.focus(),150),i.addEventListener("keydown",m=>{m.key==="Enter"&&r.click()})})}function mt(t,e,a){return new Promise(s=>{let n="kp-sync-modal",i=t==="pull",r=i?"Pull From Parent":"Push To Parent",o=i?"cloud-download":"cloud-upload",l=i?a:e,m=i?e:a,d=`
            <div id="${n}" uk-modal>
                <div class="uk-modal-dialog kp-modal uk-modal-body" style="max-width:460px">
                    ...
                    <p class="kp-muted uk-text-small uk-margin-small-bottom">
                        ...
                        <strong>${m}</strong> with data from <strong>${l}</strong>.
                        This action cannot be undone.
                    </p>
                    <p class="kp-muted uk-text-small" style="color:var(--kp-red, #e05c5c)">
                        <span uk-icon="icon: warning; ratio: 0.85"></span>
                        <strong>${m}</strong> will be temporarily unavailable during the sync.
                    </p>
                    <div class="uk-flex uk-flex-right uk-margin-top" style="gap:8px">
        ...
        </div>`}function xe(t){let e=null,a=!1,s=t.querySelector("#admin-log-output"),n=t.querySelector("#admin-log-connect"),i=t.querySelector("#admin-log-disconnect"),r=t.querySelector("#admin-log-clear"),o=t.querySelector("#admin-log-autoscroll"),l=t.querySelector("#admin-log-status");function m(k){for(k.split(`
`).forEach(p=>{if(!p)return;let g=document.createElement("div");g.className=p.match(/WAF BLOCK/i)?"kp-log-line-err":p.match(/WAF DETECT/i)?"kp-log-line-warn":p.match(/error|crit|emerg/i)?"kp-log-line-err":p.match(/warn/i)?"kp-log-line-warn":p.match(/info|notice/i)?"kp-log-line-info":"",g.textContent=p,s.appendChild(g)});s.childElementCount>ye;)s.removeChild(s.firstChild);o.checked&&(s.scrollTop=s.scrollHeight)}function d(){e&&(e.close(),e=null),a=!1,n.disabled=!1,i.disabled=!0,l&&(l.textContent="Disconnected")}n.addEventListener("click",()=>{d();let k=t.querySelector("#admin-log-source").value,p=t.querySelector("#admin-log-tail").value,g=location.protocol==="https:"?"wss":"ws",v=k==="waf"?`${g}://${location.host}/api/logs/waf?tail=${p}`:`${g}://${location.host}/api/logs/proxy?tail=${p}`;e=new WebSocket(v),e.onopen=()=>{a=!0,n.disabled=!0,i.disabled=!1,l&&(l.textContent=`Connected \u2014 ${k==="waf"?"WAF Log":"Proxy Access Log"}`)},e.onmessage=f=>m(f.data),e.onerror=()=>{},e.onclose=()=>{a=!1,n.disabled=!1,i.disabled=!0,l&&(l.textContent="Disconnected")}}),i.addEventListener("click",d),r.addEventListener("click",()=>{s.innerHTML=""}),t.querySelector("#admin-log-source").addEventListener("change",()=>{e&&e.readyState===WebSocket.OPEN&&(d(),n.click())});let b=h.go.bind(h);h.go=function(k,p={}){return e&&d(),b(k,p)}}function Tt(t){t.innerHTML=we(),xe(t)}var at=50;function Se(t,e,a){let s=Math.max(1,Math.ceil(t.total/at)),n=(t.entries??[]).map(_t).join("")||'<tr><td colspan="8" class="uk-text-center" style="color:var(--kp-text-dim)">No records found</td></tr>';return`
        ...
        </div>`}function _t(t){let e=new Date(t.ts).toLocaleString(),a=t.username?`<span style="font-family:monospace">${L(t.username)}</span>`:'<span style="color:var(--kp-text-dim)">\u2014</span>',s=$e(t.status),i=t.prior_state||t.new_state?`<button class="uk-button kp-btn-ghost kp-btn-sm al-diff-btn"
        ...
        ${s}
    </div>`}function $e(t){return`<span class="kp-badge ${t>=500?"kp-badge-error":t>=400?"kp-badge-warn":t>=300?"kp-badge-info":"kp-badge-ok"}">${t}</span>`}function L(t){return String(t??"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async function Pt(t,e){let a=new URLSearchParams({page:e,page_size:at});return t.username&&a.set("username",t.username),t.action&&a.set("action",t.action),t.target_type&&a.set("target_type",t.target_type),t.date_from&&a.set("date_from",t.date_from),t.date_to&&a.set("date_to",t.date_to),t.auth!==""&&a.set("auth",t.auth),u.get(`/audit?${a}`)}function Ee(t){return{username:t.querySelector("#al-filter-user").value.trim(),action:t.querySelector("#al-filter-action").value.trim(),target_type:t.querySelector("#al-filter-target").value.trim(),date_from:t.querySelector("#al-filter-date-from").value,date_to:t.querySelector("#al-filter-date-to").value,auth:t.querySelector("#al-filter-auth").value}}async function Le(t,e,a){async function s(r,o){let l=await Pt(r,o);t.querySelector("#al-table-body").innerHTML=(l.entries??[]).map(_t).join("")||'<tr><td colspan="8" class="uk-text-center kp-text-dim">No records found</td></tr>';let m=Math.max(1,Math.ceil(l.total/at)),d=t.querySelector("#al-pager");d&&(d.innerHTML=m>1?Ct(o,m):""),t.querySelector("#al-record-count").textContent=`${l.total} record${l.total!==1?"s":""}`,n(t,r,o,m),e=r,a=o}function n(r,o,l,m){r.querySelector("#al-prev")?.addEventListener("click",()=>s(o,l-1)),r.querySelector("#al-next")?.addEventListener("click",()=>s(o,l+1))}t.querySelector("#al-filter-apply")?.addEventListener("click",()=>{s(Ee(t),1)}),t.querySelector("#al-filter-clear")?.addEventListener("click",()=>{["al-filter-user","al-filter-action","al-filter-target","al-filter-date-from","al-filter-date-to"].forEach(r=>{let o=t.querySelector(`#${r}`);o&&(o.value="")}),t.querySelector("#al-filter-auth").value="",s({username:"",action:"",target_type:"",date_from:"",date_to:"",auth:""},1)});let i=Math.max(1,Math.ceil(parseInt(t.querySelector("#al-record-count")?.textContent??"0")/at));n(t,e,a,i),t.querySelector("#audit-log-panel")?.addEventListener("click",r=>{let o=r.target.closest(".al-diff-btn");if(!o)return;r.preventDefault(),r.stopPropagation();let l=o.dataset.prior??"",m=o.dataset.new??"",d="";l&&m?d=`=== BEFORE ===
`+et(l)+`

=== AFTER ===
`+et(m):m?d=et(m):d=et(l),document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),n=document.getElementById("cs-node-version-wrap"),i=document.getElementById("cs-dotnet-version-wrap"),r=document.getElementById("cs-start-command-wrap"),o=document.getElementById("cs-wordpress-wrap");e.show();let l=document.getElementById("cs-domains-wrap"),m=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let d=parseInt(a.value);s.classList.toggle("uk-hidden",d!==1&&d!==2||d===6),n.classList.toggle("uk-hidden",d!==4),i.classList.toggle("uk-hidden",d!==5),r.classList.toggle("uk-hidden",d!==4&&d!==5),o.classList.toggle("uk-hidden",d!==1||d===6),l.classList.toggle("uk-hidden",d===6),m.classList.toggle("uk-hidden",d!==6)}),document.getElementById("create-site-form").addEventListener("submit",async d=>{d.preventDefault();let b=d.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(d.target),g=parseInt(p.get("site_type")),v=null;g===4&&(v=parseInt(p.get("node_version"))),g===5&&(v=parseInt(p.get("dotnet_version")));let f={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:g,runtime_version:v,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(x=>x.trim()).filter(Boolean),install_wordpress:g===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=g===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;S("Creating Site",w);try{await u.post("/sites",f,6e5),y(),c.success(`Site '${f.name}' created`),h.go("sites")}catch(x){y(),c.error(x.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function B(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}function Te(t){return`${t.toFixed(1)}%`}var nt=null;function bt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function gt(t,e){return`
        ...
        <div class="kp-card uk-padding-small uk-margin-bottom">
            ...
            <div id="stats-disk-wrap">
                ...
                <tr>
                    ...
                    <td class="kp-stats-table-cell-count">${i.count.toLocaleString()}</td>
                </tr>`).join(""))}async function Bt(t){let e=document.getElementById("stats-disk-wrap");if(e){e.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>';try{let a=await u.get(`/sites/${t}/stats/disk`);e.innerHTML=`
        ...
        ${m}`}async function Ie(t,e,a){let s=document.getElementById("stats-drilldown-modal"),n=document.getElementById("stats-drilldown-title"),i=document.getElementById("stats-drilldown-body");if(!s||!i)return;n.textContent=`${a} Requests \u2014 ${new Date(e).toLocaleString([],{hour:"2-digit",minute:"2-digit",month:"short",day:"numeric"})}`,i.innerHTML='<div uk-spinner="ratio:0.8" style="color:var(--kp-blue)"></div>',UIkit.modal(s).show();let r=[],o=0,l="time",m=!0;function d(){i.innerHTML=Pe(r,o,l,m),i.querySelectorAll("th[data-dd-col]").forEach(b=>{b.addEventListener("click",()=>{let k=b.dataset.ddCol;l===k?m=!m:(l=k,m=!0),o=0,d()})}),i.querySelectorAll("[data-dd-page]").forEach(b=>{b.addEventListener("click",()=>{o=parseInt(b.dataset.ddPage,10),d()})})}try{r=await u.get(`/sites/${t}/stats/drilldown?hour=${encodeURIComponent(e)}&status=${a}`)}catch(b){i.innerHTML=`<p class="kp-muted uk-text-small">Failed to load: ${$(b.message)}</p>`;return}d()}function vt(t,e,a){let s=a===6,n=null;function i(){if(s)return;let l=t.querySelector("#stats-pod-indicator"),m=t.querySelector("#stats-pod-table-wrap");if(!m)return;let d=location.protocol==="https:"?"wss":"ws";n=new WebSocket(`${d}://${location.host}/api/sites/${e}/stats/pod`),n.onopen=()=>{l&&(l.className="kp-status kp-status-running",l.textContent="Live")},n.onmessage=b=>{try{let k=JSON.parse(b.data);m.innerHTML=Ce(k.containers??[]),m.querySelectorAll(".kp-stats-pod-role-btn").forEach(p=>{p.addEventListener("click",async()=>{let g=p.style.color;p.style.color="var(--kp-warning)";let v=p.dataset.container.split("-").pop();try{await u.post(`/sites/${e}/containers/${v}/restart`),c.success(`${v} restarted`)}catch(f){p.style.color=g,c.error(f.message)}})})}catch{}},n.onerror=()=>{l&&(l.className="kp-status kp-status-error",l.textContent="Error")},n.onclose=()=>{l&&l.textContent==="Live"&&(l.className="kp-status kp-status-stopped",l.textContent="Disconnected")}}function r(){n&&n.readyState===WebSocket.OPEN&&n.close(),n=null}t.querySelector("#stats-disk-refresh")?.addEventListener("click",()=>{Bt(e)}),i();let o=new MutationObserver(()=>{document.getElementById("stats-panel")||(r(),o.disconnect())});o.observe(document.getElementById("main")??document.body,{childList:!0,subtree:!1})}async function ht(t,e){let a=e===6;await _e(t),a||await Bt(t)}async function qt(t){let e=await u.get("/sites")??[];t.innerHTML=`
        <div class="kp-view-header">
        ...
        ${e.length===0?R("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
        ...
        <div class="kp-site-grid">
            ${e.length===0?R("world","No sites yet"):e.slice(-3).reverse().map(o=>Mt(o,e)).join("")}
        ...
        ${O(null)}`,ot(t),q(t)}function Me(t){switch(t){case"valid":case"self-signed":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Ht(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await u.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=Me(a.status)}catch{}}async function Rt(t){if(!I()){t.innerHTML=T("Access denied");return}let[e,a,s,n,i,r]=await Promise.all([u.get("/settings"),u.get("/settings/backup"),u.get("/settings/waf"),u.get("/settings/trusted-proxies"),u.get("/settings/notifications"),u.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Ht(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(o.target).get("admin_domain").trim()};try{await u.put("/settings",b),c.success("Settings saved"),Ht(b.admin_domain)}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("settings-import").addEventListener("change",async o=>{let l=o.target.files[0];if(!l)return;let m=new FormData;m.append("file",l);try{let d=await fetch("/api/settings/import",{method:"POST",body:m}),b=d.status===204?null:await d.json().catch(()=>null);if(!d.ok)throw new Error(b?.error||`HTTP ${d.status}`);c.success("Settings imported")}catch(d){c.error(d.message)}finally{o.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={backup_schedule:d.get("backup_schedule").trim(),backup_retain_days:d.get("backup_retain_days").trim()};try{await u.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("s3-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={s3_endpoint:d.get("s3_endpoint").trim(),s3_bucket:d.get("s3_bucket").trim(),s3_region:d.get("s3_region").trim(),s3_access_key:d.get("s3_access_key").trim()},k=d.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await u.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("smtp-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={smtp_host:d.get("smtp_host").trim(),smtp_port:d.get("smtp_port").trim(),smtp_username:d.get("smtp_username").trim(),smtp_from:d.get("smtp_from").trim(),smtp_tls:d.get("smtp_tls")?"true":"false"},k=d.get("smtp_password").trim();k&&(b.smtp_password=k);try{await u.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("sns-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={aws_access_key:d.get("aws_access_key").trim(),aws_region:d.get("aws_region").trim(),aws_sns_sender_id:d.get("aws_sns_sender_id").trim()},k=d.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await u.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{l.disabled=!1,l.innerHTML=m}}),document.getElementById("resource-form").addEventListener("submit",async o=>{o.preventDefault();let l=o.target.querySelector('[type="submit"]'),m=l.innerHTML;l.disabled=!0,l.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let d=new FormData(o.target),b={resource_ram_reserve_gb:d.get("resource_ram_reserve_gb").trim(),resource_poll_interval:d.get("resource_poll_interval").trim(),resource_throttle_pct:d.get("resource_throttle_pct").trim(),resource_webhook_url:d.get("resource_webhook_url").trim()};try{await u.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{l.disabled=!1,l.innerHTML=m}})}function Ut(t){return`
        ...
        </div>`}function De(t){if(!t||t.length===0)return'<p class="kp-muted uk-text-small uk-margin-remove">No snapshots yet.</p>';let e=n=>n===2?'<span class="kp-mono" style="color:var(--kp-cyan)">S3</span>':'<span class="kp-mono" style="color:var(--kp-blue)">Local</span>',a=n=>n<1024?`${n} B`:n<1048576?`${(n/1024).toFixed(1)} KB`:n<1073741824?`${(n/1048576).toFixed(1)} MB`:`${(n/1073741824).toFixed(2)} GB`;return`
        <table class="uk-table uk-table-small uk-table-divider uk-margin-remove">
            <thead>
                <tr>
                    <th>Snapshot ID</th>
        ...
        <tr>
            ...
            <td>
        ...
        </table>`}async function lt(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await u.get(`/sites/${e}/crons`);a.innerHTML=Vt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${$(s.message)}</p>`}}function Kt(t,e){let a=[],s=t.querySelector("#cron-modal"),n=t.querySelector("#cron-modal-title"),i=t.querySelector("#cron-modal-id"),r=t.querySelector("#cron-modal-label"),o=t.querySelector("#cron-modal-command"),l=t.querySelector("#cron-modal-schedule"),m=t.querySelector("#cron-schedule-preview"),d=t.querySelector("#cron-modal-enabled");l?.addEventListener("input",()=>{m.textContent=Ot(l.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{n.textContent="Add Cron Job",i.value="",r.value="",o.value="",l.value="",m.textContent="",d.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let b=o.value.trim(),k=l.value.trim();if(!b||!k){c.error("Command and schedule are required");return}let p={label:r.value.trim(),command:b,schedule:k,enabled:d.checked},g=i.value;try{g?(await u.put(`/sites/${e}/crons/${g}`,p),c.success("Cron job updated")):(await u.post(`/sites/${e}/crons`,p),c.success("Cron job created")),UIkit.modal(s).hide(),await lt(t,e),a=await u.get(`/sites/${e}/crons`)}catch(v){c.error(v.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async b=>{let k=b.target.closest(".cron-detail-btn");if(k){let f=k.dataset.id,w=a.find(ut=>String(ut.ID)===f);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let x=document.getElementById("cron-detail-modal");UIkit.modal(x).show(),x.addEventListener("hidden",()=>x.remove(),{once:!0});return}let p=b.target.closest(".cron-edit-btn");if(p){let f=p.dataset.id,w=a.find(x=>String(x.ID)===f);if(!w)return;n.textContent="Edit Cron Job",i.value=w.ID,r.value=w.Label||"",o.value=w.Command,l.value=w.Schedule,m.textContent=Ot(w.Schedule),d.checked=w.Enabled,UIkit.modal(s).show();return}let g=b.target.closest(".cron-delete-btn");if(g){let f=g.dataset.id;if(!await E("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await u.delete(`/sites/${e}/crons/${f}`),c.success("Cron job deleted"),await lt(t,e),a=await u.get(`/sites/${e}/crons`)}catch(x){c.error(x.message)}return}let v=b.target.closest(".cron-run-btn");if(v){let f=v.dataset.id;try{await u.post(`/sites/${e}/crons/${f}/run`)}catch(P){c.error(P.message);return}S("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await u.get(`/sites/${e}/crons`)).find(H=>String(H.ID)===f)?.LastRun??null}catch{}let x=Date.now()+300*1e3,ut=setInterval(async()=>{try{let P=await u.get(`/sites/${e}/crons`),H=P.find(X=>String(X.ID)===f);if(!H||H.LastRun!==w||Date.now()>x){clearInterval(ut),y(),a=P??[];let X=t.querySelector("#cron-list-wrap");X&&(X.innerHTML=Vt(P)),H?.LastError?c.error(`Job failed: ${H.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let p=k.dataset.id;try{await u.patch(`/sites/${e}/crons/${p}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(g){c.error(g.message),k.checked=!k.checked}}),u.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function wt(t){return String(t).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Ot(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,n,i,r]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&n==="*"&&i==="*"&&r==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&n==="*"&&i==="*"&&r!=="*"){let o=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${r.split(",").map(m=>o[parseInt(m)]??m).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var C="";function Xt(t){return`
        <div class="kp-card uk-padding uk-margin-top" id="fm-root" data-site="${t}">
            ...
            <div id="fm-list"></div>
        </div>`}function He(){let t=C?C.split("/"):[],e="",a=['<a href="#" data-path="">html</a>'];for(let s of t)e=e?e+"/"+s:s,a.push(`<span class="kp-fm-sep">/</span><a href="#" data-path="${$(e)}">${$(s)}</a>`);return a.join("")}function Re(t,e){if(e)return"\u2014";let a=["B","KB","MB","GB","TB"],s=t,n=0;for(;s>=1024&&n<a.length-1;)s/=1024,n++;return`${n===0?s:s.toFixed(1)} ${a[n]}`}function Fe(t){let e=new Date(t);return isNaN(e)?"":e.toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function Jt(t){return t==="d"?"folder":t==="l"?"link":"file-text"}function j(t,e){return t?t+"/"+e:e}function Ue(t){return!t||!t.length?R("folder","This folder is empty"):`
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            ...
            <tbody>${t.map(a=>{let s=j(C,a.name),n=a.is_dir,i=n?`<a href="#" class="fm-nav" data-path="${$(s)}"><span uk-icon="icon: ${Jt(a.type)}; ratio: 0.9"></span> ${$(a.name)}</a>`:`<a href="#" class="fm-edit" data-path="${$(s)}"><span uk-icon="icon: ${Jt(a.type)}; ratio: 0.9"></span> ${$(a.name)}</a>`;return`
            <tr data-path="${$(s)}" data-name="${$(a.name)}" data-dir="${n?1:0}" data-mode="${$(a.mode)}">
                <td class="kp-fm-name">${i}</td>
                <td class="uk-text-nowrap">${Re(a.size,n)}</td>
                <td><code class="kp-mono">${$(a.mode)}</code></td>
                <td class="uk-text-nowrap uk-text-small kp-muted">${Fe(a.mod_time)}</td>
                <td class="uk-text-right uk-text-nowrap">
                    ${n?"":`<a class="kp-fm-act fm-download" href="/api/sites/${document.getElementById("fm-root").dataset.site}/files/download?path=${encodeURIComponent(s)}" uk-tooltip="Download"><span uk-icon="icon: download; ratio: 0.85"></span></a>`}
            ...
            </div>`,document.body.appendChild(i),window.UIkit&&UIkit.icon(i);let r=UIkit.modal(i),o=i.querySelector("#fm-prompt-input"),l=!1,m=d=>{l||(l=!0,s(d),r.hide())};i.querySelector("#fm-prompt-ok").addEventListener("click",()=>m(o.value.trim()||null)),o.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),m(o.value.trim()||null))}),UIkit.util.on(i,"hidden",()=>{l||(l=!0,s(null)),i.remove()}),r.show(),setTimeout(()=>o.focus(),50)})}async function Ne(t,e){let a=j(C,e.name),s=await fetch(`/api/sites/${t}/files/upload?path=${encodeURIComponent(a)}`,{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:e}),n=s.status===204?null:await s.json().catch(()=>null);if(!s.ok)throw new Error(n?.error||`HTTP ${s.status}`)}function Yt(t,e){C="",t.addEventListener("click",a=>{let s=a.target.closest("#fm-breadcrumb a");if(s){a.preventDefault(),Gt(e,s.dataset.path);return}let n=a.target.closest(".fm-nav");if(n){a.preventDefault(),Gt(e,n.dataset.path);return}let i=a.target.closest(".fm-edit");if(i){a.preventDefault(),Oe(e,i.dataset.path,i.dataset.path.split("/").pop());return}}),t.querySelector("#fm-new-file")?.addEventListener("click",async()=>{let a=await G("New File","File name");if(a)try{await u.post(`/sites/${e}/files/file`,{path:j(C,a)}),_(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-new-dir")?.addEventListener("click",async()=>{let a=await G("New Folder","Folder name");if(a)try{await u.post(`/sites/${e}/files/dir`,{path:j(C,a)}),_(e)}catch(s){c.error(s.message)}}),t.querySelector("#fm-upload")?.addEventListener("change",async a=>{let s=[...a.target.files];if(s.length)try{for(let n of s)await Ne(e,n);c.success(s.length===1?"File uploaded":`${s.length} files uploaded`),_(e)}catch(n){c.error(n.message)}finally{a.target.value=""}}),t.querySelector("#fm-refresh")?.addEventListener("click",()=>_(e)),t.addEventListener("click",async a=>{let s=a.target.closest("tr[data-path]");if(!s)return;let n=s.dataset.path,i=s.dataset.name;if(a.target.closest(".fm-chmod")){let r=await G("Permissions",`Octal mode for "${i}"`,s.dataset.mode);if(!r)return;try{await u.patch(`/sites/${e}/files/chmod`,{path:n,mode:r}),_(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-rename")){let r=await G("Rename / Move","New path (relative to current folder)",i);if(!r||r===i)return;try{await u.post(`/sites/${e}/files/move`,{src:n,dst:j(C,r)}),_(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-copy")){let r=await G("Copy","Destination name",i+"-copy");if(!r)return;try{await u.post(`/sites/${e}/files/copy`,{src:n,dst:j(C,r)}),_(e)}catch(o){c.error(o.message)}return}if(a.target.closest(".fm-delete")){if(!await E("Delete",`Delete "${i}"? This cannot be undone.`))return;try{await u.delete(`/sites/${e}/files?path=${encodeURIComponent(n)}`),_(e)}catch(o){c.error(o.message)}return}})}var rt=null;function Qt(t){if(document.querySelector(`link[href="${t}"]`))return;let e=document.createElement("link");e.rel="stylesheet",e.href=t,document.head.appendChild(e)}function xt(t){return new Promise((e,a)=>{let s=document.querySelector(`script[src="${t}"]`);if(s){if(s.dataset.loaded)return e();s.addEventListener("load",()=>e()),s.addEventListener("error",()=>a(new Error("failed to load "+t)));return}let n=document.createElement("script");n.src=t,n.addEventListener("load",()=>{n.dataset.loaded="1",e()}),n.addEventListener("error",()=>a(new Error("failed to load "+t))),document.head.appendChild(n)})}function je(){if(rt)return rt;let t="https://cdn.jsdelivr.net/npm/codemirror@5";return Qt(`${t}/lib/codemirror.css`),Qt(`${t}/theme/material-darker.css`),rt=xt(`${t}/lib/codemirror.js`).then(()=>Promise.all([xt(`${t}/mode/meta.js`),xt(`${t}/addon/mode/loadmode.js`)])).then(()=>{window.CodeMirror.modeURL=`${t}/mode/%N/%N.js`}),rt}function We(t){let e=window.CodeMirror.findModeByFileName(t);return e?e.mode:null}async function Oe(t,e,a){let s;try{s=await u.get(`/sites/${t}/files/content?path=${encodeURIComponent(e)}`)}catch(k){let p=/too large/i.test(k.message)?"File is too large to edit \u2014 download it instead.":/binary/i.test(k.message)?"Binary file \u2014 download it instead of editing.":k.message;c.error(p);return}try{await je()}catch(k){c.error("Editor failed to load: "+k.message);return}let n="fm-editor-modal";document.getElementById(n)?.remove();let i=document.createElement("div");i.id=n,i.setAttribute("uk-modal",""),i.innerHTML=`
        <div class="uk-modal-dialog kp-modal kp-fm-editor-dialog">
            <div class="uk-modal-header uk-flex uk-flex-middle uk-flex-between">
                <h3 class="kp-modal-title uk-margin-remove"><span uk-icon="file-text"></span> ${$(a)} <span id="fm-ed-dirty" class="kp-muted uk-text-small" hidden>\u2022 unsaved</span></h3>
                <div class="uk-flex" style="gap:8px">
        ...
        </div>`,document.body.appendChild(i),window.UIkit&&UIkit.icon(i);let r=UIkit.modal(i,{bgClose:!1,escClose:!0}),o=i.querySelector("#fm-ed-dirty"),l=null,m=!0,d=k=>{m=!k,o.hidden=!k};UIkit.util.on(i,"shown",()=>{if(l)return;l=window.CodeMirror.fromTextArea(i.querySelector("#fm-ed-area"),{value:s.content,lineNumbers:!0,theme:"material-darker",indentUnit:4,lineWrapping:!1,extraKeys:{"Ctrl-S":b,"Cmd-S":b,"Ctrl-F":"findPersistent","Ctrl-/":"toggleComment"}}),l.setValue(s.content),l.on("change",()=>d(!0));let k=We(a);k&&(l.setOption("mode",k),window.CodeMirror.autoLoadMode(l,k)),setTimeout(()=>l.refresh(),30)}),UIkit.util.on(i,"hidden",()=>i.remove());async function b(){if(!l)return;let k=i.querySelector("#fm-ed-save"),p=k.innerHTML;k.disabled=!0,k.innerHTML='<div uk-spinner="ratio: 0.6"></div>';try{await u.put(`/sites/${t}/files/content`,{path:e,content:l.getValue()}),d(!1),c.success("Saved"),_(t)}catch(g){c.error(g.message)}finally{k.disabled=!1,k.innerHTML=p}}i.querySelector("#fm-ed-save").addEventListener("click",b),UIkit.util.on(i,"beforehide",k=>{!m&&!window.confirm("Discard unsaved changes?")&&k.preventDefault()}),r.show()}var ze=2e3;function St(t,e){return`
        <div>
            <div class="kp-log-controls">
        ...
        </div>`}function Zt(t,e){let a=null,s=!1,n=t.querySelector("#log-output"),i=t.querySelector("#log-connect"),r=t.querySelector("#log-disconnect"),o=t.querySelector("#log-clear"),l=t.querySelector("#log-autoscroll"),m=t.querySelector("#log-status");function d(p){for(p.split(`
`).forEach(g=>{if(!g)return;let v=document.createElement("div");v.className=g.match(/WAF BLOCK/i)?"kp-log-line-err":g.match(/WAF DETECT/i)?"kp-log-line-warn":g.match(/error|crit|emerg/i)?"kp-log-line-err":g.match(/warn/i)?"kp-log-line-warn":g.match(/info|notice/i)?"kp-log-line-info":"",v.textContent=g,n.appendChild(v)});n.childElementCount>ze;)n.removeChild(n.firstChild);l.checked&&(n.scrollTop=n.scrollHeight)}function b(){a&&(a.close(),a=null),s=!1,i.disabled=!1,r.disabled=!0,m&&(m.textContent="Disconnected")}i.addEventListener("click",()=>{b();let p=t.querySelector("#log-container").value,g=t.querySelector("#log-tail").value,v=location.protocol==="https:"?"wss":"ws",f=p==="waf"?`${v}://${location.host}/api/sites/${e}/logs/waf?tail=${g}`:p==="proxy"?`${v}://${location.host}/api/sites/${e}/logs/proxy?tail=${g}`:p==="access"?`${v}://${location.host}/api/sites/${e}/logs/proxy?tail=${g}`:`${v}://${location.host}/api/sites/${e}/logs?container=${p}&tail=${g}`;a=new WebSocket(f),a.onopen=()=>{s=!0,i.disabled=!0,r.disabled=!1,m&&(m.textContent=`Connected \u2014 ${p}`)},a.onmessage=w=>d(w.data),a.onerror=()=>{},a.onclose=()=>{s=!1,i.disabled=!1,r.disabled=!0,m&&(m.textContent="Disconnected")}}),r.addEventListener("click",b),o.addEventListener("click",()=>{n.innerHTML=""}),t.querySelector("#log-container").addEventListener("change",()=>{a&&a.readyState===WebSocket.OPEN&&(b(),i.click())});let k=h.go.bind(h);h.go=function(p,g={}){return a&&b(),k(p,g)}}function Ve(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function te(t,e){try{let a=await u.get(`/ssl-status?domain=${encodeURIComponent(t)}`),s=document.getElementById(`ssl-icon-${e}`);s&&(s.outerHTML=Ve(a.status))}catch{}}function ee(t){t.forEach(e=>te(e.Domain,e.ID))}function ae(t,e,a,s=0,n=null){let i=t.SiteType!==3&&t.PMAPort>0;return`
        ...
        </div>`}function pe(t,e){let a=t.querySelector("#wpcli-output"),s=t.querySelector("#wpcli-input"),n=t.querySelector("#wpcli-run"),i=t.querySelector("#wpcli-clear"),r=t.querySelector("#wpcli-status"),o=[],l=-1;function m(k,p=""){k.split(`
`).forEach(g=>{if(!g)return;let v=document.createElement("div");p?v.className=p:v.className=g.match(/error|fatal|critical/i)?"kp-log-line-err":g.match(/warning|warn/i)?"kp-log-line-warn":g.match(/success|done\]/i)?"kp-log-line-info":"",v.textContent=g,a.appendChild(v)}),a.scrollTop=a.scrollHeight}function d(k){if(k=k.trim(),!k)return;o.unshift(k),l=-1,m(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,n.disabled=!0,r&&(r.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",g=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);g.onopen=()=>{g.send(JSON.stringify({command:k}))},g.onmessage=v=>{let f=v.data;if(f.trim()==="[done]"){g.close();return}if(f.startsWith("[info]")){m(f,"kp-muted");return}if(f.startsWith("[error]")){m(f,"kp-log-line-err");return}m(f)},g.onerror=()=>{m("[error] WebSocket connection failed","kp-log-line-err")},g.onclose=()=>{s.disabled=!1,n.disabled=!1,r&&(r.textContent="Ready"),s.focus()}}n.addEventListener("click",()=>{d(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){d(s.value),s.value="",l=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),l<o.length-1&&(l++,s.value=o[l]);return}k.key==="ArrowDown"&&(k.preventDefault(),l>0?(l--,s.value=o[l]):(l=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let g=p.indexOf("''")+1;s.setSelectionRange(g,g);return}d(p)})}),i.addEventListener("click",()=>{a.innerHTML=""});let b=h.go.bind(h);h.go=function(k,p={}){return b(k,p)},s.focus()}var Q=null,M=null;function Je(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        </div>`}function ue(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),n=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function i(o,l=!1){UIkit.switcher(a).show(o),e.querySelectorAll(":scope > li[data-pill]").forEach(m=>m.classList.remove("kp-pill-active")),l?(s?.classList.add("kp-pill-active"),n?.querySelectorAll("a[data-switcher]").forEach(m=>{m.classList.toggle("kp-dd-active",parseInt(m.dataset.switcher,10)===o)})):(s?.classList.remove("kp-pill-active"),n?.querySelectorAll("a[data-switcher]").forEach(m=>m.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(o=>{o.addEventListener("click",l=>{l.preventDefault();let m=parseInt(o.closest("li").dataset.pill,10);i(m,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",o=>{o.stopPropagation(),n.hidden=!n.hidden,s.classList.toggle("kp-pill-active",!n.hidden)}),n?.querySelectorAll("a[data-switcher]").forEach(o=>{o.addEventListener("click",l=>{l.preventDefault(),n.hidden=!0,i(parseInt(o.dataset.switcher,10),!0)})}),document.addEventListener("click",o=>{n&&!s.contains(o.target)&&(n.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(0)}async function me(t){let e=document.getElementById("waf-tab-panel");if(!e)return;e.innerHTML=Je();let a=document.getElementById("waf-export-btn");a&&(a.href=`/api/sites/${t}/waf/export`);try{let s=await u.get(`/sites/${t}/waf`),n=document.getElementById("waf-override"),i=document.getElementById("waf-site-exclusions");n&&(n.value=String(s.Override??0)),i&&(i.value=s.Exclusions??"");let r=document.getElementById("waf-plugins-list");if(r){let[o,l]=await Promise.all([u.get("/settings/waf/plugins"),u.get(`/sites/${t}/waf/plugins`)]),m=new Set(l??[]);!o||o.length===0?r.innerHTML='<span class="kp-muted uk-text-small">No plugins found in local CRS install.</span>':(r.innerHTML=`
                    <div class="waf-plugin-pills">
                        ${o.map(d=>`
                        <span class="waf-plugin-pill ${m.has(d)?"active":""}"
                            data-plugin="${d}">${d}</span>
                        `).join("")}
                    </div>`,r.querySelectorAll(".waf-plugin-pill").forEach(d=>{d.addEventListener("click",()=>d.classList.toggle("active"))}))}}catch(s){c.error("Failed to load WAF settings: "+s.message)}}function Ge(t,e){Q&&Q.abort(),Q=new AbortController,t.addEventListener("submit",async a=>{if(a.target.id!=="waf-override-form")return;a.preventDefault();let s=a.target.querySelector('[type="submit"]'),n=s.innerHTML;s.disabled=!0,s.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let i=new FormData(a.target),r={override:parseInt(i.get("override"),10),exclusions:i.get("exclusions").trim()};try{await u.put(`/sites/${e}/waf`,r);let o=[...document.querySelectorAll(".waf-plugin-pill.active")].map(l=>l.dataset.plugin);await u.put(`/sites/${e}/waf/plugins`,o),c.success("WAF override saved \u2014 engine recompiling in background")}catch(o){c.error(o.message)}finally{s.disabled=!1,s.innerHTML=n}},{signal:Q.signal}),t.querySelector("#waf-import")?.addEventListener("change",async a=>{let s=a.target.files[0];if(!s)return;let n=new FormData;n.append("file",s);try{let i=await fetch(`/api/sites/${e}/waf/import`,{method:"POST",body:n}),r=i.status===204?null:await i.json().catch(()=>null);if(!i.ok)throw new Error(r?.error||`HTTP ${i.status}`);await me(e),c.success("WAF settings imported")}catch(i){c.error(i.message)}finally{a.target.value=""}})}function Qe(){return`
        <div class="kp-card uk-padding uk-margin-top">
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            ...
            <li>${oe()}</li>
            <li>${Xt(e)}</li>
        </ul>`}`,document.getElementById("sd-back").addEventListener("click",()=>h.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>tt(a)),document.getElementById("sd-site-nav")?.addEventListener("change",p=>{h.go("site-detail",{id:p.target.value})}),ot(t),q(t),Zt(t,e),Ge(t,e),me(e),m){Ye(t,e),Xe(e),vt(t,e,a.SiteType),ht(e,a.SiteType),yt(t,e),z(e),ue(t);return}document.getElementById("sd-recreate").addEventListener("click",async()=>{S("Recreating Pod","Recreating containers for this site...");try{await u.post(`/sites/${e}/recreate`),y(),c.success("Pod recreated"),h.go("site-detail",{id:e})}catch(p){y(),c.error(p.message)}}),document.getElementById("sd-clone")?.addEventListener("click",async()=>{let p=await Z(a.Name);if(p){S("Cloning Site","Copying files and database \u2014 this may take a few minutes...");try{await u.post(`/sites/${e}/clone`,{name:p},6e5),y(),c.success(`Site cloned as '${p}'`),h.go("sites")}catch(g){y(),c.error(g.message)}}}),t.querySelectorAll("[data-action]:not([data-action='wpcli-quick'])").forEach(p=>{p.addEventListener("click",async()=>{let g=p.dataset.action;if(g==="flush"){try{await u.post(`/sites/${e}/flush`),c.success("Caches flushed")}catch(f){c.error(f.message)}return}S(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[g]??g} Pod`,"Please wait...");try{await u.post(`/sites/${e}/${g}`),y(),c.success(`Site ${g} successful`),h.go("site-detail",{id:e})}catch(f){y(),c.error(f.message)}})}),Wt(t,e),ne(t,e),a.SiteType===1&&pe(t,e),ie(t,e,a),Nt(t,e),N(t,e),d&&(Kt(t,e),lt(t,e)),ce(t,e),re(e);let b=t.querySelector("#kp-site-switcher"),k=t.querySelector('a[data-switcher="files"]');b&&k&&(k.dataset.switcher=String(b.children.length-1)),Yt(t,e),_(e),ea(t,e),vt(t,e,a.SiteType),ht(e,a.SiteType),ue(t),yt(t,e),z(e),ee(s??[])}async function ct(t){let e=document.getElementById("totp-qr-img"),a=document.getElementById("totp-qr-wrap");if(!e||!a)return;if(a.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((i,r)=>{QRCode.toDataURL(t,{width:220,margin:2},(o,l)=>{o?r(o):i(l)})});e.src=n,e.style.display="";return}catch{}let s=document.createElement("p");s.className="totp-uri-text kp-muted uk-text-small",s.style.wordBreak="break-all",s.textContent=t,a.appendChild(s)}function dt(t){document.getElementById("kp-backup-codes-modal")?.remove();let a=`
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{i.textContent="Copied!"});else{let r=document.createElement("textarea");r.value=n,r.style.cssText="position:fixed;opacity:0",document.body.appendChild(r),r.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}r.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{s.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),h.go("users")})}function ge(t){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let i=UIkit.modal("#kp-edit-user-modal");i.show(),document.getElementById("edit-user-form").addEventListener("submit",async m=>{m.preventDefault();let d=m.target.querySelector('[type="submit"]'),b=d.innerHTML;d.disabled=!0,d.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(m.target),p={fname:k.get("fname").trim(),lname:k.get("lname").trim(),email:k.get("email").trim(),phone:k.get("phone").trim(),notify_email:k.get("notify_email")==="on",notify_sms:k.get("notify_sms")==="on"};if(s){p.role=parseInt(k.get("role"));let v=k.get("uname");v&&(p.uname=v.trim())}let g=k.get("password");g&&(p.password=g);try{await u.put(`/users/${e}`,p),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("User updated"),h.go("users")}catch(v){c.error(v.message),d.disabled=!1,d.innerHTML=b}});let r=document.getElementById("totp-setup-btn");r&&r.addEventListener("click",async()=>{r.disabled=!0,r.textContent="Setting up\u2026";try{let m=await u.post(`/users/${e}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=m.secret,document.getElementById("totp-setup-area").style.display="",await ct(m.uri)}catch(m){c.error(m.message),r.disabled=!1,r.textContent="Enable TOTP"}});let o=document.getElementById("totp-confirm-btn");o&&o.addEventListener("click",async()=>{let m=document.getElementById("totp-confirm-code").value.trim();if(m.length!==6){c.error("Enter a 6-digit code");return}o.disabled=!0;try{let d=await u.post(`/users/${e}/totp/confirm`,{code:m});i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),c.success("TOTP enabled"),d.backup_codes?.length?dt(d.backup_codes):h.go("users")}catch(d){c.error(d.message),o.disabled=!1}});let l=document.getElementById("totp-disable-btn");l&&l.addEventListener("click",async()=>{l.disabled=!0;try{await u.delete(`/users/${e}/totp`),c.success("TOTP disabled"),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),h.go("users")}catch(m){c.error(m.message),l.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function he(t){if(!I()){t.innerHTML=T("Access denied");return}let e=await u.get("/users");t.innerHTML=`
        <div class="kp-view-header">
