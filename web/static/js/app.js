...
"use strict";(()=>{var m={async _req(t,e,a,s=6e4){let r=new AbortController,n=setTimeout(()=>r.abort(),s),l={method:t,headers:{"Content-Type":"application/json"},signal:r.signal};t!=="GET"&&t!=="HEAD"&&(l.headers["X-CSRF-Token"]=window.KP?.csrf??""),a!==void 0&&(l.body=JSON.stringify(a));try{let i=await fetch("/api"+e,l);clearTimeout(n);let o=i.status===204?null:await i.json().catch(()=>null);if(i.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!i.ok)throw new Error(o?.error||`HTTP ${i.status}`);return o}catch(i){throw clearTimeout(n),i}},get:t=>m._req("GET",t),post:(t,e,a)=>m._req("POST",t,e,a),put:(t,e,a)=>m._req("PUT",t,e,a),delete:t=>m._req("DELETE",t),patch:(t,e)=>m._req("PATCH",t,e)};var y=t=>String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function E(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}var Z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',_=t=>`<div class="kp-empty">
...
`+et(d):d?u=et(d):u=et(o),document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);let e=UIkit.modal("#kp-create-site-modal"),a=document.getElementById("cs-site-type"),s=document.getElementById("cs-php-version-wrap"),r=document.getElementById("cs-node-version-wrap"),n=document.getElementById("cs-dotnet-version-wrap"),l=document.getElementById("cs-start-command-wrap"),i=document.getElementById("cs-wordpress-wrap");e.show();let o=document.getElementById("cs-domains-wrap"),d=document.getElementById("cs-rp-note");a.addEventListener("change",()=>{let u=parseInt(a.value);s.classList.toggle("uk-hidden",u!==1&&u!==2||u===6),r.classList.toggle("uk-hidden",u!==4),n.classList.toggle("uk-hidden",u!==5),l.classList.toggle("uk-hidden",u!==4&&u!==5),i.classList.toggle("uk-hidden",u!==1||u===6),o.classList.toggle("uk-hidden",u===6),d.classList.toggle("uk-hidden",u!==6)}),document.getElementById("create-site-form").addEventListener("submit",async u=>{u.preventDefault();let b=u.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Creating...';let p=new FormData(u.target),v=parseInt(p.get("site_type")),g=null;v===4&&(g=parseInt(p.get("node_version"))),v===5&&(g=parseInt(p.get("dotnet_version")));let h={name:p.get("name").trim(),php_version:parseInt(p.get("php_version"))||3,site_type:v,runtime_version:g,start_command:p.get("start_command")?.trim()||"",domains:p.get("domains").split(`
`).map(S=>S.trim()).filter(Boolean),install_wordpress:v===1?p.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${h.name}' as a reverse proxy...`:`Setting up '${h.name}' \u2014 pulling images and provisioning containers...`;$("Creating Site",w);try{await m.post("/sites",h,6e5),x(),c.success(`Site '${h.name}' created`),f.go("sites")}catch(S){x(),c.error(S.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var U=null;function Pe(t){return`${t.toFixed(1)}%`}var nt=null;function mt(){return nt||(nt=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),nt)}function kt(t,e){return`
        ...
        </table>`}function Be(t,e,a,s,r){if(!t||t.length===0)return'<p class="kp-muted uk-text-small">No matching requests found.</p>';let n=[...t].sort((b,k)=>{let p,v;switch(a){case"time":p=b.time,v=k.time;break;case"method":p=b.method,v=k.method;break;case"site":p=b.site_name,v=k.site_name;break;case"ip":p=b.client_ip,v=k.client_ip;break;default:p=b.status,v=k.status;break}return p<v?s?1:-1:p>v?s?-1:1:0}),l=50,i=Math.ceil(n.length/l),d=n.slice(e*l,(e+1)*l).map(b=>{let k=y(b.ua),p=b.status>=500?"kp-badge-danger":"kp-badge-warning";return`
            <tr>
...
${o}`:o),a.hide(),c.success(`${o} added to blacklist \u2014 save to apply`)})}catch(l){n.innerHTML="",c.error(l.message)}};document.getElementById("asn-lookup-go").addEventListener("click",s),document.getElementById("asn-lookup-q").addEventListener("keydown",r=>{r.key==="Enter"&&s()})}async function Rt(t){if(!q()){t.innerHTML=_("Access denied");return}t.innerHTML=`
        ...
        ${V(null)}`,ot(t),B(t)}function Re(t){switch(t){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';case"expired":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Expired certificate"></span>';case"mismatch":return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="Certificate does not match this domain"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function Dt(t){let e=document.getElementById("admin-domain-ssl");if(!(!e||!t))try{let a=await m.get(`/ssl-status?domain=${encodeURIComponent(t)}`);e.outerHTML=Re(a.status)}catch{}}async function Ft(t){if(!q()){t.innerHTML=_("Access denied");return}let[e,a,s,r,n,l]=await Promise.all([m.get("/settings"),m.get("/settings/backup"),m.get("/settings/waf"),m.get("/settings/trusted-proxies"),m.get("/settings/notifications"),m.get("/settings/resources")]);t.innerHTML=`
    <div class="kp-view-header">
    ...
    `,e.admin_domain&&Dt(e.admin_domain),document.getElementById("settings-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let b={admin_domain:new FormData(i.target).get("admin_domain").trim()};try{await m.put("/settings",b),c.success("Settings saved"),Dt(b.admin_domain)}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("settings-import").addEventListener("change",async i=>{let o=i.target.files[0];if(!o)return;let d=new FormData;d.append("file",o);try{let u=await fetch("/api/settings/import",{method:"POST",headers:{"X-CSRF-Token":window.KP?.csrf??""},body:d}),b=u.status===204?null:await u.json().catch(()=>null);if(!u.ok)throw new Error(b?.error||`HTTP ${u.status}`);c.success("Settings imported")}catch(u){c.error(u.message)}finally{i.target.value=""}}),document.getElementById("backup-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={backup_schedule:u.get("backup_schedule").trim(),backup_retain_days:u.get("backup_retain_days").trim()};try{await m.put("/settings/backup",b),c.success("Backup settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("s3-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={s3_endpoint:u.get("s3_endpoint").trim(),s3_bucket:u.get("s3_bucket").trim(),s3_region:u.get("s3_region").trim(),s3_access_key:u.get("s3_access_key").trim()},k=u.get("s3_secret_key").trim();k&&(b.s3_secret_key=k);try{await m.put("/settings/backup",b),c.success("S3 settings saved")}catch(p){c.error(p.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("smtp-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={smtp_host:u.get("smtp_host").trim(),smtp_port:u.get("smtp_port").trim(),smtp_username:u.get("smtp_username").trim(),smtp_from:u.get("smtp_from").trim(),smtp_tls:u.get("smtp_tls")?"true":"false"},k=u.get("smtp_password").trim();k&&(b.smtp_password=k);try{await m.put("/settings/notifications",b),c.success("Email notification settings saved")}catch(p){c.error(p.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("sns-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={aws_access_key:u.get("aws_access_key").trim(),aws_region:u.get("aws_region").trim(),aws_sns_sender_id:u.get("aws_sns_sender_id").trim()},k=u.get("aws_secret_key").trim();k&&(b.aws_secret_key=k);try{await m.put("/settings/notifications",b),c.success("SMS notification settings saved")}catch(p){c.error(p.message)}finally{o.disabled=!1,o.innerHTML=d}}),document.getElementById("resource-form").addEventListener("submit",async i=>{i.preventDefault();let o=i.target.querySelector('[type="submit"]'),d=o.innerHTML;o.disabled=!0,o.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let u=new FormData(i.target),b={resource_ram_reserve_gb:u.get("resource_ram_reserve_gb").trim(),resource_poll_interval:u.get("resource_poll_interval").trim(),resource_throttle_pct:u.get("resource_throttle_pct").trim(),resource_webhook_url:u.get("resource_webhook_url").trim(),shutdown_job_timeout:u.get("shutdown_job_timeout").trim()};try{await m.put("/settings/resources",b),c.success("Resource watcher settings saved")}catch(k){c.error(k.message)}finally{o.disabled=!1,o.innerHTML=d}})}async function Ut(t){let e=`
        ...
        <tr>
            ...
            <td>
        ...
        </div>`}async function it(t,e){let a=t.querySelector("#cron-list-wrap");if(a)try{let s=await m.get(`/sites/${e}/crons`);a.innerHTML=Jt(s)}catch(s){a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load cron jobs: ${y(s.message)}</p>`}}function Xt(t,e){let a=[],s=t.querySelector("#cron-modal"),r=t.querySelector("#cron-modal-title"),n=t.querySelector("#cron-modal-id"),l=t.querySelector("#cron-modal-label"),i=t.querySelector("#cron-modal-command"),o=t.querySelector("#cron-modal-schedule"),d=t.querySelector("#cron-schedule-preview"),u=t.querySelector("#cron-modal-enabled");o?.addEventListener("input",()=>{d.textContent=Vt(o.value.trim())}),t.querySelector("#cron-add-btn")?.addEventListener("click",()=>{r.textContent="Add Cron Job",n.value="",l.value="",i.value="",o.value="",d.textContent="",u.checked=!0,UIkit.modal(s).show()}),t.querySelector("#cron-modal-save")?.addEventListener("click",async()=>{let b=i.value.trim(),k=o.value.trim();if(!b||!k){c.error("Command and schedule are required");return}let p={label:l.value.trim(),command:b,schedule:k,enabled:u.checked},v=n.value;try{v?(await m.put(`/sites/${e}/crons/${v}`,p),c.success("Cron job updated")):(await m.post(`/sites/${e}/crons`,p),c.success("Cron job created")),UIkit.modal(s).hide(),await it(t,e),a=await m.get(`/sites/${e}/crons`)}catch(g){c.error(g.message)}}),t.querySelector("#cron-list-wrap")?.addEventListener("click",async b=>{let k=b.target.closest(".cron-detail-btn");if(k){let h=k.dataset.id,w=a.find(R=>String(R.ID)===h);if(!w)return;document.body.insertAdjacentHTML("beforeend",`
                ...
                </div>`);let S=document.getElementById("cron-detail-modal");UIkit.modal(S).show(),S.addEventListener("hidden",()=>S.remove(),{once:!0});return}let p=b.target.closest(".cron-edit-btn");if(p){let h=p.dataset.id,w=a.find(S=>String(S.ID)===h);if(!w)return;r.textContent="Edit Cron Job",n.value=w.ID,l.value=w.Label||"",i.value=w.Command,o.value=w.Schedule,d.textContent=Vt(w.Schedule),u.checked=w.Enabled,UIkit.modal(s).show();return}let v=b.target.closest(".cron-delete-btn");if(v){let h=v.dataset.id;if(!await L("Delete Cron Job","This will permanently remove the cron job. Continue?"))return;try{await m.delete(`/sites/${e}/crons/${h}`),c.success("Cron job deleted"),await it(t,e),a=await m.get(`/sites/${e}/crons`)}catch(S){c.error(S.message)}return}let g=b.target.closest(".cron-run-btn");if(g){let h=g.dataset.id;try{await m.post(`/sites/${e}/crons/${h}/run`)}catch(I){c.error(I.message);return}$("Running Cron Job","Executing the job inside the container \u2014 please wait.");let w=null;try{w=(await m.get(`/sites/${e}/crons`)).find(D=>String(D.ID)===h)?.LastRun??null}catch{}let S=Date.now()+300*1e3,R=setInterval(async()=>{try{let I=await m.get(`/sites/${e}/crons`),D=I.find(Y=>String(Y.ID)===h);if(!D||D.LastRun!==w||Date.now()>S){clearInterval(R),x(),a=I??[];let Y=t.querySelector("#cron-list-wrap");Y&&(Y.innerHTML=Jt(I)),D?.LastError?c.error(`Job failed: ${D.LastError}`):c.success("Cron job complete")}}catch{}},2e3);return}}),t.querySelector("#cron-list-wrap")?.addEventListener("change",async b=>{let k=b.target.closest(".cron-toggle");if(!k)return;let p=k.dataset.id;try{await m.patch(`/sites/${e}/crons/${p}/toggle`,{enabled:k.checked}),c.success(k.checked?"Cron job enabled":"Cron job disabled")}catch(v){c.error(v.message),k.checked=!k.checked}}),m.get(`/sites/${e}/crons`).then(b=>{a=b??[]}).catch(()=>{})}function Vt(t){if(!t)return"";let e=t.trim().split(/\s+/);if(e.length!==5)return"invalid expression";let[a,s,r,n,l]=e;if(t==="* * * * *")return"every minute";if(a!=="*"&&s!=="*"&&r==="*"&&n==="*"&&l==="*")return`daily at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`;if(a!=="*"&&s!=="*"&&r==="*"&&n==="*"&&l!=="*"){let i=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`weekly on ${l.split(",").map(d=>i[parseInt(d)]??d).join(", ")} at ${s.padStart(2,"0")}:${a.padStart(2,"0")}`}return a.startsWith("*/")?`every ${a.slice(2)} minutes`:s.startsWith("*/")?`every ${s.slice(2)} hours`:t}var P="";function te(t){return`
        ...
        <table class="uk-table uk-table-divider uk-table-small uk-table-middle kp-fm-table">
            ...
            <tbody>${t.map(s=>{let r=W(P,s.name),n=s.is_dir,l=je(s.name,n),i=n?`<a href="#" class="fm-nav" data-path="${y(r)}"><span uk-icon="icon: ${Gt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</a>`:`<span><span uk-icon="icon: ${Gt(s.type)}; ratio: 0.9"></span> ${y(s.name)}</span>`;return`
        ...
        </table>`}async function C(t){let e=document.getElementById("fm-list"),a=document.getElementById("fm-breadcrumb");if(e){e.innerHTML=Z(),a&&(a.innerHTML=Ue());try{let s=await m.get(`/sites/${t}/files?path=${encodeURIComponent(P)}`);e.innerHTML=We(s)}catch(s){e.innerHTML=_("Failed to list files: "+s.message)}}}function Yt(t,e){P=e||"",C(t)}function Q(t,e,a=""){return new Promise(s=>{let r="fm-prompt-modal";document.getElementById(r)?.remove();let n=document.createElement("div");n.id=r,n.setAttribute("uk-modal",""),n.innerHTML=`
...
`).forEach(v=>{if(!v)return;let g=document.createElement("div");p?g.className=p:g.className=v.match(/error|fatal|critical/i)?"kp-log-line-err":v.match(/warning|warn/i)?"kp-log-line-warn":v.match(/success|done\]/i)?"kp-log-line-info":"",g.textContent=v,a.appendChild(g)}),a.scrollTop=a.scrollHeight}function u(k){if(k=k.trim(),!k)return;i.unshift(k),o=-1,d(`wp> ${k}`,"kp-log-line-info"),s.disabled=!0,r.disabled=!0,l&&(l.textContent="Running...");let p=location.protocol==="https:"?"wss":"ws",v=new WebSocket(`${p}://${location.host}/api/sites/${e}/wpcli`);v.onopen=()=>{v.send(JSON.stringify({command:k}))},v.onmessage=g=>{let h=g.data;if(h.trim()==="[done]"){v.close();return}if(h.startsWith("[info]")){d(h,"kp-muted");return}if(h.startsWith("[error]")){d(h,"kp-log-line-err");return}d(h)},v.onerror=()=>{d("[error] WebSocket connection failed","kp-log-line-err")},v.onclose=()=>{s.disabled=!1,r.disabled=!1,l&&(l.textContent="Ready"),s.focus()}}r.addEventListener("click",()=>{u(s.value),s.value=""}),s.addEventListener("keydown",k=>{if(k.key==="Enter"){u(s.value),s.value="",o=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),o<i.length-1&&(o++,s.value=i[o]);return}k.key==="ArrowDown"&&(k.preventDefault(),o>0?(o--,s.value=i[o]):(o=-1,s.value=""))}),t.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let p=k.dataset.cmd;if(p.startsWith("search-replace")){s.value=p,s.focus();let v=p.indexOf("''")+1;s.setSelectionRange(v,v);return}u(p)})}),n.addEventListener("click",()=>{a.innerHTML=""});let b=f.go.bind(f);f.go=function(k,p={}){return b(k,p)},s.focus()}var O=null,M=null;function ve(t){let e=t.querySelector("#kp-site-pills"),a=t.querySelector("#kp-site-switcher"),s=t.querySelector("#kp-manage-pill"),r=t.querySelector("#kp-manage-dropdown");if(!e||!a)return;function n(i,o=!1){UIkit.switcher(a).show(i),e.querySelectorAll(":scope > li[data-pill]").forEach(d=>d.classList.remove("kp-pill-active")),o?(s?.classList.add("kp-pill-active"),r?.querySelectorAll("a[data-switcher]").forEach(d=>{d.classList.toggle("kp-dd-active",parseInt(d.dataset.switcher,10)===i)})):(s?.classList.remove("kp-pill-active"),r?.querySelectorAll("a[data-switcher]").forEach(d=>d.classList.remove("kp-dd-active")))}e.querySelectorAll(":scope > li[data-pill] > a").forEach(i=>{i.addEventListener("click",o=>{o.preventDefault();let d=parseInt(i.closest("li").dataset.pill,10);n(d,!1)})}),s?.querySelector(".kp-pill-dropdown-btn")?.addEventListener("click",i=>{i.stopPropagation(),r.hidden=!r.hidden,s.classList.toggle("kp-pill-active",!r.hidden)}),r?.querySelectorAll("a[data-switcher]").forEach(i=>{i.addEventListener("click",o=>{o.preventDefault(),r.hidden=!0,n(parseInt(i.dataset.switcher,10),!0)})}),document.addEventListener("click",i=>{r&&!s.contains(i.target)&&(r.hidden=!0)},{capture:!0}),UIkit.switcher(a).show(1)}function Ye(){return`
            ...
            <div class="uk-flex uk-flex-middle" style="gap:12px">
                ...
                <div class="kp-site-nav-wrap">
                    <select id="sd-site-nav" class="uk-select kp-select">
                        ${i.map(p=>`<option value="${p.ID}" ${p.ID===a.ID?"selected":""}>${p.Name}</option>`).join("")}
                    </select>
        ...
        <ul class="uk-switcher" id="kp-site-switcher">
            <li>${oe(a,s??[],r,a.ParentID??0,i.find(p=>p.ID===a.ParentID)?.Name??null)}</li>
            <li>${kt(e,a.SiteType)}</li>
            <li>${G(e,1,l[1])}</li>
            ${o?`<li>${G(e,2,l[2])}</li>`:""}
            <li>${G(e,3,l[3])}</li>
