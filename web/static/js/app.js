...
`).map(S=>S.trim()).filter(Boolean),install_wordpress:v===1?b.get("install_wordpress")==="on":!1};e.hide(),document.getElementById("kp-create-site-modal")?.remove();let w=v===6?`Setting up '${f.name}' as a reverse proxy...`:`Setting up '${f.name}' \u2014 pulling images and provisioning containers...`;x("Creating Site",w);try{await d.post("/sites",f),y(),c.success(`Site '${f.name}' created`),g.go("sites")}catch(S){y(),c.error(S.message),u.disabled=!1,u.innerHTML=m}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}var V=null;function T(t){return t===0?"0 B":t<1024?`${t} B`:t<1048576?`${(t/1024).toFixed(1)} KB`:t<1073741824?`${(t/1048576).toFixed(1)} MB`:`${(t/1073741824).toFixed(2)} GB`}function jt(t){return`${t.toFixed(1)}%`}var J=null;function st(){return J||(J=new Promise(t=>{if(window.Chart){t();return}let e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@latest/dist/chart.umd.min.js",e.onload=t,e.onerror=t,document.body.appendChild(e)}),J)}function nt(t,e){return`
        ...
        ${e.length===0?j("world","No sites yet \u2014 create one to get started"):`<div class="kp-table-wrap">
            ...
            </div>`}`,document.getElementById("sites-new-btn").addEventListener("click",()=>z()),Vt()}function zt(t,e=[]){let a=t.Domains?.[0]??null,s=t.SiteType===6,n=t.ParentID>0?e.find(i=>i.ID===t.ParentID)??null:null;return`
