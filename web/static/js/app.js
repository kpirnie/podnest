"use strict";(()=>{var d={async _req(e,t,s,a=6e4){let n=new AbortController,i=setTimeout(()=>n.abort(),a),c={method:e,headers:{"Content-Type":"application/json"},signal:n.signal};s!==void 0&&(c.body=JSON.stringify(s));try{let r=await fetch("/api"+t,c);clearTimeout(i);let o=r.status===204?null:await r.json().catch(()=>null);if(r.status===401)return window.location.href="/login?msg=Your+session+has+expired+%E2%80%94+please+log+in+again",null;if(!r.ok)throw new Error(o?.error||`HTTP ${r.status}`);return o}catch(r){throw clearTimeout(i),r}},get:e=>d._req("GET",e),post:(e,t)=>d._req("POST",e,t),put:(e,t)=>d._req("PUT",e,t),delete:e=>d._req("DELETE",e)};var z=()=>'<div class="kp-spinner"><div uk-spinner="ratio: 1.25"></div></div>',S=e=>`<div class="kp-empty">
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",s),UIkit.modal("#kp-progress-modal").show()}function h(){let e=document.getElementById("kp-progress-modal");e&&(UIkit.modal(e).hide(),setTimeout(()=>e.remove(),300))}var m={routes:{},register(e,t){this.routes[e]=t},async go(e,t={}){let s=Object.keys(t).length?e+"/"+Object.values(t).join("/"):e;window.location.hash=s,document.querySelectorAll(".kp-nav-link").forEach(i=>{i.classList.toggle("kp-active",i.dataset.view===e)});let a=this.routes[e];if(!a)return;let n=document.getElementById("kp-view");n.innerHTML=z();try{await a(n,t)}catch(i){n.innerHTML=S(i.message)}}};function O(){let t=(window.location.hash.replace("#","")||"dashboard").split("/"),s=t[0],a={};return s==="site-detail"&&t[1]&&(a.id=t[1]),{view:s,params:a}}var l={show(e,t="info",s=7e3){let a={success:"check",error:"warning",info:"info"},n=document.createElement("div");n.className=`kp-toast kp-toast-${t}`,n.innerHTML=`<span uk-icon="${a[t]||"info"}"></span><span>${e}</span>`,document.getElementById("kp-toasts").appendChild(n),UIkit.icon(n.querySelector("[uk-icon]")),setTimeout(()=>n.remove(),s)},success:e=>l.show(e,"success"),error:e=>l.show(e,"error"),info:e=>l.show(e,"info")};async function _(e){let t=`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",t);let s=UIkit.modal("#kp-edit-site-modal"),a=document.getElementById("es-site-type"),n=document.getElementById("es-php-version-wrap"),i=document.getElementById("es-node-version-wrap"),c=document.getElementById("es-dotnet-version-wrap"),r=document.getElementById("es-start-command-wrap"),o=document.getElementById("es-wordpress-wrap");s.show();let p=u=>{n.classList.toggle("uk-hidden",u!==1&&u!==2),i.classList.toggle("uk-hidden",u!==4),c.classList.toggle("uk-hidden",u!==5),r.classList.toggle("uk-hidden",u!==4&&u!==5),o.classList.toggle("uk-hidden",u!==1)};p(e.SiteType),a.addEventListener("change",()=>p(parseInt(a.value))),document.getElementById("edit-site-form").addEventListener("submit",async u=>{u.preventDefault();let b=u.target.querySelector('[type="submit"]'),k=b.innerHTML;b.disabled=!0,b.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let v=new FormData(u.target),g=parseInt(v.get("site_type")),f=null;g===4&&(f=parseInt(v.get("node_version"))),g===5&&(f=parseInt(v.get("dotnet_version")));let x={name:v.get("name").trim(),php_version:parseInt(v.get("php_version"))||3,site_type:g,runtime_version:f,start_command:v.get("start_command")?.trim()||""},ge=g===1?v.get("install_wordpress")==="on":!1;try{await d.put(`/sites/${e.ID}`,x),s.hide(),document.getElementById("kp-edit-site-modal")?.remove(),y("Applying Changes","Saving changes and recreating pod...");try{await d.post(`/sites/${e.ID}/recreate`,{install_wordpress:ge}),h(),l.success("Site updated and pod recreated")}catch(W){h(),l.error("Site saved but pod recreate failed: "+W.message)}m.go("site-detail",{id:String(e.ID)})}catch(W){l.error(W.message),b.disabled=!1,b.innerHTML=k}}),document.getElementById("kp-edit-site-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-site-modal")?.remove())}function H(){document.body.insertAdjacentHTML("beforeend",`
...
`).map(f=>f.trim()).filter(Boolean),install_wordpress:k===1?b.get("install_wordpress")==="on":!1};t.hide(),document.getElementById("kp-create-site-modal")?.remove(),y("Creating Site",`Setting up '${g.name}' \u2014 pulling images and provisioning containers...`);try{await d.post("/sites",g),h(),l.success(`Site '${g.name}' created`),m.go("sites")}catch(f){h(),l.error(f.message),p.disabled=!1,p.innerHTML=u}}),document.getElementById("kp-create-site-modal").addEventListener("hidden",()=>document.getElementById("kp-create-site-modal")?.remove())}async function K(e){let t=await d.get("/sites")??[];e.innerHTML=`
        ...
        <div class="kp-site-grid">
            ${t.length===0?q("world","No sites yet \u2014 create one to get started"):t.map(j).join("")}
        ...
        </div>`}async function Q(e){let t=await d.get("/sites")??[],s=t.filter(i=>i.SiteStatus===1).length,a=t.filter(i=>i.SiteStatus===2).length,n=t.filter(i=>i.SiteStatus===4).length;e.innerHTML=`
        <div class="kp-view-header">
        ...
        <div class="kp-site-grid">
            ${t.length===0?q("world","No sites yet"):t.slice(0,6).map(j).join("")}
    ...
    `,t.admin_domain&&Y(t.admin_domain),document.getElementById("settings-form").addEventListener("submit",async n=>{n.preventDefault();let i=n.target.querySelector('[type="submit"]'),c=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let o={admin_domain:new FormData(n.target).get("admin_domain").trim()};try{await d.put("/settings",o),l.success("Settings saved"),Y(o.admin_domain)}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=c}}),document.getElementById("backup-form").addEventListener("submit",async n=>{n.preventDefault();let i=n.target.querySelector('[type="submit"]'),c=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let r=new FormData(n.target),o={backup_schedule:r.get("backup_schedule").trim(),backup_retain_days:r.get("backup_retain_days").trim()};try{await d.put("/settings/backup",o),l.success("Backup settings saved")}catch(p){l.error(p.message)}finally{i.disabled=!1,i.innerHTML=c}}),document.getElementById("s3-form").addEventListener("submit",async n=>{n.preventDefault();let i=n.target.querySelector('[type="submit"]'),c=i.innerHTML;i.disabled=!0,i.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let r=new FormData(n.target),o={s3_endpoint:r.get("s3_endpoint").trim(),s3_bucket:r.get("s3_bucket").trim(),s3_region:r.get("s3_region").trim(),s3_access_key:r.get("s3_access_key").trim()},p=r.get("s3_secret_key").trim();p&&(o.s3_secret_key=p);try{await d.put("/settings/backup",o),l.success("S3 settings saved")}catch(u){l.error(u.message)}finally{i.disabled=!1,i.innerHTML=c}})}function X(e){return`
        ...
        <tr>
            ...
            <td>
                <div class="uk-flex" style="gap:6px">
                    ...
                    </div>`}else c.innerHTML="";let r=e.querySelector("#backup-list-wrap");r&&(r.innerHTML=he(a))}catch(s){let a=e.querySelector("#backup-list-wrap");a&&(a.innerHTML=`<p class="kp-muted uk-text-small">Failed to load backups: ${s.message}</p>`)}}function Z(e,t){e.querySelector("#backup-repo-save")?.addEventListener("click",async()=>{let s={local_enabled:e.querySelector("#backup-local-enabled")?.checked??!1,s3_enabled:e.querySelector("#backup-s3-enabled")?.checked??!1};try{await d.put(`/sites/${t}/backup-repo`,s),l.success("Backup destinations saved")}catch(a){l.error(a.message)}}),e.querySelector("#backup-run-btn")?.addEventListener("click",async()=>{let s=0;try{s=(await d.get(`/sites/${t}/backups`))?.length??0}catch{}try{await d.post(`/sites/${t}/backups`,{label:"manual"})}catch(i){l.error(i.message);return}y("Backup Running","Snapshotting files and database \u2014 this may take a few minutes.");let a=Date.now()+10*60*1e3,n=setInterval(async()=>{try{(((await d.get(`/sites/${t}/backups`))?.length??0)>s||Date.now()>a)&&(clearInterval(n),h(),await B(e,t),Date.now()<=a&&l.success("Backup complete"))}catch{}},4e3)}),e.querySelector("#backup-list-wrap")?.addEventListener("click",async s=>{let a=s.target.closest(".backup-restore-btn");if(a){let c=a.dataset.id;if(!await w("Restore Site","This will restore the site from the selected snapshot. The site will show a maintenance page during the restore. Continue?"))return;try{await d.post(`/sites/${t}/backups/${c}/restore`)}catch(b){l.error(b.message);return}y("Restore Running","Restoring files and database \u2014 the site will return automatically when complete.");let o=Date.now(),p=Date.now()+15*60*1e3,u=setInterval(async()=>{try{let b=await d.get(`/sites/${t}/backups/restore-status`);(!b?.active||Date.now()>p)&&(clearInterval(u),h(),b?.active?l.error("Restore timed out"):l.success("Restore complete"),await B(e,t))}catch{}},3e3);return}let n=s.target.closest(".backup-delete-btn");if(n){let c=n.dataset.id;if(!await w("Delete Snapshot","This will permanently remove the snapshot from all configured repositories. This cannot be undone."))return;y("Deleting Snapshot","Removing snapshot data from repositories \u2014 this may take a moment.");try{await d.delete(`/sites/${t}/backups/${c}`),h(),l.success("Snapshot deleted"),await B(e,t)}catch(o){h(),l.error(o.message)}}let i=s.target.closest(".backup-download-btn");if(i){let c=i.dataset.id;y("Preparing Download","Your backup archive is being generated \u2014 this may take a moment depending on site size. Your download will begin automatically. Do not close this tab."),setTimeout(()=>{let r=document.createElement("a");r.href=`/api/sites/${t}/backups/${c}/download`,r.style.display="none",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>{h()},5e3)},300);return}})}var N={1:"Nginx",2:"PHP",3:"MariaDB",4:"Redis",5:"Varnish"};function C(e,t,s){let a=s?Object.entries(s):[];return`
        <div>
            <div class="uk-flex uk-flex-between uk-flex-middle uk-margin-small-bottom">
        ...
        </div>`}function ee(e,t){let s=t?.enabled==="true",a=t?Object.entries(t).filter(([n])=>n!=="enabled"):[];return`
        <div>
        ...
        </div>`}function ae(e,t){let s=null,a=!1,n=e.querySelector("#log-output"),i=e.querySelector("#log-connect"),c=e.querySelector("#log-disconnect"),r=e.querySelector("#log-clear"),o=e.querySelector("#log-autoscroll"),p=e.querySelector("#log-status");function u(v){v.split(`
`).forEach(g=>{if(!g)return;let f=document.createElement("div");f.className=g.match(/error|crit|emerg/i)?"kp-log-line-err":g.match(/warn/i)?"kp-log-line-warn":g.match(/info|notice/i)?"kp-log-line-info":"",f.textContent=g,n.appendChild(f)}),o.checked&&(n.scrollTop=n.scrollHeight)}function b(){s&&(s.close(),s=null),a=!1,i.disabled=!1,c.disabled=!0,p&&(p.textContent="Disconnected")}i.addEventListener("click",()=>{b();let v=e.querySelector("#log-container").value,g=e.querySelector("#log-tail").value,f=location.protocol==="https:"?"wss":"ws";s=new WebSocket(`${f}://${location.host}/api/sites/${t}/logs?container=${v}&tail=${g}`),s.onopen=()=>{a=!0,i.disabled=!0,c.disabled=!1,p&&(p.textContent=`Connected \u2014 ${v}`)},s.onmessage=x=>u(x.data),s.onerror=()=>{},s.onclose=()=>{a=!1,i.disabled=!1,c.disabled=!0,p&&(p.textContent="Disconnected")}}),c.addEventListener("click",b),r.addEventListener("click",()=>{n.innerHTML=""}),e.querySelector("#log-container").addEventListener("change",()=>{s&&s.readyState===WebSocket.OPEN&&(b(),i.click())});let k=m.go.bind(m);m.go=function(v,g={}){return s&&b(),k(v,g)}}function ye(e){switch(e){case"valid":return'<span class="kp-ssl-valid" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Valid SSL certificate"></span>';case"self-signed":return'<span class="kp-ssl-self-signed" uk-icon="icon: lock; ratio: 0.85" uk-tooltip="Self-signed certificate"></span>';default:return'<span class="kp-ssl-none" uk-icon="icon: warning; ratio: 0.85" uk-tooltip="No SSL certificate"></span>'}}async function ne(e,t){try{let s=await d.get(`/ssl-status?domain=${encodeURIComponent(e)}`),a=document.getElementById(`ssl-icon-${t}`);a&&(a.outerHTML=ye(s.status))}catch{}}function ie(e){e.forEach(t=>ne(t.Domain,t.ID))}function oe(e,t,s){let a=e.SiteType!==3&&e.PMAPort>0;return`
        ...
        </div>`}function pe(e,t){let s=e.querySelector("#wpcli-output"),a=e.querySelector("#wpcli-input"),n=e.querySelector("#wpcli-run"),i=e.querySelector("#wpcli-clear"),c=e.querySelector("#wpcli-status"),r=[],o=-1;function p(k,v=""){k.split(`
`).forEach(g=>{if(!g)return;let f=document.createElement("div");v?f.className=v:f.className=g.match(/error|fatal|critical/i)?"kp-log-line-err":g.match(/warning|warn/i)?"kp-log-line-warn":g.match(/success|done\]/i)?"kp-log-line-info":"",f.textContent=g,s.appendChild(f)}),s.scrollTop=s.scrollHeight}function u(k){if(k=k.trim(),!k)return;r.unshift(k),o=-1,p(`wp> ${k}`,"kp-log-line-info"),a.disabled=!0,n.disabled=!0,c&&(c.textContent="Running...");let v=location.protocol==="https:"?"wss":"ws",g=new WebSocket(`${v}://${location.host}/api/sites/${t}/wpcli`);g.onopen=()=>{g.send(JSON.stringify({command:k}))},g.onmessage=f=>{let x=f.data;if(x.trim()==="[done]"){g.close();return}if(x.startsWith("[info]")){p(x,"kp-muted");return}if(x.startsWith("[error]")){p(x,"kp-log-line-err");return}p(x)},g.onerror=()=>{p("[error] WebSocket connection failed","kp-log-line-err")},g.onclose=()=>{a.disabled=!1,n.disabled=!1,c&&(c.textContent="Ready"),a.focus()}}n.addEventListener("click",()=>{u(a.value),a.value=""}),a.addEventListener("keydown",k=>{if(k.key==="Enter"){u(a.value),a.value="",o=-1;return}if(k.key==="ArrowUp"){k.preventDefault(),o<r.length-1&&(o++,a.value=r[o]);return}k.key==="ArrowDown"&&(k.preventDefault(),o>0?(o--,a.value=r[o]):(o=-1,a.value=""))}),e.querySelectorAll('[data-action="wpcli-quick"]').forEach(k=>{k.addEventListener("click",()=>{let v=k.dataset.cmd;if(v.startsWith("search-replace")){a.value=v,a.focus();let g=v.indexOf("''")+1;a.setSelectionRange(g,g);return}u(v)})}),i.addEventListener("click",()=>{s.innerHTML=""});let b=m.go.bind(m);m.go=function(k,v={}){return b(k,v)},a.focus()}function xe(){return`
        ...
        <ul class="uk-switcher">
            ...
            <li>${X(t)}</li>
        </ul>`,document.getElementById("sd-back").addEventListener("click",()=>m.go("sites")),document.getElementById("sd-edit").addEventListener("click",()=>_(s)),document.getElementById("sd-recreate").addEventListener("click",async()=>{y("Recreating Pod","Recreating containers for this site...");try{await d.post(`/sites/${t}/recreate`),h(),l.success("Pod recreated"),m.go("site-detail",{id:t})}catch(o){h(),l.error(o.message)}}),document.getElementById("sd-site-nav")?.addEventListener("change",o=>{m.go("site-detail",{id:o.target.value})}),e.querySelectorAll("[data-action]").forEach(o=>{o.addEventListener("click",async()=>{let p=o.dataset.action;if(p==="flush"){try{await d.post(`/sites/${t}/flush`),l.success("Caches flushed")}catch(b){l.error(b.message)}return}y(`${{start:"Starting",stop:"Stopping",restart:"Restarting",update:"Updating"}[p]??p} Pod`,"Please wait...");try{await d.post(`/sites/${t}/${p}`),h(),l.success(`Site ${p} successful`),m.go("site-detail",{id:t})}catch(b){h(),l.error(b.message)}})}),te(e,t),re(e,t),ae(e,t),A(e),s.SiteType===1&&pe(e,t),T(e),$e(e,t),Se(t),ce(e,t),Z(e,t),B(e,t),ie(a??[])}async function U(e){let t=document.getElementById("totp-qr-img"),s=document.getElementById("totp-qr-wrap");if(!t||!s)return;if(s.querySelectorAll(".totp-uri-text").forEach(n=>n.remove()),typeof QRCode<"u")try{let n=await new Promise((i,c)=>{QRCode.toDataURL(e,{width:220,margin:2},(r,o)=>{r?c(r):i(o)})});t.src=n,t.style.display="";return}catch{}let a=document.createElement("p");a.className="totp-uri-text kp-muted uk-text-small",a.style.wordBreak="break-all",a.textContent=e,s.appendChild(a)}function F(e){document.getElementById("kp-backup-codes-modal")?.remove();let s=`
...
`),i=document.getElementById("kp-backup-copy-btn");if(navigator.clipboard)navigator.clipboard.writeText(n).then(()=>{i.textContent="Copied!"});else{let c=document.createElement("textarea");c.value=n,c.style.cssText="position:fixed;opacity:0",document.body.appendChild(c),c.select();try{document.execCommand("copy"),i.textContent="Copied!"}catch{}c.remove()}}),document.getElementById("kp-backup-done-btn").addEventListener("click",()=>{a.hide(),document.getElementById("kp-backup-codes-modal")?.remove(),m.go("users")})}function me(e){document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`;document.body.insertAdjacentHTML("beforeend",n);let i=UIkit.modal("#kp-edit-user-modal");i.show(),document.getElementById("edit-user-form").addEventListener("submit",async p=>{p.preventDefault();let u=p.target.querySelector('[type="submit"]'),b=u.innerHTML;u.disabled=!0,u.innerHTML='<div uk-spinner="ratio: 0.6"></div> Saving...';let k=new FormData(p.target),v={fname:k.get("fname").trim(),lname:k.get("lname").trim(),email:k.get("email").trim(),phone:k.get("phone").trim()};if(a){v.role=parseInt(k.get("role"));let f=k.get("uname");f&&(v.uname=f.trim())}let g=k.get("password");g&&(v.password=g);try{await d.put(`/users/${t}`,v),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("User updated"),m.go("users")}catch(f){l.error(f.message),u.disabled=!1,u.innerHTML=b}});let c=document.getElementById("totp-setup-btn");c&&c.addEventListener("click",async()=>{c.disabled=!0,c.textContent="Setting up\u2026";try{let p=await d.post(`/users/${t}/totp/setup`,{});document.getElementById("totp-secret-text").textContent=p.secret,document.getElementById("totp-setup-area").style.display="",await U(p.uri)}catch(p){l.error(p.message),c.disabled=!1,c.textContent="Enable TOTP"}});let r=document.getElementById("totp-confirm-btn");r&&r.addEventListener("click",async()=>{let p=document.getElementById("totp-confirm-code").value.trim();if(p.length!==6){l.error("Enter a 6-digit code");return}r.disabled=!0;try{let u=await d.post(`/users/${t}/totp/confirm`,{code:p});i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),l.success("TOTP enabled"),u.backup_codes?.length?F(u.backup_codes):m.go("users")}catch(u){l.error(u.message),r.disabled=!1}});let o=document.getElementById("totp-disable-btn");o&&o.addEventListener("click",async()=>{o.disabled=!0;try{await d.delete(`/users/${t}/totp`),l.success("TOTP disabled"),i.hide(),document.getElementById("kp-edit-user-modal")?.remove(),m.go("users")}catch(p){l.error(p.message),o.disabled=!1}}),document.getElementById("kp-edit-user-modal").addEventListener("hidden",()=>document.getElementById("kp-edit-user-modal")?.remove())}async function be(e){if(!E()){e.innerHTML=S("Access denied");return}let t=await d.get("/users");e.innerHTML=`
        <div class="kp-view-header">
        ...
        </div>`,document.getElementById("users-new-btn").addEventListener("click",()=>me(e)),Le(e)}function V(e){let t=e.role===99?'<span class="kp-badge kp-badge-admin">Admin</span>':'<span class="kp-badge kp-badge-manager">Manager</span>';return`<tr data-user-id="${e.id}">
